...
import { jsonRpcError } from "./response.ts";

export const MCP_OUTLET_METADATA: Readonly<Record<string, any>> = Object.freeze({
...
const PLUGIN_MCP_OUTLET_PATTERN: any = /^meshrix\.[A-Za-z][A-Za-z0-9]*$/u;

export function mcpOutletMetadata(toolName: any = "", tool: Record<string, any> = {}) : any {
  const normalized: any = String(toolName || "").trim();
  const core: any = MCP_OUTLET_METADATA[normalized];
  if (core) return core;
  if (!PLUGIN_MCP_OUTLET_PATTERN.test(normalized)) {
    throw new Error(`Unsupported MCP outlet binding: ${normalized}.`);
  }
  const descriptor: any = tool.mcpOutletDescriptor || tool._meta?.mcpOutletDescriptor;
  if (
    !descriptor ||
...
export function mcpOutletForTool(tool: Record<string, any> = {}) : any {
  const explicitOutlet: any = String(tool.mcpOutlet || tool._meta?.mcpOutlet || "").trim();
  if (explicitOutlet) {
    return mcpOutletMetadata(explicitOutlet, tool);
  }
  const id: any = String(tool.operationId || tool.id || tool.name || "").trim();
...
export function mcpOutletSummary(operations: any = []) : any {
  ...
  for (const operation of operations) {
    const toolName: any = operation?._meta?.mcpOutlet || MCP_DISCOVERY_TOOL_NAME;
    const outlet: any = outlets[toolName] || (outlets[toolName] = {
...
export function mcpCapabilityFamilies({ operations = [] }: Record<string, any> = {}) : any {
  return Object.fromEntries((Object.values(mcpOutletSummary(operations)) as any[])
    .filter((outlet?: any) : any => !CATEGORIZED_TOOL_NAMES.has(outlet.toolName) && outlet.operationCount > 0 && outlet.capabilityFamily)
    .map((outlet?: any) : any => {
      const {
        canViewOperations = [],
      ...
      const visibleOperations: any[] = [...outlet.operations];
      const visible: any = new Set<any>(visibleOperations);
      const key: any = outlet.toolName.replace(/^meshrix\./u, "");
      return [key, {
        ...family,
        available: true,
        canView: canViewOperations.some((operation?: any) : any => visible.has(operation)),
        canOperate: canOperateOperations.every((operation?: any) : any => visible.has(operation)),
        visibleOperations
      }];
    }));
}

export function mcpToolForOperation({ operation = "", toolSkillManagementProvider, authorization = null }: Record<string, any> = {}) : any {
...
export function mcpOutletForOperation({ operation = "", toolSkillManagementProvider, authorization = null }: Record<string, any> = {}) : any {
  const operationId: any = String(operation || "").trim();
  if (operationId === "meshrix.mcp.version" || operationId === "meshrix.version" || operationId === "meshrix.capabilities.list") {
    return MCP_OUTLET_METADATA[MCP_DISCOVERY_TOOL_NAME];
  }
  const tool: any = mcpToolForOperation({ operation: operationId, toolSkillManagementProvider, authorization });
  return tool ? mcpOutletForTool(tool) : null;
}

export function operationOutletMismatchError({ id, operation, requestedTool, expectedOutlet }: Record<string, any>) : any {
  return {
    httpStatus: 200,
...
export function publicMcpTool(tool?: any) : any {
  const inputSchema: any = publicMcpInputSchema(tool.inputSchema || { type: "object" });
  const outlet: any = mcpOutletForTool(tool);
  const workspaceHint: any = schemaMentionsWorkspaceId(tool.inputSchema)
    ? " MCP clients should use workspaceRef, workspaceIndex, or workspaceName instead of internal workspaceId."
    : "";
  const scopeHint: any = (tool.requiredScopes || []).length > 0
    ? ` Requires scope: ${tool.requiredScopes.join(", ")}.`
    : "";
  const riskHint: any = tool.risk && tool.risk !== "read_only"
    ? ` Risk: ${tool.risk}.`
    : "";
  return {
    name: tool.id,
...
function schemaMentionsWorkspaceId(value?: any) : any {
...
function publicMcpInputSchema(value?: any) : any {
  if (!value || typeof value !== "object") {
    return value;
  }
  if (Array.isArray(value)) {
  ...
  if (Array.isArray(next.required)) {
    next.required = next.required.filter((item?: any) : any => {
      const key: any = String(item || "");
  ...
  if (next.properties && typeof next.properties === "object" && !Array.isArray(next.properties)) {
    ...
    for (const [key, child] of (Object.entries(next.properties) as [string, any][])) {
...
function isLocalProcessSchemaKey(key: any = "") : any {
  return [
    "args",
    "command",
    "cwd",
...
function executeToolPayload(result: Record<string, any> = {}) : any {
  return result.payload?.result !== undefined ? result.payload.result : result.payload;
}

export function meshrixCategorizedTools({ activeOutlets = CATEGORIZED_TOOL_NAMES, visibleTools = [] }: Record<string, any> = {}) : any {
  const enabledOutlets: any = new Set<any>([
    MCP_DISCOVERY_TOOL_NAME,
    ...activeOutlets
  ]);
  const commonSchema: Record<string, any> = {
    type: "object",
  ...
  const metadataByOutlet: any = new Map<any, any>();
  ...
  const pluginTools: any = [...enabledOutlets]
    .filter((toolName?: any) : any => !CATEGORIZED_TOOL_NAMES.has(toolName))
    .sort((left?: any, right?: any) : any => left.localeCompare(right))
    .map((toolName?: any) : any => {
      const outlet: any = metadataByOutlet.get(toolName) || mcpOutletMetadata(toolName);
      return {
  ...
  return [...coreTools, ...pluginTools].filter((tool?: any) : any => enabledOutlets.has(tool.name));
}
