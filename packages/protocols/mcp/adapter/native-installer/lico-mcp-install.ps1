param(
  ...
  [string]$Url = "",
  [switch]$Json,
  [string]$TokenEnv = "LICO_MCP_TOKEN",
...
$SiblingExecutable = @(
  (Join-Path $PSScriptRoot "lico-mcp.exe"),
  (Join-Path $PSScriptRoot "lico-mcp.cmd"),
  (Join-Path $PSScriptRoot "lico-mcp.ps1")
) | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf } | Select-Object -First 1
if ($SiblingExecutable) {
  Invoke-Connector $SiblingExecutable $ConnectorArguments
}

$RepositoryConnector = Join-Path $PSScriptRoot "..\gateway-installer\bin\lico-mcp.mjs"
if (Test-Path -LiteralPath $RepositoryConnector -PathType Leaf) {
  Invoke-Connector $RepositoryConnector $ConnectorArguments
}

Write-Failure "No verified LicoMesh MCP connector was found. Download a release bundle, verify its published SHA256, extract it, and run this script from that bundle."
