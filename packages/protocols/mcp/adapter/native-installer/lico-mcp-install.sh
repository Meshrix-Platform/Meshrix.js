...
resolve_and_exec_connector() {
  if [ -x "$SCRIPT_DIR/lico-mcp" ]; then
    exec_connector "$SCRIPT_DIR/lico-mcp" "$@"
  fi

  repository_connector="$SCRIPT_DIR/../gateway-installer/bin/lico-mcp.mjs"
  if [ -f "$repository_connector" ]; then
    exec_connector "$repository_connector" "$@"
  fi

  fail "No verified LicoMesh MCP connector was found. Download a release bundle, verify its published SHA256, extract it, and run this script from that bundle."
}
