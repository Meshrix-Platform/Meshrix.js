...
export function resolveWindowsDpapiCommand({ platform = process.platform, configuredCommand = process.env[PROCESS_IDENTITY_WINDOWS_DPAPI_COMMAND_ENV] } = {}) {
    if (platform === "win32") {
...
async function saveSystemProcessIdentity(target = "codex", record = {}) {
    ...
    for (const backend of processIdentitySystemBackends()) {
        ...
        if (loaded?.privateKeyPem === record.privateKeyPem &&
...
//# sourceMappingURL=process-identity-store.js.map