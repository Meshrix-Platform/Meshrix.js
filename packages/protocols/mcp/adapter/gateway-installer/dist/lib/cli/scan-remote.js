...
import { INSTALL_COMMAND_TIMEOUT_MS, REMOTE_SCAN_COMMAND_TIMEOUT_MS, SCAN_COMMAND_TIMEOUT_MS } from "./constants.js";
import { run, runWithInput, shellQuote, uniqueValues } from "./connector-process.js";
import { baseUrlWithHost, isLoopbackHost, vmBaseUrl } from "./http-json-client.js";
import { mcpProbeSupported } from "./mcp-client-probe.js";
import { detectHostOs, outputLines, systemPosixPath } from "./scan-local.js";
...
export async function remoteClientBaseUrl(context, baseUrl) {
    ...
    if (context.kind === "wsl") {
        const nameserver = await remoteLinuxShell(context, `awk '/^nameserver / { print $2; exit }' ${systemPosixPath("etc", "resolv.conf")} 2>/dev/null`, { timeoutMs: SCAN_COMMAND_TIMEOUT_MS });
