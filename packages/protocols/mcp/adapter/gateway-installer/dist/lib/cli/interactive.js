...
import { stableStringify } from "../../mcp-identity.js";
import { deleteProcessIdentity, loadProcessIdentity, saveProcessIdentity } from "../process-identity-store.js";
import { DEFAULT_TOKEN_ENV, packageJson, TARGET_LOCATIONS, msg } from "./constants.js";
import { mcpTargetHeaders, normalizeBaseUrl, normalizeTarget, option, targetLabel } from "./basic-utils.js";
import { writeServerConfigProfile } from "./device-config.js";
import { discoverMeshrixHub, resolveToken } from "./discovery.js";
import { createProcessIdentityClaim, processIdentityHeaders, sha256Hex } from "./process-identity-request.js";
import { fetchJson } from "./http-json-client.js";
import { resolveGrantRequestFields } from "./grant-request.js";
import { redactToken } from "./installer-output-safety.js";
import { canUseInstallTui, installerOptions } from "./installer-options.js";
import { isGenericRemoteLocation } from "./scan-candidates.js";
...
async function requestApprovedLocalMcpGrant(options, requestPayload) {
    ...
    const timeoutMs = boundedPositiveInteger(option(options, "authorization-timeout-ms", LOCAL_MCP_AUTHORIZATION_TIMEOUT_MS), LOCAL_MCP_AUTHORIZATION_TIMEOUT_MS, { min: 1_000, max: LOCAL_MCP_AUTHORIZATION_TIMEOUT_MS });
    const pollIntervalMs = boundedPositiveInteger(option(options, "authorization-poll-interval-ms", LOCAL_MCP_AUTHORIZATION_POLL_INTERVAL_MS), LOCAL_MCP_AUTHORIZATION_POLL_INTERVAL_MS, { min: 50, max: 5_000 });
    const deadline = Date.now() + timeoutMs;
    process.stderr.write(`MCP installation authorization ${requestId} is pending. ` +
        `Approve verification code ${verificationCode} in the authenticated Meshrix console.\n`);
    while (Date.now() < deadline) {
        let consumeResponse;
        try {
            consumeResponse = await fetchJson(`${settings.baseUrl}/api/mcp/local-grant/requests/${encodeURIComponent(requestId)}/consume`, {
                method: "POST",
                timeoutMs: 10000,
                headers: {
                    "Content-Type": "application/json",
...
async function assertNoStoredGrantWillBeOverwritten(targets) {
    for (const target of targets) {
        if (await loadProcessIdentity(target)) {
            throw new Error(`A stored MCP credential already exists for ${targetLabel(target)}. ` +
...
function localMcpGrantTargetMaterial({ settings, target, responsePayload, processIdentityClaim, issuerIdentity = null }) {
...
export async function requestLocalMcpGrantBatch(options, { targets = [], autoUpdate = false } = {}) {
    ...
    const processIdentities = Object.fromEntries(Object.entries(claimsByTarget).map(([target, claim]) => [target, claim.request]));
    echoRequestedGrantSummary(grantRequest);
    const responsePayload = await requestApprovedLocalMcpGrant(options, {
        targets: targetList,
    ...
    const issuerIdentity = issuerIdentityFromDiscovery(options.__meshrixDiscovery);
    const materials = [];
    for (const target of targetList) {
        ...
        if (!targetPayload?.token || !targetPayload?.processIdentity?.clientIdentityPackage) {
            const rollbackMaterials = materials.concat(targetList
...
async function verifyStoredIssuer(identity, verifiedIssuers) {
    ...
    if (String(actualIdentity.keyId || "") !== String(expected.keyId) ||
...
async function notifyCredentialUninstall({ target, identity, token, expectedGrantId = "", verifiedIssuers }) {
...
export function assertProcessIdentityInstallLocation(options, targets = []) {
    ...
    for (const target of [...new Set(targets.map(normalizeTarget).filter(Boolean))]) {
        ...
        if (!supportedLocations.includes(location)) {
            throw new Error(`${targetLabel(target)} installation at ${location} is not supported by this release. ` +
...
async function storedGrantForInstall(options, settings, target) {
    ...
    const currentIssuer = issuerIdentityFromDiscovery(options.__meshrixDiscovery);
    if (!token ||
        !grantId ||
        ...
        throw new Error(`The stored MCP credential for ${targetLabel(target)} cannot be reused for this server. ` +
            `Uninstall ${target} with its issuing server before requesting another grant.`);
    }
    return {
        token,
...
export async function resolveHubForInstall(options) {
    ...
    const manual = await discoverMeshrixHub({ ...options, url });
    if (!manual.ok) {
