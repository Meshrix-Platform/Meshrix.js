...
import { verifyMcpHandshakeSignature } from "../../mcp-identity.js";
import { loadProcessIdentity } from "../process-identity-store.js";
import { DEFAULT_SCAN_PORTS, DEFAULT_TOKEN_ENV, MESHRIX_MCP_DISCOVERY_FILE_ENV, MESHRIX_MCP_DISCOVERY_URL_ENV, MESHRIX_MCP_URL_ENV, MCP_INTERFACE_VERSION, MCP_SERVER_NAME, MCP_STABLE_TOOL_NAME, packageJson } from "./constants.js";
import { mcpTargetHeaders, normalizeBaseUrl, normalizeTarget, option } from "./basic-utils.js";
import { discoveryRegistryPath, readJson } from "./device-discovery-registry.js";
import { readStdin, run, uniqueValues } from "./connector-process.js";
...
export function baseUrlFromEndpoint(value) {
    ...
    try {
        ...
        if (parsed.pathname === "/api/mcp/discovery" ||
...
export async function candidateBaseUrls(options = {}) {
    ...
    for (const filePath of fileCandidates) {
        ...
        const activeProfile = payload?.serverConfig?.activeName
            ? payload?.serverConfig?.profiles?.[payload.serverConfig.activeName]
            : null;
        fromFiles.push(activeProfile?.baseUrl, baseUrlFromEndpoint(server.httpUrl), baseUrlFromEndpoint(server.url), baseUrlFromEndpoint(payload?.discovery?.preferredHttpDiscoveryUrl), baseUrlFromEndpoint(payload?.discovery?.preferredApiDiscoveryUrl), ...profiles.map((profile) => profile?.baseUrl));
    }
...
export async function fetchMeshrixDiscovery(baseUrl) {
...
export async function verifyMeshrixHandshake(baseUrl, discovery) {
...
export async function discoverMeshrixHub(options = {}) {
    const attempts = [];
    const candidates = await candidateBaseUrls(options);
    for (const baseUrl of candidates) {
        try {
            const discovery = await fetchMeshrixDiscovery(baseUrl);
            const verified = await verifyMeshrixHandshake(baseUrl, discovery);
            return {
                ...verified,
                attempts: [
                    ...attempts,
