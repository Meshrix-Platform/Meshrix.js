import { MCP_CLIENT_TARGETS } from "../../mcp-release-targets.js";
import { notDetectedTargetDetail, targetLabel } from "./basic-utils.js";
import { clientAdapterConnectorRequest, describeClientAdapter, runClientAdapter } from "./client-adapter-runner.js";
import { packageJson, SUPPORTED_TARGETS } from "./constants.js";
import { withInstallCandidateGuidance } from "./guidance.js";
import { installerOptions } from "./installer-options.js";
import { detectLocalCommandPaths } from "./scan-local.js";
export function candidateLocation() {
    return "local";
}
export function isGenericRemoteLocation(_location) {
    return false;
}
export function candidateIdentity(candidate) {
    const command = String(candidate?.optionOverrides?.__meshrixAdapterClient?.command || "").trim();
...
export async function candidateHasInstalledMeshrixMcp(settings, candidate) {
...
export async function annotateInstalledCandidates(settings, candidates) {
    for (const candidate of candidates) {
        if (typeof candidate.installed !== "boolean") {
            candidate.installed = await candidateHasInstalledMeshrixMcp(settings, candidate).catch(() => false);
        }
    }
    return candidates;
}
async function scanTarget(settings, target, explicitClient = {}) {
    ...
    for (const command of [...new Set(commands)]) {
        ...
        if (scanned.result.available === false)
            continue;
        candidates.push({
        ...
        for (const { target } of MCP_CLIENT_TARGETS) {
            try {
                const explicitClient = explicitTarget === target && settings.clientCommand
                    ...
                    : {};
                for (const candidate of await scanTarget(settings, target, explicitClient))
...
//# sourceMappingURL=scan-candidates.js.map