import { BOOTSTRAP_INSTALL_SCRIPT_ZH_CN, DEFAULT_TOKEN_ENV, MESHRIX_MCP_DISCOVERY_FILE_ENV, MESHRIX_MCP_DISCOVERY_URL_ENV, MESHRIX_MCP_URL_ENV, MCP_INTERFACE_VERSION, MCP_SERVER_NAME, MCP_STABLE_TOOL_NAME, PRIORITY_INSTALL_TARGET, PRIORITY_INSTALL_TARGETS, SUPPORTED_TARGETS, packageJson, sharedHubContract, supportedTargetDetails } from "./constants.js";
import { option, targetInstallMode } from "./basic-utils.js";
import { discoverMeshrixHub, explicitBaseUrl, publishLaunchctlEnv } from "./discovery.js";
import { deviceDiscoveryEnv, discoveryRegistryPath, readJson, writeJson } from "./device-discovery-registry.js";
import { githubOneLineMcpInstallCommand, run, shellQuote } from "./connector-process.js";
import { redactToken } from "./installer-output-safety.js";
export function buildDeviceHubManifest({ baseUrl, targets, tokenEnv = DEFAULT_TOKEN_ENV, discoveryPath = discoveryRegistryPath() }) {
...
export async function serverConfigCommand(options) {
    ...
    if (options.set) {
        ...
        const discovered = await discoverMeshrixHub({ ...options, url });
        if (!discovered.ok) {
    ...
    if (options.switch) {
        ...
        const discovered = await discoverMeshrixHub({ ...options, url: profile.baseUrl });
        if (!discovered.ok) {
