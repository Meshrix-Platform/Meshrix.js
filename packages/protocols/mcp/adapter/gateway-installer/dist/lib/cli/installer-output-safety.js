...
export function redactSensitiveText(value, secrets = []) {
    ...
    const uniqueSecrets = new Set(secrets.map((item) => String(item || "")).filter((item) => item.length > 0));
...
export function redactInstallerJsonOutput(value) {
    ...
    if (value && typeof value === "object") {
        return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, redactInstallerJsonOutput(child)]));
    }
...
//# sourceMappingURL=installer-output-safety.js.map