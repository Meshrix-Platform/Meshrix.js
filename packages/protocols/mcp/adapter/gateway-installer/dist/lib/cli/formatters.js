...
import { targetLabel } from "./basic-utils.js";
import { commandFailureGuidance } from "./guidance.js";
import { redactInstallerJsonOutput, redactSensitiveText, sensitiveOptionValues } from "./installer-output-safety.js";
...
export function formatTargetInstallLine(target, item = {}) {
    ...
    if (item.httpVerification) {
        lines.push(`      MCP verify: tools=${item.httpVerification.toolCount}, stableTool=${item.httpVerification.stableToolName || ""}, health=${yesNo(item.httpVerification.systemHealthOk)}`);
    }
    return lines;
}
export function resultHasInstallRepair(result) {
    ...
    return commands.some((command) => /\bmeshrix-mcp\s+install\b/.test(String(command)));
}
...
export function formatInstallResult(result) {
    ...
    lines.push("Server:", `  MCP URL: ${result.baseUrl ? `${result.baseUrl}/mcp` : "unknown"}`);
