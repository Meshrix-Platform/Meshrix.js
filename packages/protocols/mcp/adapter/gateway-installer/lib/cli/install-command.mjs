...
import { isAutoTargetRequest, option, parseTargets, targetInstallMode } from "./basic-utils.mjs";
import { clientAdapterConnectorRequest, runClientAdapter } from "./client-adapter-runner.mjs";
import { writeDeviceDiscovery } from "./device-config.mjs";
import { ensureService, verifyMcpTools } from "./discovery.mjs";
import {
...
import { discoveryRegistryPath } from "./device-discovery-registry.mjs";
import { redactSensitiveText, redactToken } from "./installer-output-safety.mjs";
    ...
    try {
      if (!targetToken && tokenInfo?.grantsByTarget?.[target]?.token) {
        ...
        targetTokenInfo = await requestLocalMcpGrant(mergedOptions, {
          targets: [target],
          autoUpdate: Boolean(mergedOptions.__licoAutoUpdate ?? tokenInfo.autoUpdate)
        });
        targetToken = targetTokenInfo.token;
      }
      ...
      const authorizationRollback = await rollbackIssuedInstallGrant(
        mergedOptions,
        target,
        targetTokenInfo
      ).catch(() => ({
...
export async function installTuiCommand(options) {
  ...
  options.__licoAutoUpdate = autoUpdate;
...
export async function installAutoDetectedCommand(resolvedOptions) {
  ...
  resolvedOptions.__licoAutoUpdate = autoUpdate;
  const selectedTargets = [...new Set(selected.map((candidate) => candidate.target))];
...
export async function installCommand(options) {
  ...
  if (resolvedOptions.__licoSkippedDiscovery) {
    return {
      ok: false,
      skipped: true,
      packageName: packageJson.name,
      packageVersion: packageJson.version,
      ...resolvedOptions.__licoSkippedDiscovery
    };
  }
