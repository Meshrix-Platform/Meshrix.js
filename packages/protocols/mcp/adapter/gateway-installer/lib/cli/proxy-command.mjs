...
} from "../mcp-proxy-session.mjs";
import { createBoundedStdioOutput } from "./bounded-stdio-output.mjs";
...
import { optionsWithDiscoveredBaseUrl, resolveToken, signedAuthHeaders } from "./discovery.mjs";
...
export function extractStdioMessage(buffer, {
...
export function createProxyRequestDispatcher({
  ...
  const activeRequestLimit = positiveInteger(
    maxActiveRequests,
    MCP_PROXY_MAX_ACTIVE_REQUESTS,
    "maxActiveRequests"
  );
  const pendingDispatchLimit = positiveInteger(
