...
// - risk levels: tool-skill-management-provider-local-mcp.mjs LOCAL_GRANT_RISK_RANK
// - capability pattern: operation-permission-core/store-utils.mjs
//   DYNAMIC_UPSTREAM_CAPABILITY_PATTERN
// - service ids: the server stores allowedServiceIds as plain non-empty strings
//   (operation-permission-core/store-utils.mjs normalizeStringList) with no
...
export function resolveGrantRequestFields(options = {}) {
  ...
  const declaredMaxRisk = toolsets.reduce(
    (max, toolset) => (riskRank(GRANTABLE_TOOLSET_MAX_RISK[toolset]) > riskRank(max) ? GRANTABLE_TOOLSET_MAX_RISK[toolset] : max),
    "read_only"
  );
