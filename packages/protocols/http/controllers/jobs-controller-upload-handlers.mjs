...
} from "./jobs-controller-upload-trace.mjs";
...
export function createUploadSessionHandlers({
  ...
  return {
    async handleUploadWorkspaceMaterialize({ input, requestBody, response, authSession, governanceReceipt }) {
      ...
      const payload = input && typeof input === "object"
        ? input
        : requestBody.length ? JSON.parse(requestBody.toString("utf8")) : {};
      const result = await uploadWorkspaceMaterializationProvider.submit({
        ...payload,
        subject: requestOwnerSubjectFromSession(authSession),
        governanceReceipt
      });
      sendJson(response, result.deduped ? 200 : 202, result);
    },
    async handleUploadWorkspaceMaterializationCancel({ requestRef, response, authSession }) {
      if (!uploadWorkspaceMaterializationProvider) {
        throw new Error("Upload workspace materialization provider is unavailable.");
      }
      const cancelled = await uploadWorkspaceMaterializationProvider.cancel(requestRef, {
        subject: requestOwnerSubjectFromSession(authSession)
      });
      if (!cancelled) {
    ...
    async handleCreateUploadSession({ requestBody, response, authSession }) {
    ...
    async handleGetUploadSession({ sessionId, response, authSession }) {
      const requestId = randomUUID();
      const ownerSubject = requestOwnerSubjectFromSession(authSession);
      const trace = createUploadTracePublisher(protocolEventBus, requestId, {
        http: {
    ...
    async handleUploadChunk({ sessionId, fileIndex, offset, requestBody, response, authSession }) {
      const requestId = randomUUID();
      const ownerSubject = requestOwnerSubjectFromSession(authSession);
      const trace = createUploadTracePublisher(protocolEventBus, requestId, {
        http: {
