...
export function createStartQueuedJob(ctx) {
  const {
    userDataPath,
  ...
  async function startQueuedJob(nextEntry) {
    ...
    try {
      ...
      await durableWorkflows.startActivity(currentJob.workflowId || workflowIdForJob(currentJob), "job-execution").catch((error) => {
    ...
    let preserveForRecoveryPromise = null;
    ...
    const trackBackgroundTask = (label, task) => {
    ...
    const finalizeJob = async ({
      ...
      const uploadSessionCleanupReady =
        completed &&
        uploadSessionConsumptionComplete(result?.uploadSessionConsumption);
      if (currentJob.uploadSessionId && uploadSessionCleanupReady) {
        await deleteUploadSession(userDataPath, currentJob.uploadSessionId);
      } else if (currentJob.uploadSessionId) {
        logJob("warn", "jobs.upload_session.retained", {
          jobId: currentJob.id,
          reason: completed ? "persistence_receipt_missing" : "job_not_completed"
        });
      }
    ...
    const runFinalizeJob = (input) => finalizeJob(input).catch((error) => {
    ...
    const activeController = {
      ...
      fail: async ({ stage = "队列执行失败", errorMessage = "Queue execution failed." } = {}) => {
    ...
    const runGuardedFinalizeJob = async (input) => {
      try {
    ...
    const abortFromQueueLease = () => {
      void preserveForLostLease().catch((error) => {
