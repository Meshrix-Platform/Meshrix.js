...
function eventStorePath(userDataPath) {
...
function updateCounters(db, counters) {
...
function removeLatestAtOffset(db, offset, counters) {
  const latest = prepareCached(db,
...
function pruneForAdmission(db, counters, policy, incomingBytes, nowMs) {
  const oldest = prepareCached(db, `
    SELECT offset,event_bytes,published_at_ms
    FROM protocol_events
    ORDER BY offset ASC
    LIMIT 1
  `);
  const deleteEvent = prepareCached(db, "DELETE FROM protocol_events WHERE offset=?");
  const expiresBefore = nowMs - policy.maxAgeMs;
  let removed = 0;
  while (removed < policy.retentionBatch) {
    const entry = oldest.get();
    if (!entry) break;
    const overCapacity =
...
function upsertLatest(db, event, serialized, eventBytes, counters, policy) {
...
export function createSqliteProtocolEventStore({
  ...
  const db = openSqliteDatabase(selectedPath);
  ...
  `);
  const publishTransaction = db.transaction((candidate, retain) => {
    ...
    async getLatest(topics = []) {
      ...
      if (normalizedTopics.length === 0) {
        return prepareCached(db, `
          ...
          FROM protocol_event_latest
          ORDER BY offset ASC
        `).all().map((entry) => JSON.parse(String(entry.event_json)));
      }
      return prepareCached(db, `
        SELECT event_json
        FROM protocol_event_latest
      ...
      `).all(...normalizedTopics)
        .map((entry) => JSON.parse(String(entry.event_json)));
    },
    async getRevision() {
      requireOpen();
