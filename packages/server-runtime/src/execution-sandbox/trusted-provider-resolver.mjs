...
function normalizedConfiguration(configuration) {
  ...
  const allowedProviderClasses = Array.isArray(configuration.allowedProviderClasses)
    ? [...new Set(configuration.allowedProviderClasses.map(text).filter((value) => PROVIDER_ORDER.has(value)))]
    : [];
  if (
...
function conformanceState(descriptor, configuration, now) {
  const receipt = descriptor?.conformanceReceipt;
  const restrictions = new Set(Array.isArray(descriptor?.enforcedRestrictions)
...
function administrativeProjection(state) {
  return Object.freeze({
    state: state.state,
...
export function createTrustedSandboxProviderResolver({
  ...
  const boundedTtl = Number.isSafeInteger(Number(ttlMs)) && Number(ttlMs) > 0
  ...
  let selected = null;
  let state = configured.state !== "configured"
    ? { state: configured.state }
    : { state: "unavailable" };

  function expireCachedSelection() {
  ...
  async function refresh({ force = false } = {}) {
    ...
    refreshPromise = (async () => {
      ...
      candidates.sort((left, right) => {
        const rank = PROVIDER_ORDER.get(left.adapter.providerClass) -
