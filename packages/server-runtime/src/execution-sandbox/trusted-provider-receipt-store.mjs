...
function normalizedReceipts(value) {
  ...
  return Object.freeze(Object.fromEntries(Object.entries(value)
    .filter(([providerId, receipt]) => providerId.trim() && receipt && typeof receipt === "object" && !Array.isArray(receipt))
    .map(([providerId, receipt]) => [providerId.trim(), Object.freeze({ ...receipt })])));
}

function ownedByCurrentProcess(metadata) {
  return typeof process.getuid !== "function" || metadata.uid === process.getuid();
}

export function loadTrustedSandboxProviderReceipts({ userDataPath } = {}) {
...
export async function writeTrustedSandboxProviderReceipts({ userDataPath, receipts } = {}) {
  const target = storePath(userDataPath);
