...
function containerIdentity(runId, backendNonce) {
...
function boundedBufferLimit(value, fallback = 64 * 1024) {
  const number = Number(value);
  return Number.isSafeInteger(number) && number > 0 ? number : fallback;
}

function runtimeCreateArguments(engine, runtimeClass) {
...
async function assertOwnedDirectory(directoryPath, { uid, gid, label, writable }) {
...
function runCommand(binary, args, {
  ...
  return new Promise((resolve, reject) => {
    ...
    const consume = (chunk, capture = false) => {
      bytes += chunk.length;
      if (bytes > maxBytes) {
        child.kill("SIGKILL");
      }
