...
export async function executeSystemObservationOperation({ operationId, input = {}, context }) {
...
export async function executeJobObservationOperation({ operationId, input = {}, context }) {
  const id = String(operationId || "");
...
export async function executeSystemCoreOperation({ operationId, context }) {
  const id = String(operationId || "");
...
export async function executeConsoleStateOperation({ operationId, context }) {
  if (operationId === "readiness.baseline.status") {
    const provider = readinessBaselineProviderFor(context);
...
export async function executeSystemInterfaceOperation({ operationId, context }) {
...
export async function executeRuntimePathBrowseOperation({ operationId, input = {}, context }) {
...
export async function executeProductionReadinessOperation({ operationId, input = {}, context }) {
  const id = String(operationId || "");
  const handledOperations = new Set([
    "production.health",
  ...
  if (id.startsWith("executive_report.")) {
    try {
      const provider = executiveReportProviderFor(context);
  ...
  if (id.startsWith("sample_capability_pack.")) {
    const store = sampleCapabilityPackStoreFor(context);
