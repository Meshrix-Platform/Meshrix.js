...
export async function executeAuthorizationFacadeOperation({ operationId, input = {}, context }) {
  ...
  if (id === "tag_management.tags.list") {
    const items = securityPermissions?.listTags?.({
      kind: input.kind || "",
      status: input.status || "",
  ...
  if (id === "tag_management.tags.get") {
    const tag = securityPermissions?.getTag?.(input.tagId || input.id || "");
    return tag
  ...
  if (id === "tag_management.tags.upsert") {
    ...
    const tag = securityPermissions.upsertTag(input);
  ...
  if (id === "tag_management.tags.archive") {
    ...
    const tag = securityPermissions.archiveTag(input.tagId || input.id || "", input);
    const governanceUpdate = await publishAuthorizationGovernanceUpdate({
      context,
      operationId: id,
      entityType: "tag",
  ...
  if (id === "tag_management.tags.restore") {
    ...
    const tag = securityPermissions.restoreTag(input.tagId || input.id || "");
    const governanceUpdate = await publishAuthorizationGovernanceUpdate({
      context,
      operationId: id,
      entityType: "tag",
  ...
  if (id === "tag_management.projections.list") {
    const items = securityPermissions?.listTagProjections?.({
  ...
  if (id === "tag_management.audit.list") {
    const items = securityPermissions?.listTagEvents?.({
  ...
  if (id === "authorization.departments.list") {
    const items = securityPermissions?.listGovernanceDepartments?.() || [];
    return result(200, protocolPayload({ items, count: items.length }));
  }
  ...
  if (id === "authorization.departments.upsert") {
    ...
    const department = securityPermissions.upsertGovernanceDepartment(input);
    const governanceUpdate = await publishAuthorizationGovernanceUpdate({
      context,
      operationId: id,
