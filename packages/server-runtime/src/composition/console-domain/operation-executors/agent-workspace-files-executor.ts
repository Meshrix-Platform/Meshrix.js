...
export async function executeAgentWorkspaceFileOperation({ operationId, input, context }: Record<string, any>) : Promise<any> {
  ...
  if (isManagedWorkspaceAssetWriteOperation(id) && context.workspaceAssetManagedWriteActive !== true) {
    ...
    return runManagedWorkspaceAssetWrite({
      ...
      run: () : any => executeAgentWorkspaceFileOperation({
        operationId: id,
        input,
        context: {
          ...context,
  ...
  const actorId: any = context.authSession?.user?.username || context.authSession?.user?.userId || "";
