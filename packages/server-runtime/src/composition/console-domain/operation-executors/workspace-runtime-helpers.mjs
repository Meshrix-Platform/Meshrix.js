...
export function workspaceFileSnapshotFromCheckpointPlan(plan = {}, input = {}) {
  ...
  const files = Array.isArray(snapshot.files)
    ...
    : Array.isArray(snapshot.entries)
      ...
      : Array.isArray(input.files)
        ? input.files
        : [];
  const localDirectorySnapshots = Array.isArray(snapshot.localDirectorySnapshots)
...
export async function runCheckpointWorkspaceFileRestore({ plan, input = {}, context = {}, dryRun }) {
...
export function applyWorkspaceRuntimeContext(payload = {}, agentWorkspace = null, options = {}) {
