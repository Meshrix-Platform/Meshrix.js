...
} from "./settings-agent-gateway-models.ts";
import { applyWorkspaceRuntimeContext } from "./workspace-runtime-helpers.ts";
import { publicAgentGatewayError } from "#meshrix/agents/agent-gateway/index";

export async function executeSettingsAgentGatewayOperation({ operationId, input = {}, context }: Record<string, any>) : Promise<any> {
  ...
  const settingsPortResult: any = requireSettingsPort(context);
  if (settingsPortResult.error) {
    return settingsPortResult.error;
  }
  const { settingsPort } = settingsPortResult;

  const settingsInput: any = input && typeof input === "object" && !Array.isArray(input) ? input : {};
  ...
  const registry: any = agentConfigRegistryFrom(context);
  const requiresRegistry: any = id !== "model_routing.health";
  ...
  if (id === "settings.set") {
    const shouldAuditModelLibrary: any =
      Object.hasOwn(settingsInput, "modelLibraryAgents") ||
      Object.hasOwn(settingsInput, "modelLibraryEntries");
    const beforeSettings: any = shouldAuditModelLibrary
      ? await loadAgentRuntimeSettings(context, { redactSecrets: true })
      : null;
    const beforeModelLibrarySummary: any = shouldAuditModelLibrary
      ? normalizeModelLibraryAuditList((beforeSettings?.modelLibraryAgents || []).concat())
      : null;
    const saved: any = await settingsPort.saveSettings(context.userDataPath, settingsInput, {
      redactSecrets: false
    });
    const runtimeSettings: any = saved;
    ...
    if (shouldAuditModelLibrary) {
      const afterModelLibrarySummary: any = normalizeModelLibraryAuditList(saved.modelLibraryAgents);
      const modelLibraryDiff: any = diffModelLibraryAgents(
  ...
  if (id === "settings.model_probe") {
    const provider: any = String(settingsInput.provider || settingsInput.modelProvider || "").trim();
    const modelAlias: any = String(
    ...
    let message: any = "";
    try {
      const current: any = await loadAgentRuntimeSettings(context);
      const candidateSettings: any = mergeSettingsForModelProbe(
        current,
  ...
  if (id === "agent_gateway.call") {
    ...
    const settings: any = await loadAgentRuntimeSettings(context);
    let gatewayResult: any;
    try {
      gatewayResult = await agentRuntimeProvider.callAgentGateway({
        settings,
      ...
      const projected: any = publicAgentGatewayError(error);
      return result(Number(error?.statusCode || 503), {
  ...
  if (id === "agents.create") {
    const startedAt: any = Date.now();
    const patch: any = normalizeAgentModelPayload(settingsInput);
    const provider: any = patch.provider || "";
    const model: any = patch.model || patch.engine || "";
    const current: any = await loadAgentRuntimeSettings(context);
    ...
    const models: any[] = [entry, ...(current.modelLibraryAgents || [])];
    try {
  ...
  if (id === "agents.update") {
    const startedAt: any = Date.now();
    const agentId: any = settingsInput.agentId;
    const patch: any = normalizeAgentModelPayload(settingsInput);
    const current: any = await loadAgentRuntimeSettings(context);
    const models: any[] = [...(current.modelLibraryAgents || [])];
    ...
    const previous: any = models[index];
    const next: any = applyAgentModelPatch(previous, patch);
    models[index] = next;
    try {
      const { registry: gatewayRegistry } = await saveAgentModelLibrary(context, current, models);
