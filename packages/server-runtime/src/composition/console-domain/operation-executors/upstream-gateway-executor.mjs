...
const CONFIG_MUTATION_OPERATION_IDS = new Set([
...
const CONFIG_MUTATION_ACTIONS = Object.freeze({
...
export function createUpstreamGatewayOperationExecutor(dependencies = {}) {
  const errorPayload = requiredFunction("errorPayload", dependencies.errorPayload);
  const objectOrNull = requiredFunction("objectOrNull", dependencies.objectOrNull);
  ...
  const subjectFromAuthSession = requiredFunction("subjectFromAuthSession", dependencies.subjectFromAuthSession);
  const upstreamGatewayRegistryFor = requiredFunction("upstreamGatewayRegistryFor", dependencies.upstreamGatewayRegistryFor);

  function upstreamGatewayServiceId(input = {}, context = {}) {
    return String(
      context.serviceId ||
        input.serviceId ||
        input["service-id"] ||
  ...
  return async function executeUpstreamGatewayOperation({ operationId, input = {}, context = {} }) {
    ...
    const registry = upstreamGatewayRegistryFor(context);
    ...
    const subject = {
      ...
      approvedPendingOperation: context.request?.__licoToolRuntimeAuthorization?.approvedPendingOperation || null
    };
    ...
    try {
      ...
      if (CONFIG_MUTATION_OPERATION_IDS.has(id)) {
        ...
        const published = await context.upstreamPublishingApplication.execute(context.rawRequestBody, subject, {
          signal: context.signal || null,
      ...
      const serviceId = upstreamGatewayServiceId(input, context);
      ...
      if (id.startsWith("upstream_operation.")) {
        const forward = await registry.forwardProjectedOperation(id, input, subject, {
          signal: context.signal || null,
      ...
      if (id === "gateway.forward") {
        const forward = await registry.forward(input, subject, {
          signal: context.signal || null,
          responseAdapter: context.transport === "mcp" ? "artifact" : "structured"
        });
        return result(forward.status === "pending_approval" ? 202 : 200, protocolPayload(forward));
