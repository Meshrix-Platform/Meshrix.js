...
export function clientVersionString(value) {
  const text = String(value || "").trim().slice(0, 80);
  return /^[A-Za-z0-9._:@+ -]*$/.test(text) ? text : hashClientString(text, "client.version");
}

export function stripClientDiscoveryStrings(value = {}) {
  return {
...
export async function executeDiscoveryOperation({ operationId, input = {}, context }) {
  ...
  if (id === "discovery.check_in") {
    ...
    const record = clientRegistryService.recordClientCheckIn({
      clientId,
  ...
  if (id === "discovery.clients.alignment_command") {
    ...
    const client = clientRegistryService.findClientRegistration({
  ...
  if (id === "discovery.set_config") {
    const discoveryPortResult = requireDiscoveryPort(context);
    ...
    const nextDiscoveryState = await discoveryPort.saveDiscoveryConfig(
    ...
    const publicDiscoveryState = buildConsoleDiscoveryConfig(nextDiscoveryState);
    await publishProtocolEvent(
      context.protocolEventBus,
      "discovery.config",
      {
