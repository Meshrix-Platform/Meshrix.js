...
export async function executeOperationPermissionPassthroughOperation({ operationId, context }) {
  const id = String(operationId || "operation_permission.http.passthrough");
  ...
  const handled = await provider.handleOperationPermissionHttpRequest({
