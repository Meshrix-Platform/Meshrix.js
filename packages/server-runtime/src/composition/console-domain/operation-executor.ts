import { createUpstreamGatewayOperationExecutor } from "./operation-executors/upstream-gateway-executor.ts";
import {
  errorPayload,
  objectOrNull,
  protocolPayload,
...
} from "./operation-executors/shared.ts";
import { upstreamGatewayRegistryFor } from "./operation-executors/registry-services.ts";
import { executeWorkspaceAssetOperation } from "./operation-executors/workspace-asset-executor.ts";
import { executeWorkspaceContributionOperation } from "./operation-executors/workspace-contribution-executor.ts";
import {
  executeMonitorAlertOperation,
  executeStorageOperation
} from "./operation-executors/storage-client-monitor-executors.ts";
import {
...
} from "./operation-executors/runtime-admin-executors.ts";
import { executeDiscoveryOperation } from "./operation-executors/discovery-executor.ts";
import { executeAgentWorkspaceFileOperation } from "./operation-executors/agent-workspace-files-executor.ts";
import { executeAgentWorkspaceManagementOperation } from "./operation-executors/agent-workspace-management-executor.ts";

const executeUpstreamGatewayOperation: any = createUpstreamGatewayOperationExecutor({
  errorPayload,
  objectOrNull,
  protocolPayload,
  result,
...
export const CONSOLE_DOMAIN_OPERATION_EXECUTOR_REGISTRY: readonly any[] = Object.freeze([
...
export function listConsoleDomainOperationExecutors() : any {
  return CONSOLE_DOMAIN_OPERATION_EXECUTOR_REGISTRY.map((entry?: any) : any => ({
    featureId: entry.featureId,
    handlerName: entry.handler.name
  }));
}

export async function executeConsoleDomainOperation({ operationId, input = {}, context = {} }: Record<string, any> = {}) : Promise<any> {
  for (const executor of CONSOLE_DOMAIN_OPERATION_EXECUTOR_REGISTRY) {
    const operationResult: any = await executor.handler({ operationId, input, context });
    if (operationResult) {
