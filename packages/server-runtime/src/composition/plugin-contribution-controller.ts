...
import { PLUGIN_WORKSPACE_ACCESS_METHODS } from "./plugin-workspace-access.ts";
import { PLUGIN_HOST_PORT_NAMES } from "./plugin-contribution-registry.ts";
import {
  boundedCallStrings,
...
} from "./plugin-call-context.ts";

const HOST_PORT_NAMES: any = new Set<any>(PLUGIN_HOST_PORT_NAMES);
const PLUGIN_PERMISSION_LOAN_FIELDS: any = new Set<any>([
  "loanRecordId",
  "contributionGrantId",
  "contributionId",
  "granteeId",
...
const EXTERNAL_SERVICE_REQUEST_FIELDS: any = new Set<any>([
  "serviceRef",
  "operationRef",
  "input",
  "idempotencyKey",
...
function boundedGrantText(value?: any, label?: any, { required = true, maximum = 256 }: Record<string, any> = {}) : any {
...
function normalizedPluginPermissionLoan(value?: any) : any {
  if (!isPlainObject(value)) throw pluginPermissionGrantError("plugin_permission_grant_record_invalid");
  if (Object.keys(value).some((field?: any) : any => !PLUGIN_PERMISSION_LOAN_FIELDS.has(field))) {
  ...
  const actions: any = [...new Set<any>(value.actions.map((action?: any) : any => boundedGrantText(action, "action", { maximum: 128 })))].sort();
  if (actions.length !== value.actions.length) {
    throw pluginPermissionGrantError("plugin_permission_grant_actions_invalid");
  }
  const expiresAt: any = boundedGrantText(value.expiresAt, "expiry", { required: false, maximum: 64 });
  if (expiresAt && (!Number.isFinite(Date.parse(expiresAt)) || Date.parse(expiresAt) <= Date.now())) {
    throw pluginPermissionGrantError("plugin_permission_grant_expiry_invalid");
  }
  const createdAt: any = boundedGrantText(value.createdAt, "created_at", { maximum: 64 });
...
function operationPermissionGrantFacade(source?: any, record?: any, call?: any) : any {
  return Object.freeze({
    async recordPluginGrant(input: Record<string, any> = {}) : Promise<any> {
      if (!isPlainObject(input) || Object.keys(input).some((field?: any) : any => field !== "loanRecord")) {
        throw pluginPermissionGrantError("plugin_permission_grant_request_invalid");
      }
      const governance: any = currentSandboxGovernance(call);
      ...
      const loanRecord: any = normalizedPluginPermissionLoan(input.loanRecord);
      const callProjection: any = createPluginCallProjection(call);
      const evidence: Readonly<Record<string, any>> = Object.freeze({
      ...
      const storedRef: any = boundedGrantText(stored?.loanRecordId, "host_record_id");
      return Object.freeze({
        ok: true,
...
function pluginExternalServiceError(code?: any, status: any = 502) : any {
...
function externalServiceJson(value?: any, label?: any, depth: any = 0) : any {
  ...
  if (Array.isArray(value)) {
    ...
    return Object.freeze(value.map((entry?: any) : any => externalServiceJson(entry, label, depth + 1)));
  }
  ...
  if (entries.length > 1_000 || entries.some(([key]: any[]) : any => UNSAFE_JSON_KEYS.has(key))) {
    throw pluginExternalServiceError("plugin_external_service_input_invalid", 400);
  }
  return Object.freeze(Object.fromEntries(entries.map(([key, entry]: any[]) : any => [
    key,
    externalServiceJson(entry, label, depth + 1)
  ])));
}

function externalServicePagination(value?: any) : any {
  const fields: any = new Set<any>(["nextCursor", "page", "perPage"]);
  ...
  if (value.nextCursor !== undefined) {
    const nextCursor: any = String(value.nextCursor || "").trim();
...
function externalServiceRateLimit(value?: any) : any {
  const fields: any = new Set<any>(["remaining", "resetAt", "retryAfterMs"]);
  if (!isPlainObject(value) || Object.keys(value).some((field?: any) : any => !fields.has(field))) {
    throw pluginExternalServiceError("plugin_external_service_response_invalid", 502);
  }
  const output: Record<string, any> = {};
  ...
  if (value.resetAt !== undefined) {
    const resetAt: any = String(value.resetAt || "").trim();
...
function externalServiceRequestFacade(source?: any, record?: any, call?: any) : any {
  return Object.freeze({
    async request(input: Record<string, any> = {}, options: Record<string, any> = {}) : Promise<any> {
      if (!isPlainObject(input) || Object.keys(input).some((field?: any) : any => !EXTERNAL_SERVICE_REQUEST_FIELDS.has(field)) ||
          !isPlainObject(options) || Object.keys(options).some((field?: any) : any => !EXTERNAL_SERVICE_OPTIONS_FIELDS.has(field))) {
      ...
      const serviceRef: any = String(input.serviceRef || "").trim();
      const operationRef: any = String(input.operationRef || "").trim();
      ...
      const requestedTimeout: any = input.timeoutMs === undefined ? 0 : Number(input.timeoutMs);
      ...
      const callSignal: any = call.signal || call.operationLock?.signal || null;
      ...
      const runtimeAuthorization: any = call.request?.__meshrixToolRuntimeAuthorization ||
      ...
      try {
        ...
        const status: any = Number(error?.status || 0);
        const code: any = callSignal?.aborted === true || status === 499
      ...
      const status: any = Number(response.status || response.statusCode || 0);
      ...
      const data: any = externalServiceJson(response.data ?? null, "Plugin external service response");
      ...
      const sourceReceipt: any = String(response.receiptRef || response.auditId || "").trim();
      if (sourceReceipt) {
...
function sandboxExecutionFacade(source?: any, record?: any, call?: any) : any {
  const pluginId: any = record.pluginId;
  const bindRequest: any = (request: Record<string, any> = {}) : any => Object.freeze({
    ...request,
  ...
  const bindOpaqueInputs: any = (request?: any, opaqueInputs?: any) : any => {
    const boundRequest: any = bindRequest(request);
    return Object.freeze({
      request: boundRequest,
      opaqueInputs: Object.freeze((Array.isArray(opaqueInputs) ? opaqueInputs : []).map((input?: any) : any => Object.freeze({
        ...input,
  ...
  return Object.freeze({
    execute: (request?: any, resolveInput?: any) : any => source.execute(bindRequest(request), {
      resolveInput,
      currentGovernance: currentSandboxGovernance(call, request),
      pluginId
    }),
    executeConfigured: (request?: any, resolveInput?: any, options: Record<string, any> = {}) : any => source.executeConfigured(bindRequest(request), resolveInput, {
      currentGovernance: currentSandboxGovernance(call, request),
      pluginId,
      signal: options.signal || null
    }),
    executeOpaque: (request?: any, opaqueInputs?: any) : any => {
      const bound: any = bindOpaqueInputs(request, opaqueInputs);
      return source.executeOpaque(bound.request, bound.opaqueInputs, {
        currentGovernance: bound.request.governance,
        pluginId
      });
    },
    executeConfiguredOpaque: (request?: any, opaqueInputs?: any, options: Record<string, any> = {}) : any => {
      const bound: any = bindOpaqueInputs(request, opaqueInputs);
    ...
    cancel: (runId?: any) : any => source.cancel(runId, { pluginId }),
    getStatus: (runId?: any) : any => source.getStatus(runId, { pluginId }),
    getReceipt: (runId?: any) : any => source.getReceipt(runId, { pluginId }),
    resolveQuarantinedOutput: (outputHandle?: any) : any => source.resolveQuarantinedOutput(outputHandle, { pluginId }),
    disposeOutput: (outputHandle?: any, disposition?: any) : any => source.disposeOutput(outputHandle, disposition, {
      pluginId,
...
const OPAQUE_INPUT_HANDLE_SCHEMA_VERSION: any = "v0.0.1:plugin:opaque-input-handle-1";
const OPAQUE_INPUT_DIGEST_PATTERN: any = /^[a-f0-9]{64}$/u;
const OPAQUE_INPUT_HANDLE_PATTERN: any = /^custody:[A-Za-z0-9._-]{1,160}$/u;
const OPAQUE_INPUT_CHUNK_CHARACTERS: any = 64 * 1024;

function opaqueInputError(code?: any) : any {
  const error: Error & Record<string, any> = new Error("Plugin opaque input preprocessing failed.");
  error.code = code;
  return error;
}

function strictBase64Metrics(value?: any, maxBytes?: any) : any {
  ...
  const tail: any = value.slice(-4);
  if (Buffer.from(tail, "base64").toString("base64") !== tail) {
    throw opaqueInputError("plugin_opaque_input_base64_invalid");
  }
  const padding: any = value.endsWith("==") ? 2 : value.endsWith("=") ? 1 : 0;
  const byteCount: any = value.length / 4 * 3 - padding;
...
async function *base64ByteSource(value?: any) : AsyncGenerator<any, any, any> {
  for (let offset: any = 0; offset < value.length; offset += OPAQUE_INPUT_CHUNK_CHARACTERS) {
    const bytes: any = Buffer.from(value.slice(offset, offset + OPAQUE_INPUT_CHUNK_CHARACTERS), "base64");
    try {
      yield bytes;
    } finally {
      bytes.fill(0);
...
function opaqueSealIdempotencyKey({ record, declaration, ownerBinding, governance, metrics, metadataDigest, requestRef }: Record<string, any>) : any {
  const digest: any = createHash("sha256").update(JSON.stringify({
...
async function preprocessPluginOpaqueInputs(record?: any, input?: any, call?: any, custody?: any) : Promise<any> {
  const declarations: any = record.implementation.opaqueInputPreprocessing || [];
  ...
  try {
    for (const declaration of declarations) {
      ...
      const encoded: any = output[declaration.sourceField];
      const metrics: any = strictBase64Metrics(encoded, declaration.maxBytes);
      const metadata: Record<string, any> = { ...output };
      delete metadata[declaration.sourceField];
      delete metadata[declaration.targetField];
      const metadataDigest: any = createHash("sha256").update(JSON.stringify(metadata)).digest("hex");
    ...
    if (typeof custody.delete === "function") {
      await Promise.allSettled(newlyStored.map((handle?: any) : any => custody.delete({
        handle,
...
function hostPathInputError(code?: any, statusCode?: any) : any {
...
async function preprocessPluginHostPathInputs(record?: any, input?: any, call?: any, agentWorkspace?: any) : Promise<any> {
  const declarations: any = record.implementation.hostPathInputPreprocessing || [];
  if (declarations.length === 0) return input;
  ...
  const authority: any = pluginWorkspaceAuthority(call);
  const user: any = call.authSession?.user || {};
  ...
  for (const declaration of declarations) {
    ...
    const selected: any = await agentWorkspace.createLocalDirectoryMountSelection({
    ...
    if (selected?.ok !== true || !/^local-directory-selection:[a-f0-9]{32}$/u.test(String(selected.mountSelectionRef || ""))) {
      const statusCode: any = [400, 403, 404].includes(Number(selected?.status)) ? Number(selected.status) : 403;
...
function opaqueArtifactCustodyFacade(source?: any, call?: any) : any {
  return Object.freeze({
    describe: (handle?: any, input: Record<string, any> = {}) : any => source.describe(handle, custodyOwnerBinding(call)),
    delete: (input: Record<string, any> = {}) : any => source.delete({
      ...input,
      ownerBinding: custodyOwnerBinding(call)
    })
  });
...
const AGENT_WORKSPACE_HOST_METHODS: readonly any[] = Object.freeze([
...
function agentWorkspaceFacade(source?: any, call?: any) : any {
  const authority: any = pluginWorkspaceAuthority(call);
  const user: any = call.authSession?.user || {};
  const facade: Record<string, any> = {};
  for (const name of AGENT_WORKSPACE_HOST_METHODS) {
    if (typeof source?.[name] !== "function") continue;
    facade[name] = (input: Record<string, any> = {}) : any => {
      const suppliedWorkspaceRef: any = String(input.workspaceId || input.workspaceRef || input.workspace || "").trim();
...
function selectHostPorts(record?: any, hostPorts?: any, call: Record<string, any> = {}, rawHostPorts: Record<string, any> = {}) : any {
...
function responseEnded(response?: any) : any {
  return response?.writableEnded === true || response?.ended === true;
}

function sendExecutionResult(response?: any, result?: any) : any {
  if (responseEnded(response)) return;
  if (result === undefined) throw new Error("Plugin operation completed without a response.");
  const envelope: any = isPlainObject(result) && (
    Object.hasOwn(result, "statusCode") || Object.hasOwn(result, "headers") || Object.hasOwn(result, "body")
  ) ? result : { statusCode: 200, body: result };
  const statusCode: any = Number(envelope.statusCode || 200) || 200;
  const headers: any = isPlainObject(envelope.headers) ? envelope.headers : {};
  const body: any = envelope.body === undefined ? null : envelope.body;
  if (Buffer.isBuffer(body) || typeof body === "string") {
    response.writeHead(statusCode, headers);
    response.end(body);
    return;
  }
  for (const [name, value] of (Object.entries(headers) as [string, any][])) response.setHeader?.(name, value);
  sendJson(response, statusCode, body);
}

function methodFacade(source?: any, methodNames?: any) : any {
  const facade: Record<string, any> = {};
...
function projectHostPort(name?: any, value?: any) : any {
...
export function createPluginContributionController({
  ...
  const ports: any = Object.freeze(Object.fromEntries(
    (Object.entries(hostPorts) as [string, any][]).map(([name, value]: any[]) : any => [name, projectHostPort(name, value)])
  ));

  return Object.freeze({
    async executePluginOperation(call: Record<string, any> = {}) : Promise<any> {
      const record: any = typeof registry.operations?.get === "function"
      ...
      const opaqueInput: any = await preprocessPluginOpaqueInputs(
        record,
        call.input || {},
        call,
      ...
      const host: any = selectHostPorts(record, ports, call, hostPorts);
      const signal: any = call.signal || call.operationLock?.signal || null;
      const baseProjection: any = createPluginCallProjection(call);
      let invocationAuthorization: any = "";
      ...
      const result: any = await record.implementation.execute({
    ...
    async verifyPluginExternalAuth(call: Record<string, any> = {}) : Promise<any> {
      const record: any = typeof registry.operations?.get === "function"
        ? registry.operations.get(call.operation?.id) || null
        : registry.requireOperation?.(call.operation?.id) || null;
      ...
      const host: any = selectHostPorts(record, ports, call);
