 ...
 */
export async function registerSecurityProviders({ userDataPath }: Record<string, any>) : Promise<any> {
  ...
  const { createTagStoreAdapter } = await import(
    "../state/tags/tag-store.adapter.ts"
  );
  ...
  const adapter: any = createTagStoreAdapter({ userDataPath });
