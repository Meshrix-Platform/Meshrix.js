...
export const UPLOAD_SESSION_MAX_FILE_COUNT: any = 256;
export const UPLOAD_SESSION_MAX_FILE_BYTES: any = 512 * 1024 * 1024;
export const UPLOAD_SESSION_MAX_TOTAL_BYTES: any = 2 * 1024 * 1024 * 1024;
export const UPLOAD_SESSION_MAX_CHUNK_BYTES: any = 8 * 1024 * 1024;
export const UPLOAD_SESSION_MAX_SOURCE_METADATA_BYTES: any = 64 * 1024;
export const UPLOAD_SESSION_MAX_ACTIVE_PER_OWNER_SCOPE: any = 8;
export const UPLOAD_SESSION_MAX_RETAINED: any = 4_096;
export const UPLOAD_SESSION_TTL_MS: any = 24 * 60 * 60 * 1_000;
export const UPLOAD_SESSION_EXPIRY_CLEANUP_BATCH: any = 32;

const DATABASE_FILE_NAME: any = "admission.sqlite";

export function uploadAdmissionError(code?: any, statusCode?: any, message?: any) : any {
...
export function validateUploadSessionDeclaration(files?: any) : any {
  ...
  for (const [index, file] of files.entries()) {
    const byteSize: any = Number(file?.byteSize || 0);
    ...
    const sourceMetadata: any = file?.sourceMetadata;
...
function openAdmissionDatabase(userDataPath?: any) : any {
  const databasePath: any = path.join(userDataPath, "upload-sessions", DATABASE_FILE_NAME);
  ...
  for (const suffix of ["-wal", "-shm"]) {
    const auxiliaryPath: any = `${databasePath}${suffix}`;
...
function withImmediateTransaction(database?: any, operation?: any) : any {
...
export function listExpiredUploadSessionAdmissions(
  userDataPath?: any,
  nowMs: any = Date.now(),
  limit: any = UPLOAD_SESSION_EXPIRY_CLEANUP_BATCH
) : any {
  const database: any = openAdmissionDatabase(userDataPath);
  try {
...
export function reserveUploadSessionAdmission({
  ...
  const database: any = openAdmissionDatabase(userDataPath);
  try {
    return withImmediateTransaction(database, () : any => {
      ...
      const retainedCount: any = database.prepare(
      ...
      if (status === "uploading") {
        const activeCount: any = database.prepare(`
          SELECT COUNT(*) AS count FROM upload_session_admission
      ...
      const expiresAtMs: any = nowMs + UPLOAD_SESSION_TTL_MS;
      database.prepare(`
...
export function readUploadSessionAdmission(userDataPath?: any, sessionId?: any, nowMs: any = Date.now()) : any {
  const database: any = openAdmissionDatabase(userDataPath);
  try {
...
export function updateUploadSessionAdmissionStatus(userDataPath?: any, sessionId?: any, status?: any) : any {
  const database: any = openAdmissionDatabase(userDataPath);
  try {
    database.prepare(
...
export function deleteUploadSessionAdmission(userDataPath?: any, sessionId?: any) : any {
  const database: any = openAdmissionDatabase(userDataPath);
  try {
