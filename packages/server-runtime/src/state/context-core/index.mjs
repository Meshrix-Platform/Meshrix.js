...
import { assertContextProfileComplete, normalizeProfiles } from "./profile.mjs";
import {
...
import { createContextCoreStorage } from "./storage.mjs";

export const CONTEXT_RUNTIME_PROTOCOL_VERSION = "v0.0.1:agent:context-1";
export { estimateTokens } from "./validation.mjs";
...
export function createContextRuntime({
  ...
  async function compact(input = {}) {
    ...
    const requestedTarget = input.targetTokens === undefined || input.targetTokens === null || input.targetTokens === ""
  ...
  async function assemble(input = {}) {
    ...
    let compressionDroppedReferenceIds = [];
    if (profile.compression.enabled === true && totalTokens > thresholdTokens) {
      ...
      const nextReferenceBudget = Math.max(0, Math.floor(budgets.reference * profile.compression.targetRatio));
      const nextReference = selectEvidenceByBudget(pack.evidencePack, nextReferenceBudget, protectedEvidenceIds);
      pack.evidencePack = nextReference.selected;
      compressionDroppedReferenceIds = nextReference.dropped.map((entry) => entry.item.evidenceId).filter(Boolean);
  ...
  async function runEvaluation(input = {}) {
    const requestedProfiles = asArray(input.profiles);
    ...
    for (const profileRef of profiles) {
      const contextProfileId = typeof profileRef === "string" ? profileRef : profileRef.profileId;
