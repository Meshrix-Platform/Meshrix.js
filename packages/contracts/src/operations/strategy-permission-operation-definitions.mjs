const stringValue = Object.freeze({ type: "string", minLength: 1, maxLength: 512 });
const booleanValue = Object.freeze({ type: "boolean" });
const numberValue = Object.freeze({ type: "number" });
...
const safetyInput = closedObject({
  risk: stringValue,
  blocked: booleanValue,
  requiresConfirmation: booleanValue
});
const operationInput = closedObject({ id: stringValue, safety: safetyInput });
const strategyCommonInput = Object.freeze({
  risk: stringValue,
  blocked: booleanValue,
  operationId: stringValue,
  operation: operationInput
});
const arbitraryToolInput = Object.freeze({ type: "object" });
const toolExecutionInput = closedObject({
  toolId: stringValue,
  input: arbitraryToolInput,
  confirm: booleanValue,
  idempotencyKey: stringValue
}, ["toolId"]);
const toolBatchCallInput = closedObject({
  toolId: stringValue,
  input: arbitraryToolInput,
  confirm: booleanValue,
  idempotencyKey: stringValue
}, ["toolId"]);
const toolBatchInput = closedObject({
  calls: Object.freeze({ type: "array", items: toolBatchCallInput, minItems: 1, maxItems: 100 }),
  atomic: booleanValue
}, ["calls"]);
const grantMutationInput = closedObject({
  grantId: stringValue,
  label: stringValue,
  type: stringValue,
...
export const STRATEGY_PERMISSION_OPERATION_DEFINITIONS = Object.freeze([
