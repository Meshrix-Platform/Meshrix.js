import { PROTOCOL_OPERATION_DEFINITIONS } from "./protocol-operation-definitions.ts";
import { listStaticSemanticFamilyPublicSummaries } from "./static-semantic-family-catalog.ts";

export const STATIC_SEMANTIC_FAMILY_COUNT: any = listStaticSemanticFamilyPublicSummaries().length;

function parseOperationBody(value?: any) : any {
...
function isReadOnlyPrompt(context: Record<string, any> = {}) : any {
  const input: Record<string, any> = {
    ...parseOperationBody(context.requestBody),
    ...(context.params && typeof context.params === "object" ? context.params : {})
  };
  const mode: any = String(input.mode || input.promptMode || input.askMode || "").trim().toLowerCase();
...
export function resolveAcpPromptRisk(context: Record<string, any> = {}) : any {
  return isReadOnlyPrompt(context) ? "read_only" : "repair_write";
}

const UPSTREAM_GATEWAY_SERVICE_ID_PARAMS: readonly any[] = Object.freeze([
  { name: "serviceId", aliases: ["service-id", "serviceId", "upstreamId", "upstream-id", "id"], required: true }
]);

function upstreamGatewayInputSchema(required: any = []) : any {
  return {
    type: "object",
    required,
...
const UPSTREAM_GATEWAY_OPERATION_DEFINITIONS: readonly any[] = Object.freeze([
...
const SECURITY_ALERT_OPERATION_DEFINITIONS: readonly any[] = Object.freeze([
  {
...
export const OPERATION_REGISTRY_GOVERNED_DEFINITIONS: readonly any[] = Object.freeze([
