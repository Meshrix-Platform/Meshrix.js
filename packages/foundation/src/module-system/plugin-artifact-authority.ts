...
const ENVELOPE_SCHEMA: any = "meshrix.plugin-artifact-envelope/1";
const INSTALL_JOURNAL_SCHEMA: any = "meshrix.plugin-artifact-install-journal/1";
const REMOVAL_JOURNAL_SCHEMA: any = "meshrix.plugin-artifact-removal-journal/1";
const CURRENT_SCHEMA: any = "meshrix.plugin-artifact-current/1";
const TOMBSTONE_SCHEMA: any = "meshrix.plugin-artifact-tombstone/1";
const METADATA_FILE: any = "plugin-artifact.json";
const MAX_FILE_COUNT: any = 4096;
const MAX_FILE_BYTES: any = 16 * 1024 * 1024;
const MAX_TOTAL_BYTES: any = 64 * 1024 * 1024;
const MAX_RECORD_BYTES: any = 256 * 1024;
const MAX_DIRECTORY_DEPTH: any = 32;
const MAX_PATH_BYTES: any = 1024;
const MAX_PLUGIN_COUNT: any = 1024;
...
function generation(value?: any) : any {
...
function requiredDigest(value?: any, label?: any) : any {
...
export function pluginOwnerGenerationDigest({
  ...
  const artifactDigest: any = requiredDigest(rawArtifactDigest, "Plugin artifact owner digest");
  const artifactGeneration: any = generation(rawGeneration);
...
function exactObject(value?: any, fields?: any, label?: any) : any {
  if (!value || typeof value !== "object" || Array.isArray(value) ||
      Object.keys(value).sort().some((field?: any, index?: any) : any => field !== [...fields].sort()[index]) ||
...
async function ensureRealDirectory(directory?: any, parent: any = null) : Promise<any> {
...
async function existingRealDirectory(directory?: any) : Promise<any> {
  const resolved: any = await fs.realpath(directory);
  const stat: any = await fs.lstat(directory);
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
...
async function readBoundedJson(filePath?: any, { missing = null }: Record<string, any> = {}) : Promise<any> {
  let handle: any;
  try {
...
async function durableJson(filePath?: any, value?: any, lifecycleStatePort?: any) : Promise<any> {
  const bytes: any = Buffer.from(canonicalJson(value), "utf8");
  if (bytes.byteLength > MAX_RECORD_BYTES) throw new TypeError("Plugin artifact record exceeds its bounded size.");
  const temporary: any = `${filePath}.tmp-${crypto.randomUUID()}`;
  let handle: any;
  try {
...
async function regularFileBytes(filePath?: any) : Promise<any> {
  let handle: any;
  try {
    handle = await fs.open(filePath, fsConstants.O_RDONLY | (fsConstants.O_NOFOLLOW || 0));
    const stat: any = await handle.stat();
...
async function inventory(root?: any, relative: any = "", accumulator: Record<string, any> = { files: [], total: 0, directories: 0 }, depth: any = 0) : Promise<any> {
  ...
  const directory: any = relative ? path.join(root, relative) : root;
  const directoryStat: any = await fs.lstat(directory);
  ...
  for (const entry of entries.sort((left?: any, right?: any) : any => left.name.localeCompare(right.name))) {
    const child: any = safeName(relative ? `${relative}/${entry.name}` : entry.name);
    ...
    const bytes: any = await regularFileBytes(path.join(root, child));
...
async function copyTree(source?: any, target?: any) : Promise<any> {
  await ensureRealDirectory(target, path.dirname(target));
  for (const entry of await fs.readdir(source, { withFileTypes: true })) {
    const name: any = safeName(entry.name);
    const from: any = path.join(source, name);
    const to: any = path.join(target, name);
    ...
    if (entry.isDirectory()) await copyTree(from, to);
      ...
      const bytes: any = await regularFileBytes(from);
      let handle: any;
      try {
...
async function durableFile(filePath?: any, bytes?: any) : Promise<any> {
  let handle: any;
  try {
...
async function sealTree(directory?: any) : Promise<any> {
...
async function makeWritableTree(directory?: any) : Promise<any> {
  if (process.platform !== "win32") await fs.chmod(directory, 0o700);
  for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
    const target: any = path.join(directory, entry.name);
...
function normalizeDependencyClosure(value?: any) : any {
  ...
  }).sort((left?: any, right?: any) : any => left.pluginId.localeCompare(right.pluginId)));
}

function unsignedEnvelope(envelope?: any) : any {
  return Object.freeze({
...
function validateEnvelopeShape(value?: any, expectedPluginId: any = "") : any {
  exactObject(value, [
...
async function verifyEnvelope({ contentRoot, envelopePath, trustedPublicKeys, expectedPluginId = "" }: Record<string, any>) : Promise<any> {
  ...
  const envelope: any = validateEnvelopeShape(await readBoundedJson(envelopePath), expectedPluginId);
  const actualFiles: any = await inventory(contentRoot);
  ...
  const expectedDigest: any = digest(canonicalJson(unsignedEnvelope(envelope)));
  ...
  const signature: any = exactObject(envelope.signature, [
    "algorithm", "payloadEncoding", "keyId", "payloadDigest", "contextDigest", "signedEnvelope", "value"
  ], "Plugin artifact signature");
  const key: any = trustedKey(trustedPublicKeys, signature.keyId);
  ...
  const expectedSignedEnvelope: Readonly<Record<string, any>> = Object.freeze({
...
function validateLifecyclePort(port?: any, expectedPluginId?: any) : any {
...
function currentRecord(value?: any, expectedPluginId?: any) : any {
...
export async function createPluginArtifactAuthority({
  ...
  const boundCoreContractDigest: any = requiredDigest(coreContractDigest, "Plugin artifact Core contract digest");
  const root: any = await ensureRealDirectory(path.resolve(artifactRoot));
  const bundlesRoot: any = await ensureRealDirectory(path.join(root, "bundles"), root);
  const installedRoot: any = await ensureRealDirectory(path.join(root, "installed"), root);
  const currentRoot: any = await ensureRealDirectory(path.join(root, "current"), root);
  const tombstoneRoot: any = await ensureRealDirectory(path.join(root, "tombstones"), root);
  const fault: any = async (phase?: any) : Promise<any> => { if (typeof faultInjector === "function") await faultInjector(phase); };

  async function verifyInstalledContent(record?: any) : Promise<any> {
    const id: any = pluginId(record.pluginId);
    const contentRoot: any = path.join(installedRoot, id, String(record.generation), "content");
    const envelopePath: any = path.join(installedRoot, id, String(record.generation), METADATA_FILE);
    const verified: any = await verifyEnvelope({ contentRoot, envelopePath, trustedPublicKeys, expectedPluginId: id });
  ...
  function installedSnapshot(record?: any, verified?: any) : any {
    const facts: Readonly<Record<string, any>> = Object.freeze({
    ...
    return Object.freeze({
      ...
      async verify() : Promise<any> {
        const verified: any = await verifyInstalledContent(record);
        return Object.freeze({
          ...facts,
          coreContractDigest: verified.coreContractDigest,
          dependencyClosure: verified.dependencyClosure
      ...
      async readFile(filePath?: any) : Promise<any> {
        const verified: any = await verifyInstalledContent(record);
        const relative: any = safeName(String(filePath || "").replace(/^\.\//u, ""));
        const candidate: any = path.join(verified.contentRoot, relative);
        const bytes: any = await regularFileBytes(candidate);
        const resolved: any = await fs.realpath(candidate);
        const within: any = path.relative(verified.contentRoot, resolved);
        if (!within || within.startsWith("..") || path.isAbsolute(within)) {
          throw controlledError("PLUGIN_ARTIFACT_PATH_INVALID", "Plugin artifact file escaped immutable content.");
        }
        return Buffer.from(bytes);
      },
      async resolveRuntimeModule(modulePath?: any) : Promise<any> {
        const verified: any = await verifyInstalledContent(record);
        const relative: any = safeName(String(modulePath || "").replace(/^\.\//u, ""));
        const candidate: any = path.join(verified.contentRoot, relative);
        const bytes: any = await regularFileBytes(candidate);
  ...
  async function loadCurrent(id?: any) : Promise<any> {
    const value: any = await readBoundedJson(path.join(currentRoot, `${pluginId(id)}.json`));
    if (!value) return null;
    return currentRecord(value, id);
  }

  function forPlugin({ pluginId: rawPluginId, lifecycleStatePort }: Record<string, any> = {}) : any {
    const id: any = pluginId(rawPluginId);
    const statePort: any = validateLifecyclePort(lifecycleStatePort, id);
    const bundlePluginRoot: any = path.join(bundlesRoot, id);
    const installedPluginRoot: any = path.join(installedRoot, id);
    const currentPath: any = path.join(currentRoot, `${id}.json`);

    async function retireSupersededCode(keepGeneration?: any, keepDigest?: any) : Promise<any> {
      for (const entry of await fs.readdir(installedPluginRoot, { withFileTypes: true })) {
        if (entry.name === String(keepGeneration)) continue;
        const retired: any = path.join(installedPluginRoot, entry.name);
      ...
      for (const entry of await fs.readdir(bundlePluginRoot, { withFileTypes: true })) {
        ...
        const retired: any = path.join(bundlePluginRoot, entry.name);
    ...
    return Object.freeze({
      ...
      pluginId: id,
      async publish({ sourceRoot, version, generation: rawGeneration, dependencyClosure = [] }: Record<string, any> = {}) : Promise<any> {
        ...
        const artifactGeneration: any = generation(rawGeneration);
        const dependencies: any = normalizeDependencyClosure(dependencyClosure);
        if (typeof sourceRoot !== "string" || !sourceRoot.trim()) throw new TypeError("Plugin artifact source root is required.");
        const source: any = await existingRealDirectory(path.resolve(sourceRoot));
        const rawManifest: any = await readBoundedJson(path.join(source, "plugin.json"));
        if (rawManifest?.id !== id || String(rawManifest?.version || "").trim() !== String(version || "").trim() ||
            canonicalJson([...(rawManifest?.dependencies || [])].sort()) !== canonicalJson(dependencies.map((entry?: any) : any => entry.pluginId))) {
        ...
        if (!files.some((entry?: any) : any => entry.path === "plugin.json") || !files.some((entry?: any) : any => entry.path === runtimeModule)) {
          throw controlledError("PLUGIN_ARTIFACT_CONTENT_INVALID", "Plugin artifact requires its declared runtime module.");
        }
        const unsigned: Readonly<Record<string, any>> = Object.freeze({
        ...
        const artifactDigest: any = digest(canonicalJson(unsigned));
        const context: Readonly<Record<string, any>> = Object.freeze({
          pluginId: id,
        ...
        const signed: any = await artifactSigner.sign({
        ...
        const envelope: Readonly<Record<string, any>> = Object.freeze({
          ...unsigned,
          artifactDigest,
        ...
        const pluginBundles: any = await ensureRealDirectory(bundlePluginRoot, bundlesRoot);
        const target: any = path.join(pluginBundles, artifactDigest.slice("sha256:".length));
        const staging: any = path.join(pluginBundles, `.publish-${crypto.randomUUID()}`);
        ...
        try {
          ...
          const pendingEntries: any = await fs.readdir(pluginBundles, { withFileTypes: true });
          ...
          for (const entry of pendingEntries) {
            ...
            const retired: any = path.join(pluginBundles, entry.name);
          ...
          await fs.rm(staging, { recursive: true, force: true }).catch(() : any => {});
        }
      },
      async install({ artifactDigest: rawDigest, generation: rawGeneration, operation = "install", expectedCurrent = null }: Record<string, any> = {}) : Promise<any> {
        const expectedDigest: any = requiredDigest(rawDigest, "Plugin artifact install digest");
        const expectedGeneration: any = generation(rawGeneration);
        if (!["install", "update", "reinstall"].includes(operation)) throw new TypeError("Plugin artifact install operation is invalid.");
        return statePort.runExclusive(async () : Promise<any> => {
          const currentBefore: any = await loadCurrent(id);
          const ledgerBefore: any = await statePort.readRecord("ledger");
          const expectedPrevious: any = expectedCurrent && typeof expectedCurrent === "object"
            ...
            : null;
          const latestTombstone: any = await readBoundedJson(path.join(tombstoneRoot, `${id}.json`));
          if (latestTombstone) {
            exactObject(latestTombstone, ["schemaVersion", "pluginId", "generation", "artifactDigest", "removed"], "Plugin artifact tombstone");
          }
          const priorInstallJournal: any = await statePort.readRecord("artifact-install-journal");
          const highWater: any = Math.max(0, currentBefore?.generation || 0, Number(priorInstallJournal?.generation || 0), Number(latestTombstone?.generation || 0));
          const derivedOperation: any = !currentBefore && !ledgerBefore && highWater === 0
            ? "install"
          ...
          const bundle: any = path.join(bundlePluginRoot, expectedDigest.slice("sha256:".length));
          const verifiedBundle: any = await verifyEnvelope({
          ...
          const pluginInstallRoot: any = await ensureRealDirectory(installedPluginRoot, installedRoot);
          const target: any = path.join(pluginInstallRoot, String(expectedGeneration));
          const staging: any = path.join(pluginInstallRoot, `.install-${transactionId}`);
          ...
          try {
            ...
            if (!["EEXIST", "ENOTEMPTY"].includes(error?.code)) throw error;
            const existing: any = await verifyEnvelope({ contentRoot: path.join(target, "content"), envelopePath: path.join(target, METADATA_FILE), trustedPublicKeys, expectedPluginId: id });
          ...
          const current: Readonly<Record<string, any>> = Object.freeze({
      ...
      async recoverInstall() : Promise<any> {
        return statePort.runExclusive(async () : Promise<any> => {
          const journal: any = await statePort.readRecord("artifact-install-journal");
          ...
          const target: any = path.join(installedPluginRoot, String(generation(journal.generation)));
          try {
            await fs.lstat(target);
          } catch (error: any) {
            if (error?.code !== "ENOENT" || journal.phase !== "prepared") throw error;
            const bundle: any = path.join(bundlePluginRoot, String(journal.artifactDigest).slice("sha256:".length));
            const staging: any = path.join(installedPluginRoot, `.recover-install-${journal.transactionId}`);
          ...
          const verified: any = await verifyEnvelope({ contentRoot: path.join(target, "content"), envelopePath: path.join(target, METADATA_FILE), trustedPublicKeys, expectedPluginId: id });
          ...
          const current: Readonly<Record<string, any>> = Object.freeze({ schemaVersion: CURRENT_SCHEMA, pluginId: id, version: verified.envelope.version,
      ...
      async remove({ expectedArtifactDigest, expectedGeneration }: Record<string, any> = {}) : Promise<any> {
        const expectedDigest: any = requiredDigest(expectedArtifactDigest, "Plugin artifact removal digest");
        const expectedArtifactGeneration: any = generation(expectedGeneration);
        return statePort.runExclusive(async () : Promise<any> => {
          const tombstonePath: any = path.join(tombstoneRoot, `${id}.json`);
          const existingTombstone: any = await readBoundedJson(tombstonePath);
          const current: any = await loadCurrent(id);
          ...
          const journal: Readonly<Record<string, any>> = Object.freeze({ schemaVersion: REMOVAL_JOURNAL_SCHEMA, recordType: "artifact_removal", pluginId: id,
          ...
          const target: any = path.join(installedPluginRoot, String(expectedArtifactGeneration));
          const verified: any = await verifyEnvelope({ contentRoot: path.join(target, "content"), envelopePath: path.join(target, METADATA_FILE), trustedPublicKeys, expectedPluginId: id });
          ...
          const tombstone: Readonly<Record<string, any>> = Object.freeze({ schemaVersion: TOMBSTONE_SCHEMA, pluginId: id, generation: expectedArtifactGeneration,
      ...
      async recoverRemoval() : Promise<any> {
        return statePort.runExclusive(async () : Promise<any> => {
          const journal: any = await statePort.readRecord("artifact-removal-journal");
          if (!journal || journal.phase === "completed") return Object.freeze({ ok: true, recovered: false });
          ...
          const expectedDigest: any = requiredDigest(journal.artifactDigest, "Plugin artifact removal digest");
          const expectedGeneration: any = generation(journal.generation);
          const target: any = path.join(installedPluginRoot, String(expectedGeneration));
          let targetPresent: any = false;
          try {
            await fs.lstat(target);
            targetPresent = true;
          } catch (error: any) {
      ...
      async loadSnapshot() : Promise<any> {
        const record: any = await loadCurrent(id);
  ...
  return Object.freeze({
    ...
    async discover() : Promise<any> {
      const entries: any = await fs.readdir(currentRoot, { withFileTypes: true });
      ...
      for (const entry of entries.sort((left?: any, right?: any) : any => left.name.localeCompare(right.name))) {
        ...
        const id: any = pluginId(entry.name.slice(0, -5));
        const record: any = currentRecord(await readBoundedJson(path.join(currentRoot, entry.name)), id);
        const verified: any = await verifyInstalledContent(record);
...
export const PLUGIN_ARTIFACT_METADATA_FILE: any = METADATA_FILE;
export const PLUGIN_ARTIFACT_ENVELOPE_SCHEMA: any = ENVELOPE_SCHEMA;
