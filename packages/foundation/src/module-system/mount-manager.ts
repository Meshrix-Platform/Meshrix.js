...
import { activatePluginDeployment } from "./plugin-runtime.ts";
import { createPluginDataCapability } from "./plugin-data-capability.ts";
import {
  HOST_OPAQUE_PAYLOAD_TTL_MS,
  createHostOpaquePayloadCustody
} from "./opaque-payload-custody.ts";
import { createPluginLifecycleStatePort } from "./plugin-lifecycle-state-port.ts";
import { normalizePluginArtifactTrustedPublicKeys } from "./plugin-artifact-trust.ts";
import { createIsolatedPluginProcessHost } from "./isolated-plugin-process-host.ts";
...
function normalizeProfile(value?: any) : any {
...
function immutableConfigurationSnapshot(value?: any, seen: any = new WeakSet<object>()) : any {
  if (value === null || typeof value === "string" || typeof value === "boolean") return value;
  if (typeof value === "number" && Number.isFinite(value)) return value;
  if (Array.isArray(value)) {
    if (seen.has(value)) throw new TypeError("Plugin configuration must not contain cycles.");
    seen.add(value);
    const snapshot: any = value.map((entry?: any) : any => immutableConfigurationSnapshot(entry, seen));
    seen.delete(value);
  ...
  if (value && typeof value === "object") {
    ...
    for (const [key, entry] of (Object.entries(value) as [string, any][])) {
...
export function normalizeRuntimeOptions(runtimeOptions: Record<string, any> = {}) : any {
  const testHooks: any =
    runtimeOptions.testHooks && typeof runtimeOptions.testHooks === "object"
      ? { jobDelayMs: Number(runtimeOptions.testHooks.jobDelayMs || 0) }
      : {};
  const cwd: any = String(runtimeOptions.cwd || process.cwd()).trim();
  if (!cwd) throw new TypeError("Runtime cwd is required.");
  const pluginConfigurations: any = runtimeOptions.pluginConfigurations === undefined
    ? {}
  ...
  const normalizedPluginConfigurations: Record<string, any> = {};
  for (const [pluginId, configuration] of (Object.entries(pluginConfigurations) as [string, any][])) {
...
function mountModuleField(entry?: any, field?: any, fallback: any = "") : any {
  return typeof entry === "object" && entry
    ? String(entry[field] || fallback || "").trim()
    : String(fallback || "").trim();
}

function createConfiguredMounts(runtimeOptions: Record<string, any> = {}, builtinMountProviders: Record<string, any> = {}) : any {
  const mounts: Record<string, any> = {};
  for (const [name, entry] of (Object.entries(runtimeOptions.mountModules || {}) as [string, any][])) {
    const enabled: any = typeof entry === "object" && entry ? entry.enabled !== false : true;
    if (!enabled) continue;
    const pluginId: any = mountModuleField(entry, "pluginId");
    if (pluginId) {
      throw new Error(`Configured mount ${name} cannot claim plugin ownership; plugin mounts come from plugin manifests.`);
    }
    const providerId: any = mountModuleField(entry, "provider") || mountModuleField(entry, "kind", name);
    const provider: any = builtinMountProviders[name] || builtinMountProviders[providerId] || null;
...
function mergeDistinctRouting(base: Record<string, any> = {}, patch: Record<string, any> = {}) : any {
  const left: any = normalizeMountRouting(base);
  const right: any = normalizeMountRouting(patch);
  const output: Record<string, any> = { kindRoutes: {}, extensionRoutes: {}, mediaTypeRoutes: {} };
  for (const routeType of Object.keys(output)) {
    output[routeType] = { ...left[routeType] };
    for (const [key, target] of (Object.entries(right[routeType]) as [string, any][])) {
...
function combineMounts(pluginMounts?: any, configuredMounts?: any) : any {
  const output: Record<string, any> = { ...pluginMounts };
  for (const [name, mount] of (Object.entries(configuredMounts) as [string, any][])) {
...
function validateRouteTargets(routing?: any, mounts?: any) : any {
...
function routeForInput(routing?: any, input: Record<string, any> = {}) : any {
  const kind: any = String(input.kind || input.sourceKind || "").trim();
  const extension: any = String(input.extension || input.ext || "").trim().toLowerCase();
  const normalizedExtension: any = extension
    ? extension.startsWith(".") ? extension : `.${extension}`
    : "";
  const mediaType: any = String(input.mediaType || input.mimeType || "").trim().toLowerCase();
...
async function ensureRealDirectory(directory: any, { parentRealPath = "", label }: Record<string, any>) : Promise<any> {
  try {
  ...
  if (process.platform !== "win32") {
    ...
    const secured: any = await fs.lstat(directory);
    if ((secured.mode & 0o777) !== 0o700) {
      throw new Error(`${label} permissions could not be restricted.`);
    }
  }
  const realPath: any = await fs.realpath(directory);
  if (parentRealPath) {
...
async function preparePluginDataPath(userDataPath?: any, pluginId?: any) : Promise<any> {
  const dataRoot: any = await fs.realpath(path.resolve(userDataPath));
  const pluginRoot: any = await ensureRealDirectory(path.join(dataRoot, "plugins"), {
...
async function preparePluginDataCapability(userDataPath?: any, pluginId?: any) : Promise<any> {
  return createPluginDataCapability(await preparePluginDataPath(userDataPath, pluginId));
}

function createExecutionView(runtimeOptions?: any, pluginRuntime?: any, builtinMountProviders?: any) : any {
  const configuredMounts: any = createConfiguredMounts(runtimeOptions, builtinMountProviders);
  const mounts: any = combineMounts(pluginRuntime.mounts, configuredMounts);
  const routing: any = mergeDistinctRouting(pluginRuntime.mountRouting, runtimeOptions.mountRouting);
  validateRouteTargets(routing, mounts);
  const postCommitHooks: any[] = [...new Set<any>((Object.values(mounts) as any[])
    .flatMap((mount?: any) : any => [mount.onPostCommit, mount.postCommitHook, mount.onBatchCompleted])
    .filter((hook?: any) : any => typeof hook === "function"))];
  return Object.freeze({
    ...
    runtimeOptions,
    resolveDocumentRoute(input: Record<string, any> = {}) : any {
      const route: any = routeForInput(routing, input);
...
function assertMountConfigShape(value?: any) : any {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new TypeError("Mount config must be an object.");
  }
  for (const field of Object.keys(value)) {
    if (!new Set<any>(["mountModules", "mountRouting"]).has(field)) {
...
export async function createMountManager({
  ...
  let normalizedRuntimeOptions: any = normalizeRuntimeOptions(runtimeOptions);
  const persistedMountConfig: any = await loadMountConfig(userDataPath);
  const artifactAuthority: any = pluginHostPorts.artifactAuthority || null;
  ...
  if (artifactAuthority?.id === "PluginArtifactAuthority") {
    ...
    for (const id of selectedIds) {
      ...
      if (ledger?.state === "removal_pending" && ledger.operation === "uninstall" &&
        ...
        await lifecycleStatePort.runExclusive(() : any => lifecycleStatePort.writeRecord("ledger", {
          ...ledger,
          state: "uninstalled"
        }));
  ...
  if (retiredPluginIds.size > 0) {
    normalizedRuntimeOptions = normalizeRuntimeOptions({
      ...
      enabledPlugins: normalizedRuntimeOptions.enabledPlugins.filter((id?: any) : any => !retiredPluginIds.has(id)),
      pluginConfigurations: Object.fromEntries((Object.entries(normalizedRuntimeOptions.pluginConfigurations) as [string, any][])
        .filter(([id]: any[]) : any => !retiredPluginIds.has(id)))
    });
  }
  const pluginDeployment: any = providedPluginDeployment || (await loadPluginRegistry({
    artifactAuthority
  })).resolveDeployment({
  ...
  const expectedPluginIds: any = [...normalizedRuntimeOptions.enabledPlugins].sort();
  if (
    expectedPluginIds.length !== pluginDeployment.enabledPluginIds.length ||
    expectedPluginIds.some((id?: any, index?: any) : any => id !== pluginDeployment.enabledPluginIds[index])
  ) {
    throw new Error("Resolved plugin deployment does not match runtime.enabledPlugins.");
  }
  const expectedConfiguredPluginIds: any = Object.keys(normalizedRuntimeOptions.pluginConfigurations).sort();
  if (
    expectedConfiguredPluginIds.length !== pluginDeployment.configuredPluginIds.length ||
    expectedConfiguredPluginIds.some((id?: any, index?: any) : any => id !== pluginDeployment.configuredPluginIds[index])
  ) {
  ...
  let currentRuntimeOptions: any;
  let generation: any = 1;
  let mutationTail: any = Promise.resolve();
  let closing: any = false;
  let closePromise: any = null;
  let observedPluginLifecycleGeneration: any = 1;
  const lifecycleListeners: any = new Set<any>();
  const lifecycleAdmissions: any = new Map<any, any>();
  ...
  try {
    pluginRuntime = await activatePluginDeployment({
      ...
      createContext: async (manifest?: any) : Promise<any> => {
        const lifecycleStatePort: any = await createPluginLifecycleStatePort({ userDataPath, pluginId: manifest.id });
        ...
        const clock: any = typeof pluginHostPorts.pluginClockForPlugin === "function"
        ...
        ...(typeof registerPluginRuntimeMeasurementSource === "function"
          ? {
              registerRuntimeMeasurementSource(provider?: any) : any {
  ...
  function assertOpen() : any {
    if (closing) throw new Error("Mount manager is closing.");
  }

  function enqueueMutation(mutation?: any) : any {
    assertOpen();
    const task: any = mutationTail.then(mutation);
    mutationTail = task.catch(() : any => {});
    return task;
  }
  ...
  return Object.freeze({
    ...
    applyMountConfig(config: Record<string, any> = {}) : any {
      assertMountConfigShape(config);
      return enqueueMutation(async () : Promise<any> => {
        const candidate: any = normalizeRuntimeOptions({
        ...
        const view: any = createExecutionView(candidate, pluginRuntime, builtinMountProviders);
    ...
    reloadMounts() : any {
      return enqueueMutation(async () : Promise<any> => {
        const view: any = createExecutionView(currentRuntimeOptions, pluginRuntime, builtinMountProviders);
        generation += 1;
        return view;
      });
    },
    async refreshMounts() : Promise<any> {
      assertOpen();
      await mutationTail;
      assertOpen();
      return createExecutionView(currentRuntimeOptions, pluginRuntime, builtinMountProviders);
    ...
    transitionPluginLifecycle(request: Record<string, any> = {}) : any {
      const expectedGeneration: any = Number(request.expectedGeneration);
      if (!Number.isSafeInteger(expectedGeneration) || expectedGeneration !== generation) {
        const error: Error & Record<string, any> = new Error("Mount manager lifecycle generation fence does not match.");
        error.code = "PLUGIN_LIFECYCLE_FENCE_MISMATCH";
        error.generation = generation;
        throw error;
      }
      const admissionKey: any = canonicalJson([
      ...
      const admission: any = enqueueMutation(async () : Promise<any> => {
        ...
        try {
          ...
          try {
            for (const listener of lifecycleListeners) {
              const artifactGenerationDigest: any = pluginRuntime.getPluginArtifactGenerationDigest(
                String(request.pluginId || "").trim()
              );
              const transaction: any = await listener.prepare(Object.freeze({
            ...
            const listenerError: Error & Record<string, any> = new Error("Plugin lifecycle admission update did not complete.");
            listenerError.code = "PLUGIN_LIFECYCLE_ADMISSION_UPDATE_FAILED";
            listenerError.generation = generation;
            throw listenerError;
          }
          const transition: any = pluginRuntime.transitionPluginLifecycle({
            ...request,
            expectedGeneration: pluginRuntime.lifecycleGeneration,
            async commitAdmission() : Promise<any> {
              try {
          ...
          const receipt: any = await transition;
      ...
      }).finally(() : any => lifecycleAdmissions.delete(admissionKey));
    ...
    onPluginLifecycleTransition(listener?: any) : any {
      ...
      return () : any => lifecycleListeners.delete(listener);
    },
    ...
    close() : any {
      ...
      closing = true;
      closePromise = mutationTail.then(() : any => pluginRuntime.close()).catch((error?: any) : any => {
        closePromise = null;
        throw error;
      });
      return closePromise;
