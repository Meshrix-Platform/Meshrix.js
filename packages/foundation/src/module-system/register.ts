...
export function registerModuleManagementPlatformServices(registry?: any, {
  ...
  const runtimeState: any = moduleManagement?.getRuntimeState
    ? moduleManagement.getRuntimeState()
    : {
        profile: runtimeOptions?.profile || "",
        mountNames: Object.keys(runtime?.mounts || {})
      };
  const mountList: any = moduleManagement?.listMounts
    ? moduleManagement.listMounts()
    : (Object.entries(runtime?.mounts || {}) as [string, any][]).map(([name, mount]: any[]) : any => ({
        name,
        id: mount?.id || "",
  ...
  const mountNames: any = mountList.map((mount?: any) : any => mount.name).filter(Boolean);
  const architectureComponents: any = moduleManagement?.getArchitectureComponentInventory
