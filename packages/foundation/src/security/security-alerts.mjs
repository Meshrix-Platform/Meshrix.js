...
} from "../observability/alert-service.mjs";
import {
  ALERT_LIFECYCLE_STATES,
  sanitizeAlertText,
  stableAlertReference
} from "../observability/alert-contract.mjs";
import {
  finalizeSensitiveReport,
  sanitizeSensitiveReport
} from "../observability/sensitive-report-scan.mjs";
import { OBSERVABILITY_BUDGETS } from "../observability/observability-budgets.mjs";
...
function randomId(prefix = "alert") {
  return `${prefix}_${Date.now().toString(36)}_${crypto.randomBytes(5).toString("hex")}`;
}

function resolveDataDir(userDataPath = "") {
  return path.resolve(text(userDataPath) || ServerConfig.getDataDir());
}

export function getSecurityAlertsDatabasePath(userDataPath = "") {
...
function ensureSchema(db) {
  ...
  `);
  const columns = new Set(db.prepare("PRAGMA table_info(security_alerts)").all().map((column) => column.name));
...
export function redactSecurityAlertValue(value) {
...
function alertSeverity(value = "medium") {
...
function lifecycleRecordFromRow(row = {}) {
  const stored = parseJson(row.lifecycle_json, {});
  return {
...
export function createSecurityAlertStore({ userDataPath = "", db: injectedDatabase = null } = {}) {
  ...
  try {
    ...
    return withPrivateFileCreationMask(() => {
      ...
      const store = createSecurityAlertStoreFromDatabase({ db, ownsDatabase, dbPath });
...
function createSecurityAlertStoreFromDatabase({ db, ownsDatabase, dbPath }) {
  let closed = false;
  ensureSchema(db);

  function writeLifecycle(alertId, lifecycle, {
    acknowledgedBy = "",
    retentionArchivedAt = ""
  } = {}) {
  ...
  function hydrateStoredLifecycleMetadata() {
    ...
    const migrate = db.transaction(() => {
      for (const row of rows) {
        ...
        if (!ALERT_LIFECYCLE_STATES.includes(desiredStatus)) {
          const error = new Error("Stored security alert lifecycle status is invalid.");
        ...
        let lifecycle = desiredStatus === "rule_loaded"
          ...
          : activateAlertRecord(signal, null, { actor: "migration", now: () => createdAt });
        if (desiredStatus === "acknowledged") {
          lifecycle = transitionAlertRecord(lifecycle, "acknowledge", { actor: "migration", now: () => createdAt });
        } else if (["resolved", "archived"].includes(desiredStatus)) {
          lifecycle = transitionAlertRecord(lifecycle, "resolve", { actor: "migration", now: () => createdAt });
        } else if (desiredStatus === "suppressed") {
          lifecycle = transitionAlertRecord(lifecycle, "suppress", { actor: "migration", now: () => createdAt });
        } else if (desiredStatus === "notification_failed") {
          lifecycle = transitionAlertRecord(lifecycle, "notification_failed", { actor: "migration", now: () => createdAt });
        }
        if (desiredStatus === "archived") {
          lifecycle = transitionAlertRecord(lifecycle, "archive", { actor: "migration", now: () => createdAt });
        }
  ...
  function acknowledgeAlert(input = {}) {
  ...
  function transitionAlert(alertIdInput = "", event = "", input = {}) {
    const alertId = text(alertIdInput);
    if (!alertId) {
      return { ok: false, status: 400, reasonCode: "alert_id_required", error: "alertId is required." };
    }
    const row = db.prepare("SELECT * FROM security_alerts WHERE alert_id = ? AND retention_archived_at = ''").get(alertId);
    if (!row) {
