...
export function capabilityKernelLocalSealingKeyPath({ dataDir = "", alias = DEFAULT_ALIAS } = {}) {
  return path.join(resolveDataDir(dataDir), "security", "capability-kernel", `${safeAlias(alias)}.sealing-key`);
}

export function capabilityKernelLockPath({ dataDir = "", alias = DEFAULT_ALIAS } = {}) {
...
export function canonicalOpaqueCapabilities(capabilities = [], trustedCapabilityPermissions = []) {
  const trusted = new Set(normalizeRegisteredToolCapabilities(trustedCapabilityPermissions));
  return uniqueStrings(stringsFrom(
    normalizeKernelCapabilities(capabilities),
    stringsFrom(capabilities).filter((capability) => trusted.has(capability))
  )).sort();
}

export function rejectUnknownOpaqueCapabilities(capabilities = [], trustedCapabilityPermissions = []) {
  const trusted = new Set(normalizeRegisteredToolCapabilities(trustedCapabilityPermissions));
  const unknown = unknownKernelCapabilities(capabilities).filter((capability) => !trusted.has(capability));
  if (unknown.length > 0) {
    throw new Error(`Unknown opaque capability permission: ${unknown.join(", ")}`);
  }
}

export function opaqueCapabilityHash(capabilities = [], trustedCapabilityPermissions = []) {
...
export function publicKeyRecord(record = null) {
  if (!record) {
    return null;
  }
  return {
...
export function validateKeyRecord(record = null, { now = nowIso(), minGrantVersion = 0 } = {}) {
  if (!record) {
...
export function normalizeKernelState(input = {}) {
...
export function mergeKeyRecord(existing = null, candidate = null) {
...
export function mergePermissionRecord(existing = null, candidate = null) {
  if (!existing) {
    return candidate;
  }
  if (!candidate) {
...
export function mergeKernelStates(persistedInput = {}, hotInput = {}) {
...
export function kernelStateRoot(state = {}) {
  const normalized = {
    stateVersion: Number(state.stateVersion || KERNEL_STATE_VERSION),
...
export function publicKernelRecord(record = {}) {
  return {
    protocolVersion: OPAQUE_CAPABILITY_KEY_PROTOCOL_VERSION,
...
export function createKernelRecord({ alias = DEFAULT_ALIAS, provider = "local-file", securityMode = "degraded_file_fallback", state = null, sealingKeyBase64 = "" } = {}) {
  const timestamp = nowIso();
...
export function isClosedPipeError(error) {
  return error?.code === "EPIPE" || error?.code === "ECONNRESET";
}

export function markNeedsInitialWrite(record) {
...
export function stateFromKernelRecord(record = {}) {
