 ...
 */
export function createPolicyEnforcementPoint(options: Record<string, any> = {}) : any {
  const authorizationEngine: any = options.authorizationEngine || createAuthorizationEngine();
  const auditStore: any = options.auditStore || null;
  const approvalReceiptStore: any = options.approvalReceiptStore || null;
   ...
   */
  async function enforce(params: Record<string, any> = {}) : Promise<any> {
    const {
      operation = {},
      tool = null,
      subject = {},
    ...
    const operationRisk: any = String(tool?.risk || operation?.safety?.risk || operation?.risk || "");
    const isHighRisk: any =
    ...
    if (isHighRisk) {
      const hasReceipt: any = await _checkApprovalReceipt(decision, input, context);
      if (!hasReceipt?.valid) {
   ...
   */
  async function httpMiddleware(req?: any, res?: any, next?: any) : Promise<any> {
    const result: any = await enforce({
    ...
    if (!result.allowed) {
      const statusCode: any = _effectToStatusCode(result.decision.effect);
      const errorBody: any = _buildErrorEnvelope(req, result.decision);

      if (res && typeof res.status === "function") {
        return res.status(statusCode).json(errorBody);
      }
      const err: Error & Record<string, any> = new Error(errorBody.message);
  ...
  async function _audit(decision?: any, context?: any) : Promise<any> {
  ...
  async function _checkApprovalReceipt(decision?: any, input?: any, context?: any) : Promise<any> {
  ...
  function _effectToStatusCode(effect?: any) : any {
    switch (effect) {
      case "deny":
        return 403;
  ...
  function _buildErrorEnvelope(req?: any, decision?: any) : any {
    // External callers must NOT see internal operationId
    const isExternalCaller: any = !req?.authorizationSubject ||
...
function isApprovalEffect(effect: any = "") : any {
 ...
 * @returns {object}
 */
export function createConsoleRouteGuard(options: Record<string, any> = {}) : any {
  const pep: any = createPolicyEnforcementPoint(options);
   ...
   */
  async function guardRoute({ requiredScopes = [], requiredCapabilities = [], subject = {}, request = null }: Record<string, any> = {}) : Promise<any> {
