 ...
 * ## Contract
 *
 * - **Consumer**: `authorization-governance-store.mjs`, `security-permissions-provider.mjs`
 *   and other security modules use `registry.getProvider()` and call through the
 *   port interface.
 * - **Provider**: `packages/server-runtime/src/state/tag-store-adapter.mjs` implements
 *   this port and registers itself at composition time.
 * - **Composition**: `packages/server-runtime/src/composition/register-domain-loaders.mjs`
...
export const TAG_STORE_PORT_VERSION = "v0.0.1:authorization:tag-store-port-0.4.0";
 ...
 */
export function createTagStoreProviderRegistry() {
 ...
 */
export function validateTagStoreProvider(store) {
  const required = [
    "getPolicyRevision",
 ...
 */
export function createNoopTagStoreProvider() {
  const failClosed = (method) => {
    const fn = () => {
      const diagnostic = {
        method,
        provider: "noop",
