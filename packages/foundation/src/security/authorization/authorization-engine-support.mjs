...
import {
  ...
  stringsFrom,
  uniqueStrings
} from "./authorization-engine-common.mjs";

export function hasConfirmation(input = {}, request = null) {
...
export function maxRiskAllowed(profile = null, grant = null, subject = null, fallback = "safe_write") {
  const candidates = [
    profile?.maxRisk,
    grant?.maxRisk,
...
export function inferOperationAction(operation = {}, tool = null) {
  if (operation?.action) {
...
export function subjectScopes(subject = {}, actor = null, authSession = null, grant = null) {
  return uniqueStrings([
...
export function grantHasToolset(grant = null, tool = null) {
...
export function toolsetMisses(grant = null, tool = null) {
  if (!tool || !grant?.toolsets?.length) {
    return [];
  }
...
function subjectHasResourceBoundaryBypass(subject = {}) {
  return subject.roleId === "owner" ||
    subject.scopes?.includes("auth:admin");
}

export function abacDenyDetails({ subject = {}, grant = null, profile = null, resource = {} } = {}) {
  ...
  const accountPolicy = firstString(subject.accountId, grant?.accountId, grant?.metadata?.accountId, profile?.accountId);
  const allowedAccountIds = stringsFrom(
  ...
  const endpointPolicy = firstString(subject.endpointId, grant?.endpointId, grant?.metadata?.endpointId, profile?.endpointId);
  const allowedEndpointIds = stringsFrom(
  ...
  for (const [resourceKey, resourceListKey, allowedKey, reasonCode, reason] of semanticChecks) {
    const allowed = stringsFrom(subject[allowedKey], grant?.[allowedKey], grant?.metadata?.[allowedKey], profile?.[allowedKey]);
