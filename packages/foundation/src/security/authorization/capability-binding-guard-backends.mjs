...
export async function writeLocalRecord({ dataDir = "", alias = DEFAULT_ALIAS } = {}, record = {}) {
  const filePath = capabilityBindingGuardStatePath({ dataDir, alias });
...
export async function readMacosRecord({ alias = DEFAULT_ALIAS } = {}) {
...
export async function writeMacosRecord({ alias = DEFAULT_ALIAS } = {}, record = {}) {
  if (process.platform !== "darwin") {
    throw new Error("macos-keychain capability binding guard backend is only available on macOS.");
  }
...
export async function readLinuxKeyringRecord({ alias = DEFAULT_ALIAS } = {}) {
...
export async function writeLinuxKeyringRecord({ alias = DEFAULT_ALIAS } = {}, record = {}) {
  const description = linuxKeyringDescription(alias);
  try {
...
export async function readSecretServiceRecord({ alias = DEFAULT_ALIAS } = {}) {
  try {
...
export async function writeSecretServiceRecord({ alias = DEFAULT_ALIAS } = {}, record = {}) {
...
export async function readPassRecord({ alias = DEFAULT_ALIAS } = {}) {
  try {
...
export async function writePassRecord({ alias = DEFAULT_ALIAS } = {}, record = {}) {
  await runText("pass", ["insert", "-m", "-f", passEntryName(alias)], { input: JSON.stringify(record) });
  return record;
}

export async function readWindowsDpapiRecord({ dataDir = "", alias = DEFAULT_ALIAS } = {}) {
...
export async function writeWindowsDpapiRecord({ dataDir = "", alias = DEFAULT_ALIAS } = {}, record = {}) {
  const filePath = windowsDpapiProtectedPath({ dataDir, alias });
...
export function degradedLocalRecord(record = {}, { alias = DEFAULT_ALIAS } = {}) {
...
export function rewrapBindingRecordForProvider(record = {}, { alias = DEFAULT_ALIAS, provider = "local-file" } = {}) {
...
export async function readLinuxAutoRecord({ dataDir = "", alias = DEFAULT_ALIAS } = {}) {
...
export async function writeLinuxAutoRecord({ dataDir = "", alias = DEFAULT_ALIAS } = {}, record = {}) {
...
export async function readRecord({ backend = "auto", dataDir = "", alias = DEFAULT_ALIAS } = {}) {
...
export async function writeRecord({ backend = "auto", dataDir = "", alias = DEFAULT_ALIAS } = {}, record = {}) {
  if (backend === "auto" && process.platform === "linux") {
