...
} from "./authorization-capabilities.mjs";
import { nowIso, randomId, riskRank, stringSet, uniqueStrings, effectDetails } from "./authorization-engine-common.mjs";
import { resolveResourceContext } from "./authorization-resource-context.mjs";
import {
  abacDenyDetails,
  grantHasToolset,
  hasConfirmation,
  ...
  sourceIpFromRequest,
  toolsetMisses
} from "./authorization-engine-support.mjs";
...
export { resolveAuthorizationSubject } from "./authorization-engine-support.mjs";

function subjectCanUseCapabilityWildcards(subject = {}) {
  return subject.roleId === "admin" ||
    subject.roleId === "owner" ||
    subject.scopes?.includes("auth:admin");
}

function effectiveSubjectCapabilities(subject = {}) {
...
function approvedPendingOperationMatches(context = {}, operation = {}) {
...
export function evaluateAuthorizationPolicy({
  ...
  const subjectCapabilitiesForDecision = effectiveSubjectCapabilities(resolvedSubject);
  const missingCapabilities = capabilityMode
    ? requiredCapabilities.filter((capability) => !hasCapability(subjectCapabilitiesForDecision, capability))
    : [];
