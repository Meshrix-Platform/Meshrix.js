...
export function createProcessIdentityService({
  ...
  async function issueLocalMcpClientIdentityPackage({ input = {} }: Record<string, any> = {}) : Promise<any> {
    return enqueueMutation(async () : Promise<any> => {
      const source: any = asObject(input);
      let normalizedClient: any;
      try {
      ...
      const timestamp: any = nowIso();
      const packageId: any = text(source.packageId) || `cidpkg_${crypto.randomUUID()}`;
      const identityGeneration: any = Math.max(1, Number(source.identityGeneration || 1) || 1);
      const credentialId: any = `procid_${packageId}`;
      const issued: any = await resolvedCapabilityKeyProvider.issue({
        credentialId,
        capabilities: normalizedClient.capabilities,
      ...
      state = {
        ...
        claimCount: Number(state.claimCount || 0) + 1,
        clients: [
          ...state.clients.filter((existing?: any) : any =>
  ...
  async function revokeIssuedLocalMcpClientIdentityPackage({ clientIdentityPackage = {}, reason = "" }: Record<string, any> = {}) : Promise<any> {
    return enqueueMutation(async () : Promise<any> => {
      const packageObject: any = asObject(clientIdentityPackage);
      const capability: any = asObject(packageObject.capability);
      const packageId: any = text(packageObject.packageId);
      const capabilityKey: any = text(capability.key);
      const client: any = state.clients.find((candidate?: any) : any => candidate.packageId === packageId) || null;
      if (
        !client ||
        client.status !== "valid" ||
        !capabilityKey ||
      ...
      const rollbackReason: any = text(reason) || "local_mcp_grant_issue_rolled_back";
      ...
      state = {
        ...state,
        clients: state.clients.map((candidate?: any) : any => candidate.packageId === packageId
          ? normalizeClientRecord({
              ...candidate,
              status: "revoked",
              revokedAt: timestamp,
