...
} from "./process-identity-revocation-receipt.mjs";
...
export function createProcessIdentityService({
  ...
  async function issueLocalMcpClientIdentityPackage({ input = {} } = {}) {
    return enqueueMutation(async () => {
      const source = asObject(input);
      let normalizedClient;
      try {
        normalizedClient = normalizeClientInput({
          ...source,
        ...
        return deny(error.status || 400, error.reasonCode || "local_mcp_identity_invalid", error.message);
      }
      const timestamp = nowIso();
      const packageId = text(source.packageId) || `cidpkg_${crypto.randomUUID()}`;
      const identityGeneration = Math.max(1, Number(source.identityGeneration || 1) || 1);
      const credentialId = `procid_${packageId}`;
      const issued = await resolvedCapabilityKeyProvider.issue({
        credentialId,
        capabilities: normalizedClient.capabilities,
      ...
      state = {
        ...state,
        claimed: true,
        claimedAt: state.claimedAt || timestamp,
        claimCount: Number(state.claimCount || 0) + 1,
        clients: [
          ...state.clients.filter((existing) =>
            existing.packageId !== client.packageId &&
            !(
              existing.clientId === client.clientId &&
              existing.processKeyId === client.processKeyId &&
  ...
  async function revokeIssuedLocalMcpClientIdentityPackage({ clientIdentityPackage = {}, reason = "" } = {}) {
    return enqueueMutation(async () => {
      const packageObject = asObject(clientIdentityPackage);
      const capability = asObject(packageObject.capability);
      const packageId = text(packageObject.packageId);
      const capabilityKey = text(capability.key);
      const client = state.clients.find((candidate) => candidate.packageId === packageId) || null;
      if (
        !client ||
        client.status !== "valid" ||
        !capabilityKey ||
        text(capability.credentialId) !== client.capabilityCredentialId
      ) {
        return deny(
          409,
          "local_mcp_identity_rollback_unavailable",
          "Issued local MCP process identity could not be rolled back."
        );
      }
      const rollbackReason = text(reason) || "local_mcp_grant_issue_rolled_back";
      await resolvedCapabilityKeyProvider.invalidate({ capabilityKey, reason: rollbackReason });
      ...
      state = {
        ...state,
        clients: state.clients.map((candidate) => candidate.packageId === packageId
          ? normalizeClientRecord({
              ...candidate,
              status: "revoked",
              revokedAt: timestamp,
      ...
      await save();
      return { ok: true, status: 200, packageId, revokedAt: timestamp };
    });
  }
  ...
  async function verifyClientIdentityRevocationReceipt({ receipt = null, expected = {} } = {}) {
    await mutationQueue.catch(() => {});
    await load();
    ...
    const payload = signatureDecision.payload || {};
  ...
  function ownerProcessBindingContext(input = {}) {
  ...
  function ownerProcessBindingDecision(input = {}, binding = null) {
  ...
  async function issueOwnerProcessBinding(input = {}) {
    return enqueueMutation(async () => {
  ...
  async function inspectOwnerProcessBinding(input = {}) {
    await mutationQueue.catch(() => {});
    await load();
  ...
  async function revokeOwnerProcessBinding(input = {}) {
    return enqueueMutation(async () => {
      ...
      const next = { ...binding, status: "revoked", revokedAt, receiptDigest };
      const previousState = state;
      const previousRecord = record;
      state = {
        ...state,
  ...
  async function revokeOwnerProcessBindings(input = {}) {
    return enqueueMutation(async () => {
      const ownerId = text(input.ownerId);
      const ownerGenerationDigest = text(input.ownerGenerationDigest);
  ...
  async function verifyOwnerProcessBindingsRevoked(input = {}) {
    await mutationQueue.catch(() => {});
    await load();
    const ownerId = text(input.ownerId);
    const ownerGenerationDigest = text(input.ownerGenerationDigest);
