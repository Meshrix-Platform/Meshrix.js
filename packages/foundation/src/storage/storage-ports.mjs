...
function looksLikeHighEntropyBase64(value) {
...
const REFERENCE_SCHEMES = Object.freeze({
...
export function serviceManifestError(code, message, cause) {
...
function normalizeSafeString(value, field, { maxLength = 512, pattern = null } = {}) {
  ...
  if (FORBIDDEN_VALUE_PATTERNS.some((candidate) => candidate.test(normalized)) || looksLikeHighEntropyBase64(normalized)) {
    throw serviceManifestError(
...
export function validateOpaqueServiceId(value) {
...
export function validateManifestDigest(value, field = "digest") {
...
export function validateManifestRevision(value, field) {
...
export function normalizeManifestResourceBudget(input = {}) {
  assertPlainObject(
    input,
...
function normalizeJsonValue(value, context, path = "manifest", depth = 0, schemaContext = false) {
  context.nodes += 1;
...
export function stableManifestJson(value) {
  ...
  return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${stableManifestJson(value[key])}`).join(",")}}`;
}

export function sha256ManifestBytes(bytes) {
  return createHash("sha256").update(bytes).digest("hex");
}

export function deepFreezeManifest(value) {
  if (!value || typeof value !== "object" || Object.isFrozen(value)) return value;
  for (const child of Object.values(value)) deepFreezeManifest(child);
  return Object.freeze(value);
}

export function canonicalizeTypedReferenceManifest(value, budgetInput = {}) {
  const budget = Object.isFrozen(budgetInput) && BUDGET_KEYS.size === Object.keys(budgetInput).length
    ? budgetInput
...
export function createDurableManifestWriterPort(commitManifestSet) {
...
export function createManifestSnapshotReaderPort(getSnapshot) {
