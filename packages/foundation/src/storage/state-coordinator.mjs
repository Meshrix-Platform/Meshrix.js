...
export const STATE_MUTATION_POLICY = Object.freeze({
...
function stateMutationError(code, message) {
...
function positiveIntegerAtMost(value, fallback, maximum) {
...
function lowerPolicyLimit(value, hardLimit) {
  return positiveIntegerAtMost(value, hardLimit, hardLimit);
}

function normalizeStateMutationPolicy(input = {}) {
...
export class BoundedMutationDeque {
  constructor(capacity, { onOperation = null } = {}) {
...
export function createStateMutationCoordinator({
  ...
  const resolveLogger = typeof loggerProvider === "function"
    ? loggerProvider
    : () => loggerProvider || null;
  let queuedCount = 0;

  function noteReason(code) {
    if (!code) return;
    reasonCounts[code] = Number(reasonCounts[code] || 0) + 1;
  }

  function rejectWith(logger, event, error, fields = {}) {
  ...
  function releaseQueuedEntry(entry) {
  ...
  function settleLaneIfIdle(key, lane) {
  ...
  function rejectQueuedMutations(key, lane) {
    let entry;
  ...
  function startNextMutation(key, lane) {
    ...
    const fenceActiveEntry = (error, event) => {
    ...
    if (entry.signal) {
      ...
      detachActiveAbort = () => entry.signal.removeEventListener("abort", abortActive);
  ...
  function queueStateMutation(key, task, options = {}) {
    ...
    const cancelQueued = (error, event) => {
      if (entry.slot < 0) return false;
      const removed = lane.queue.remove(entry.slot);
    ...
    entry.queueWaitTimer = setTimeout(() => {
      cancelQueued(
        schedulerError(
    ...
    if (entry.signal) {
      const abortQueued = () => cancelQueued(queueAbortedError(), "state.queue.queue_aborted");
      entry.signal.addEventListener("abort", abortQueued, { once: true });
      entry.detachQueuedAbort = () => entry.signal.removeEventListener("abort", abortQueued);
    }
...
const defaultStateMutationCoordinator = createStateMutationCoordinator();

export function queueStateMutation(key, task, options = {}) {
  return defaultStateMutationCoordinator.queueStateMutation(key, task, options);
}

export function stateMutationSchedulerSnapshot() {
  return defaultStateMutationCoordinator.snapshot();
}

export function createStateMutationDispatcher({ logger = null, coordinator = null } = {}) {
  ...
  async function mutate({
    ...
    return mutationCoordinator.queueStateMutation(normalizedKey, async (mutationContext) => {
      const startedAt = Date.now();
...
export function waitForStateIdle(key) {
...
function normalizeAtomicWriteOptions(options = "utf8") {
...
async function ensurePrivateStateDirectory(directoryPath) {
...
async function syncStateDirectory(directoryPath) {
  let handle = null;
  try {
...
export async function atomicWriteJson(filePath, value, { trailingNewline = true, ignoreMissingParent = false } = {}) {
...
async function truncateTornJsonLineTail(handle) {
