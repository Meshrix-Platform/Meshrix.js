...
type RestoreActionKind = "create" | "replace" | "noop" | "blocked" | "delete";
...
function recordFromAction(action: RestoreAction, entry: RestoreEntry | null = null): RestoreTransactionRecord {
...
async function sqliteSidecarRecords({
  rootPath,
  entry,
  explicitPaths
}: {
...
}): Promise<RestoreTransactionRecord[]> {
  if (!isSqliteDataFile(entry.relativePath)) return [];
  const records: RestoreTransactionRecord[] = [];
...
export async function buildRestoreTransactionRecords({
  rootPath,
  entries,
  actions
}: {
