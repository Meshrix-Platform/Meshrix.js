...
export const PLUGIN_TRANSPARENCY_LEDGER_PROVIDER: any = "pactium.plugin-transparency-ledger";
...
export function createPluginTransparencyLedger({
  ...
  const resolvedPluginId: any = text(pluginId, "plugin");
  const resolvedLedgerId: any = text(ledgerId, `plugin:${resolvedPluginId}:transparency`);
  const root: any = inMemory
    ? ""
  ...
  async function appendDigestLeaf({
    ...
    const digest: any = text(leafDigest);
    if (!digest) {
      throw new Error("leafDigest is required for transparency ledger append.");
    }
    const appendResult: any = await log.append({
    ...
    const head: any = appendResult.head || await log.head();
    return {
  ...
  async function createInclusionProof(index?: any, head: any = null) : Promise<any> {
    return log.createInclusionProof(index, head);
  }

  async function createConsistencyProof(oldHead?: any, newHead: any = null) : Promise<any> {
    const normalizedOld: any = asObject(oldHead);
    const size: any = Number(normalizedOld.size || 0);
    const rootHash: any = text(normalizedOld.rootHash) || (size === 0 ? emptyTreeHash() : "");
  ...
  async function getHead(id: any = "current") : Promise<any> {
    return log.getHead(id);
  }

  async function getLeaf(index?: any) : Promise<any> {
    return log.getLeaf(index);
  }

  function verifyInclusion(proofInput: Record<string, any> = {}) : any {
    return verifyLedgerInclusionProof(proofInput);
  }

  function verifyConsistency(proofInput: Record<string, any> = {}) : any {
...
export function projectionLeafFromAppend(appendResult: Record<string, any> = {}, endpointRecord: Record<string, any> = {}) : any {
  return {
