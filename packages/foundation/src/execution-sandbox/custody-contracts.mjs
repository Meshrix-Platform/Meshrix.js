import { SANDBOX_PROVIDER_CONFORMANCE_SCHEMA, sandboxDigest } from "./contracts.mjs";
...
export function normalizeCustodyHandle(value) {
...
export function custodyPromotionAuthorizationDigest({ promotionDigest, ownerBinding, governance } = {}) {
  const owner = ownerBinding && typeof ownerBinding === "object" && !Array.isArray(ownerBinding)
    ? ownerBinding
    : {};
  const currentGovernance = governance && typeof governance === "object" && !Array.isArray(governance)
    ? governance
    : {};
  return sandboxDigest({
    promotionSchemaVersion: SANDBOX_CUSTODY_PROMOTION_SCHEMA,
...
export function normalizeCustodyPromotionRequest(value = {}, { now = new Date() } = {}) {
