...
function deny(reasonCode, detail = "") {
...
function intersectRequested(requested, allowed) {
  const allowedSet = stringSet(allowed);
  return requested.filter((item) => allowedSet.has(item));
}

export function normalizeSandboxConfiguration(value) {
...
export function compileSandboxAdmission({
  ...
  const governance = currentGovernance || {};
  if (
  ...
  const allowedCapabilities = plainObject(profile.capabilities) ? profile.capabilities : {};
  const narrowed = {
  ...
  const resourceLimits = plainObject(profile.resourceLimits) ? profile.resourceLimits : {};
  const resources = {};
