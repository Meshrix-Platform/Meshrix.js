...
export function ensureSqliteWorkQueueSchema(db) {
  db.exec(`
  ...
  runMigrations(db, [
    ...
    },
    {
      ...
      up: (migrationDb) => {
        const columns = new Set(
          migrationDb.prepare("PRAGMA table_info(work_items)").all().map((column) => column.name)
        );
    ...
    },
    {
      ...
      up: (migrationDb) => {
        ...
        if (["queue_definition_id", "state", "updated_at_ms", "work_item_id"].every((column) => columns.has(column))) {
          migrationDb.exec(`
            CREATE INDEX IF NOT EXISTS idx_work_queue_retention
              ON work_items(queue_definition_id, state, updated_at_ms, work_item_id);
          `);
        }
        const hasJournal = Boolean(migrationDb.prepare(`
