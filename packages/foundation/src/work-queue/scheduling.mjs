...
export const WORK_QUEUE_PRIORITY_WEIGHTS = Object.freeze({
...
export const WORK_QUEUE_PRIORITY_CYCLE = Object.freeze(
  Object.entries(WORK_QUEUE_PRIORITY_WEIGHTS)
    .flatMap(([priorityClass, weight]) => Array.from({ length: weight }, () => priorityClass))
);

export function normalizeWorkQueuePriority(value) {
...
export function priorityClassAtCursor(cursor = 0) {
...
export function nextPriorityCursor(cursor = 0) {
  const normalized = Math.max(0, Math.trunc(Number(cursor) || 0));
...
export function hierarchicalScopeParts(scope = {}) {
...
export function assertCapacityBelow({ count, limit, reason }) {
  if (Number(count) >= Number(limit)) {
    throw new WorkQueueCapacityError(reason, limit);
  }
}

export function assertCapacityAtMost({ count, limit, reason }) {
  if (Number(count) > Number(limit)) {
    throw new WorkQueueCapacityError(reason, limit);
  }
}
