...
import { asInt, getPolicy, jsonString, nowFrom, parseJson, rowToWorkItem, toText } from "./store-serialization.mjs";
import { WorkQueueCapacityError } from "./scheduling.mjs";

export function createSqliteWorkQueueRuntime({ statements, timeSource, identityGenerator, resolvedPolicy }) {
  function policyForRow(row) {
  ...
  function boundedCount(statement, parameters, limit) {
    return Number(statement.get({ ...parameters, limit: Number(limit) + 1 })?.count || 0);
  }

  function insertRetentionSnapshot(row, nowMs) {
  ...
  function compactWorkJournal(workItemId, nowMs, { force = false } = {}) {
  ...
  function deleteTerminalRows(queueDefinitionId, excludeWorkItemId, limit) {
  ...
  function maintainRetentionBeforeAppend(row, nowMs) {
    const { retention } = policyForRow(row);
  ...
  function makeFailedCapacityRoom(row, nowMs) {
  ...
  function appendTransitionInternal({
    ...
    const forceCompactCurrent = maintainRetentionBeforeAppend(row, nowMs);
    const result = statements.insertJournal.run({
      journal_entry_id: identityGenerator.journalEntryId(),
      work_item_id: row.work_item_id,
      queue_definition_id: row.queue_definition_id,
  ...
  function transitionProjection({ row, transition, toState, patch = {}, nowMs, operationId, actor, reason, policyVersion }) {
  ...
  function expireEligibleLocked({ nowMs, queueDefinitionId = "", scopeKey = "", limit = 1000 } = {}) {
