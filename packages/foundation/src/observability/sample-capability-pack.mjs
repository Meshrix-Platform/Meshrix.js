...
const DEFAULT_PACK_ID = "gateway-forwarding-pilot";
const SAMPLE_PACK_ROOT = "sample-capability-packs";
...
export function listSampleCapabilityPacks() {
  return {
    schemaVersion: "v0.0.1:schema:definition-1",
    protocolVersion: SAMPLE_CAPABILITY_PACK_PROTOCOL_VERSION,
...
export function getSampleCapabilityPack(packId = DEFAULT_PACK_ID) {
...
export async function materializeSampleCapabilityPack(input = {}, options = {}) {
  const packId = slug(input.packId || DEFAULT_PACK_ID);
  const pack = getPackDefinition(packId);
  if (!pack) {
...
export function createSampleCapabilityPackStore({ userDataPath } = {}) {
  return {
