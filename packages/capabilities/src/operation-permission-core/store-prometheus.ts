import { prometheusSample } from "./store-metrics-utils.ts";

export function createMetricsPrometheus({ metricsHealth }: Record<string, any>) : any {
  function metricsPrometheus(options: Record<string, any> = {}) : any {
  const health: any = metricsHealth(options);
  const lines: any[] = [
    ...
    ...["ok", "warn", "critical"].map((status?: any) : any =>
    ...
    ...health.toolCalls.topTools.map((item?: any) : any =>
    ...
    ...health.requests.topRoutes.map((item?: any) : any =>
