...
export function createGrantProcessIdentityMethods(ctx: any, { appendGrantEvent }: Record<string, any>) : any {
  ...
  function recordMcpTargetBindingDenial({ grant, request, decision }: Record<string, any> = {}) : any {
    const sourceIp: any = sourceIpFromRequest(request);
    const userAgent: any = headerValue(request, "user-agent");
  ...
  async function verifyLocalMcpProcessIdentity({
    ...
    if (request?.__meshrixProcessIdentity?.ok === true) {
      return { ok: true, applicable: true, reasonCode: "process_identity_reused", verification: request.__meshrixProcessIdentity };
    }
    ...
    const requestUrl: any = url instanceof URL
      ? url
      : new URL(String(request?.url || "/mcp"), "http://127.0.0.1");
    const verification: any = await ctx.securityPermissions.verifyProcessIdentity({
      request,
