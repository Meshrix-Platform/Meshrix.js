export function createDelegatedGrantSecurity({
  ...
  function delegatedParentId(grant = null) {
    return String(grant?.metadata?.delegatedMcp?.sourceGrantId || "").trim();
  }

  function delegatedChildren(parentGrantId) {
    const resolvedParentGrantId = String(parentGrantId || "").trim();
    if (!resolvedParentGrantId) return [];
    return db.prepare("SELECT * FROM tool_grants WHERE type = 'delegated-mcp-child'")
      .all()
      .map(rowToGrant)
      .filter((grant) => delegatedParentId(grant) === resolvedParentGrantId);
  }

  async function invalidateGrantCredential(grant, reason) {
    if (!grant) return;
  ...
  async function revokeDelegatedDescendants(parentGrantId, reason) {
    const queue = [String(parentGrantId || "").trim()].filter(Boolean);
  ...
  function policyIntegrityDenial(grant, { parent = false } = {}) {
    if (grant?.policyIntegrity?.valid !== false) return null;
    const reasonCode = parent ? "delegated_parent_policy_corrupt" : "grant_policy_corrupt";
  ...
  function activeDelegatedParent(grant) {
    if (String(grant?.type || "") !== "delegated-mcp-child") {
      return { ok: true, parentGrant: null };
    }
    const parentGrantId = delegatedParentId(grant);
    const parentGrant = parentGrantId ? getGrant(parentGrantId) : null;
    ...
    const integrityDenial = policyIntegrityDenial(parentGrant, { parent: true });
