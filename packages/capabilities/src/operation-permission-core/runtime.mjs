...
import { completeToolExecutionFailure } from "./runtime-execution-failure.mjs";
import { completeHandlerPendingApproval } from "./runtime-handler-pending.mjs";
import { createPendingOperationRuntime } from "./runtime-pending.mjs";
import { jsonByteLength, resultSummaryFromPayload } from "./runtime-result-summary.mjs";
import { validateInputSchema } from "./runtime-schema.mjs";
import {
...
} from "./runtime-transport.mjs";
import { toolActorFromAuthorization } from "./runtime-tool-actor.mjs";
import { denyUnknownToolExecution } from "./runtime-unknown-tool.mjs";

const RISK_RANK = Object.freeze({ read_only: 0, safe_write: 1, repair_write: 2, destructive: 3 });

function toolWithDynamicCapability(tool = null, context = {}) {
  const descriptor = context?.dynamicCapability && typeof context.dynamicCapability === "object" && !Array.isArray(context.dynamicCapability)
    ? context.dynamicCapability
    : null;
  if (!tool || !descriptor) return tool;
  const staticRisk = String(tool.risk || "read_only");
  const dynamicRisk = String(descriptor.risk || "read_only");
  const risk = (RISK_RANK[dynamicRisk] ?? 0) > (RISK_RANK[staticRisk] ?? 0) ? dynamicRisk : staticRisk;
...
export function createToolExecutionRuntime({
  ...
  async function executeTool({
    ...
    const tool = toolWithDynamicCapability(registry.getTool(toolId), context);
    ...
    const profile = context.profileId
      ...
      : null;
    const delegatedChildOperation = context.delegatedChildOperation && typeof context.delegatedChildOperation === "object" && !Array.isArray(context.delegatedChildOperation)
      ? context.delegatedChildOperation
      : null;
    const appendExecution = async (entry = {}) => {
      const payload = {
        ...entry,
    ...
    const approvalOperationBindingCore = {
    ...
    if (trustedApproval) {
      const persistedBinding = trustedApproval.requiredApproval?.operationBinding;
      const bindingCurrent = persistedBinding &&
    ...
    const approvalAlreadySatisfiesCurrentPolicy = approvalAlreadySatisfiesPolicy(policy, trustedApproval);
    ...
    if (
      ...
      const durationMs = Date.now() - startedAtMs;
      const approvalReasonCode = governanceApprovalRequired
        ? policy.reasonCode || "governance_approval_required"
        : "tool_approval_required";
      const approvalReason = governanceApprovalRequired
        ? policy.redactedReason || "Governance approval is required before execution."
        : `Tool ${tool.id} requires approval before execution.`;
      const policyRequiredApproval = policy.requiredApproval && typeof policy.requiredApproval === "object" && !Array.isArray(policy.requiredApproval)
        ? policy.requiredApproval
        : {};
      const toolRequiredApproval = tool.requiredApproval && typeof tool.requiredApproval === "object" && !Array.isArray(tool.requiredApproval)
        ? tool.requiredApproval
        : {};
    ...
    if (dryRun || policy.effect === "dry_run_only") {
      ...
      const operationInput = directRequest.input && typeof directRequest.input === "object" && !Array.isArray(directRequest.input)
        ? directRequest.input
        : {
    ...
    request.__licoToolRuntimeAuthorization = {
      ...
      approvedPendingOperation: trustedApproval
        ? (() => {
            const operationBinding = trustedApproval.requiredApproval.operationBinding;
            const expiresAt = String(trustedApproval.expiresAt || "");
            return {
    ...
    try {
      const toolActor = toolActorFromAuthorization({
        authorization,
        trustedApproval,
        operation,
        tool
      ...
      const handlerPendingResult = await completeHandlerPendingApproval({
        payload,
        statusCode,
        store,
        traceId,
      ...
      request.__licoToolRuntimeAuthorization = previousAuthorization;
    }
  }

  const resumePendingOperation = createPendingOperationRuntime({ store, executeTool, publishEvent, securityPermissions });
