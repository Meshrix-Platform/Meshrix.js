...
import { createGrantEventStore } from "./store-grant-events.mjs";
import { publicGrant, rowToGrant, summarizeValue } from "./store-models.mjs";
import { createGrantProcessIdentityMethods } from "./store-process-identity.mjs";
import { createDelegatedGrantSecurity } from "./store-delegated-grant-security.mjs";
import { createConsumeGrantUseStatement } from "./store-grant-use.mjs";

export function createGrantStoreMethods(ctx) {
  const {
    db,
    registry,
    capabilityResolver,
  ...
  const toPublicGrant = (grant) => publicGrant(grant, {
  ...
  const consumeGrantUseStmt = createConsumeGrantUseStatement(db);
  const ownerRowsStmt = db.prepare(`
  ...
  const insertOwnerStmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `);
  const insertOwnerAuthorityStmt = db.prepare(`
  ...
  function grantOwners(grantId) {
    return ownerRowsStmt.all(String(grantId || "")).map((owner) => Object.freeze(owner));
  }

  function hydrateGrant(row) {
    const grant = rowToGrant(row);
    if (!grant) return null;
    const owners = grantOwners(grant.id);
    return {
      ...grant,
      owners,
  ...
  function hydrateGrants(rows) {
    if (rows.length === 0) return [];
    const grantIds = new Set(rows.map((row) => row.id));
    ...
    `).all().filter((owner) => grantIds.has(owner.grant_id));
    const ownersByGrantId = new Map();
    for (const owner of ownerRows) {
      const list = ownersByGrantId.get(owner.grant_id) || [];
    ...
    return rows.map((row) => {
      const grant = rowToGrant(row);
      const owners = ownersByGrantId.get(grant.id) || [];
      return { ...grant, owners, ownerIntegrity: { valid: owners.length > 0 } };
    });
  }

  function normalizeGrantAgainstCurrentCatalog(input = {}, fallback = {}) {
    const grant = normalizeGrantInput(input, fallback);
    if (!grant.toolsets?.length || !registry || typeof registry.resolveToolset !== "function") return grant;
    const resolution = registry.resolveToolset({
      toolsets: grant.toolsets,
      toolAllow: grant.toolAllow,
      toolDeny: grant.toolDeny
    });
    const resolvedToolsets = new Set(Array.isArray(resolution.toolsets) ? resolution.toolsets : grant.toolsets);
    const missingToolset = grant.toolsets.find((toolsetId) => !resolvedToolsets.has(toolsetId));
    if (missingToolset) {
      const error = new Error("Operation Permission grant references an unknown toolset.");
      error.code = "operation_permission_grant_toolset_unknown";
      throw error;
    }
    return {
      ...grant,
      scopes: [...new Set((Array.isArray(resolution.requiredScopes) ? resolution.requiredScopes : grant.scopes)
        .map(String).filter(Boolean))].sort()
    };
  }

  function rejectCallerGrantOwner(input = {}) {
    const metadata = input?.metadata && typeof input.metadata === "object" && !Array.isArray(input.metadata)
      ? input.metadata
      : {};
    const forbidden = ["owner", "ownerKind", "ownerId", "owners", "pluginId", "grantOwner"];
    if (forbidden.some((field) => Object.hasOwn(input, field) || Object.hasOwn(metadata, field))) {
      const error = new Error("Operation Permission grant owner is resolved by the active catalog authority.");
      error.code = "operation_permission_grant_owner_not_caller_controlled";
      throw error;
    }
  }

  function closedOwner(tool = {}) {
    const ownerKind = String(tool.ownerKind || "").trim();
    const ownerId = String(tool.ownerId || "").trim();
    if ((ownerKind !== "core" && ownerKind !== "plugin") || !ownerId) {
      const error = new Error("Operation catalog tool owner is missing or unknown.");
    ...
    if (!authority?.ownerGeneration) {
      const error = new Error("Plugin grant owner generation is not active.");
  ...
  function resolveGrantOwners(grant) {
    const delegatedParentId = String(grant?.metadata?.delegatedMcp?.sourceGrantId || "").trim();
    if (String(grant?.type || "") === "delegated-mcp-child") {
      const parent = delegatedParentId ? getGrant(delegatedParentId) : null;
      if (!parent?.ownerIntegrity?.valid) {
        const error = new Error("Delegated grant parent has no valid catalog owner binding.");
    ...
    if (registry && typeof registry.resolveToolset === "function" && typeof registry.listTools === "function") {
      ...
      const selected = new Map((resolved.tools || []).map((tool) => [tool.id, tool]));
      const capabilities = resolveGrantCapabilities(grant, { registry, capabilityResolver });
      const allTools = registry.listTools();
      if (capabilities.includes("cap:tool:*")) {
        for (const tool of allTools) selected.set(tool.id, tool);
      } else {
        const allowedCapabilityIds = new Set(capabilities);
    ...
    const error = new Error("Operation Permission grant owner authority is unavailable.");
    error.code = "operation_permission_grant_owner_authority_unavailable";
    throw error;
  }

  function replaceGrantOwners(grantId, owners, createdAt) {
    ...
    for (const owner of owners) {
      ...
      if (authority?.state !== "active") {
        const error = new Error("Grant owner generation changed before persistence.");
  ...
  function assertGrantOwnersActive(owners = []) {
    if (!Array.isArray(owners) || owners.length === 0) {
      const error = new Error("Grant owner binding is missing.");
    ...
    for (const owner of owners) {
      ...
      `).get(owner.ownerKind, owner.ownerId, owner.ownerGeneration);
      if (authority?.state !== "active") {
        const error = new Error("Grant owner generation is no longer active.");
        error.code = "operation_permission_plugin_owner_generation_inactive";
        throw error;
      }
    }
  ...
  function grantOwnersAreActive(grantId) {
  ...
  async function recoverOwnerMigrationInvalidations() {
    const targets = db.prepare(`
      SELECT migration_id, grant_id, capability_invalidated, binding_invalidated
      FROM tool_grant_owner_migration_invalidations
      WHERE capability_invalidated = 0 OR binding_invalidated = 0
      ORDER BY migration_id, grant_id
    `).all();
    for (const target of targets) {
      try {
        if (!target.capability_invalidated) {
          await resolvedCapabilityKeyProvider.invalidateCredential({
            credentialId: target.grant_id,
            reason: "grant_owner_migration_retired"
          });
          db.prepare(`
            UPDATE tool_grant_owner_migration_invalidations
            SET capability_invalidated = 1, updated_at = ?
            WHERE migration_id = ? AND grant_id = ?
          `).run(nowIso(), target.migration_id, target.grant_id);
        }
        if (!target.binding_invalidated && typeof resolvedCapabilityBindingGuard?.invalidateCapabilityKeyBinding === "function") {
          ...
          db.prepare(`
            ...
            WHERE migration_id = ? AND grant_id = ?
          `).run(nowIso(), target.migration_id, target.grant_id);
        } else if (!target.binding_invalidated) {
          db.prepare(`
            UPDATE tool_grant_owner_migration_invalidations
            SET binding_invalidated = 1, updated_at = ?
            WHERE migration_id = ? AND grant_id = ?
        ...
        // The durable target remains pending and is retried on the next store open.
      }
    }
  }
  ...
  const ownerlessRows = db.prepare(`
    SELECT grant.* FROM tool_grants AS grant
    WHERE NOT EXISTS (
      SELECT 1 FROM tool_grant_owners AS owner WHERE owner.grant_id = grant.id
    )
    ORDER BY grant.id
  `).all();
  if (ownerlessRows.length > 0) {
    const migrationAt = nowIso();
    let revokedCount = 0;
    db.transaction(() => {
      for (const row of ownerlessRows) {
        const grant = rowToGrant(row);
        const tools = typeof registry?.resolveToolset === "function"
        ...
        for (const tool of tools) {
          ...
          const ownerId = String(tool?.ownerId || "").trim();
          if ((ownerKind === "core" || ownerKind === "plugin") && ownerId) {
            owners.set(`${ownerKind}:${ownerId}`, { ownerKind, ownerId });
          }
        }
        const pluginOwned = [...owners.values()].some((owner) => owner.ownerKind === "plugin");
        const mustRevoke = pluginOwned || owners.size === 0;
        const effectiveOwners = owners.size > 0
          ? [...owners.values()]
          : [{ ownerKind: "core", ownerId: "unknown-retired" }];
        for (const owner of effectiveOwners) {
          const generation = owner.ownerKind === "plugin" || owner.ownerId === "unknown-retired"
            ? "legacy-retired"
            : "core";
          if (generation === "legacy-retired") {
            db.prepare(`
              INSERT INTO tool_grant_owner_authorities (
                owner_kind, owner_id, owner_generation, state, first_seen_at, updated_at, retired_at
              ) VALUES (?, ?, ?, 'retired', ?, ?, ?)
              ON CONFLICT(owner_kind, owner_id, owner_generation) DO NOTHING
            `).run(owner.ownerKind, owner.ownerId, generation, migrationAt, migrationAt, migrationAt);
          }
          insertOwnerStmt.run(grant.id, owner.ownerKind, owner.ownerId, generation, migrationAt);
        }
        if (mustRevoke && !grant.revokedAt && grant.enabled !== false) {
          revokedCount += 1;
          upsertGrant({
            ...grant,
            enabled: false,
            revokedAt: migrationAt,
            updatedAt: migrationAt,
            reason: "grant_owner_migration_retired"
          });
          appendGrantEvent(grant.id, "revoked", {
            reasonCode: "grant_owner_migration_retired",
            migrationReceipt: true
          });
          db.prepare(`
            INSERT INTO tool_grant_owner_migration_invalidations (
              migration_id, grant_id, created_at, updated_at
            ) VALUES ('catalog-owner-binding', ?, ?, ?)
            ON CONFLICT(migration_id, grant_id) DO NOTHING
          `).run(grant.id, migrationAt, migrationAt);
        }
      }
      const receiptDigest = crypto.createHash("sha256").update(JSON.stringify({
        protocol: "operation-permission-grant-owner-migration",
        migratedCount: ownerlessRows.length,
        revokedCount
      })).digest("hex");
      db.prepare(`
        INSERT INTO tool_grant_owner_migrations (
          migration_id, migrated_count, revoked_count, receipt_digest, completed_at
        ) VALUES ('catalog-owner-binding', ?, ?, ?, ?)
        ON CONFLICT(migration_id) DO UPDATE SET
          migrated_count = tool_grant_owner_migrations.migrated_count + excluded.migrated_count,
          revoked_count = tool_grant_owner_migrations.revoked_count + excluded.revoked_count,
          receipt_digest = excluded.receipt_digest,
          completed_at = excluded.completed_at
      `).run(ownerlessRows.length, revokedCount, receiptDigest, migrationAt);
    })();
  }
  void recoverOwnerMigrationInvalidations();
  ...
  async function createGrant(input = {}) {
    ...
    const policyRevision = ctx.currentGovernancePolicyRevision();
    const baseGrant = normalizeGrantAgainstCurrentCatalog({
      ...input,
      createdAt: nowIso(),
      updatedAt: nowIso()
    });
    ...
    if (getGrant(baseGrant.id)) {
      const error = new Error("Operation Permission grant identifier is already terminally reserved.");
      error.code = "operation_permission_grant_id_reserved";
      throw error;
    }
    const owners = resolveGrantOwners(baseGrant);
    let token = "";
    ...
    const grant = normalizeGrantAgainstCurrentCatalog({
      ...baseGrant,
      metadata: {
    ...
    const persistedGrant = getGrant(grant.id);
    const publicCreatedGrant = toPublicGrant(persistedGrant);
  ...
  async function updateGrant(grantId, patch = {}) {
    ...
    if (existing.revokedAt || existing.enabled === false) {
      const error = new Error("Terminal Operation Permission grants cannot be updated.");
      error.code = "operation_permission_grant_terminal";
      throw error;
    }
    rejectUnknownGrantCapabilities(patch);
    const updated = normalizeGrantAgainstCurrentCatalog(
      {
        ...patch,
        id: existing.id,
    ...
    const owners = resolveGrantOwners(updated);
    db.transaction(() => {
    ...
    const publicUpdatedGrant = toPublicGrant(getGrant(updated.id));
  ...
  async function rotateGrantToken(grantId) {
    ...
    if (existing.revokedAt || existing.enabled === false) {
      const error = new Error("Terminal Operation Permission grants cannot be rotated.");
      error.code = "operation_permission_grant_terminal";
      throw error;
    }
    assertGrantOwnersActive(existing.owners);
  ...
  function normalizePluginOwnerGeneration(value) {
    const generation = String(value || "").trim().toLowerCase();
    if (!/^[a-f0-9]{64}$/u.test(generation)) {
      const error = new Error("Plugin grant owner generation must be an artifact digest.");
  ...
  function registerPluginGrantOwner({ pluginId, generationDigest } = {}) {
    const ownerId = String(pluginId || "").trim();
    const generation = normalizePluginOwnerGeneration(generationDigest);
    if (!/^[a-z0-9][a-z0-9-]{0,127}$/u.test(ownerId)) {
      const error = new Error("Plugin grant owner identifier is invalid.");
    ...
    db.transaction(() => {
      ...
      if (exact?.state === "retiring" || exact?.state === "retired") {
        const error = new Error("Plugin grant owner generation is not eligible for activation.");
      ...
      if (current && current.owner_generation !== generation) {
        const error = new Error("A different plugin grant owner generation is still current.");
        error.code = "operation_permission_plugin_owner_generation_conflict";
        throw error;
      }
      if (current?.state === "retiring") {
        const error = new Error("Plugin grant owner generation is still retiring.");
  ...
  function ownerRevocationReceipt(job) {
    return Object.freeze({
      ok: true,
  ...
  function assertPluginOwnerAuthority(pluginId, ownerGeneration) {
    ...
    if (!persistedOwner && !persistedJob) {
      const error = new Error("Plugin grant owner is unknown to Operation Permission.");
      error.code = "operation_permission_unknown_plugin_owner";
      throw error;
    }
  }

  async function invalidateOwnerRevocationTargets(idempotencyKey, batchSize) {
    const targets = db.prepare(`
    ...
    for (const target of targets) {
      try {
        ...
        const error = new Error("Plugin-owner grant credential revocation is pending recovery.");
  ...
  function finalizeOwnerRevocationTargets(idempotencyKey) {
    return db.transaction(() => {
      const pending = db.prepare(`
        ...
        ORDER BY grant_id
      `).all(idempotencyKey);
      if (pending.some((target) => !target.capability_invalidated || !target.binding_invalidated)) return false;
      if (pending.length === 0) return true;
      const newlyRevoked = pending.reduce((count, target) => count + (target.newly_revoked ? 1 : 0), 0);
      const lastGrantId = pending.at(-1).grant_id;
      ...
      const committedCursor = revocationDigest({
        key: idempotencyKey,
  ...
  async function revokeGrantsByPluginOwner({
    ...
    const ownerId = String(pluginId || "").trim();
    const ownerGeneration = normalizePluginOwnerGeneration(generationDigest);
    const key = String(idempotencyKey || "").trim();
    if (!/^[a-z0-9][a-z0-9-]{0,127}$/u.test(ownerId) || !key || key.length > 256) {
      const error = new Error("Plugin-owner grant revocation requires a closed owner and idempotency key.");
      error.code = "operation_permission_plugin_owner_revocation_invalid";
      throw error;
    }
    const limit = Math.max(1, Math.min(256, Math.trunc(Number(batchSize) || 64)));
    assertPluginOwnerAuthority(ownerId, ownerGeneration);
    const timestamp = nowIso();
    db.transaction(() => {
      const existingJob = db.prepare("SELECT plugin_id, owner_generation FROM tool_grant_owner_revocations WHERE idempotency_key = ?")
        .get(key);
      ...
      if (authority?.state === "retiring") {
        const error = new Error("Plugin grant owner generation is retiring under another request.");
        error.code = "operation_permission_plugin_owner_generation_retiring";
        throw error;
      }
      ...
      if (retired.changes !== 1) {
        const error = new Error("Plugin grant owner generation is not active for retirement.");
        error.code = "operation_permission_plugin_owner_generation_inactive";
        throw error;
      }
      db.prepare(`
    ...
    let job = db.prepare("SELECT * FROM tool_grant_owner_revocations WHERE idempotency_key = ?").get(key);
    if (!job || job.plugin_id !== ownerId || job.owner_generation !== ownerGeneration) {
      const error = new Error("Plugin-owner grant revocation idempotency key conflicts with another owner.");
      error.code = "operation_permission_plugin_owner_revocation_conflict";
      throw error;
    }
    if (cursor && cursor !== job.cursor_token) {
      const error = new Error("Plugin-owner grant revocation cursor is stale or invalid.");
    ...
    db.transaction(() => {
      ...
      for (const { grant_id: grantId } of ownerRows) {
        ...
        const newlyRevoked = !grant.revokedAt && grant.enabled !== false;
        if (newlyRevoked) {
          upsertGrant({
            ...grant,
            enabled: false,
  ...
  function finishGrantAuthorization({
    ...
    const integrityDenial = policyIntegrityDenial(grant);
    ...
    const authorizationDecision = evaluateAuthorizationPolicy(policyInput);
    const grantBoundaryAllowsRuntimeGate = authorizationDecision.allowed ||
    ...
    if (parentGrant) {
      const parentAuthorizationDecision = evaluateAuthorizationPolicy({
        ...policyInput,
      ...
      const parentBoundaryAllowsRuntimeGate = parentAuthorizationDecision.allowed ||
    ...
    const usedAt = nowIso();
    const parentGrantId = parentGrant?.id || "";
    const consumed = consumeGrantUseStmt.run(
      usedAt,
      grant.id,
      grant.tokenHash,
      grant.updatedAt,
    ...
    if (consumed.changes !== 1) {
      const current = getGrant(grant.id);
      const reasonCode = current && Number(current.maxUses || 0) > 0 && Number(current.useCount || 0) >= Number(current.maxUses || 0)
        ? "grant_max_uses"
        : parentGrantId
    ...
    const updated = getGrant(grant.id);
    return {
      ok: true,
  ...
  async function authorizeOpaqueToolCapability({
    ...
    const dynamicCapability = context?.dynamicCapability && typeof context.dynamicCapability === "object" && !Array.isArray(context.dynamicCapability)
      ? context.dynamicCapability
      : null;
    if (dynamicCapability) {
      const capabilityId = String(dynamicCapability.capabilityId || "").trim();
      const serviceId = String(dynamicCapability.serviceId || dynamicCapability.resourceContext?.serviceId || "").trim();
      const grantedDynamicCapabilities = new Set(normalizeDynamicUpstreamCapabilities(grant.dynamicCapabilities));
      ...
      const allowedServiceIds = new Set(grant.allowedServiceIds || []);
      ...
      const allowedSecretBindings = new Set(grant.allowedSecretBindings || []);
      const credentialBindingIds = [...new Set((Array.isArray(dynamicCapability.credentialBindingIds)
        ? dynamicCapability.credentialBindingIds
        : []).map((value) => String(value || "").trim()).filter(Boolean))];
      const missingCredentialBindings = credentialBindingIds.filter((bindingId) =>
      ...
      if (missingCredentialBindings.length > 0) {
        return {
          ...
          missingCapabilities: missingCredentialBindings.map((bindingId) => `${capabilityId}:${bindingId}`),
          missingScopes: [],
          grant: toPublicGrant(grant),
          authorizationDecision: {
            ok: false,
            reasonCode: "upstream_credential_binding_denied",
            requiredCapabilities: missingCredentialBindings.map((bindingId) => `${capabilityId}:${bindingId}`)
          }
        };
      }
    }
  ...
  async function authorizeRequest({
    ...
    const grant = rows.map(hydrateGrant).find((item) => safeCompare(item.tokenHash, tokenHash));
    if (!grant) {
      return {
        ok: false,
        status: 401,
    ...
    const delegatedParent = activeDelegatedParent(grant);
    ...
    const targetBindingDecision = localMcpTargetBindingDecision(grant, request);
    ...
    const processIdentityDecision = await verifyLocalMcpProcessIdentity({
      grant,
      request,
      requestBody,
      url,
