...
function parseStoredJson(value, fallback, expectedType) {
  try {
...
export function rowToGrant(row) {
  if (!row) {
    return null;
  }
  const storedPolicy = {
  ...
  const metadata = storedPolicy.metadata.value;
  const invalidFields = Object.entries(storedPolicy)
...
export function publicGrant(grant, { catalogFingerprint = "" } = {}) {
  if (!grant) {
    return null;
  }
...
export function grantProjectionDescriptor(grant = {}, {
  metadata = {},
  catalogFingerprint = ""
} = {}) {
  ...
  const projection = {
    ...
    owners: (Array.isArray(grant.owners) ? grant.owners : [])
      ...
      .filter((owner) => owner.ownerKind && owner.ownerId && owner.ownerGeneration)
      .sort((left, right) =>
...
export function summarizeValue(value) {
  const redacted = redactOperationAuditValue(value);
  ...
  const summary = {};
  for (const [key, nested] of Object.entries(redacted).slice(0, 40)) {
...
function summarizeMetadataOnlyValue(value) {
...
function delegatedChildOperationSummary(value = null) {
  ...
  const mismatches = Array.isArray(value.requestBindingMismatches)
    ...
    : [];
  const missingBindings = Array.isArray(value.missingRequestBindings)
    ? value.missingRequestBindings.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 16)
    : [];
  const summary = {
...
export function executionResultSummary(entry = {}) {
  const base = entry.resultSummary !== undefined
    ? entry.resultSummary
    : summarizeValue(entry.result || {});
  const delegatedChildOperation = delegatedChildOperationSummary(
    {
...
export function rowToPendingOperation(row, { includeOriginalInput = false } = {}) {
  if (!row) {
    return null;
  }
  const pending = {
