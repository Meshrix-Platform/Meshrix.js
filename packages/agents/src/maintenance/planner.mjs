...
} from "../agent-gateway/index.mjs";
import { maxRisk } from "./config.mjs";

const MAINTENANCE_AGENT_MODULE_ID = "maintenance-agent-runbooks";
...
function buildDailyStorageMaintenancePlan() {
...
export function createMaintenancePlanner({
  ...
  async function runbookPlan(input = {}) {
    const rawPlan = buildRunbookPlan(input.runbook, input.options || {});
  ...
  async function plan(input = {}, config = {}) {
    ...
    const plannerMode = String(config.plannerMode || "").trim();
    ...
    if (plannerMode === "gateway") {
      ...
      const settings = await loadRuntimeSettings(userDataPath);
      const requestedModelAlias = String(effectiveInput.modelAlias || effectiveInput.alias || "").trim();
