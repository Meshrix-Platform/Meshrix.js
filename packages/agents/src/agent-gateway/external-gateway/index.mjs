...
function normalizeRoutes(routes, activePluginRoutes) {
  ...
  if (!normalized.some((route) => route.path === "/")) {
    normalized.push(cloneRoute({ routeId: "lico-http", path: "/", trafficClass: "default" }));
  }
...
export function normalizeExternalGatewayProfile(input = {}) {
  ...
  const profile = Object.freeze({
    schemaVersion: "v0.0.1:schema:definition-1",
    protocol: EXTERNAL_GATEWAY_PROTOCOL_VERSION,
    profileId: String(input.profileId || `lico-external-gateway-${adapterId}`).trim(),
    mode,
    adapterId,
    ...
    trustedHeaderPolicy: Object.freeze({
      ...
      gatewayHeaders: Object.freeze([
        "X-LicoMesh-Gateway",
        "X-LicoMesh-Gateway-Route",
        "X-LicoMesh-Gateway-Request-Id",
        "X-Request-Id",
        "X-Forwarded-For",
        "X-Forwarded-Host",
        "X-Forwarded-Proto"
...
function renderCaddyProxyBlock({ matcher = "", upstreams, adapterId, streaming = false }) {
  ...
  const lines = [
    ...
    `    header_up X-LicoMesh-Gateway ${adapterId}`,
    "    header_up X-LicoMesh-Gateway-Route {http.request.uri.path}",
    "    header_up X-LicoMesh-Gateway-Request-Id {http.request.uuid}",
...
export function renderCaddyConfig(profileInput = {}) {
  ...
  return [
    "# Generated by LicoMesh. Edit the gateway profile and regenerate instead of hand-editing.",
    "# LicoMesh must keep working through directMode.baseUrl when this gateway is removed.",
    "{",
    "  auto_https off",
    "}",
    "",
    ...
    "  }",
    "",
    `  @lico_streaming path ${streamingMatchers.join(" ")}`,
    renderCaddyProxyBlock({
      matcher: "@lico_streaming",
...
function renderNginxProxySettings({ profile, adapterId = "nginx", streaming = false }) {
  const lines = [
    ...
    `      proxy_set_header X-LicoMesh-Gateway ${adapterId};`,
    "      proxy_set_header X-LicoMesh-Gateway-Route $uri;",
    "      proxy_set_header X-LicoMesh-Gateway-Request-Id $request_id;",
...
function renderNginxLocation(route, profile, scheme) {
  ...
  const upstreamName = route.streaming ? "lico_backend_stream" : "lico_backend";
  return [
    `    location ${modifier} ${route.path} {`,
    `      client_max_body_size ${route.bodyLimit};`,
    "      limit_req zone=lico_default burst=40 nodelay;",
...
export function renderNginxConfig(profileInput = {}) {
  ...
  return [
    ...
    "",
    "http {",
    "  limit_req_zone $binary_remote_addr zone=lico_default:10m rate=120r/m;",
    "  map $http_upgrade $connection_upgrade {",
    "    default upgrade;",
    "    '' '';",
    "  }",
    "",
    "  upstream lico_backend {",
    "    least_conn;",
    ...profile.gatewayMode.upstreams.map((upstream) => `    server ${upstream.authority};`),
    "    keepalive 32;",
    "  }",
    "",
    "  upstream lico_backend_stream {",
...
export function resolveExternalGatewayRuntimePlan(input = {}, env = process.env) {
  ...
  const configuredBinary = String(
    input.runtimeBinary ||
      input.binary ||
      env[`LICO_${adapter.adapterId.toUpperCase()}_BINARY`] ||
      env.LICO_GATEWAY_RUNTIME_BINARY ||
      ""
  ).trim();
  const runtimeUrl = String(
    input.runtimeUrl ||
      input.url ||
      env[`LICO_${adapter.adapterId.toUpperCase()}_RUNTIME_URL`] ||
      env.LICO_GATEWAY_RUNTIME_URL ||
      ""
  ).trim();
  return Object.freeze({
    schemaVersion: "v0.0.1:schema:definition-1",
