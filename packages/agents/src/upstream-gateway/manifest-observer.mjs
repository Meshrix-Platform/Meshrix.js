import { normalizeService } from "./support.mjs";
...
function assertUniqueOperations(service) {
  const keys = new Set();
...
function deepFreezeRuntimeValue(value) {
  ...
  for (const nested of Object.values(value)) deepFreezeRuntimeValue(nested);
  return Object.isFrozen(value) ? value : Object.freeze(value);
}

function runtimeServiceFromRecord(record) {
  assertRecordIdentity(record);
  const state = record?.manifest?.payload?.state;
  if (state === "removed") return null;
  const descriptor = record?.manifest?.payload?.descriptor;
  ...
  const credentialReferences = record.manifest.references
    .filter((reference) => reference.type === "credential")
    .map((reference) => Object.freeze({
      ...reference,
      ...(reference.scopes ? { scopes: Object.freeze([...reference.scopes]) } : {})
    }));
  const credentialRefs = credentialReferences.map((reference) => reference.reference);
  ...
  return deepFreezeRuntimeValue({
    ...
    updatedAt: `revision-${record.serviceRevision}`,
    credentialReferences: service.credentialReferences.map((reference) => ({
      ...reference,
...
function immutableRuntimeSnapshot(storageSnapshot) {
  const records = storageSnapshot.listServices();
  ...
  for (const record of records) {
    ...
    const service = runtimeServiceFromRecord(record);
    services.set(record.serviceId, service);
  }
  const serviceEntries = Object.freeze(
    [...services.entries()]
      .filter(([, service]) => service !== null)
      .map(([serviceId, service]) => Object.freeze([serviceId, service]))
  );
...
export function createUpstreamManifestObserver({
  ...
  const interval = Math.max(10, Math.min(Number(pollIntervalMs || DEFAULT_POLL_INTERVAL_MS), 60_000));
  let acceptedSetRevision = -1;
  let acceptedSetDigest = "";
  let building = false;
  let pending = false;
  let closed = false;
  let timer = null;
  ...
  function report(error, reasonCode) {
    try {
  ...
  function schedule(delay = interval) {
    ...
    timer = setTimeout(() => {
      timer = null;
      void scan().finally(() => schedule(interval));
    }, delay);
    timer.unref?.();
  }

  async function executeScan({ signal, force }) {
    try {
      const scanSignal = signal
        ? AbortSignal.any([signal, lifecycleAbort.signal])
        : lifecycleAbort.signal;
      const storageSnapshot = await readerPort.getSnapshot({ signal: scanSignal });
      ...
      const candidate = immutableRuntimeSnapshot(storageSnapshot);
      if (closed) return Object.freeze({ outcome: "closed", setRevision: acceptedSetRevision });
  ...
  function scan({ signal, force = false } = {}) {
    if (closed) {
    ...
    building = true;
    activeScan = executeScan({ signal, force }).finally(() => {
      building = false;
      activeScan = null;
      if (pending && !closed) {
        pending = false;
