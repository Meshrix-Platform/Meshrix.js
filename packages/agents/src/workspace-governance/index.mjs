...
function approvalRevision(value = {}) {
...
function evaluateCanonicalApproval(approvalFact, binding = {}, now = new Date()) {
...
function evaluatePolicy(policy = {}, request = {}, trusted = {}) {
...
export function createWorkspaceGovernanceRegistry({ userDataPath = "" } = {}) {
  ...
  const api = {
    ...
    async createShareGrant(input = {}, trusted = {}) {
      const registry = await readRegistry();
    ...
    async findIncompleteUnshare(input = {}) {
      const registry = await readRegistry();
    ...
    async recordIncompleteUnshare(input = {}) {
      const registry = await readRegistry();
      const workspaceId = text(input.workspaceId || "");
      const targetWorkspaceId = text(input.targetWorkspaceId || "");
    ...
    async markIncompleteUnshareStage(input = {}) {
      const registry = await readRegistry();
      const idempotencyKey = text(input.idempotencyKey || "");
      ...
      const updated = { ...record, stage, updatedAt: nowIso() };
  ...
  for (const methodName of [
    ...
    const method = api[methodName].bind(api);
    api[methodName] = (...args) => enqueueMutation(() => method(...args));
  }
  for (const methodName of ["describe", "findIncompleteUnshare"]) {
    const method = api[methodName].bind(api);
    api[methodName] = (...args) => afterMutations(() => method(...args));
  }
