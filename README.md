...
[Overview](#overview) · [Quick Start](#quick-start) · [Architecture](#architecture) · [Documentation](docs/README.md) · [Runbook](docs/RUNBOOK.md) · **[简体中文](README.zh-CN.md)**
...
LicoMesh runs as a Node.js server that forwards server-configured upstream
...
LicoMesh's product boundary is the server-side governance layer of a private
...
| Variable | Purpose |
| --- | --- |
| `LICO_SERVER_DATA_DIR` | Places runtime state in an explicit deployment directory. |
| `LICO_SERVER_HOST` | Server listen address. |
| `LICO_SERVER_PORT` | Server listen port. |
...
delivered as external `LicoMesh-Plugins` adapter packages rather than Core
...
<div align="center">
  <sub>LicoMesh — self-contained by default, built for private deployment.</sub>
</div>
