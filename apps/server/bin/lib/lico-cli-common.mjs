...
export const DEFAULT_SERVER_URL = process.env.LICO_SERVER_URL || getDefaultServerUrl();
...
export function applyCommonSafetyHeaders(args, headers = {}) {
  ...
  return {
    ...headers,
    "x-lico-safety-confirm": headers["x-lico-safety-confirm"] || "true"
  };
}
...
export function readHeaders(args) {
  const headers = {
    ...
    "x-lico-client-id": process.env.LICO_CLIENT_ID || "lico-cli",
    ...envAuthHeaders()
  };
  for (const [index, entry] of (args.header || []).entries()) {
    ...
    if (SENSITIVE_ARG_HEADER_NAMES.has(name.toLowerCase())) {
      throw new Error(`禁止通过命令行参数传入敏感请求头 ${name}；请使用 LicoMesh 密钥入口或受控环境变量`);
    }
...
export function envAuthHeaders() {
  const headers = {};
  if (process.env.LICO_CONSOLE_COOKIE) {
    headers.Cookie = process.env.LICO_CONSOLE_COOKIE;
  }
  if (process.env.LICO_CONSOLE_CSRF) {
    headers["x-lico-csrf"] = process.env.LICO_CONSOLE_CSRF;
  }
  if (["1", "true", "yes"].includes(String(process.env.LICO_SAFETY_CONFIRM || "").toLowerCase())) {
