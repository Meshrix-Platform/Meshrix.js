...
} from "#meshrix/operation-registry";
import {
...
export async function runRpc(args?: any) : Promise<any> {
  const method: any = String(args.method || args.m || "GET").toUpperCase();
  const apiPath: any = normalizeApiPath(requireValue(args, "path"));
...
export async function runServerRpcCall(args?: any) : Promise<any> {
  const rpcMethod: any = args["rpc-method"] || args._[1];
  if (!rpcMethod || rpcMethod === true) {
    throw new Error("rpc-call requires a RPC method, for example: meshrix rpc-call jobs.list");
  }
  const params: any = applyCommonSafetyFlags(args, await readRpcParams(args));
...
function toolIdArg(args?: any) : any {
  return String(args["tool-id"] || args.toolId || args.id || args._[2] || "").trim();
}

export async function runToolsCommand(args?: any) : Promise<any> {
  const command: any = String(args._[1] || "catalog");
  const subcommand: any = String(args._[2] || "");
  if (command === "catalog") {
    await writeResponse({
      args,
  ...
  if (command === "execute" || command === "dry-run") {
    const body: any = await readBody(args);
    const toolId: any = String(body.toolId || toolIdArg(args));
    if (!toolId) {
  ...
  if (command === "grants") {
    ...
    if (subcommand === "rotate" || subcommand === "revoke") {
      const grantId: any = String(args["grant-id"] || args.grantId || args.id || args._[3] || "").trim();
      if (!grantId) {
...
async function getActiveCliOperations(args: Record<string, any> = {}) : Promise<any> {
  ...
  const interfaces: any = Array.isArray(payload?.interfaces) ? payload.interfaces : [];
  return interfaces.map((entry?: any) : any => ({
    id: entry.id,
  ...
  })).filter((operation?: any) : any => operation.cli?.command?.length > 0);
}

function writeLocalInterfaceCatalog(args?: any, operations?: any) : any {
...
export async function runNamedRpc(args?: any, { usage = () : any => "" }: Record<string, any> = {}) : Promise<any> {
  if (args._[0] === "interfaces" && writeLocalInterfaceCatalog(args)) {
    return;
  }
  const activeOperations: any = await getActiveCliOperations(args);
  const cliMatch: any = findCliOperation(args._, activeOperations);
  if (!cliMatch) {
    throw new Error(`未知命令：${args._[0] || ""}\n${usage()}`);
  }
  const operation: any = cliMatch.operation;
  if (operation.id === "system.interfaces" && writeLocalInterfaceCatalog(args, activeOperations)) {
    return;
  }

  const apiPath: any = buildApiPathForCliOperation(operation, args);
  let body: any;
  let headers: any = applyCommonSafetyHeaders(args, readHeaders(args));
  if (operation.http.method !== "GET" && operation.http.method !== "HEAD") {
    const cliBody: any = args.body === undefined && !args["body-file"]
      ? buildBodyFromCliParams(operation, args)
      : null;
    if (cliBody) {
      body = cliBody;
    } else {
      const payload: any = await readHttpPayload(args, operation.http.method);
      body = payload.body;
