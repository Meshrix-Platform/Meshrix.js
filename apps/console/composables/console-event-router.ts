...
export const CONSOLE_EVENT_TOPICS = Object.freeze([
...
export function createConsoleEventRouter(options: ConsoleEventRouterOptions) {
  return function applyServerEvent(event: ProtocolEvent) {
    const payload = objectValue(event.payload) || {};
    if (event.topic === "system.console_state") {
      const state = serverConsoleStateValue(payload.state);
      if (!state) return false;
      options.applyConsoleState(state);
      return true;
    }
    if (event.topic === "jobs.job") {
      const job = objectValue(payload.job) as SplitJob | null;
      return Boolean(job?.id && options.upsertJob(job));
    }
    if (event.topic === "jobs.deleted") {
      const job = objectValue(payload.job) || objectValue(payload.deletedJob);
      const jobId = String(job?.id || payload.jobId || payload.batchId || "");
      return Boolean(jobId && options.removeJob(jobId));
    }
    if (event.topic === "settings.current") {
      const currentState = options.getConsoleState();
      if (!currentState) return false;
      const settings = objectValue(payload.value) || payload;
