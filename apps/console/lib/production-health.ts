...
import type { ProductionHealthGate, ProductionHealthResponse, ReadinessBaselineStatus } from "./types";
...
export function statusLabel(status: string) : any {
...
export function formatDateTime(value: string) : any {
  if (!value) return "未生成";
...
export function elapsedText(gate: ProductionHealthGate) : any {
  const ms: any = Number(gate.commandSummary?.elapsedMs || 0);
  if (!ms) return "0ms";
...
export type { ProductionHealthGate, ProductionHealthResponse, ReadinessBaselineStatus };
