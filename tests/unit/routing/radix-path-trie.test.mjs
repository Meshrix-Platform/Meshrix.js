...
import { RadixPathTrie } from "../../../packages/server-runtime/src/routing/radix-path-trie.mjs";

describe("RadixPathTrie", () => {
  describe("insert and lookup", () => {
    it("inserts and looks up a static path", () => {
      const trie = new RadixPathTrie();
      trie.insert("/api/health", { handler: "health" });
      const result = trie.lookup("/api/health");
    ...
    it("matches named parameters", () => {
      ...
      const result = trie.lookup("/api/workspaces/abc123");
      expect(result).not.toBeNull();
  ...
  describe("wildcard routes", () => {
    it("matches wildcard catch-all", () => {
      ...
      const result = trie.lookup("/static/css/main.css");
      expect(result).not.toBeNull();
  ...
  describe("conflict detection", () => {
    ...
    it("detects param name conflicts at same position", () => {
      const trie = new RadixPathTrie();
      ...
      const r1 = trie.lookup("/api/123/read");
      expect(r1).not.toBeNull();
      const r2 = trie.lookup("/api/456/write");
      expect(r2).not.toBeNull();
    });
  });

  describe("size and paths", () => {
    ...
    it("returns all paths", () => {
      ...
      trie.insert("/api/a", 1);
      trie.insert("/api/b", 2);
      const paths = trie.paths();
      expect(paths).toHaveLength(2);
    });
  });

  describe("performance", () => {
    it("handles 1000+ routes efficiently", () => {
      const trie = new RadixPathTrie();
      const COUNT = 1000;
