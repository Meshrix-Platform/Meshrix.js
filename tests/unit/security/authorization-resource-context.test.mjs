...
import { createAuthorizationEngine } from "../../../packages/foundation/src/security/authorization/authorization-engine.mjs";
...
describe("authorization resource context mapping", () => {
  it("maps operation resourceContext fieldMap aliases into workspace ABAC decisions", () => {
  ...
  it("uses nested resource objects for service and secret binding ABAC decisions", () => {
    const serviceDecision = evaluate({
      operation: {
        id: "gateway.forward",
  ...
  it("denies grant management inputs that delegate outside the subject resource boundary", () => {
    const decision = evaluate({
      operation: {
  ...
  it("denies grant metadata resource limits outside the subject boundary", () => {
    const decision = evaluate({
      operation: {
  ...
  it("denies grant metadata resource limits with the real registry mapping", async () => {
    const { SERVER_API_OPERATIONS } = await import("../../../packages/contracts/src/operations/operation-registry.mjs");
    const operation = SERVER_API_OPERATIONS.find((item) => item.id === "operation_permission.create_grant");
    const decision = evaluate({
      operation,
      authSession: {
        user: {
  ...
  it("denies gateway forwarding when credential or secret refs exceed the subject boundary", () => {
    const decision = evaluate({
      operation: {
        id: "gateway.forward",
        requiredScopes: ["gateway:write"],
  ...
  it("keeps legitimate mapped resource access allowed", () => {
    const decision = evaluate({
      operation: {
