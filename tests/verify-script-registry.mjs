#!/usr/bin/env node
/**
 * verify-script-registry.mjs — Script Registry Coverage & Consistency Verifier
 *
 * Validates:
 ...
 *
 * Usage:
 *   node tests/verify-script-registry.mjs
 */
...
import { scanPublicArtifactFiles } from "../tools/server-scripts/lib/public-artifact-boundary.mjs";
...
const FORBIDDEN_PACKAGED_INTERNAL_PATH_PATTERN =
  /(^|\/)docs\/(?:plan|report|decisions)(?:\/|$)/u;

// Import script registry
const scriptRegUrl = pathToFileURL(
  path.join(repoRoot, "tools/scripts/package-script-registry.mjs")
).href;
const scriptReg = await import(scriptRegUrl);
const { createReleaseEvidenceReadiness } = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/lib/release-evidence-readiness.mjs")
).href);
const { validateFactSourceAuthorityFindings } = await import(pathToFileURL(
  path.join(repoRoot, "tools/verifiers/registry-fact-source-authority.mjs")
).href);
const {
  ...
  path.join(repoRoot, "tools/server-scripts/verify-platform-acceptance.mjs")
).href);
const {
  createReleaseCommandSchedule
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/lib/release-command-dag-runner.mjs")
).href);
const {
  ...
  path.join(repoRoot, "tools/server-scripts/lib/upstream-fixture-transit-evidence.mjs")
).href);
const {
  UPSTREAM_MCP_GATEWAY_REPORT_PATH,
  UPSTREAM_MCP_GATEWAY_REQUIRED_TEST_NAMES
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/lib/upstream-mcp-gateway-evidence.mjs")
).href);
const {
  ...
  path.join(repoRoot, "tools/server-scripts/lib/downstream-agent-tool-loop-evidence.mjs")
).href);
const {
  MCP_SUPPORTED_TARGETS
} = await import(pathToFileURL(
  path.join(repoRoot, "packages/protocols/mcp/adapter/mcp-release-targets.mjs")
).href);
const {
  SERVER_VERIFY_COMMANDS
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/verify-server-runtime.mjs")
).href);
const {
  SERVER_HEADLESS_VERIFY_COMMANDS
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/verify-server-headless.mjs")
).href);
const {
  SERVER_CHECKPOINTS_VERIFY_COMMANDS
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/verify-server-checkpoints.mjs")
).href);
const {
  SERVER_OPS_VERIFY_COMMANDS
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/verify-server-ops.mjs")
).href);
const {
  SERVER_REBUILD_VERIFY_COMMANDS
} = await import(pathToFileURL(
  path.join(repoRoot, "tools/server-scripts/verify-server-rebuild.mjs")
).href);

const suiteRegistry = JSON.parse(
  await fs.readFile(path.join(repoRoot, "tools/registry/tests.registry.json"), "utf8")
);
const repoLayoutRegistry = JSON.parse(
  await fs.readFile(path.join(repoRoot, "tools/registry/repo-layout.registry.json"), "utf8")
);

function registeredReleaseReportFixture(reportPath, report = {}) {
...
function registeredRepoOrganizationReportFixture(overrides = {}) {
  const base = {
    summary: {
      releaseReady: true,
      reportLeakScan: true,
...
const patternMatchedEntries = [];
const allowlistedEntries = [];
const releaseStrictnessFindings = [];
const releaseSourceOfTruthFindings = [];
const releaseProfileReadinessFindings = [];
const mcpReleaseTargetSourceOfTruthFindings = [];
const packagePackFindings = [];
const factSourceAuthorityFindings = [];

const RELEASE_CHAIN_SOURCES = Object.freeze([
  "tools/server-scripts/verify-platform-acceptance.mjs",
  "tools/server-scripts/production-readiness-gate.mjs"
]);
const PLATFORM_ACCEPTANCE_SOURCE =
  "tools/server-scripts/verify-platform-acceptance.mjs";
const PLATFORM_ACCEPTANCE_COMMAND_CATALOG_SOURCE =
  "tools/server-scripts/lib/platform-acceptance-command-catalog.mjs";
const PRIVATE_DEPLOYMENT_OPEN_PLATFORM_E2E_CATALOG_SOURCE =
...
const REQUIRED_FACT_AUTHORITY_PATHS = Object.freeze([
  ...
  "tools/server-scripts/lib/upstream-mcp-gateway-evidence.mjs",
  ...
  "packages/foundation/src/security/process-identity/index.mjs",
...
const REQUIRED_FACT_AUTHORITY_KEYS = Object.freeze({
  ...
  "process-identity.runtime-contract": "packages/foundation/src/security/process-identity/index.mjs",
  ...
  "upstream-mcp.gateway-evidence": "tools/server-scripts/lib/upstream-mcp-gateway-evidence.mjs",
...
const RELEASE_PROFILE_SOURCES = Object.freeze([
  "tools/server-scripts/stress-gateway-platform-profile.mjs"
]);
const MCP_RELEASE_TARGET_CONSUMER_SOURCES = Object.freeze([
...
const REPORT_ONLY_SCRIPT_PATTERN = /(?::report|:audit:report)$/u;
const REPORT_ONLY_FLAG_PATTERN = /--(?:allow-open-gaps|report-only)\b/u;
const RELEASE_EVIDENCE_READINESS_HELPER =
  "tools/server-scripts/lib/release-evidence-readiness.mjs";
const RELEASE_EVIDENCE_FRESHNESS_HELPER =
  "tools/server-scripts/lib/release-evidence-freshness.mjs";
const UPSTREAM_FIXTURE_TRANSIT_HELPER_PATTERN = /createUpstreamFixtureTransitReadiness/u;
const UPSTREAM_MCP_GATEWAY_HELPER_PATTERN = /createUpstreamMcpGatewayReadiness/u;
const DOWNSTREAM_AGENT_TOOL_LOOP_HELPER_PATTERN = /createDownstreamAgentToolLoopReadiness/u;
const MCP_CREDENTIAL_STORE_HELPER_PATTERN = /createMcpProcessIdentityCredentialStoreReadiness/u;
const MCP_CREDENTIAL_STORE_REPORT_PATH_PATTERN = /MCP_PROCESS_IDENTITY_CREDENTIAL_STORE_REPORT_PATH/u;
const PLATFORM_ACCEPTANCE_REPORT_CATALOG_IMPORT_PATTERN = /platform-acceptance-report-catalog\.mjs/u;
const RELEASE_AGGREGATOR_FULL_AGGREGATION_PATTERN = /commandExecutionMode:\s*["']dag-parallel-full-aggregation["']/u;
const RELEASE_COMMAND_DAG_RUNNER_IMPORT_PATTERN = /release-command-dag-runner\.mjs/u;
const RELEASE_COMMAND_DAG_SCHEDULE_PATTERN = /commandSchedule/u;
const RELEASE_COMMAND_DAG_CHILD_PROCESS_PATTERN = /spawn\s*\(/u;
const RELEASE_AGGREGATOR_FAIL_FAST_PATTERN =
  /for\s*\(\s*const\s+command\s+of\s+COMMANDS\s*\)\s*\{[\s\S]{0,600}result\.status\s*!==\s*["']passed["'][\s\S]{0,160}break\s*;/u;
const GATEWAY_PLATFORM_PROFILE_STRICT_EXIT_PATTERN =
  /if\s*\(\s*!report\.summary\.releaseReady\s*\)\s*\{\s*process\.exitCode\s*=\s*1\s*;/u;
const RELEASE_PROFILE_AGGREGATE_REDUCER_PATTERN =
  /createAggregateReleaseEvidenceReadiness/u;
const RELEASE_PROFILE_SUMMARY_SOURCE_PATTERN =
  /releaseReadinessSourceOfTruth:\s*aggregateReadiness\.sourceOfTruth/u;
const RELEASE_PROFILE_FRESHNESS_HELPER_PATTERN =
  /release-evidence-freshness\.mjs/u;
const RELEASE_PROFILE_CANONICAL_CONSUMPTION_PATTERN =
  /consumedCanonicalReport:\s*true/u;
const RELEASE_PROFILE_CHILD_LEAK_SCAN_MISSING_EVIDENCE_PATTERN =
  /report-leak-scan-not-passed/u;
const RELEASE_PROFILE_AGGREGATE_LEAK_SCAN_PATTERN =
  /reportLeakScan:\s*reportLeakScanReady/u;
const MCP_GATEWAY_LOAD_RESOURCE_CUTOFF_REASON_PATTERN = /resource-safety-cutoff/u;
const MCP_GATEWAY_LOAD_INCOMPLETE_PHASE_REASON_PATTERN = /phase-incomplete/u;
const MCP_GATEWAY_LOAD_SUMMARY_LEAK_SCAN_PATTERN = /reportLeakScan:\s*true/u;
const RELEASE_EVIDENCE_READINESS_IMPORT_PATTERN = /release-evidence-readiness\.mjs/u;
const RELEASE_EVIDENCE_READINESS_REDUCER_PATTERN =
  /createReleaseEvidenceReadiness|releaseEvidenceReady/u;
const AGGREGATE_RELEASE_EVIDENCE_READINESS_PATTERN = /createAggregateReleaseEvidenceReadiness/u;
const RELEASE_EVIDENCE_FRESHNESS_PATTERN = /release-evidence-freshness\.mjs/u;
const PLATFORM_ACCEPTANCE_REPORT_RESET_PATTERN = /ACCEPTANCE_REQUIRED_REPORTS\.map\(removeReport\)/u;
const PLATFORM_ACCEPTANCE_TIMESTAMP_VALIDATION_PATTERN = /minimumTimestampMs/u;
const MCP_RELEASE_TARGET_SOURCE_PATTERN = /mcp-release-targets\.mjs/u;
const MCP_RELEASE_TARGET_HARDCODED_LIST_PATTERN = new RegExp(
  `\\[\\s*${MCP_SUPPORTED_TARGETS.map((target) => `["']${target.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}["']`).join("\\s*,\\s*")}\\s*\\]`,
  "u"
);
const GENERIC_PRODUCTION_BLOCKED_SKIP_PATTERN =
  /if\s*\(\s*evidence\.productionReleaseReady\s*===\s*false\s*\|\|\s*evidence\.productionReleaseStatus\s*===\s*["']blocked["']\s*\)\s*\{\s*continue\s*;/u;
const RAW_REPORT_PRODUCTION_FALLBACK_PATTERN =
...
const PRIVATE_REDUCER_ONLY_PATTERN =
  /commandExecutionMode:\s*["']platform-acceptance-existing-evidence-reduction["']/u;
const OPEN_DIRECT_AGGREGATE_RELEASE_READY_PATTERN =
  /releaseReady\s*=\s*failedCommands\.length\s*===\s*0\s*&&\s*missingEvidence\.length\s*===\s*0/u;
const PRODUCTION_GATE_REDUCER_CONTEXT_PATTERN =
  /createReleaseEvidenceReadiness\(\s*PRODUCTION_READINESS_GATES_REPORT_PATH\s*,\s*report\s*\)/u;
const PRODUCTION_GATE_DIRECT_PRODUCTION_READY_EXIT_PATTERN =
  /productionReadiness\.productionReleaseReady\s*!==\s*true/u;
const PRODUCTION_GATE_SUMMARY_MISSING_EVIDENCE_PATTERN =
  /missingEvidenceCount:\s*missingEvidence\.length[\s\S]{0,120}missingEvidence/u;
const PRODUCTION_GATE_PROJECTION_ONLY_PATTERN =
  /projectionOnly:\s*true/u;
const SOURCE_PATH_PATTERN = /^(?:apps|content|docs|packages|plugins|tests|tools)\/.+\.(?:cjs|css|html|js|json|mjs|svg|ts|tsx|vue|yaml|yml)$/u;
...
function packageScriptSourcePaths(command = "") {
  const paths = new Set();
  const sourceArgPattern = /(?:^|\s)(?:node|vitest|tsx|ts-node|bash|sh)\s+(?!-[^\s]*\s)(["']?[^"'`\s]+\.(?:cjs|js|json|mjs|sh|ts|tsx)["']?)/gu;
  const configArgPattern = /(?:--config|-p|--project)\s+["']?([^"'`\s]+\.(?:json|ts|js|mjs))["']?/gu;
  for (const pattern of [sourceArgPattern, configArgPattern]) {
    for (const match of command.matchAll(pattern)) {
      const normalized = normalizePackPath(match[1]);
...
function suiteSourcePaths(suite = {}) {
  return (Array.isArray(suite.args) ? suite.args : [])
    .map(normalizePackPath)
    .filter((candidate) => SOURCE_PATH_PATTERN.test(candidate) || ROOT_SOURCE_PATHS.has(candidate));
}
...
async function validateFactSourceAuthorityRegistry() {
  let registry;
  try {
  ...
  const authorityPaths = new Set(authorities.map((authority) =>
  ...
  const authorityPathByFactKey = new Map(authorities.map((authority) => [
    String(authority.factKey || "").trim(),
    normalizePackPath(authority.authorityPath)
  ]));
  for (const [requiredFactKey, requiredPath] of Object.entries(REQUIRED_FACT_AUTHORITY_KEYS)) {
    const authorityPath = authorityPathByFactKey.get(requiredFactKey);
...
function validateCommandReportCatalogConsistency({ source, commands, requiredReports }) {
  const requiredReportSet = new Set((Array.isArray(requiredReports) ? requiredReports : [])
    .map(normalizePackPath)
    .filter(Boolean));
  const reportOwners = new Map();
  for (const command of Array.isArray(commands) ? commands : []) {
    const report = normalizePackPath(command.report);
    if (report) {
      const previousOwner = reportOwners.get(report);
      if (previousOwner) {
...
function validateReleaseDagCatalogConsistency({ source, commands }) {
  const commandList = Array.isArray(commands) ? commands : [];
  const schedule = createReleaseCommandSchedule(commandList);
  ...
  for (const command of commandList) {
    const id = String(command.id || "(missing command id)");
...
const unregistered = allScripts.filter((s) => !scriptReg.isClassified(s));
...
for (const scriptName of explicitEntries) {
  const entry = scriptReg.getDeclaredEntry(scriptName);
  const pkgCommand = packageScripts[scriptName];
  const expectedAlias = `npm run ${scriptName}`;
...
for (const scriptName of patternMatchedEntries) {
  const entry = scriptReg.getEntry(scriptName, packageScripts);
...
const missing = [...suiteScriptRefs].filter(
  (s) => !scriptReg.isClassified(s)
);
if (missing.length > 0) {
...
const verifyPattern = /npm\s+run\s+(server:verify:[^\s"']+)/g;
const verifyCompositeScripts = [
  "server:verify", "server:verify:security-hardening",
];
const requiredServerRegressionGroups = Object.freeze([
  {
...
let verifySubIssues = 0;
for (const compName of verifyCompositeScripts) {
  if (!allScripts.includes(compName)) continue;
  const scriptCmd = packageScripts[compName];
  const matches = [...scriptCmd.matchAll(verifyPattern)];
...
for (const group of requiredServerRegressionGroups) {
  ...
  const groupScripts = group.commands.map((entry) => entry.script).filter(Boolean);
...
const dangerousSideEffectScripts = Object.values(scriptReg.SCRIPT_REGISTRY)
  .filter((s) => ["docker", "network-service", "destructive"].includes(s.sideEffects));
...
const freshContainerScripts = scriptReg.scriptsRequiringFreshContainer();
...
for (const [kind, names] of declarationGroups) {
  for (const scriptName of names) {
    const previousKind = declarationOwners.get(scriptName);
    if (previousKind) {
...
const stale = [...declarationOwners.keys()].filter((scriptName) => !allScripts.includes(scriptName));
if (stale.length > 0) {
...
function parseNpmPackFiles(packOutputText = "", source = "npm pack") {
  let parsed;
  try {
  ...
  if (parsed && typeof parsed === "object") {
    ...
    const groupedRecords = Object.values(parsed).flatMap((value) => {
...
function assertNoInternalPackFiles(packRecords = [], source = "npm pack") {
  for (const record of packRecords) {
    for (const file of record.files || []) {
      const packedPath = normalizePackPath(file.path);
...
const packResult = spawnSync(npmCommand, ["pack", "--dry-run", "--json", "--ignore-scripts", "--silent"], {
  ...npmPackSpawnOptions
});
if (packResult.status !== 0) {
  ...
  const packOutput = parseNpmPackFiles(packResult.stdout, "npm pack --dry-run --json");
  assertNoInternalPackFiles(packOutput, "root npm pack");
  packedFiles = new Set((packOutput[0]?.files || []).map((file) => normalizePackPath(file.path)));
  ...
  const artifactBoundary = await scanPublicArtifactFiles(repoRoot, [...packedFiles], {
  ...
  const requiredPackSources = new Set(["package.json", ".gitignore"]);
...
const workspacePackResult = spawnSync(npmCommand, ["pack", "--dry-run", "--json", "--workspaces", "--ignore-scripts", "--silent"], {
  ...npmPackSpawnOptions
});
await fs.rm(npmPackCachePath, { recursive: true, force: true }).catch(() => {});
...
for (const relativePath of RELEASE_CHAIN_SOURCES.filter((source) =>
  source !== "tools/server-scripts/production-readiness-gate.mjs"
)) {
  ...
  const npmRunMatches = [...source.matchAll(/args:\s*\[\s*["']run["']\s*,\s*["']([^"']+)["']/gu)]
    .map((match) => match[1]);
  const commandStringMatches = [...source.matchAll(/npm\s+run\s+([^\s"'`]+)/gu)]
    .map((match) => match[1]);
  for (const scriptName of [...new Set([...npmRunMatches, ...commandStringMatches])]) {
  ...
  const nodeCommandMatches = [...source.matchAll(/args:\s*\[\s*["']([^"']+\.mjs)["']/gu)]
    .map((match) => match[1]);
  for (const scriptPath of nodeCommandMatches) {
    if (scriptPath.endsWith("core-platform-gap-audit.mjs") && source.includes("--allow-open-gaps")) {
      releaseStrictnessFindings.push({
        source: relativePath,
...
const releaseAggregatorSource = await fs.readFile(
  path.join(repoRoot, PLATFORM_ACCEPTANCE_SOURCE),
  "utf8"
);
const platformAcceptanceCommandCatalogSource = await fs.readFile(
...
const releaseEvidenceReadinessHelperSource = await fs.readFile(
  path.join(repoRoot, RELEASE_EVIDENCE_READINESS_HELPER),
  "utf8"
);
const scriptRegistryWithMissingPackSource = createReleaseEvidenceReadiness("build/reports/script-registry.json", registeredReleaseReportFixture("build/reports/script-registry.json", {
...
const scriptRegistryWithFactSourceAuthorityFinding = createReleaseEvidenceReadiness("build/reports/script-registry.json", registeredReleaseReportFixture("build/reports/script-registry.json", {
  totalPackageScripts: 1,
  commandMismatchCount: 0,
...
const governanceCoverageWithUnmappedOperation = createReleaseEvidenceReadiness("build/reports/enterprise-governance-coverage.json", registeredReleaseReportFixture("build/reports/enterprise-governance-coverage.json", {
  summary: {
    releaseReady: true,
    reportLeakScan: true,
...
const reportWithReleaseBlockingWarnings = createReleaseEvidenceReadiness("build/reports/repo-organization.json", registeredRepoOrganizationReportFixture({
  summary: {
    releaseReady: true,
    reportLeakScan: true,
...
const reportWithReleaseBlockingFindings = createReleaseEvidenceReadiness("build/reports/repo-organization.json", registeredRepoOrganizationReportFixture({
  summary: {
    releaseReady: true,
    reportLeakScan: true,
...
const releaseEvidenceFreshnessHelperSource = await fs.readFile(
...
const releaseCommandDagRunnerSource = await fs.readFile(
  path.join(repoRoot, "tools/server-scripts/lib/release-command-dag-runner.mjs"),
  "utf8"
);
if (!RELEASE_COMMAND_DAG_CHILD_PROCESS_PATTERN.test(releaseCommandDagRunnerSource) ||
  ...
  releaseSourceOfTruthFindings.push({
    source: "tools/server-scripts/lib/release-command-dag-runner.mjs",
...
const openReleaseSource = releaseAggregatorSource;
...
const productionReadinessGateSource = await fs.readFile(
  path.join(repoRoot, "tools/server-scripts/production-readiness-gate.mjs"),
  "utf8"
);
if (!PRODUCTION_GATE_REDUCER_CONTEXT_PATTERN.test(productionReadinessGateSource)) {
  releaseSourceOfTruthFindings.push({
    ...
    detail: "production-readiness-gate must reduce its server-owned report through release-evidence-readiness.mjs"
  });
}
...
const privateDeploymentSource = await fs.readFile(
  path.join(repoRoot, "tools/server-scripts/verify-private-deployment-open-platform-e2e.mjs"),
  "utf8"
);
if (!PRIVATE_REDUCER_ONLY_PATTERN.test(privateDeploymentSource)) {
  releaseSourceOfTruthFindings.push({
    source: "tools/server-scripts/verify-private-deployment-open-platform-e2e.mjs",
...
for (const relativePath of [
  ...
  if (!RELEASE_EVIDENCE_READINESS_IMPORT_PATTERN.test(source) ||
    ...
    releaseSourceOfTruthFindings.push({
      ...
      detail: "release/profile aggregators must reduce reports through release-evidence-readiness.mjs"
    });
  }
}
if (!UPSTREAM_FIXTURE_TRANSIT_HELPER_PATTERN.test(releaseEvidenceReadinessHelperSource) ||
  ...
  releaseSourceOfTruthFindings.push({
    ...
    detail: "Upstream fixture transit evidence must be reduced through upstream-fixture-transit-evidence.mjs from the shared release evidence reducer"
  });
}
if (!UPSTREAM_MCP_GATEWAY_HELPER_PATTERN.test(releaseEvidenceReadinessHelperSource) ||
  ...
  releaseSourceOfTruthFindings.push({
    ...
    detail: "Upstream MCP gateway approval evidence must be reduced through upstream-mcp-gateway-evidence.mjs from the shared release evidence reducer"
  });
}
if (!DOWNSTREAM_AGENT_TOOL_LOOP_HELPER_PATTERN.test(releaseEvidenceReadinessHelperSource) ||
  ...
  releaseSourceOfTruthFindings.push({
    ...
    detail: "Downstream agent tool loop evidence must be reduced through downstream-agent-tool-loop-evidence.mjs from the shared release evidence reducer"
  });
}
const upstreamMcpGatewayEvidenceByTestName = Object.freeze({
...
const upstreamMcpGatewayBaseReport = registeredReleaseReportFixture(UPSTREAM_MCP_GATEWAY_REPORT_PATH, {
  summary: {
    reportLeakScan: true,
    failedCount: 0,
    releaseReady: true,
  ...
  tests: UPSTREAM_MCP_GATEWAY_REQUIRED_TEST_NAMES.map((name) => ({
    name,
    status: "passed",
    evidence: structuredClone(upstreamMcpGatewayEvidenceByTestName[name] || {})
  }))
});
const upstreamMcpGatewayReady = createReleaseEvidenceReadiness(
  UPSTREAM_MCP_GATEWAY_REPORT_PATH,
...
for (const [kind, mutate] of [
  ["missing-required-test", (report) => { report.tests.pop(); }],
  ["approval-replayed", (report) => {
    report.tests.find((item) => item.name.startsWith("approval-required upstream MCP"))
      .evidence.upstreamHitDelta = 2;
  }],
  ["credential-binding-missing", (report) => {
    report.tests.find((item) => item.name.startsWith("approval-required upstream MCP"))
      .evidence.credentialBindingAuthorized = false;
  }],
  ["rejection-forwarded", (report) => {
    report.tests.find((item) => item.name.startsWith("rejected and duplicate"))
      .evidence.upstreamHitDelta = 1;
  }],
  ["expiry-resolution-accepted", (report) => {
    report.tests.find((item) => item.name.startsWith("expired upstream MCP"))
      .evidence.resolveRejected = false;
  }],
  ["audit-completed-twice", (report) => {
    report.tests.find((item) => item.name.startsWith("upstream MCP approval lifecycle"))
      .evidence.gatewayCompletedCount = 2;
  }],
  ["exactly-once-summary-missing", (report) => {
    report.summary.approvalExactlyOnceVerified = false;
  }]
]) {
  const mutated = structuredClone(upstreamMcpGatewayBaseReport);
  mutate(mutated);
  const readiness = createReleaseEvidenceReadiness(UPSTREAM_MCP_GATEWAY_REPORT_PATH, mutated);
...
const upstreamFixtureTransitIdentityProof = {
...
const upstreamFixtureTransitBaseReport = registeredReleaseReportFixture(UPSTREAM_FIXTURE_TRANSIT_REPORT_PATH, {
  summary: {
    reportLeakScan: true,
...
const upstreamFixtureTransitReady = createReleaseEvidenceReadiness(
  UPSTREAM_FIXTURE_TRANSIT_REPORT_PATH,
...
for (const [kind, mutate] of [
  ["missing-rest-credential-injection", (report) => { report.evidence.restForwarding.credentialHeaderInjectionProven = false; }],
  ["missing-mcp-identity-proof", (report) => { delete report.evidence.mcpTransit.identityProof; }],
  ["missing-mcp-stateful-increment-probe", (report) => { report.evidence.mcpTransit.stdioStatefulIncrementProbeProven = false; }],
  ["missing-mcp-stateful-session", (report) => { report.evidence.mcpTransit.stdioStatefulSessionReuseProven = false; }],
  ["inline-credential-in-descriptor", (report) => { report.evidence.secretStoreCredentialBinding.descriptorHasInlineCredential = true; }],
  ["destructive-tool-visible-downstream", (report) => { report.evidence.downstreamAgentProjection.destructiveToolHidden = false; }],
  ["missing-secret-store-credential-binding", (report) => { delete report.evidence.secretStoreCredentialBinding; }]
]) {
  const report = structuredClone(upstreamFixtureTransitBaseReport);
  mutate(report);
  const readiness = createReleaseEvidenceReadiness(UPSTREAM_FIXTURE_TRANSIT_REPORT_PATH, report);
  if (readiness.releaseReady === true) {
    releaseSourceOfTruthFindings.push({
      source: RELEASE_EVIDENCE_READINESS_HELPER,
...
const downstreamAgentCancellationEvidence = {
...
const downstreamAgentToolLoopBaseReport = registeredReleaseReportFixture(DOWNSTREAM_AGENT_TOOL_LOOP_REPORT_PATH, {
  summary: {
    reportLeakScan: true,
    serviceConfigured: true,
    selfContained: true
  ...
  evidence: {
    ...
    proxyClientTargets: DOWNSTREAM_AGENT_CLIENT_TARGETS.map((target) => ({
      target,
      status: "passed",
...
const downstreamAgentToolLoopReady = createReleaseEvidenceReadiness(
  DOWNSTREAM_AGENT_TOOL_LOOP_REPORT_PATH,
  downstreamAgentToolLoopBaseReport
);
if (JSON.stringify([...DOWNSTREAM_AGENT_CLIENT_TARGETS]) !== JSON.stringify([...MCP_SUPPORTED_TARGETS])) {
  releaseSourceOfTruthFindings.push({
    source: "tools/server-scripts/lib/downstream-agent-tool-loop-evidence.mjs",
...
for (const [kind, mutate] of [
  ["missing-proxy-target", (report) => { report.evidence.proxyClientTargets.pop(); }],
  ["destructive-tool-visible", (report) => { report.evidence.proxyClientTargets[0].destructiveToolHidden = false; }],
  ["missing-target-credential-proof", (report) => { report.evidence.proxyClientTargets[0].credentialProof.tokenProofMatchesIssuedCredential = false; }],
  ["missing-initialized-notification", (report) => { report.evidence.proxyClientTargets[0].initializedNotificationSent = false; }],
  ["wrong-client-framing", (report) => { report.evidence.proxyClientTargets[0].clientProtocolProfile.framing = "content-length"; }],
  ["missing-core-turn", (report) => { report.evidence.proxyClientTargets[0].completedTurnIds = ["initialize"]; }],
  ["missing-cancellation-closure", (report) => { report.evidence.cancellationPropagation.upstreamCancellationObserved = false; }],
  ["missing-secret-store-credential-binding", (report) => { delete report.evidence.secretStoreCredentialBinding; }]
]) {
  const report = structuredClone(downstreamAgentToolLoopBaseReport);
  mutate(report);
  const readiness = createReleaseEvidenceReadiness(DOWNSTREAM_AGENT_TOOL_LOOP_REPORT_PATH, report);
  if (readiness.releaseReady === true) {
    releaseSourceOfTruthFindings.push({
      source: RELEASE_EVIDENCE_READINESS_HELPER,
...
const downstreamEvidenceSourceOfTruthFindings = [];
const downstreamEvidenceSourceChecks = [
  {
    source: RELEASE_EVIDENCE_READINESS_HELPER,
    ...
    detail: "MCP process identity credential-store release evidence must be reduced through mcp-process-identity-credential-store-evidence.mjs from the shared release evidence reducer"
  },
  {
    source: "tools/server-scripts/verify-mcp-process-identity-credential-store.mjs",
    helperPattern: MCP_CREDENTIAL_STORE_HELPER_PATTERN,
...
for (const check of downstreamEvidenceSourceChecks) {
  const source = await fs.readFile(path.join(repoRoot, check.source), "utf8");
  const registryReducerMatches = !check.registeredReportPath ||
...
for (const relativePath of MCP_RELEASE_TARGET_CONSUMER_SOURCES) {
  ...
  if (!MCP_RELEASE_TARGET_SOURCE_PATTERN.test(source)) {
    mcpReleaseTargetSourceOfTruthFindings.push({
      ...
      detail: "MCP release target consumers must import packages/protocols/mcp/adapter/mcp-release-targets.mjs"
    });
  }
...
const gatewayPlatformProfileSource = await fs.readFile(
  path.join(repoRoot, "tools/server-scripts/stress-gateway-platform-profile.mjs"),
  "utf8"
);
if (
  ...
  releaseSourceOfTruthFindings.push({
    source: "tools/server-scripts/stress-gateway-platform-profile.mjs",
...
if (
  ...
  releaseSourceOfTruthFindings.push({
    ...
    detail: "gateway platform profile must consume canonical reports and validate freshness through release-evidence-freshness.mjs"
  });
}
...
const mcpGatewayLoadSource = await fs.readFile(
  path.join(repoRoot, "tools/server-scripts/stress-mcp-gateway.mjs"),
  "utf8"
);
if (
  ...
  releaseProfileReadinessFindings.push({
    source: "tools/server-scripts/stress-mcp-gateway.mjs",
...
const releaseProfileSourceFindings = releaseSourceOfTruthFindings.filter((finding) =>
...
const snapshot = generateScriptRegistrySnapshot();
await fs.writeFile(
...
function generateScriptRegistrySnapshot() {
  const scriptsByTier = {};
  const scriptsBySideEffects = {};

  for (const entry of Object.values(scriptReg.SCRIPT_REGISTRY)) {
    const tier = entry.tier || "unknown";
    if (!scriptsByTier[tier]) scriptsByTier[tier] = [];
    scriptsByTier[tier].push(entry.scriptName);

    const se = entry.sideEffects || "none";
  ...
  return {
    ...
    verifier: "tests/verify-script-registry.mjs",
    summary: {
    ...
    commandMismatches: commandMismatches.map((m) => ({
    ...
    entries: Object.values(scriptReg.SCRIPT_REGISTRY).map((s) => ({
      ...
      commandMatch: commandMismatches.some((m) => m.scriptName === s.scriptName) ? "mismatch" : "ok",
    })),
    patternMatched: patternMatchedEntries.map((name) => ({
      scriptName: name,
      packageCommand: packageScripts[name],
      classification: "pattern",
    })),
    allowlisted: allowlistedEntries.map((name) => ({
      scriptName: name,
