...
describe("upstream service publishing client contract", () => {
  it("sends an explicit create command without synthesizing omitted configuration", async () => {
    ...
    const payload = bridge.sendJson.mock.calls[0][2];
  ...
  it("binds replacement and removal to the server service id and both expected revisions", async () => {
  ...
  it("waits for a terminal publication with a bounded poll and then exposes runtime health", async () => {
    bridge.getJson
  ...
  it("fails after the configured publication poll bound", async () => {
