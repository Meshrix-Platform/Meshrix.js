...
describe("model credential prefix normalization", () => {
  it("preserves the separator required by Bearer authorization", () => {
    const entry = normalizeAgentModelEntry({
  ...
  it("rejects header-injection control characters", () => {
    expect(() => normalizeAgentModelEntry({
      uid: "agent_openai_explicit",
      provider: "openai",
      tokenPrefix: "Bearer\nInjected: yes",
    })).toThrow(/不能包含换行/u);
