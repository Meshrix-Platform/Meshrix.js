...
import { runControlledExecutionSandboxVerification } from "../../../tools/server-scripts/verify-controlled-execution-sandbox.mjs";
import { createReleaseEvidenceReadiness } from "../../../tools/server-scripts/lib/release-evidence-readiness.mjs";

describe("controlled execution sandbox", () => {
  it("fails closed across admission and lifecycle paths without claiming a production backend", async () => {
    const report = await runControlledExecutionSandboxVerification({ writeReport: false });
