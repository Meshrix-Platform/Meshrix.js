...
} from "../../../tools/server-scripts/publish-release-set.mjs";
...
function filenameFor(name, version) {
  return `${name.replace(/^@/u, "").replace(/\//gu, "-")}-${version}.tgz`;
}

function tagsKey(name) {
  return `dist-tags:${name}`;
}

function publishedDistribution(name, version, integrity = integrityFor(name)) {
  return {
    integrity,
    signatures: [{
...
function addPublishedVersion(registry, packageRecord, {
...
function createInjectedNpmRunner({ registry = new Map() } = {}) {
  const calls = [];
  const publishCalls = [];
  ...
  const runner = async (args, { cwd }) => {
    ...
    if (args[0] === "pack") {
      ...
      const filename = filenameFor(manifest.name, manifest.version);
      const integrity = integrityFor(manifest.name);
      const destination = args[args.indexOf("--pack-destination") + 1];
      const tarballPath = path.join(destination, filename);
    ...
    if (args[0] === "view") {
      const key = args[2] === "dist-tags" ? tagsKey(args[1]) : args[1];
      if (!registry.has(key)) {
    ...
    if (args[0] === "publish") {
      const tarball = tarballs.get(args[1]);
      if (!tarball) return { exitCode: 1, stdout: "", stderr: "fixture missing tarball" };
      const tag = args[args.indexOf("--tag") + 1];
      const version = tarball.spec.slice(tarball.spec.lastIndexOf("@") + 1);
      registry.set(
        tarball.spec,
...
describe("npm release-set publication", () => {
  it("discovers public workspaces plus the connector and orders every internal dependency before the root", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const names = releaseSet.packages.map(({ name }) => name);
  ...
  it("keeps dry-run fully offline while packing the complete real release set", async () => {
    const injected = createInjectedNpmRunner();
    const result = await publishReleaseSet({
      rootDir: ROOT,
      dryRun: true,
    ...
    expect(result.packages.map(({ action }) => action)).toEqual(Array(9).fill("planned"));
    expect(injected.calls).toHaveLength(9);
    expect(injected.calls.every(({ args }) => args[0] === "pack")).toBe(true);
  });

  it("preflights every package against the registry without publication credentials or mutations", async () => {
    const injected = createInjectedNpmRunner();
    const result = await publishReleaseSet({
      rootDir: ROOT,
      preflight: true,
    ...
    expect(result.packages.map(({ action }) => action)).toEqual(Array(9).fill("publish"));
    expect(injected.calls.filter(({ args }) => args[0] === "pack")).toHaveLength(9);
    expect(injected.calls.filter(({ args }) => args[0] === "view")).toHaveLength(18);
    expect(injected.calls.some(({ args }) => (
      args[0] === "publish" || args[0] === "install" || args[0] === "audit"
    ))).toBe(false);
  });

  it("keeps local dry-run and registry preflight as separate modes", () => {
    expect(parsePublishArguments(["--preflight", "--tag", "next"]))
      .toEqual({ dryRun: false, preflight: true, tag: "next", help: false });
    expect(() => parsePublishArguments(["--dry-run", "--preflight"]))
      .toThrowError(expect.objectContaining({ code: "release_set_argument_conflict" }));
  });

  it("publishes missing tarballs with provenance once and skips matching immutable versions on rerun", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const alreadyPublished = releaseSet.packages[0];
    const registry = new Map();
    addPublishedVersion(registry, alreadyPublished);
    const injected = createInjectedNpmRunner({ registry });

    const first = await publishReleaseSet({
      rootDir: ROOT,
      runner: injected.runner,
      environment: {}
    });
    expect(first.packages.filter(({ action }) => action === "skipped").map(({ name }) => name))
      .toEqual([alreadyPublished.name]);
    expect(first.packages.filter(({ action }) => action === "published")).toHaveLength(8);
    expect(injected.calls.slice(0, 9).every(({ args }) => args[0] === "pack")).toBe(true);
    expect(injected.publishCalls.map(({ name }) => name)).toEqual(
      first.packages.filter(({ action }) => action === "published").map(({ name }) => name)
    );
    ...
    expect(injected.calls.some(({ args }) => args[0] === "install")).toBe(true);
    expect(injected.calls.some(({ args }) => (
      args[0] === "audit" && args[1] === "signatures" && args.includes("--include-attestations")
    ))).toBe(true);

    const publishedCallCount = injected.publishCalls.length;
    const second = await publishReleaseSet({
      rootDir: ROOT,
      runner: injected.runner,
      environment: {}
    });
    expect(second.packages.every(({ action }) => action === "skipped")).toBe(true);
    expect(injected.publishCalls).toHaveLength(publishedCallCount);
  });

  it("fails closed when an immutable registry version has different content", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const first = releaseSet.packages[0];
    const registry = new Map();
  ...
  it("preflights every package before publishing when a late immutable version conflicts", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const last = releaseSet.packages.at(-1);
    const registry = new Map();
  ...
  it("requires signatures and provenance before accepting an existing immutable version", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const first = releaseSet.packages[0];
    const registry = new Map();
  ...
  it("preserves a newer tag for an existing version and rejects tag regression for a missing version", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const first = releaseSet.packages[0];
    const existingRegistry = new Map();
    addPublishedVersion(existingRegistry, first, { taggedVersion: "9.0.0" });
    const existingInjected = createInjectedNpmRunner({ registry: existingRegistry });
    const result = await publishReleaseSet({
      rootDir: ROOT,
      runner: existingInjected.runner,
      environment: {}
    });
    expect(result.packages.find(({ name }) => name === first.name)?.action).toBe("skipped");
    expect(existingRegistry.get(tagsKey(first.name))).toEqual({ latest: "9.0.0" });

    const missingRegistry = new Map([[tagsKey(first.name), { latest: "9.0.0" }]]);
    const missingInjected = createInjectedNpmRunner({ registry: missingRegistry });
    await expect(publishReleaseSet({
      rootDir: ROOT,
  ...
  it("fails closed when an existing version would require an OIDC dist-tag repair", async () => {
    const releaseSet = await discoverReleaseSet({ rootDir: ROOT });
    const first = releaseSet.packages[0];
    const registry = new Map();
  ...
  it("fails closed when npm cannot cryptographically verify the published package set", async () => {
    const injected = createInjectedNpmRunner();
    const runner = async (args, context) => {
      if (args[0] === "audit" && args[1] === "signatures") {
  ...
  it("uses only latest or next and refuses raw npm token publication", async () => {
