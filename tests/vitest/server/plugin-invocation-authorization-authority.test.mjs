...
function lifecycle(pluginId = "plugin-a", generation = 1) {
  let state = "active";
  let currentGeneration = generation;
  return {
    id: "PluginLifecycleStatePort",
    readRecord: async () => ({ pluginId, generation: currentGeneration, state }),
...
function issue(authority, patch = {}) {
...
describe("Plugin invocation authorization authority", () => {
  it("rejects forgery, cross-operation/request/digest replay, and oversized tokens", async () => {
    const authority = createPluginInvocationAuthorizationAuthority();
    authority.registerOwner({
      ownerId: "plugin-a", ownerGenerationDigest: digestA, ownerGeneration: 1, lifecycleStatePort: lifecycle()
    });
    const token = await issue(authority);
  ...
  it("atomically replaces the current generation and invalidates old tokens", async () => {
    const authority = createPluginInvocationAuthorizationAuthority();
    ...
    expect(() => authority.registerOwner({
      ownerId: "plugin-a", ownerGenerationDigest: digestA, ownerGeneration: 1, lifecycleStatePort: firstLifecycle
  ...
  it("permits an empty outer target while enforcing an explicitly signed target and lifecycle expiry", async () => {
    let clock = 1_000;
    const ownerLifecycle = lifecycle();
    const authority = createPluginInvocationAuthorizationAuthority({ ttlMs: 10, now: () => clock });
    authority.registerOwner({
      ownerId: "plugin-a", ownerGenerationDigest: digestA, ownerGeneration: 1, lifecycleStatePort: ownerLifecycle
    });
    const unbound = await issue(authority);
    ...
    const bound = await issue(authority, { targetRef: "signed-target" });
  ...
  it("bounds replay state, sweeps expiry, and rejects malformed clocks and oversized required claims", async () => {
    expect(() => createPluginInvocationAuthorizationAuthority({ maxSpentNonces: Number.POSITIVE_INFINITY })).toThrow(/capacity/u);
    expect(() => createPluginInvocationAuthorizationAuthority({ maxSpentNonces: -1 })).toThrow(/capacity/u);
    expect(() => createPluginInvocationAuthorizationAuthority({ maxSpentNonces: 16_385 })).toThrow(/capacity/u);
    let clock = 10;
    const authority = createPluginInvocationAuthorizationAuthority({ ttlMs: 5, now: () => clock, maxSpentNonces: 1 });
    authority.registerOwner({
      ownerId: "plugin-a", ownerGenerationDigest: digestA, ownerGeneration: 1, lifecycleStatePort: lifecycle()
    });
    const first = await issue(authority);
    await authority.verify(first, { audience: "controlled-execution" });
    const second = await issue(authority, { requestRef: "request-two" });
    ...
    clock = 16;
    const third = await issue(authority, { requestRef: "request-three" });
    ...
    const broken = createPluginInvocationAuthorizationAuthority({ now: () => Number.NaN });
