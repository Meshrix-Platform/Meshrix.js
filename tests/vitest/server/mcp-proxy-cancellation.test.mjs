...
import { fetchJson } from "../../../packages/protocols/mcp/adapter/gateway-installer/lib/cli/http-json-client.mjs";
import {
  createProxyStdioTransport,
  encodeStdioJsonRpc,
  MCP_STDIO_FRAMING_CONTENT_LENGTH
} from "../../../packages/protocols/mcp/adapter/gateway-installer/lib/cli/proxy-command.mjs";

afterEach(() => {
  vi.unstubAllGlobals();
});

describe("MCP stdio proxy cancellation", () => {
  it("cancels only the matching concurrent request and cleans active dispatches", async () => {
    const writes = [];
    const forwardedMessages = [];
    let cancelledRequestObserved = false;
    let cancellationSignalReason;
    let unrelatedRequestCompleted = false;

    const forwardMessage = ({ message, signal }) => {
    ...
    expect(forwardedMessages.map(({ method }) => method)).toEqual([
      "tools/call",
      "tools/call",
    ...
    expect(writes.every(({ framing }) => framing === MCP_STDIO_FRAMING_CONTENT_LENGTH)).toBe(true);
    expect(writes.some(({ payload }) => payload.id === "slow-request")).toBe(false);
    expect(writes.find(({ payload }) => payload.id === "unrelated-request")?.payload.result).toEqual({ ok: true });
    expect(writes.some(({ payload }) => payload.id === undefined)).toBe(false);
    expect(transport.activeRequestCount).toBe(0);
    expect(transport.pendingDispatchCount).toBe(0);
  });

  it("bounds JSONL frames, Content-Length frames, and incomplete input buffers", async () => {
    const forwardedMessages = [];
    const jsonlWrites = [];
    const jsonlTransport = createProxyStdioTransport({
      maxFrameBytes: 96,
      maxBufferBytes: 512,
    ...
    const contentLengthWrites = [];
    const contentLengthTransport = createProxyStdioTransport({
      maxFrameBytes: 96,
      maxBufferBytes: 512,
    ...
    const bufferWrites = [];
    const bufferTransport = createProxyStdioTransport({
      maxFrameBytes: 128,
      maxBufferBytes: 16,
      forwardMessage: vi.fn(),
      writeMessage(payload, framing) {
  ...
  it("rejects excess active requests while accepting cancellation at full capacity", async () => {
    const forwardedMessages = [];
    const writes = [];
  ...
  it("caps pending notification and dispatch work independently of active requests", async () => {
    const forwardedMessages = [];
    const completions = [];
    const writes = [];
    const transport = createProxyStdioTransport({
    ...
    expect(forwardedMessages.map(({ params }) => params?.progress)).toEqual([1, 2]);
    expect(transport.activeRequestCount).toBe(0);
  ...
  it("keeps cancellation responsive while stdout is waiting for drain", async () => {
    const writable = new EventEmitter();
    const frames = [];
    writable.write = vi.fn((frame) => {
    ...
    await vi.waitFor(() => expect(transport.outputSnapshot.blocked).toBe(true));
  ...
  it("fails input without growing a permanently backpressured stdout buffer", async () => {
    const writable = new EventEmitter();
    const frames = [];
    writable.write = vi.fn((frame) => {
      frames.push(Buffer.from(frame));
    ...
    transport.push(Buffer.concat([1, 2, 3].map((id) => encodeStdioJsonRpc({
      jsonrpc: "2.0",
      id,
      method: "tools/list"
    }))));
  ...
  it("keeps the timeout active when an external abort signal is supplied", async () => {
    let combinedSignal;
  ...
  it("propagates an external abort before the timeout", async () => {
    ...
    const externalController = new AbortController();
    const expectedReason = new Error("client cancellation");
    const response = fetchJson("http://gateway.invalid/mcp", {
      signal: externalController.signal,
      timeoutMs: 1000
    });
