...
function workspaceFolderPath(userDataPath, workspaceId) {
...
describe("agent workspace behavior", () => {
  ...
  it("restores snapshot entries from CAS bytes and rebases them under the snapshot base path", async () => {
    ...
    const merkleState = {
      ...
      merkleIndex: {
        prefix: vi.fn(async () => []),
        prove: vi.fn(async () => ({ proofHash: sha256("empty-proof") }))
      },
      stateCommit: {
        commit: vi.fn(async ({ mutations, contentRefs, payload }) => {
          commitSequence += 1;
          const beforeRoot = currentRoot;
          const afterRoot = `after-root-${commitSequence}`;
          const commitId = `commit-${commitSequence}-${mutations.length}-${contentRefs.length}`;
          const eventHash = `event-hash-${commitSequence}`;
          currentRoot = afterRoot;
          stateEvents.push({ eventId: `event-${commitSequence}`, eventHash, payload, afterRoot });
          const persistedContentRefs = [...contentRefs];
      ...
      eventLog: {
        listEvents: vi.fn(async () => [...stateEvents])
      }
    };
    const checkpointTreeApi = {
      checkpointTreeId: vi.fn(() => "tree-sandbox-output"),
      loadCheckpointTree: vi.fn(async () => Object.keys(checkpointNodes).length > 0
        ? { treeId: "tree-sandbox-output", nodes: checkpointNodes }
        : null),
      startCheckpointTree: vi.fn(async () => ({ started: true })),
      upsertCheckpointNode: vi.fn(async (node) => {
        checkpointNodes[node.nodeId] = node;
        return { ok: true };
      })
    };

    await withWorkspaceRuntime(async (runtime, root) => {
      const workspace = runtime.createWorkspace({ title: "Snapshot Rebase Workspace" }).workspace;
      const workspacePath = workspaceFolderPath(root, workspace.workspaceId);
      ...
      const persistedReceipt = await runtime.getWorkspaceSandboxMutationReceipt({
        workspaceId: workspace.workspaceId,
      ...
      const rootBeforeFailedReceipt = currentRoot;
      merkleState.stateCommit.begin = vi.fn(async () => ({ currentRoot }));
      merkleState.stateCommit.restoreRoot = vi.fn(async ({ targetRoot, expectedCurrentRoot, operationId, payload }) => {
      ...
      const failedBindings = { runRef: "failed-run", artifactDigest: sha256("failed-artifact"), outputDigest: sha256("failed-output") };
      const receiptFailure = await runtime.restoreWorkspaceFiles({
        workspaceId: workspace.workspaceId,
        operationId: "synthetic.plugin.sandbox.output.commit",
      ...
      expect(await fs.access(path.join(workspacePath, "docs", "obsolete.txt")).then(() => true).catch(() => false)).toBe(false);
