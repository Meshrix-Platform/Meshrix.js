...
describe("MCP process identity Windows DPAPI backend", () : any => {
  it("selects one current-user DPAPI backend for Windows", () : any => {
  ...
  it.skipIf(process.platform === "win32")("round-trips through the DPAPI command boundary without plaintext persistence", async () : Promise<any> => {
    const root: any = await fs.mkdtemp(path.join(os.tmpdir(), "meshrix-mcp-dpapi-test-"));
    temporaryRoots.push(root);
    const command: any = path.join(root, "fake-powershell.ts");
