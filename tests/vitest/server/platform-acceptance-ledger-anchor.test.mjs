...
} from "../../../tools/server-scripts/lib/platform-acceptance-ledger-anchor.mjs";
...
describe("platform acceptance ledger anchoring", () => {
  ...
  it("fails closed when releaseId is missing", async () => {
    const anchor = await anchorAcceptanceEvidence({
      proofSubstrate: { recordAcceptanceEvidence: async () => ({ ledgerEventId: "x" }) },
  ...
  it("anchors an exact Plan receipt digest and verifies the Pactium bundle", async () => {
    ...
    try {
      ...
      const entry = await proofSubstrate.getReceipt(anchor.ledgerEventId);
  ...
  it("rejects unreadable reports and report-set drift", async () => {
    ...
    try {
      ...
      const contextMismatch = await verifyAcceptanceEvidenceAnchor({
        proofSubstrate,
        ledgerEventId: anchor.ledgerEventId,
        envelopeId: anchor.envelopeId,
        workspaceId: anchor.workspaceId,
