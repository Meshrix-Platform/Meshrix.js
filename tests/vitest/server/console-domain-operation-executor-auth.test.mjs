...
describe("console domain auth and authorization facade coverage", () => {
  ...
  it("covers workspace audit query, history, and revert scope payloads", async () => {
    const checkpointTreeApi = {
      checkpointTreeId: vi.fn(() => "tree-workspace-1"),
    ...
    const agentWorkspace = {
      restoreWorkspaceFiles: vi.fn(async (input) => ({
        ok: true,
    ...
    const context = { operationAuditStore: createAuditStore(), checkpointTreeApi, agentWorkspace };
  ...
  it("covers operation permission MCP request branches and rejects removed legacy grant branches", async () => {
    const provider = {
