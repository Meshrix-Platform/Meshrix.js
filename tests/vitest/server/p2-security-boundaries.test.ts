...
const TOOLSET_RISK: Readonly<Record<string, any>> = Object.freeze({
...
function localPairingRequest(headers: Record<string, any> = {}) : any {
  return {
    headers: {
      host: "127.0.0.1:7391",
      ...headers
...
function bridgeClientRequest(headers: Record<string, any> = {}) : any {
  return {
    headers: {
      host: "127.0.0.1:7391",
      ...headers
...
function createRegistry() : any {
  let resolvedToolIds: any[] = [];
  const toolsets: any = Object.keys(TOOLSET_RISK).map((id?: any) : any => ({ id, grantable: true }));
  return {
    setResolvedToolIds(toolIds: any = []) : any {
      resolvedToolIds = [...toolIds];
    },
    listToolsets: () : any => toolsets,
    resolveToolset(input: Record<string, any> = {}) : any {
      const selected: any[] = [...new Set<any>(input.toolsets || [])];
      const maxRisk: any = selected.reduce((max?: any, toolsetId?: any) : any => {
        const risk: any = TOOLSET_RISK[toolsetId] || "read_only";
      ...
      return {
        ...
        requiredScopes: selected.map((toolsetId?: any) : any => `${toolsetId}:scope`),
        maxRisk
      };
    }
  };
...
function createProvider({ securityPermissions }: Record<string, any> = {}) : any {
  let authorizationRequest: any = null;
  const registry: any = createRegistry();
  const store: Record<string, any> = {
    createGrant: vi.fn(async (grant?: any) : Promise<any> => {
      const target: any = grant.metadata?.mcpTarget || "target";
      return {
        grant: { id: `grant-${target}`, tokenPrefix: `tok_${target}_`, ...grant },
        token: `token-${target}`
      };
    }),
    revokeGrant: vi.fn(async () : Promise<any> => ({ id: "revoked-grant" })),
    ...
    updateGrant: vi.fn(async (grantId?: any, patch?: any) : Promise<any> => ({ id: grantId, ...patch })),
    createMcpAuthorizationRequest: vi.fn((input?: any) : any => {
    ...
    getMcpAuthorizationRequest: vi.fn(() : any => authorizationRequest ? { ...authorizationRequest } : null),
    resolveMcpAuthorizationRequest: vi.fn(({ resolution, resolvedBy = "" }: Record<string, any>) : any => {
    ...
    claimMcpAuthorizationRequest: vi.fn(({ claimTokenHash }: Record<string, any>) : any => {
    ...
    completeMcpAuthorizationRequest: vi.fn(({ status, replayEnvelope = "", replayExpiresAt = "" }: Record<string, any>) : any => {
...
function processIdentityPermissions(extra: Record<string, any> = {}) : any {
  return {
    ...extra,
    processIdentity: {
      issueLocalMcpClientIdentityPackage: vi.fn(async ({ input = {} }: Record<string, any> = {}) : Promise<any> => ({
        ok: true,
      ...
      revokeIssuedLocalMcpClientIdentityPackage: vi.fn(async () : Promise<any> => ({ ok: true }))
    }
  };
}

async function createLocalGrant(provider?: any, body?: any, headers: Record<string, any> = {}) : Promise<any> {
...
describe("P2 security boundaries", () : any => {
  ...
  it("defaults authenticated known local MCP targets to read-only toolsets", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });

    const result: any = await createLocalGrant(provider, {
      targets: ["codex"],
  ...
  it("rejects unauthenticated read-only local MCP grants before identity or grant issuance", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: false, status: 401, error: "unauthenticated" }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });
  ...
  it("rejects forged local pairing Host and Origin headers before authorization", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });
    ...
    const forgedHost: any = await createLocalGrant(provider, body, { host: "example.invalid" });
    const forgedOrigin: any = await createLocalGrant(provider, body, { origin: "http://example.invalid" });
  ...
  it("rejects every proxied local pairing request including multi-hop loopback-first spoofing", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });
    const result: any = await createLocalGrant(provider, {
  ...
  it("admits direct bridge peers only to the approved device flow, not direct grant issuance", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });
    const claimToken: any = "bridge_peer_device_claim_abcdefghijklmnopqrstuvwxyz";
    const body: Record<string, any> = {
      targets: ["codex"],
      processIdentity: { processPublicKeyPem: "bridge-public-key" }
    };

    const directGrant: any = await provider.createLocalMcpGrant({
      request: bridgeClientRequest(),
      requestBody: Buffer.from(JSON.stringify(body)),
      url: new URL("http://127.0.0.1:7391/api/mcp/local-grant")
    });
    const deviceRequest: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: bridgeClientRequest(),
    ...
    const consumed: any = await provider.consumeLocalMcpGrantAuthorizationRequest({
  ...
  it("rejects forwarded metadata on bridge device authorization requests", () : any => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const result: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: bridgeClientRequest({
        "x-forwarded-for": "127.0.0.1, 198.51.100.44",
    ...
    const originMismatch: any = provider.createLocalMcpGrantAuthorizationRequest({
  ...
  it("allows a direct bridge uninstall only with the bound grant authorization path", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    ...
    const result: any = await provider.markLocalMcpGrantUninstalled({
      request: bridgeClientRequest({
    ...
    const originMismatch: any = await provider.markLocalMcpGrantUninstalled({
      request: bridgeClientRequest({
        authorization: "Bearer test-token",
        "x-meshrix-mcp-target": "codex",
        "x-meshrix-process-key-id": "process-key",
    ...
    const forwarded: any = await provider.markLocalMcpGrantUninstalled({
      request: bridgeClientRequest({
        authorization: "Bearer test-token",
        "x-meshrix-mcp-target": "codex",
        "x-meshrix-process-key-id": "process-key",
  ...
  it("uninstalls only the authenticated installation when another grant shares the target", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const authorizedGrant: Record<string, any> = {
  ...
  it("issues multi-target local MCP grants in one authorization batch while keeping per-target identity binding", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });
  ...
  it("rejects write-capable local MCP grants when console authorization is unavailable", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
  ...
  it("creates an immutable device authorization request and replays the same encrypted result after response loss", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const claimToken: any = "device_claim_abcdefghijklmnopqrstuvwxyz0123456789";
    ...
    const pending: any = provider.createLocalMcpGrantAuthorizationRequest({
    ...
    const wrongClaim: any = await provider.consumeLocalMcpGrantAuthorizationRequest({
    ...
    const issued: any = await provider.consumeLocalMcpGrantAuthorizationRequest({
    ...
    const replay: any = await provider.consumeLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest({ "x-meshrix-authorization-claim": claimToken }),
      requestId: pending.body.requestId
    });
  ...
  it("rejects oversized or non-canonical process identity payloads before persistence", () : any => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const claimTokenHash: any = hashLocalMcpAuthorizationClaim(
      "bounded_device_claim_abcdefghijklmnopqrstuvwxyz"
    );
    const oversized: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: Buffer.from(JSON.stringify({
        targets: ["codex"],
        processIdentity: {
    ...
    const unknownNestedField: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: Buffer.from(JSON.stringify({
        targets: ["codex"],
        processIdentity: {
  ...
  it("assigns distinct verification codes to concurrent same-name installation requests", () : any => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const body: any = (claimToken?: any) : any => Buffer.from(JSON.stringify({
      targets: ["codex"],
    ...
    const first: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: body("first_device_claim_abcdefghijklmnopqrstuvwxyz")
    });
    const second: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
  ...
  it("revokes issued credentials when authorization completion loses its CAS", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const claimToken: any = "completion_conflict_claim_abcdefghijklmnopqrstuvwxyz";
    const pending: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: Buffer.from(JSON.stringify({
        targets: ["codex"],
    ...
    const result: any = await provider.consumeLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest({ "x-meshrix-authorization-claim": claimToken }),
      requestId: pending.body.requestId
    });
  ...
  it("revokes issued credentials when authorization completion persistence throws", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const claimToken: any = "completion_throw_claim_abcdefghijklmnopqrstuvwxyz";
    const pending: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: Buffer.from(JSON.stringify({
        targets: ["codex"],
    ...
    store.completeMcpAuthorizationRequest.mockImplementation(() : any => {
  ...
  it("fails closed when an approved request payload is rebound before consumption", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, store } = createProvider({ securityPermissions });
    const claimToken: any = "immutable_request_claim_abcdefghijklmnopqrstuvwxyz";
    const pending: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: Buffer.from(JSON.stringify({
        targets: ["codex"],
    ...
    const stored: any = store.getMcpAuthorizationRequest();
  ...
  it("fails closed when a toolset resolves to different tool IDs after approval", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions();
    const { provider, registry, store } = createProvider({ securityPermissions });
    registry.setResolvedToolIds(["tool.before-approval"]);
    const claimToken: any = "tool_catalog_drift_claim_abcdefghijklmnopqrstuvwxyz";
    const pending: any = provider.createLocalMcpGrantAuthorizationRequest({
      request: localPairingRequest(),
      requestBody: Buffer.from(JSON.stringify({
        targets: ["codex"],
  ...
  it("denies unauthenticated direct local grants at the HTTP adapter boundary", async () : Promise<any> => {
    const securityPermissions: any = processIdentityPermissions({
      authorizeOperation: vi.fn(async () : Promise<any> => ({ ok: false, status: 401, error: "unauthenticated" }))
    });
    const { provider, store } = createProvider({ securityPermissions });
  ...
  it("rolls back earlier grants and all issued process identities when a multi-target batch fails", async () : Promise<any> => {
    const authorizeOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    const securityPermissions: any = processIdentityPermissions({ authorizeOperation });
    const { provider, store } = createProvider({ securityPermissions });
    store.createGrant
