...
import { PLUGIN_MANIFEST_SCHEMA_VERSION } from "../../../packages/foundation/src/module-system/plugin-registry.mjs";
...
async function writePlugin(root, source) {
  const directory = path.join(root, "plugins", "demo");
...
async function createManager(root, runtime = {}) {
...
describe("plugin data capability", () => {
  it("provides scoped storage operations without exposing a host path", async () => {
...
export async function activatePlugin({ manifest, context }) {
  ...
  return { id: manifest.id, mounts: {}, close() {} };
    ...
    if (process.platform !== "win32") {
      const pluginRoot = path.join(root, "data", "plugins", "demo");
  ...
  it("rejects traversal, absolute names, and symlink escapes", async () => {
    const root = await fixtureRoot();
