...
} from "../../../packages/server-runtime/src/composition/queue-observation-projection.mjs";

describe("work queue observation projection", () => {
  it("projects canonical work items without owner, payload, lease, or journal data", () => {
    const result = projectQueueObservation({
      stateCounts: [
  ...
  it("reads through the workflow provider without exposing mutation methods", async () => {
    const inspectWorkQueue = vi.fn(async () => ({
      stateCounts: [{ state: "running", count: 1 }],
      items: [{ workItemId: "work-running", queueDefinitionId: "queue.jobs.import-parse", state: "running" }]
    }));
    const projection = createWorkQueueObservationProjection({
      getJobWorkflowProvider: () => ({ inspectWorkQueue })
    });
  ...
  it("returns an empty read model until the workflow provider is available", async () => {
    const projection = createWorkQueueObservationProjection();
