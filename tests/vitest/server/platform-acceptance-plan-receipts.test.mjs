...
function dependencyMap() {
  const provider = (directory, finalNodeId) => ({
    ...
    parent_integrations: [],
    final_validations: [{ node_id: finalNodeId, profiles: ["local"] }],
  ...
  return {
    ...
    plans: [
      ...
      provider("end-to-end-release/deployment/linux-container", "linux-final"),
      {
        ...
        parent_contract_node_id: null,
        parent_integrations: [],
        final_validations: [{ node_id: "release-final", profiles: ["local"] }],
        prerequisite_receipts: [
          {
...
describe("platform acceptance Plan receipt preflight", () => {
  it("requires the exact Release Acceptance prerequisite final receipts", () => {
    expect(requiredPlatformAcceptancePlanReceipts(dependencyMap())).toEqual([
      { plan: "end-to-end-release/platform-foundation", finalNodeId: "foundation-final", planProfile: "local" },
      { plan: "end-to-end-release/deployment/linux-container", finalNodeId: "linux-final", planProfile: "local" },
    ]);
  });
  ...
  it("verifies the Plan DAG and every current proof receipt", async () => {
    ...
    expect(result.planProfile).toBe("local");
    expect(result.requiredReceiptCount).toBe(2);
    expect(result.bindings).toHaveLength(2);
    expect(result.bindings).toEqual(expect.arrayContaining([
      expect.objectContaining({ platform: "linux", profiles: ["local"] }),
      expect.objectContaining({ platform: "any", profiles: ["local"] }),
    ]));
  ...
  it("rejects a binding whose profile, platform, or requirement coverage is absent", async () => {
    ...
    await expect(verify({ profiles: ["ha"] })).rejects.toThrow("required-plan-receipt-profile-mismatch");
