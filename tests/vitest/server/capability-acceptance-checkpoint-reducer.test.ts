...
import { reduceCapabilityCheckpoints as reduceCapabilityCheckpointsRaw } from "../../../tools/server-scripts/capability-acceptance-checkpoint-reducer.ts";
import {
...
} from "../../../tools/server-scripts/verify-capability-acceptance-machines.ts";

const ROOT: any = path.resolve(import.meta.dirname, "../../..");
const FIXTURE_EVIDENCE_COMMAND_AUTHORITY: any = new Map<any, any>([
  ...CAPABILITY_EVIDENCE_COMMAND_AUTHORITY,
...
function reduceCapabilityCheckpoints(checkpoints?: any, options: Record<string, any> = {}) : any {
...
function criterion(checked: any = true, text: any = "Fixture acceptance criterion") : any {
  return {
    checked,
    text,
...
function externalBlocker() : any {
  return {
    ...
    verificationCommand: "node tools/server-scripts/verify-supported-platform-receipt.ts"
  };
}

function completedCheckpoints() : any {
  return [
    {
      id: "implementation",
      role: "implementation",
...
describe("reduceCapabilityCheckpoints", () : any => {
  it("marks a complete and valid checkpoint graph ready for release reduction", () : any => {
    const result: any = reduceCapabilityCheckpoints(completedCheckpoints());
  ...
  it("treats pending local work as failed and not as an external blocker", () : any => {
    const checkpoints: any = completedCheckpoints().map((checkpoint?: any) : any => ({
      ...checkpoint,
      status: "pending",
      acceptance_criteria: [criterion(false)]
    }));

    const result: any = reduceCapabilityCheckpoints(checkpoints);
    ...
    expect(result.findings.map(({ code }: Record<string, any>) : any => code)).toEqual(expect.arrayContaining([
      "checkpoint-not-completed",
      "checkpoint-acceptance-criterion-unchecked"
    ]));
  });

  it("fails closed on an unsupported checkpoint status", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("fails closed when a criterion is unchecked or malformed", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("requires reproducible evidence for completed implementation and validation criteria", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("rejects unknown command ids instead of accepting arbitrary command claims", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("distinguishes a structured external evidence blocker from local incomplete work", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("fails closed when blocked status has no valid external evidence blocker", () : any => {
    const checkpoints: any = completedCheckpoints();
    checkpoints[0].status = "blocked";
    checkpoints[0].acceptance_criteria = [criterion(false)];
    checkpoints[1].status = "blocked";
  ...
  it("rejects missing prerequisites and impossible completion order", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("rejects prerequisite cycles", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("requires implementation and final-validation roles", () : any => {
    const result: any = reduceCapabilityCheckpoints([
      {
        id: "evidence",
        role: "evidence",
        status: "completed",
  ...
  it("requires exactly one implementation and final-validation role", () : any => {
    const checkpoints: any = completedCheckpoints();
    checkpoints.push({
      ...checkpoints[1],
  ...
  it("requires final validation to transitively depend on implementation", () : any => {
    const checkpoints: any = completedCheckpoints();
  ...
  it("allows callers to inject a different required-role fixture", () : any => {
    const result: any = reduceCapabilityCheckpoints([
      {
        id: "evidence",
        role: "evidence",
...
describe("capability acceptance authority and report safety", () : any => {
  it("rejects unknown proof methods, incomplete parameters, and unprotected verification edges", () : any => {
    const definition: any = JSON.parse(fs.readFileSync(
      path.join(ROOT, "packages/foundation/src/workflow/state-machine/definitions/acceptance/state-machine-governance.json"),
      "utf8"
    ));

    const unknownMethod: any = structuredClone(definition);
    ...
    const missingParams: any = structuredClone(definition);
    missingParams.proofMappings[0].params = {};
    expect(verifyMachineDefinition(missingParams, { throwOnError: false }).ok).toBe(false);

    const unprotected: any = structuredClone(definition);
    const event: any = unprotected.events.find((item?: any) : any => item.id === "capability_verifiers_pass");
    event.riskLevel = "medium";
    const cell: any = unprotected.totalMatrix.find((item?: any) : any =>
  ...
  it("uses only tracked checkpoint authorities, not ignored local plan files", () : any => {
    const registry: any = JSON.parse(fs.readFileSync(
    ...
    for (const entry of registry.entries) {
      ...
      const definition: any = JSON.parse(fs.readFileSync(path.join(ROOT, entry.definitionPath), "utf8"));
      expect(definition.states.map(({ id }: Record<string, any>) : any => id)).toEqual([
        "planned",
        "implemented",
        "verified",
        "blocked",
      ...
      expect(definition.states.find(({ id }: Record<string, any>) : any => id === "verified")?.terminal).toBe(true);
      expect(definition.acceptance.platformReducerCommand).toBe("npm run verify:acceptance");
    }

    for (const relativePath of [
      "tools/generators/generate-capability-acceptance-definitions.ts",
      "tools/server-scripts/verify-capability-acceptance-machines.ts"
    ]) {
      const source: any = fs.readFileSync(path.join(ROOT, relativePath), "utf8");
      expect(source).not.toContain("planPath");
    }
  });

  it("sets reportLeakScan only after the report has actually been scanned", () : any => {
    const observedFlags: any[] = [];
    const report: Record<string, any> = { summary: { reportLeakScan: true } };

    const finalized: any = finalizeCapabilityAcceptanceReport(report, (candidate?: any) : any => {
  ...
  it("uses exit code 2 only for a structurally legal blocked report", () : any => {
  ...
  it("deduplicates one external blocker referenced by implementation and validation checkpoints", () : any => {
    const shared: any = externalBlocker();
    const blockers: any = reduceCapabilityBlockers([{
