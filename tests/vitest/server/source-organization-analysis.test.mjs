...
} from "../../../tools/server-scripts/lib/repo-organization-ast-advisory.mjs";

describe("source organization AST advisory", () => {
  it("keeps its table-driven parser contract current", async () => {
  ...
  it("reports independent exported behavior components as a review candidate", async () => {
  ...
  it("reports shared mutable state as a caution instead of a split recommendation", async () => {
    const result = await analyzeSourceText({
  ...
  it("does not guess across Vue template bindings", async () => {
    const result = await analyzeSourceText({
