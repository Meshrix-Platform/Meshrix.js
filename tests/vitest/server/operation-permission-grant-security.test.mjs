...
describe("Operation Permission grant security invariants", () => {
  ...
  it("deterministically retires pre-owner plugin grants during schema reconciliation", async () => {
    const userDataPath = await fs.mkdtemp(path.join(os.tmpdir(), "lico-grant-owner-migration-"));
    roots.push(userDataPath);
    const generationDigest = "d".repeat(64);
    const pluginTool = Object.freeze({
      id: "fixture.migration.execute",
      owner: "migration-plugin",
      ownerKind: "plugin",
      ownerId: "migration-plugin",
      toolsets: ["fixture.migration"],
      requiredScopes: []
    });
    const registry = {
      getCatalog: () => ({
    ...
    const invalidated = [];
    const invalidatedBindings = [];
    let failMigrationInvalidation = false;
    const capabilityKeyProvider = {
      async issue({ credentialId }) {
        return {
          capabilityKey: `ock_${credentialId}_migration_fixture`,
          credentialId,
          capabilitySetHash: "migration",
          capabilityCount: 1,
          expiresAt: "9999-12-31T23:59:59.999Z"
        };
      },
      async invalidateCredential({ credentialId }) {
        if (failMigrationInvalidation) throw new Error("fixture migration invalidation failure");
        invalidated.push(credentialId);
      },
      close() {}
    };
    const capabilityBindingGuard = {
      async invalidateCapabilityKeyBinding({ credentialId }) {
        if (failMigrationInvalidation) throw new Error("fixture migration binding invalidation failure");
        invalidatedBindings.push(credentialId);
      },
      close() {}
    };
    const openStore = () => createOperationPermissionStore({
    ...
    let store = openStore();
    store.registerPluginGrantOwner({ pluginId: "migration-plugin", generationDigest });
    const issued = await store.createGrant({
      id: "legacy-plugin-grant",
      toolsets: ["fixture.migration"],
      capabilities: ["cap:tool:*"]
    });
    store.db.prepare("DELETE FROM tool_grant_owners WHERE grant_id = ?").run(issued.grant.id);
    store.close();

    failMigrationInvalidation = true;
    store = openStore();
    await Promise.resolve();
    expect(store.getGrant(issued.grant.id)).toMatchObject({
      enabled: false,
      ...
      owners: [{
        ownerKind: "plugin",
        ownerId: "migration-plugin",
        ownerGeneration: "legacy-retired"
      }]
    });
    expect(store.db.prepare(`
      SELECT migrated_count, revoked_count, receipt_digest
      FROM tool_grant_owner_migrations WHERE migration_id = 'catalog-owner-binding'
    `).get()).toMatchObject({
      migrated_count: 1,
      revoked_count: 1,
    ...
    expect(store.db.prepare(`
      SELECT capability_invalidated, binding_invalidated
      FROM tool_grant_owner_migration_invalidations
      WHERE migration_id = 'catalog-owner-binding' AND grant_id = ?
    `).get(issued.grant.id)).toEqual({ capability_invalidated: 0, binding_invalidated: 0 });
    store.close();

    failMigrationInvalidation = false;
    store = openStore();
    await Promise.resolve();
    await Promise.resolve();
    expect(store.db.prepare(`
      ...
      FROM tool_grant_owner_migration_invalidations
      WHERE migration_id = 'catalog-owner-binding' AND grant_id = ?
    `).get(issued.grant.id)).toEqual({ capability_invalidated: 1, binding_invalidated: 1 });
    expect(invalidated).toContain(issued.grant.id);
    expect(invalidatedBindings).toContain(issued.grant.id);
    store.close();
  });
