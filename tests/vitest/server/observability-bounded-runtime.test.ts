...
} from "../../../packages/foundation/src/observability/metric-registry.ts";
import {
...
} from "../../../packages/foundation/src/observability/upstream-publication.ts";
import {
  assertNoSensitiveReportLeak as assertNoSensitiveReportLeakFromVerifierPath
} from "../../../tools/server-scripts/lib/sensitive-report-scan.ts";

describe("bounded observability primitives", () : any => {
  it("uses one shared scanner for runtime and verifier report finalization", () : any => {
    expect(assertNoSensitiveReportLeakFromVerifierPath).toBe(assertNoSensitiveReportLeak);
    const protectedValue: any = ["private", "token", "value"].join("-");
    ...
    expect(() : any => assertNoSensitiveReportLeak({ value: "/tmp/private" }, "unsafe report"))
  ...
  it("rejects invalid report ownership before atomically publishing a finalized report", async () : Promise<any> => {
    ...
    try {
      ...
      const report: any = await finalizeAndPublishSensitiveReport({
        schemaVersion: "observability-test",
        verifier: "owned-verifier",
        generatedAt: "2026-01-01T00:00:00.000Z",
      ...
      const published: any = JSON.parse(await fs.readFile(filePath, "utf8"));
  ...
  it("admits only finite metric dimensions and enforces series and cancellation budgets", () : any => {
    const registry: any = createBoundedMetricRegistry({
      families: ["alerts"],
  ...
  it("rejects oversized metric descriptors and accumulator overflow", () : any => {
    expect(() : any => createBoundedMetricRegistry({
      families: Array.from({ length: OBSERVABILITY_BUDGETS.maxMetricVocabularyValues + 1 }, (_?: any, index?: any) : any => `family-${index}`),
      statuses: ["ok"],
      reasons: ["accepted"]
    ...
    expect(() : any => registry.record({ family: "family", status: "ok", reason: "accepted" }))
      .toThrowError(expect.objectContaining({ code: "observability_metric_accumulator_budget_exceeded" }));
  });

  it("reduces production publishing events without exposing partition identities", () : any => {
    const tracker: any = createUpstreamPublicationTracker();
    ...
    const snapshot: any = tracker.snapshot();
  ...
  it("rejects incomplete publication, unknown fields, raw partition ids, and cancelled updates", () : any => {
    const tracker: any = createUpstreamPublicationTracker();
  ...
  it("rejects out-of-order stages and non-monotonic affected-partition edges", () : any => {
    const tracker: any = createUpstreamPublicationTracker();
    const base: Record<string, any> = {
      outcome: "succeeded",
      reason: "accepted",
  ...
  it("fails closed on duration, CPU, and RSS budget overruns", () : any => {
    const duration: any = startObservabilityBudgetObservation({
      now: (() : any => {
        const values: any[] = [0, 6];
        return () : any => values.shift();
      })(),
      cpuUsage: () : any => ({ user: 0, system: 0 }),
      rss: () : any => 0
    });
    expect(() : any => duration.finish({ budgets: {
      ...OBSERVABILITY_BUDGETS,
      maxCycleDurationMs: 5
    } })).toThrowError(expect.objectContaining({ code: "observability_duration_budget_exceeded" }));

    const cpu: any = startObservabilityBudgetObservation({
      now: () : any => 0,
      cpuUsage: (started?: any) : any => started ? { user: 2_000, system: 0 } : { user: 0, system: 0 },
      rss: () : any => 0
    });
    expect(() : any => cpu.finish({ budgets: {
      ...OBSERVABILITY_BUDGETS,
      maxCycleCpuMs: 1
    } })).toThrowError(expect.objectContaining({ code: "observability_cpu_budget_exceeded" }));

    const rss: any = startObservabilityBudgetObservation({
      now: () : any => 0,
      cpuUsage: () : any => ({ user: 0, system: 0 }),
      rss: (() : any => {
        const values: any[] = [0, 2];
        return () : any => values.shift();
      })()
    });
    expect(() : any => rss.finish({ budgets: {
      ...OBSERVABILITY_BUDGETS,
      maxCycleRssDeltaBytes: 1
    } })).toThrowError(expect.objectContaining({ code: "observability_rss_budget_exceeded" }));
  });

  it("bounds queued work and snapshot cache capacity under pressure", async () : Promise<any> => {
    const queue: any = createBoundedWorkQueue({ maxConcurrent: 1, maxPending: 1 });
    let release: any;
    const first: any = queue.run(() : any => new Promise((resolve?: any) : any => {
      release = resolve;
    }));
    const second: any = queue.run(() : any => "second");
    expect(queue.snapshot()).toMatchObject({ active: 1, pending: 1 });
    expect(() : any => queue.run(() : any => "overflow"))
    ...
    const strictCache: any = createBoundedSnapshotCache({ maxEntries: 1, overflow: "error" });
    strictCache.set("a", 1);
    expect(() : any => strictCache.set("b", 2))
      .toThrowError(expect.objectContaining({ code: "observability_snapshot_cache_budget_exceeded" }));
    const evictingCache: any = createBoundedSnapshotCache({ maxEntries: 1 });
  ...
  it("bounds publication partitions and revision windows under stress", () : any => {
    const tracker: any = createUpstreamPublicationTracker({
