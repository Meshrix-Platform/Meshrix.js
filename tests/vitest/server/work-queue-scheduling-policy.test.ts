...
} from "../../../packages/foundation/src/work-queue/policies.ts";
import {
  assertCapacityBelow,
...
} from "../../../packages/foundation/src/work-queue/scheduling.ts";
import {
...
async function createFixture(policy: Record<string, any> = {}) : Promise<any> {
  const userDataPath: any = await fs.mkdtemp(path.join(os.tmpdir(), "work-queue-scheduling-"));
...
async function enqueue(fixture?: any, workItemId?: any, scope?: any, priority: any = 0) : Promise<any> {
...
describe("work queue scheduling policy", () : any => {
  it("persists immutable queue definition versions without label aliasing", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const replay: any = fixture.store.registerQueueDefinition(fixture.definition);
    expect(replay).toMatchObject({ registered: false, idempotent: true });

    expect(() : any => fixture.store.registerQueueDefinition({
      ...fixture.definition,
      policy: { ...fixture.definition.policy, maxInFlight: 99 }
    })).toThrow(expect.objectContaining({ code: "work_queue_definition_conflict" }));

    const nextVersion: any = fixture.registry.registerQueueDefinition({
      queueDefinitionId: fixture.definition.queueDefinitionId,
  ...
  it("evolves one stable queue definition through immutable versions", () : any => {
    const registry: any = createQueueDefinitionRegistry();
    const first: any = registry.registerQueueDefinition({
      queueDefinitionId: "queue.stable.definition",
      label: "queue.stable.definition",
      ownerCapability: "queue-definition-test"
    });
    const second: any = registry.registerQueueDefinition({
      queueDefinitionId: first.queueDefinitionId,
      label: first.label,
      ownerCapability: first.ownerCapability,
      policy: { maxInFlight: 2 }
    });
    const renamed: any = registry.registerQueueDefinition({
      queueDefinitionId: first.queueDefinitionId,
    ...
    expect(() : any => registry.registerQueueDefinition({
  ...
  it("uses a finite deterministic weighted priority cycle", () : any => {
    const observed: any = Object.fromEntries(
      (Object.values(WORK_QUEUE_PRIORITY_CLASSES) as any[]).map((priorityClass?: any) : any => [priorityClass, 0])
    );
    let cursor: any = 0;
    for (let index: any = 0; index < WORK_QUEUE_PRIORITY_CYCLE.length * 3; index += 1) {
      observed[priorityClassAtCursor(cursor)] += 1;
      cursor = nextPriorityCursor(cursor);
    }
    for (const [priorityClass, weight] of (Object.entries(WORK_QUEUE_PRIORITY_WEIGHTS) as [string, any][])) {
      expect(observed[priorityClass]).toBe(weight * 3);
    }
    expect(cursor).toBe(0);
  });

  it("normalizes arbitrary numeric priority into the closed class set", () : any => {
  ...
  it("promotes waiting work through the finite priority classes", () : any => {
  ...
  it("normalizes the tenant workspace project hierarchy without inferred values", () : any => {
  ...
  it("rejects capacity at the exact bound with a stable reason", () : any => {
    expect(() : any => assertCapacityBelow({
      count: 4,
      limit: 4,
    ...
    expect(() : any => assertCapacityBelow({ count: 3, limit: 4, reason: "project_outstanding" }))
      .not.toThrow();
  });

  it("derives bounded retry jitter deterministically from stable queue inputs", () : any => {
    ...
    const first: any = computeDeterministicRetryDelay(input);
    const replay: any = computeDeterministicRetryDelay(input);
  ...
  it("enforces project capacity transactionally without partial admission", async () : Promise<any> => {
    const fixture: any = await createFixture({
      capacity: {
    ...
    const scope: Record<string, any> = { tenantId: "tenant-a", workspaceId: "workspace-a", projectId: "project-a" };
    ...
    expect(inspected.items.map((item?: any) : any => item.workItemId).sort()).toEqual([
      "capacity-1",
      "capacity-2"
    ]);
    fixture.store.close();
  ...
  it("scopes state counts to the same queue boundary as listed items", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const firstScope: Record<string, any> = { tenantId: "tenant-a", workspaceId: "first" };
    const secondScope: Record<string, any> = { tenantId: "tenant-a", workspaceId: "second" };
    ...
    expect(inspected.items.map((item?: any) : any => item.workItemId)).toEqual(["scope-first"]);
    expect(inspected.stateCounts).toEqual([{ state: "queued", count: 1 }]);
    fixture.store.close();
  });

  it("never claims work outside the authorization scope boundary", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const ownedScope: Record<string, any> = { tenantId: "authorization", workspaceId: "owned" };
    const otherScope: Record<string, any> = { tenantId: "authorization", workspaceId: "other" };
    await enqueue(fixture, "other-scope-older", otherScope);
    await enqueue(fixture, "owned-scope-newer", ownedScope);

    const claim: any = fixture.store.claim({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: ownedScope,
    ...
    expect(claim.claimed.map(({ workItem }: Record<string, any>) : any => workItem.workItemId))
  ...
  it("keeps authorization scope fixed while fairly rotating per-item scheduling hierarchy", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const boundary: Record<string, any> = { tenantId: "platform", workspaceId: "job-workflow" };
    const enqueueScheduled: any = (workItemId?: any, tenantId?: any) : any => {
      const resolved: any = fixture.registry.resolveQueueDefinitionForEnqueue({
        queueDefinitionId: fixture.definition.queueDefinitionId,
        scope: boundary,
        dedupeKey: workItemId
      });
    ...
    expect(claim.claimed.map(({ workItem }: Record<string, any>) : any => workItem.schedulingScope.tenantId))
      .toEqual(["tenant-a", "tenant-b"]);
    expect(claim.claimed.every(({ workItem }: Record<string, any>) : any => workItem.scopeKey === claim.claimed[0].workItem.scopeKey))
      .toBe(true);
    fixture.store.close();
  });

  it("fills a bounded batch when only the sparsest weighted priority class has work", async () : Promise<any> => {
    const capacity: any = Object.fromEntries(
      Object.keys(DEFAULT_QUEUE_POLICY.capacity).map((key?: any) : any => [key, 1_000])
    );
    capacity.maxPayloadRefBytes = DEFAULT_QUEUE_POLICY.capacity.maxPayloadRefBytes;
    const fixture: any = await createFixture({ capacity });
    const batchSize: any = 280;
    for (let index: any = 0; index < batchSize; index += 1) {
    ...
    expect(claim.claimed.every(({ workItem }: Record<string, any>) : any => workItem.priorityClass === "low")).toBe(true);
    fixture.store.close();
  });

  it("ages old claimable work without a global priority sort", async () : Promise<any> => {
    const fixture: any = await createFixture({
      fairness: { ...DEFAULT_QUEUE_POLICY.fairness, agingIntervalMs: 1_000, agingBatchSize: 8 }
    });
    const boundary: Record<string, any> = { tenantId: "platform", workspaceId: "aging" };
    const enqueueAt: any = (workItemId?: any, priority?: any, availableAtMs?: any) : any => fixture.store.enqueue({
      ...fixture.registry.resolveQueueDefinitionForEnqueue({
        queueDefinitionId: fixture.definition.queueDefinitionId,
        scope: boundary,
        dedupeKey: workItemId
  ...
  it("reserves parent lease capacity for an active underserved tenant", async () : Promise<any> => {
    const fixture: any = await createFixture({
      capacity: {
        ...DEFAULT_QUEUE_POLICY.capacity,
        maxLeased: 2,
    ...
    const boundary: Record<string, any> = { tenantId: "platform", workspaceId: "reservation" };
    const enqueueScheduled: any = (workItemId?: any, tenantId?: any) : any => fixture.store.enqueue({
      ...fixture.registry.resolveQueueDefinitionForEnqueue({
        queueDefinitionId: fixture.definition.queueDefinitionId,
        scope: boundary,
        dedupeKey: workItemId
    ...
    const first: any = fixture.store.claim({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: boundary,
    ...
    const hotClaim: any = fixture.store.claim({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: boundary,
      schedulingScope: { tenantId: "tenant-a" },
    ...
    const reservedClaim: any = fixture.store.claim({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: boundary,
  ...
  it("requires the current lease fence before failing running work", async () : Promise<any> => {
    const fixture: any = await createFixture();
  ...
  it("persists a lease-fenced opaque checkpoint across retry and reclaim", async () : Promise<any> => {
    const fixture: any = await createFixture();
    await enqueue(fixture, "checkpoint-resume", {}, 0);
    const firstClaim: any = fixture.store.claim({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: {},
    ...
    const saved: any = fixture.store.checkpoint({
    ...
    expect(() : any => fixture.store.checkpoint({
      workItemId: firstClaim.workItem.workItemId,
      leaseId: firstClaim.lease.leaseId,
    ...
    const resumed: any = fixture.store.claim({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: {},
  ...
  it("returns from dispatcher drain at the deadline while a slow consumer remains active", async () : Promise<any> => {
    let claimed: any = false;
    ...
    const workerRuntime: Record<string, any> = {
      ...
      async runLeased() : Promise<any> {
        await new Promise((resolve?: any) : any => { release = resolve; });
      }
    };
  ...
  it("reports claim-time infrastructure terminals to the owning projection", async () : Promise<any> => {
    const terminal: Record<string, any> = {
      workItemId: "claim-terminal",
      queueDefinitionId: "queue.terminal",
      state: "failed"
    };
    const onTerminal: any = vi.fn(async () : Promise<any> => null);
    const dispatcher: any = createQueuePushDispatcher({
      store: {
        claim: vi.fn(async () : Promise<any> => ({ claimed: [], failed: [terminal], expired: [] }))
      },
  ...
  it("does not claim after dispatch cancellation and signals already leased work", async () : Promise<any> => {
    const preAbortedStore: Record<string, any> = { claim: vi.fn() };
    const preAborted: any = createQueuePushDispatcher({
    ...
    const dispatcher: any = createQueuePushDispatcher({
      ...
      workerRuntime: {
        runLeased: vi.fn(async ({ signal }: Record<string, any>) : Promise<any> => {
  ...
  it("validates and bounds payload references before persistence", async () : Promise<any> => {
    const fixture: any = await createFixture({
      capacity: { ...DEFAULT_QUEUE_POLICY.capacity, maxPayloadRefBytes: 24 }
    });
    const base: any = fixture.registry.resolveQueueDefinitionForEnqueue({
      queueDefinitionId: fixture.definition.queueDefinitionId,
      scope: {},
      dedupeKey: "payload-bound"
    });
    expect(() : any => fixture.store.enqueue({
      ...base,
    ...
    const circular: Record<string, any> = { ref: "circular" };
  ...
  it("retires the oldest failed item at the failed retention bound", async () : Promise<any> => {
    const fixture: any = await createFixture({
  ...
  it("rejects a dedupe key rebound to a different immutable request", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const request: any = fixture.registry.resolveQueueDefinitionForEnqueue({
      queueDefinitionId: fixture.definition.queueDefinitionId,
  ...
  it("bounds terminal projections and transition journals without breaking replay", async () : Promise<any> => {
    const fixture: any = await createFixture({
      retention: {
    ...
    const retained: any = fixture.store.inspect({ workItemId: "terminal-current", includeJournal: true });
  ...
  it("detects a journal row missing from the projection and repairs it on apply", async () : Promise<any> => {
    const fixture: any = await createFixture();
  ...
  it("rotates deterministically across tenant workspace project partitions", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const boundary: Record<string, any> = { tenantId: "platform", workspaceId: "fairness" };
    for (const work of [
  ...
  it("applies weighted service across the finite priority classes", async () : Promise<any> => {
    const fixture: any = await createFixture();
    const boundary: Record<string, any> = { tenantId: "platform", workspaceId: "weighted" };
    const schedulingScope: Record<string, any> = { tenantId: "tenant-a", workspaceId: "workspace-a", projectId: "project-a" };
    for (const [priority, count, prefix] of [
      ...
      for (let index: any = 0; index < count; index += 1) {
        const workItemId: any = `${prefix}-${index}`;
        fixture.store.enqueue({
          ...fixture.registry.resolveQueueDefinitionForEnqueue({
            queueDefinitionId: fixture.definition.queueDefinitionId,
            scope: boundary,
    ...
    const counts: any = claimed.reduce((result?: any, item?: any) : any => {
