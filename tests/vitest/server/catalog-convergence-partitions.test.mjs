...
} from "../../../packages/protocols/mcp/adapter/http-mcp-adapter-replies.mjs";
import { configureMcpNotificationBus } from "../../../packages/protocols/mcp/adapter/mcp-notification-bus.mjs";
import { broadcastMcpNotification as sseBroadcast } from "../../../packages/server-runtime/src/state/sse-connection-state.mjs";
...
describe("MCP catalog convergence partitions", () => {
  it("delivers revision-only invalidation only to affected partitions and digests", () => {
    configureMcpNotificationBus({ broadcastNotification: sseBroadcast });
    const affected = connectionFixture({
      grantId: "grant-affected",
      remoteAddress: "10.0.0.1",
      partitionKeys: ["part-a"]
    });
    const other = connectionFixture({
      grantId: "grant-other",
    ...
    const delivery = broadcastAudienceCatalogInvalidation({
      sourceRevision: 3,
  ...
  it("binds partition keys after registration and scopes grant-digest fan-out", () => {
  ...
  it("keeps list_changed change payload free of catalog tool arrays", () => {
    ...
    const result = broadcastMcpToolListChanged({
      sourceRevision: 1,
  ...
  it("does not deliver list changes to clients that did not negotiate the capability", () => {
    const unsupported = connectionFixture({
  ...
  it("projects only each connection's affected opaque partitions into its notification", () => {
  ...
  it("tracks applied cohort acknowledgement against grant, session, revisions, and partitions", () => {
    const fixture = connectionFixture({
      grantId: "grant-ack",
  ...
  it("requires an exact partition set and keeps the latest revision timeout after an older acknowledgement", async () => {
    vi.useFakeTimers();
    const fixture = connectionFixture({
      grantId: "grant-exact",
    ...
    const notify = (sourceRevision, audienceRevision, affectedPartitions) => broadcastMcpNotification({
      jsonrpc: "2.0",
      method: "notifications/tools/list_changed",
      params: { change: {
        sourceRevision,
  ...
  it("disconnects all active streams when their grant is retired", () => {
    const first = connectionFixture({
  ...
  it("disconnects and fences a cohort that misses its acknowledgement deadline", async () => {
    vi.useFakeTimers();
    const fixture = connectionFixture({
    ...
    const freshSession = connectionFixture({
      grantId: "grant-timeout",
