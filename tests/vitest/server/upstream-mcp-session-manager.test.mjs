...
async function createHttpFixture({
  ...
  let initializedMissingDelivered = false;
  const server = http.createServer(async (request, response) => {
    ...
    if (message.method === "initialize") {
      const createdSessionId = `session-${nextSessionId++}`;
    ...
    if (message.method === "notifications/cancelled") {
      ...
      const timer = pendingEffects.get(message.params.requestId);
      if (timer) clearTimeout(timer);
    ...
    setTimeout(() => response.end(`${result.slice(Math.floor(result.length / 2))}\n\n`), 5);
  });
...
describe("upstream MCP managed sessions", () => {
  it("keeps stdio state, routes concurrent ids, and isolates cancellation", async () => {
    ...
    const listed = await manager.listTools(config);
    expect(listed.tools.map((tool) => tool.name)).toContain("state.increment");
    await manager.callTool(config, { name: "state.increment" });
    const probe = await manager.callTool(config, { name: "state.probe" });
    expect(probe.result.structuredContent.value).toBe(1);
    const ping = await manager.callTool(config, { name: "ping-check" });
    ...
    const cancelled = manager.callTool(config, { name: "cancel-me" }, { signal: controller.signal });
    const survivor = manager.callTool(config, { name: "survivor" });
    ...
    await delay(100);
    const afterCancel = await manager.callTool(config, { name: "state.probe" });
  ...
  it("enforces concurrency and rotates a session scope without reusing old state", async () => {
    const manager = createUpstreamMcpSessionManager({
      maxSessions: 2,
    ...
    managers.add(manager);
    const first = stdioConfig();
    await manager.listTools(first);
    const slow = manager.callTool(first, { name: "slow" });
    await delay(10);
    ...
    const rotated = stdioConfig({ sessionKey: "stdio-generation-two" });
    const probe = await manager.callTool(rotated, { name: "state.probe" });
    ...
    const current = await manager.callTool(rotated, { name: "state.probe" });
    expect(current.result.structuredContent.value).toBe(0);
  });

  it("rejects stale or conflicting explicit generations without unbounded tombstones", async () => {
    const manager = createUpstreamMcpSessionManager({
      maxSessions: 2,
      idleTtlMs: 5000
    });
  ...
  it("streams HTTP notifications, rebuilds a missing session, propagates cancel, and closes with DELETE", async () => {
    const fixture = await createHttpFixture();
    let openedTransports = 0;
    let closedTransports = 0;
    const manager = createUpstreamMcpSessionManager({
      idleTtlMs: 5000,
      async fetchTransport(url, init) {
        openedTransports += 1;
        try {
          return {
    ...
    const order = [];
    const echo = await manager.callTool(config, { name: "echo" }, {
    ...
    const callbackDidRun = [];
    const nonBlockingNotification = await Promise.race([
    ...
    const recovered = await manager.callTool(config, { name: "recover" });
    expect(recovered.result.structuredContent.name).toBe("recover");
    expect(fixture.evidence.initializations).toHaveLength(2);
    expect(fixture.evidence.calls.filter((call) => call.name === "recover")).toHaveLength(2);

    const controller = new AbortController();
    const cancelled = manager.callTool(config, { name: "cancel-http" }, { signal: controller.signal });
    setTimeout(() => controller.abort("private caller reason"), 10);
    await expect(cancelled).rejects.toMatchObject({ name: "AbortError" });
    await delay(120);
    const probe = await manager.callTool(config, { name: "probe" });
    expect(probe.result.structuredContent.value).toBe(0);
  ...
  it("evicts at capacity and reaps idle HTTP sessions", async () => {
    const fixture = await createHttpFixture();
    const manager = createUpstreamMcpSessionManager({ maxSessions: 1, idleTtlMs: 30 });
    managers.add(manager);
    const base = { transport: "streamable-http", url: fixture.url, timeoutMs: 1000 };
    await Promise.all([
  ...
  it("bounds HTTP notification callbacks and fails a flooding upstream session closed", async () => {
    const fixture = await createHttpFixture();
    const manager = createUpstreamMcpSessionManager({ idleTtlMs: 5000 });
    managers.add(manager);
    const config = {
      transport: "streamable-http",
      url: fixture.url,
  ...
  it("retires sessions at their maximum lifetime", async () => {
    const fixture = await createHttpFixture();
    const manager = createUpstreamMcpSessionManager({
      maxSessions: 1,
      idleTtlMs: 5000,
  ...
  it("deletes issued HTTP sessions when initialization cannot complete", async () => {
    const rejectedNotification = await createHttpFixture({ rejectInitialized: true });
    const firstManager = createUpstreamMcpSessionManager();
    ...
    const unsupported = await createHttpFixture({ negotiatedProtocolVersion: "2024-11-05" });
    const secondManager = createUpstreamMcpSessionManager();
  ...
  it("retries once when an issued HTTP session disappears during initialization", async () => {
    const fixture = await createHttpFixture({ missingInitializedOnce: true });
    const manager = createUpstreamMcpSessionManager({ maxSessions: 1 });
    managers.add(manager);
  ...
  it("uses the injected header environment and waits for active requests during close", async () => {
    const fixture = await createHttpFixture();
    const envName = "MCP_SESSION_HEADER_FIXTURE";
    ...
    try {
      ...
      const slowOutcome = manager.callTool(config, { name: "slow-close" })
