...
import { readHeaders } from "../../../apps/server/bin/lib/lico-cli-common.mjs";

const repoRoot = path.resolve(fileURLToPath(new URL("../../..", import.meta.url)));
const cliEntrypoint = path.join(repoRoot, "apps", "server", "bin", "lico.mjs");

describe("lico public CLI entrypoint", () => {
  it("delegates to the canonical command dispatcher", () => {
    ...
    expect(result.stdout).toContain("lico rpc-call jobs.list");
    expect(result.stdout).toContain("lico tools catalog");
    expect(result.stdout).toContain("lico secret init --target-file TARGET.json --json-stdin");
