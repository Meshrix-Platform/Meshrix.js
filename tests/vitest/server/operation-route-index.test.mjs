...
describe("operation route index", () => {
  it("compiles an immutable canonical registry snapshot without route conflicts", () => {
    const index = createOperationRouteIndex(SERVER_API_OPERATIONS, { strict: true });
  ...
  it("uses indexed HTTP, RPC, and operation-id lookup with decoded route params", () => {
    const operations = [
    ...
    const index = createOperationRouteIndex(operations, { strict: true });
  ...
  it("does not become stale or mutable when the source catalog changes", () => {
    const operations = [operation("first", "GET", "/first", "first")];
    const index = createOperationRouteIndex(operations, { strict: true });
  ...
  it("reuses the provider's immutable route index for its configured source catalog", () => {
    const operations = [
  ...
  it("caches dynamic route indices by immutable catalog identity instead of operation IDs", () => {
    ...
    let current = first;
    const provider = createCorePlatformProvider({ operations: [], getOperations: () => current });
    const discoveryState = {
      mode: "forward",
      advertisedBaseUrl: "https://local.example",
      forwardBaseUrl: "https://upstream.example",
  ...
  it("normalizes encoded unreserved static segments before parameter routes", () => {
    const index = createOperationRouteIndex(SERVER_API_OPERATIONS, { strict: true });
  ...
  it("backs off from an incomplete static branch to a parameter branch", () => {
    const trie = new RadixPathTrie();
  ...
  it("orders static, parameter, and wildcard routes deterministically", () => {
    const trie = new RadixPathTrie();
    ...
    const index = createOperationRouteIndex([
  ...
  it("rejects invalid route shapes and malformed encoded params", () => {
    const trie = new RadixPathTrie();
    expect(() => trie.insert("/assets/*/nested", {})).toThrow("Wildcard must be the final segment");
    expect(() => trie.insert("/api/:/value", {})).toThrow("Parameter name is required");
    ...
    const wildcard = new RadixPathTrie();
    wildcard.insert("/assets/*", { id: "wildcard" });
    expect(wildcard.lookup(`/assets/${"x".repeat(ROUTE_WILDCARD_MAX_BYTES + 1)}`))
      .toBeNull();
  });

  it("fails strict compilation for duplicate ids, HTTP routes, and RPC methods", () => {
    expect(() => createOperationRouteIndex([
  ...
  it("keeps explicit empty provider catalogs empty and local prefixes local", () => {
    const provider = createCorePlatformProvider({ operations: [] });
    expect(provider.listInterfaceCatalog()).toEqual([]);
    expect(provider.describeOperationRegistry().summary.total).toBe(0);

    const localOperation = operation("local.jobs", "GET", "/api/jobs/:jobId", "local.jobs");
