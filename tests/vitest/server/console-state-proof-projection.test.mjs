...
} from "../../../packages/protocols/http/controllers/system-controller.mjs";

describe("console state proof projection", () => {
  it("hashes only stable status, revision and count fields", () => {
    const state = {
      ...
      agentConfigs: { generation: "g1", revision: 7, modelManifest: { private: true } },
      readinessBaseline: { ok: true, status: "ready", checkedAt: "volatile" },
      maintenanceAgent: { status: "idle", enabled: true, lastRunAt: "volatile" },
    ...
    const first = consoleStateProofChangeProjection(state);
    const privateOnlyChange = consoleStateProofChangeProjection({
      ...state,
    ...
    const countChange = consoleStateProofChangeProjection({
      ...state,
