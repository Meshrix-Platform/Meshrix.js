...
} from "../../../apps/server/runtime/http-server-middleware.ts";
import { clientIpFromRequest } from "../../../packages/foundation/src/security/trusted-client-ip.ts";
import {
  isSecureRequest,
  isTrustedProxy,
  requestTargetOrigin
...
const originalTrustedProxies: any = process.env.MESHRIX_TRUSTED_PROXIES;

function requestFrom(remoteAddress?: any, forwardedFor: any = "") : any {
  return {
...
describe("HTTP middleware client IP normalization", () : any => {
  it("keeps rotating spoofed forwarded addresses from direct loopback in one rate-limit bucket", () : any => {
    delete process.env.MESHRIX_TRUSTED_PROXIES;
    const limiter: any = createFixedWindowRateLimiter({
      limit: 1,
      windowMs: 60_000,
      label: "direct-loopback",
      maxBuckets: 8
    });
    const firstIp: any = normalizeClientIp(requestFrom("127.0.0.1", "198.51.100.10"));
    const secondIp: any = normalizeClientIp(requestFrom("127.0.0.1", "203.0.113.20"));
  ...
  it("honors a valid forwarded chain only for an explicitly trusted socket peer", () : any => {
  ...
  it("uses the same explicit proxy trust boundary for authorization, cookies, and origin resolution", () : any => {
    delete process.env.MESHRIX_TRUSTED_PROXIES;
    const directLoopback: Record<string, any> = {
      headers: {
        host: "127.0.0.1:7228",
...
describe("fixed-window rate limiter capacity", () : any => {
  it("does not let quota races or rejected retries extend a fixed window", async () : Promise<any> => {
    vi.useFakeTimers();
    ...
    const outcomes: any = await Promise.all(
      Array.from({ length: 100 }, () : any => Promise.resolve().then(() : any => limiter.shouldAllow("same-key")))
    );
    expect(outcomes.filter((outcome?: any) : any => outcome.allowed)).toHaveLength(1);
    for (let retry: any = 0; retry < 1_000; retry += 1) {
  ...
  it("aggregates overflow without evicting active buckets and recovers after TTL", () : any => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2030-01-01T00:00:00.000Z"));
    const limiter: any = createFixedWindowRateLimiter({
      limit: 1,
  ...
  it("stays strictly bounded under high-cardinality identifier churn", () : any => {
    const limiter: any = createFixedWindowRateLimiter({
      limit: 2,
      windowMs: 60_000,
      label: "bounded-churn",
    ...
    for (let index: any = 0; index < 5_000; index += 1) {
