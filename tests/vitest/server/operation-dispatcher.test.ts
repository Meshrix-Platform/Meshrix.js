...
function controllers(handler?: any) : any {
  return {
    unit: {
      handle: handler
    }
...
function createProofSubstrate() : any {
  return {
...
function governedDispatchOptions() : any {
  return {
...
describe("operation dispatcher behavior", () : any => {
  it("coerces the context build-record limit before schema validation", async () : Promise<any> => {
    const operation: any = SERVER_API_OPERATIONS.find(({ id }: Record<string, any>) : any => id === "context.build_records");
    expect(operation?.http?.coerce).toEqual({ limit: "number" });

    const response: any = createResponse();
    const handler: any = vi.fn(({ input, response: innerResponse }: Record<string, any>) : any => {
  ...
  it("keeps the workspace asset console query and submit payload aligned with the contract", async () : Promise<any> => {
    const listOperation: any = SERVER_API_OPERATIONS.find(({ id }: Record<string, any>) : any => id === "workspace.asset.list");
    const submitOperation: any = SERVER_API_OPERATIONS.find(({ id }: Record<string, any>) : any => id === "workspace.asset.submit");
    ...
    const lockManager: Record<string, any> = {
  ...
  it("declares the complete console checkpoint restore payload for preview and apply", () : any => {
    for (const operationId of [
      "workspace.checkpoint.restore.preview",
      "workspace.checkpoint.restore"
    ]) {
      const operation: any = SERVER_API_OPERATIONS.find(({ id }: Record<string, any>) : any => id === operationId);
  ...
  it("keeps tag, queue control, and tool policy console payloads aligned with their contracts", () : any => {
    const tagUpsert: any = SERVER_API_OPERATIONS.find(({ id }: Record<string, any>) : any => id === "tag_management.tags.upsert");
    ...
    const policyPreview: any = SERVER_API_OPERATIONS.find(
      ({ id }: Record<string, any>) : any => id === "operation_permission.policy_preview"
    );
    ...
    const assemblyBuild: any = SERVER_API_OPERATIONS.find(({ id }: Record<string, any>) : any => id === "runtime.assembly.build");
  ...
  it("lets write-capable operations marked concurrencySafe use their own runtime concurrency policy", async () : Promise<any> => {
    ...
    const lockManager: Record<string, any> = { acquire: vi.fn() };
    const operationControllers: any = controllers(async ({ response }: Record<string, any>) : Promise<any> => {
          inFlight += 1;
  ...
  it("covers validation, empty parsing, malformed parsing, and proxy edge branches", async () : Promise<any> => {
    ...
    const okResponse: any = createResponse();
    await expect(dispatchOperation({
    ...
    expect(okResponse.json().error).toContain("input schema is invalid");

    const arrayResponse: any = createResponse();
    await expect(dispatchOperation({
    ...
    const typeResponse: any = createResponse();
    await expect(dispatchOperation({
      operation: baseOperation({
        inputSchema: {
          type: "object",
    ...
    const protocolResponse: any = createResponse();
    await expect(dispatchOperation({
      operation: baseOperation({
        http: {
          method: "POST",
    ...
    for (const requestBody of ["   ", "{not-json", { direct: true }]) {
      ...
      await expect(dispatchOperation({
        operation: baseOperation(),
        controllers: controllers(({ response: innerResponse }: Record<string, any>) : any => {
          innerResponse.writeHead(200, { "Content-Type": "application/json" });
          innerResponse.end(JSON.stringify({ ok: true }));
        }),
        request: {},
  ...
  it("captures fixed startup snapshot text and binary responses with case-insensitive headers", async () : Promise<any> => {
    const textOperation: any = baseOperation({
      id: "system.interfaces",
      http: { method: "POST", path: "/api/unit/text" }
    });
    const textPort: any = createCorePlatformProvider({
    ...
    const binaryOperation: any = baseOperation({
      id: "storage.summary",
      http: { method: "POST", path: "/api/unit/binary" },
      binary: true
    });
    const binaryPort: any = createCorePlatformProvider({
  ...
  it("maps RPC body encodings, URL params, query aliases, and error responses", async () : Promise<any> => {
    ...
    const rpcControllers: any = controllers(({ requestBody, url, response }: Record<string, any>) : any => {
      observations.push({
    ...
    const allowRpcOperation: any = vi.fn(async () : Promise<any> => ({ ok: true }));
    ...
    expect(observations.map((item?: any) : any => item.body)).toEqual([
      "from-base64",
      "from-text",
      "from-string",
      ""
    ...
    const failingOperation: any = baseOperation({
      id: "unit.rpc.failing",
    ...
    const requiredOperation: any = baseOperation({
  ...
  it("keeps raw JSON RPC carriers out of operation proof input", async () : Promise<any> => {
    const marker: any = "synthetic-sensitive-marker";
    const proofInputs: any[] = [];
    const operationProofSubstrate: Record<string, any> = {
      beginLifecycle: vi.fn(async (input?: any) : Promise<any> => {
        proofInputs.push(input);
        return { ledgerEventId: "proof-entry" };
      }),
      finishLifecycle: vi.fn(async () : Promise<any> => ({ ok: true }))
    };
    const operation: any = baseOperation({
  ...
  it("normalizes proof profiles without retaining the retired binding contract", () : any => {
    ...
    expect(SERVER_API_OPERATIONS.find((operation?: any) : any => operation.id === "system.console_state")?.proof).toMatchObject({
      profile: OPERATION_PROOF_PROFILES.ON_CHANGE,
      changeProjection: "console-state-v1"
    });
    expect(() : any => decorateServerApiOperations([definitions[0], {
      ...definitions[1],
  ...
  it("prepares and finishes a mandatory lifecycle for private read-only operations", async () : Promise<any> => {
    const operation: any = baseOperation({
      id: "unit.profile.receipt",
      proof: { profile: OPERATION_PROOF_PROFILES.RECEIPT }
    });
    const operationProofSubstrate: Record<string, any> = {
      beginLifecycle: vi.fn(async () : Promise<any> => ({ ledgerEventId: "receipt-entry" })),
      finishLifecycle: vi.fn(async (input?: any) : Promise<any> => ({ ledgerEventId: input.ledgerEventId })),
      recordReceipt: vi.fn(async (input?: any) : Promise<any> => ({
    ...
    await dispatchTestOperation({
      operation,
      operationControllers: controllers(({ response }: Record<string, any>) : any => {
        response.writeHead(200, { "Content-Type": "application/json" });
    ...
    expect(operationProofSubstrate.beginLifecycle).toHaveBeenCalledTimes(1);
    expect(operationProofSubstrate.finishLifecycle).toHaveBeenCalledTimes(1);
    expect(operationProofSubstrate.recordReceipt).not.toHaveBeenCalled();
    const lifecycle: any = operationProofSubstrate.finishLifecycle.mock.calls[0][0];
    expect(lifecycle).toMatchObject({ ledgerEventId: "receipt-entry", status: "succeeded" });
    expect(JSON.stringify(lifecycle)).not.toContain("privateValue");
    expect(JSON.stringify(lifecycle)).not.toContain("not-proof-input");
  });

  it("records on-change receipts only from an explicit privacy-safe digest projection", async () : Promise<any> => {
    const secret: any = "private-console-payload";
    ...
    const beginLifecycle: any = vi.fn(async () : Promise<any> => ({ ledgerEventId: "change-entry" }));
    const finishLifecycle: any = vi.fn(async (input?: any) : Promise<any> => ({ ledgerEventId: input.ledgerEventId }));

    await dispatchTestOperation({
      operation,
      operationControllers: controllers(({ response }: Record<string, any>) : any => {
        response.__meshrixProofChangeProjection = {
          changeProjection: "console-state-v1",
          changeDigest: digest,
          changed: false
        };
      ...
      operationProofSubstrate: { beginLifecycle, finishLifecycle }
    });

    expect(finishLifecycle).toHaveBeenCalledTimes(1);
    expect(finishLifecycle.mock.calls[0][0].workspaceEffectEvidence).toMatchObject({
      changeProjection: "console-state-v1",
      changeDigest: digest
    });
    expect(JSON.stringify(finishLifecycle.mock.calls[0][0])).not.toContain(secret);

    beginLifecycle.mockClear();
    finishLifecycle.mockClear();
    await dispatchTestOperation({
      operation,
      operationControllers: controllers(({ response }: Record<string, any>) : any => {
    ...
    expect(finishLifecycle).toHaveBeenCalledTimes(1);
    expect(finishLifecycle.mock.calls[0][0].workspaceEffectEvidence.changeDigest).toBe("");
  });

  it("forces denied on-change operations to a receipt and keeps metadata-only audit payloads empty", async () : Promise<any> => {
    const operation: any = baseOperation({
    ...
    const response: any = createResponse();
    const operationAuditStore: Record<string, any> = {
      append: vi.fn(() : any => ({ auditId: "denied-audit" }))
    };
    const recordReceipt: any = vi.fn(async () : Promise<any> => ({ disposition: "recorded", ledgerEventId: "denied-receipt" }));

    const dispatchResult: any = await dispatchOperation({
      operation,
      controllers: controllers(() : any => {}),
      request: {},
      response,
  ...
  it("suppresses only successful audit rows when audit.write is false", async () : Promise<any> => {
    const operation: any = baseOperation({
    ...
    const response: any = createResponse();
    const operationAuditStore: Record<string, any> = { append: vi.fn() };

    const dispatchResult: any = await dispatchOperation({
      operation,
      controllers: controllers(({ response: targetResponse }: Record<string, any>) : any => {
      ...
      operationAuditStore,
      operationProofSubstrate: {
        beginLifecycle: vi.fn(async () : Promise<any> => ({ ledgerEventId: "audit-receipt" })),
        finishLifecycle: vi.fn(async (input?: any) : Promise<any> => ({ ledgerEventId: input.ledgerEventId }))
      },
      ...governedDispatchOptions(),
      logger: { debug: vi.fn(), warn: vi.fn(), error: vi.fn() }
