...
import {
  createReport,
  scanText
} from "../../../tools/config-scanner.ts";

const scanKey: any = Buffer.alloc(32, 7);

function macosHome(username: any, ...parts: any[]) : any {
  return ["", "Users", username, ...parts].join("/");
}

function linuxHome(username: any, ...parts: any[]) : any {
  return ["", "home", username, ...parts].join("/");
}

function windowsHome(username: any, ...parts: any[]) : any {
  return ["C:", "Users", username, ...parts].join("\\");
}

describe("repository local-info hygiene report redaction", () : any => {
  it("records only location, category, length, and an irreversible fingerprint", () : any => {
    const sensitiveValue: any = ["/", "Users", "/", "fixture-account", "/", "private-project"].join("");
    const findings: any = scanText("fixtures/local-path.txt", sensitiveValue, scanKey);
    ...
    const report: any = createReport(findings, "2026-07-10T00:00:00.000Z");
    expect(report.summary.reportLeakScan).toBe(true);
    expect(JSON.stringify(report)).not.toContain(sensitiveValue);
  });

  it("uses a keyed fingerprint so identical values cannot be correlated across scan keys", () : any => {
    const sensitiveValue: any = ["/", "home", "/", "fixture-account", "/", "private-project"].join("");
    const first: any = scanText("fixture.txt", sensitiveValue, Buffer.alloc(32, 1));
    const second: any = scanText("fixture.txt", sensitiveValue, Buffer.alloc(32, 2));
  ...
  it("fails closed when report metadata itself contains local information", () : any => {
    const unsafeFile: any = ["/", "Users", "/", "fixture-account", "/", "finding.txt"].join("");
    let thrown: any;
    try {
      createReport([{
        severity: "warning",
...
describe("repository local-info hygiene home-path classification", () : any => {
  it("allows syntactic username placeholders on macOS, Linux, and Windows", () : any => {
    const placeholderPaths: any[] = [
  ...
  it("reports realistic ASCII account names, including bare user and example", () : any => {
    const realisticPaths: any[] = [
  ...
  it("still reports a real account when a later path component is a placeholder", () : any => {
    const realisticPaths: any[] = [
  ...
  it("does not partially match invalid account components", () : any => {
