...
} from "../../../packages/server-runtime/src/composition/plugin-contribution-registry.mjs";
import { createPluginContributionController } from "../../../packages/server-runtime/src/composition/plugin-contribution-controller.mjs";
import { createPluginWorkspaceAccess } from "../../../packages/server-runtime/src/composition/plugin-workspace-access.mjs";
import { createCapturedResponse, parseCapturedResult } from "../../../packages/server-runtime/src/composition/dispatch-operation-captured-response.mjs";
import {
...
const TEST_ARTIFACT_IDENTITY = Object.freeze({
  pluginId: "demo",
  version: "0.0.1",
...
const TEST_ARTIFACT_OPTIONS = Object.freeze({
  artifactIdentityResolver: () => TEST_ARTIFACT_IDENTITY,
  artifactFileReader: async (_manifest, filePath) => Buffer.from(`export const asset = ${JSON.stringify(filePath)};`)
});

function definition(id = "demo.run", patch = {}) {
  const resource = {
...
function opaqueInputContract({ maxBytes = 1024 * 1024 } = {}) {
...
function hostPathInputContract({ targetField = "mountSelectionRef" } = {}) {
  return Object.freeze([Object.freeze({
...
function opaqueRegistry({ execute, contract = opaqueInputContract() } = {}) {
  const plugin = manifest({ opaqueInputPreprocessing: { "demo.run": contract } });
...
function record(pluginId, kind, id, implementation) {
...
function enabledContributions(operationPatch = {}) {
  const execute = operationPatch.execute || vi.fn(async ({ host }) => ({
    ok: true,
...
function createRegistry(contributions = enabledContributions(), patch = {}) {
...
describe("plugin contribution registry", () => {
  it("projects one enabled definition into the shared HTTP/RPC/tool surface", () => {
  ...
  it("binds declared console assets to the active immutable artifact identity", async () => {
    const artifactFileReader = vi.fn(async (_manifest, filePath) =>
      Buffer.from(`export const asset = ${JSON.stringify(filePath)};`));
    const registry = createRegistry(enabledContributions(), { artifactFileReader });
    ...
    const asset = await registry.readConsoleAsset(entry.assetUrl);
  ...
  it("rejects unsafe or incomplete console artifact declarations", () => {
    for (const patch of [
      ...
      const contributions = enabledContributions();
      const original = contributions.consoleEntries["admin.demo"];
      contributions.consoleEntries["admin.demo"] = record("demo", "consoleEntries", "admin.demo", {
        ...original.implementation,
        ...patch
      });
      expect(() => createRegistry(contributions)).toThrow(/console entry .* is invalid/u);
    }
  });

  it("admits each plugin contribution once into an immutable snapshot", () => {
    const source = enabledContributions();
    const reads = Object.fromEntries(CONTRIBUTION_KINDS.map((kind) => [kind, 0]));
    const contributions = {};
    ...
    const registry = createRegistry(contributions);
    ...
    expect(reads).toEqual(Object.fromEntries(CONTRIBUTION_KINDS.map((kind) => [kind, 1])));
  ...
  it("removes every dynamic surface and rejects a captured operation after deactivation", async () => {
    const registry = createRegistry();
    const capturedOperation = registry.activeOperations[0];
    const controller = createPluginContributionController({
      registry,
      hostPorts: { securityPermissions: { appendLoanRecord() {} } }
    });
  ...
  it("atomically replaces one active plugin contribution set without deactivating its product", async () => {
    const registry = createRegistry();
    const capturedOperation = registry.activeOperations[0];
    const controller = createPluginContributionController({ registry });
    const empty = Object.fromEntries(CONTRIBUTION_KINDS.map((kind) => [kind, {}]));
    const change = registry.preparePluginContributionReplacement("demo", empty);

    change.commit();
    expect(registry.enabledPlugins).toHaveLength(1);
    const emptySnapshot = registry.currentActiveOperations();
    ...
    change.rollback();
    const restoredSnapshot = registry.currentActiveOperations();
  ...
  it("injects only declared facade methods and writes the canonical result envelope", async () => {
    const registry = createRegistry();
    const controller = createPluginContributionController({
      registry,
      hostPorts: {
  ...
  it("passes plugins only an immutable serializable call projection and a separate cancellation capability", async () => {
    const execute = vi.fn(async () => ({ ok: true }));
    const registry = createRegistry(enabledContributions({ execute, requiredHostPorts: [] }));
    const controller = createPluginContributionController({ registry });
    const response = createCapturedResponse();
    const signal = new AbortController().signal;
    const credential = "fixture-mobile-credential-0123456789abcdef";
    const request = {
      method: "POST",
      headers: {
      ...
      __licoTraceContext: { traceId: "trace-fixture", requestId: "request-fixture" },
  ...
  it("does not emit reusable credential digests for low-entropy bearer values", async () => {
    const execute = vi.fn(async () => ({ ok: true }));
    const registry = createRegistry(enabledContributions({ execute, requiredHostPorts: [] }));
    ...
    const projection = execute.mock.calls[0][0].call;
  ...
  it("projects opaque workspace methods and enforces the core-owned path boundary", async () => {
    ...
    try {
      ...
      const workspaceAccess = createPluginWorkspaceAccess({ workspaceRoot: root });
      const execute = vi.fn(async ({ host }) => ({
        ok: true,
        methods: Object.keys(host.workspaceAccess).sort(),
        content: await host.workspaceAccess.readTextFile({ path: "existing.txt" })
      }));
      const registry = createRegistry(enabledContributions({
        execute,
  ...
  it("projects only delegated MCP grant broker methods into plugin operations", async () => {
    const execute = vi.fn(async ({ host }) => ({
      ok: true,
  ...
  it("projects only appendAlert through the security alert host port", async () => {
    const execute = vi.fn(async ({ host }) => ({
      ok: true,
  ...
  it("records plugin grant evidence only under the current Operation Permission authorization", async () => {
    const loanRecord = Object.freeze({
    ...
    const appendLoanRecord = vi.fn(async () => ({ loanRecordId: "host-loan-record-1" }));
    const execute = vi.fn(async ({ host, input }) => ({
      ok: true,
    ...
    const operation = registry.activeOperations[0];
    ...
    const authorizedCall = (input, response) => controller.executePluginOperation({
      operation,
      input,
      request: {
        __licoToolRuntimeAuthorization: {
  ...
  it("binds external service requests to the current authorized plugin operation", async () => {
    const requestPluginExternalService = vi.fn(async () => ({
      ok: true,
      status: 200,
    ...
    const execute = vi.fn(async ({ host, input, signal }) => ({
    ...
    const authorizedCall = (input, response, signal = null) => controller.executePluginOperation({
      operation,
      input,
      signal,
      request: {
  ...
  it("seals declared base64 before plugin execution and preserves HTTP/MCP parity", async () => {
    const raw = Buffer.alloc(70 * 1024, 0xa5);
    ...
    const execute = vi.fn(async ({ input, host }) => ({
      ok: true,
      input,
      custodyMethods: Object.keys(host.opaqueArtifactCustody)
    }));
    const registry = opaqueRegistry({ execute });
    const controller = createPluginContributionController({
      registry,
      hostPorts: {
        opaqueArtifactCustody: { store, describe() {}, delete() {} }
      }
    });
    for (const transport of ["http", "mcp"]) {
      ...
      const result = parseCapturedResult({ operation: registry.activeOperations[0], captured: response });
    ...
    expect(observed.every((entry) => entry.chunks === Math.ceil(encoded.length / (64 * 1024)))).toBe(true);
    expect(observed.every((entry) => entry.content.equals(raw))).toBe(true);
  ...
  it("fails opaque preprocessing closed on invalid transport, bounds, ownership, substitution, and receipt replay", async () => {
    const execute = vi.fn();
    const validBytes = Buffer.from("opaque-input");
    const validDigest = createHash("sha256").update(validBytes).digest("hex");
    const store = vi.fn(async ({ source }) => {
    ...
    const registry = opaqueRegistry({ execute, contract: opaqueInputContract({ maxBytes: 32 }) });
    const removeStored = vi.fn();
    const controller = createPluginContributionController({
      registry,
      hostPorts: { opaqueArtifactCustody: { store, describe() {}, delete: removeStored } }
    });
    const base = {
      operation: registry.activeOperations[0],
      authSession: { user: { userId: "subject:fixture", tenantId: "tenant:fixture" } },
  ...
  it("isolates opaque custody for an authenticated tool grant without a console tenant", async () => {
    const bytes = Buffer.from("tool-grant-opaque-input");
    const ownerBindings = [];
    ...
    const controller = createPluginContributionController({
      ...
      hostPorts: {
        opaqueArtifactCustody: {
          async store({ source, ownerBinding }) {
            const chunks = [];
  ...
  it("binds sandbox execution to the same authenticated custody owner", async () => {
    const executeConfiguredOpaque = vi.fn(async (request) => ({ request }));
    const file = {
      path: "input.bin",
    ...
    const promotionDigest = custodyPromotionSetDigest({ files: [file] });
    const execute = vi.fn(async ({ host }) => host.sandboxExecution.executeConfiguredOpaque({
      principal: {
  ...
  it("keeps external authentication in the pre-execution verifier phase", async () => {
    const execute = vi.fn();
    const verifyExternalAuth = vi.fn(async () => ({
      ok: false,
      status: 503,
    ...
    const result = await createPluginContributionController({ registry }).verifyPluginExternalAuth({
      operation,
      input: {}
    });
  ...
  it("keeps disabled contributions absent and rejects core takeover", () => {
    const plugin = manifest();
    const empty = Object.fromEntries([
      "operations", "routes", "mcpTools", "consoleEntries", "stateMachines", "verifierHooks"
    ].map((kind) => [kind, {}]));
    const disabled = createPluginContributionRegistry({
      manifests: [plugin],
      loadedPlugins: [],
      contributions: empty,
      coreOperations: [],
    ...
    expect(() => disabled.requireOperation("demo.run")).toThrow(/not enabled/u);
    expect(() => createRegistry(enabledContributions(), {
      coreOperations: [definition()]
    })).toThrow(/conflicts with a plugin-owned operation claim/u);
  });

  it("rejects unknown fields, mismatched routes, duplicate MCP bindings, and undeclared ports", () => {
    expect(() => createRegistry(enabledContributions({ typoExecute: () => {} }))).toThrow(/unsupported field/u);
    expect(() => createRegistry(enabledContributions({ requiredHostPorts: ["runtime"] }))).toThrow(/unsupported host port/u);
    const mismatchedOpaqueContract = opaqueInputContract({ maxBytes: 2048 });
    expect(() => createRegistry(enabledContributions({
      requiredHostPorts: ["opaqueArtifactCustody"],
    ...
    const wrongRoute = enabledContributions();
  ...
  it("requires plugin-owned feature, toolset, and resource metadata without core inference", () => {
    expect(() => createRegistry(enabledContributions({
  ...
  it("projects only validated enabled-plugin gateway route descriptors", () => {
    const contributions = enabledContributions();
    ...
    const invalid = enabledContributions();
    ...
    expect(() => createRegistry(invalid)).toThrow(/gateway descriptor is invalid/u);
  });
});
