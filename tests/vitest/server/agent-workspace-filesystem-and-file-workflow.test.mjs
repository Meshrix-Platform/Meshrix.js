...
describe("agent workspace behavior helper branches", () => {
...
describe("agent workspace behavior filesystem edges", () => {
  it("covers local directory mount validation and traversal guards", async () => {
    ...
    await withRuntime(async (runtime, root) => {
      ...
      const localSourcesRoot = path.join(root, "local-sources");
      await fsPromises.mkdir(localSourcesRoot, { recursive: true });
      const sourceRoot = await fsPromises.mkdtemp(path.join(localSourcesRoot, "source-"));
      const cleanSource = await fsPromises.mkdtemp(path.join(localSourcesRoot, "clean-source-"));
      try {
        ...
        const selection = runtime.createLocalDirectoryMountSelection({
          workspaceId: workspace.workspaceId,
        ...
        const directWrite = await runtime.writeLocalDirectoryFile({
          workspaceId: workspace.workspaceId,
          mountRef: firstMount.mount.mountRef,
        ...
        const workspaceSymlink = path.join(workspacePath, "workspace-symlink");
        ...
        const cleanMount = runtime.connectLocalDirectory({
          workspaceId: workspace.workspaceId,
          sourcePath: cleanSource,
...
describe("agent workspace behavior file operations", () => {
