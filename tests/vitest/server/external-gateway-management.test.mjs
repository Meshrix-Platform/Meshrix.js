...
function createProbeFetch({ identity, adapterId = "caddy", mcpOk = true } = {}) {
...
describe("External Gateway management provider", () => {
  it("normalizes one default policy and renders capability-valid Caddy and Nginx configs", () => {
    ...
    expect(profile.delegatedControls.filter((control) => profile.platformControls.includes(control))).toEqual([]);
    ...
    expect(profile.routes.find((route) => route.routeId === "mcp-stream")).toMatchObject({
      streaming: true,
      sticky: true,
    });

    const caddy = renderExternalGatewayConfig({ ...profile, adapterId: "caddy" });
    const nginx = renderExternalGatewayConfig({ ...profile, adapterId: "nginx" });
  ...
  it("atomically activates one generation and preserves it on validation failure", async () => {
    const persist = vi.fn(async () => {});
    const authority = createExternalGatewayAuthority({
      persist,
      validateRuntime: async ({ profile }) => profile.adapterId === "caddy"
        ? { ok: true }
        : { ok: false, reason: "adapter_validation_failed" },
      probe: async () => ({ ok: true }),
    });
    const provider = createExternalGatewayManagementProvider({ externalGateway: authority });

    expect(provider.getState()).toEqual({ mode: "direct", generation: 0 });
    expect(provider.listAdapters().map((adapter) => adapter.adapterId)).toEqual(["caddy", "nginx"]);
  ...
  it("requires an explicit endpoint and preserves the generation when the live probe fails", async () => {
    const persist = vi.fn(async () => {});
    const authority = createExternalGatewayAuthority({
      persist,
      probe: async () => ({ ok: false, reason: "external_gateway_health_probe_failed" }),
    });
  ...
  it("fails closed when no live endpoint probe is composed", async () => {
    const authority = createExternalGatewayAuthority();
    await expect(authority.apply({
      expectedGeneration: 0,
      mode: "external",
  ...
  it.each(["caddy", "nginx"])("passes only after %s health, signed transit, and MCP probes", async (adapterId) => {
    const identity = createProbeIdentity();
    const fetchImpl = createProbeFetch({ identity, adapterId });
    const profile = normalizeExternalGatewayProfile({
      mode: "external",
      adapterId,
      publicBaseUrl: "https://gateway.example.invalid",
    ...
    expect(fetchImpl.mock.calls.map(([url, options]) => [new URL(url).pathname, options?.method || "GET"]))
      .toEqual([
        ["/api/healthz", "GET"],
  ...
  it("rejects wrong adapter evidence and an invalid MCP endpoint", async () => {
    const identity = createProbeIdentity();
    const profile = normalizeExternalGatewayProfile({
      mode: "external",
      adapterId: "caddy",
  ...
  it("signs bounded gateway transit evidence into the platform handshake", () => {
    const identity = createProbeIdentity();
  ...
  it("registers and executes the bounded External Gateway management operations", async () => {
    ...
    const externalGatewayManagement = {
      getState: vi.fn(() => ({ mode: "direct", generation: 0 })),
      validate: vi.fn(async () => ({ ok: true })),
      apply: vi.fn(async () => ({ ok: true, mode: "external", generation: 1 })),
      switchDirect: vi.fn(async () => ({ ok: true, mode: "direct", generation: 2 })),
    };
    const context = { externalGatewayManagement };
