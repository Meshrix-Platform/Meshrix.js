...
import { createArtifactSignerPort } from "../../../packages/foundation/src/security/artifact-signer-port.ts";
import {
...
const roots: any[] = [];
const secretRef: any = "secret://fixture/artifact-signing";
...
async function fixture() : Promise<any> {
  const dataDir: any = await fs.mkdtemp(path.join(os.tmpdir(), "meshrix-artifact-signer-test-"));
  roots.push(dataDir);
  const { privateKey } = generateKeyPairSync("ed25519");
  const privateKeyJwk: any = privateKey.export({ format: "jwk" });
  const secretKeyProvider: any = createMemoryLocalSecretKeyProvider();
  await initializeLocalSecret({ dataDir, target, payload: { privateKeyJwk }, keyProvider: secretKeyProvider });
  return { dataDir, privateKeyJwk, secretKeyProvider, port: createArtifactSignerPort({
    dataDir,
    secretKeyProvider,
...
describe("ArtifactSignerPort", () : any => {
  it("returns only public verification facts and a context-bound receipt", async () : Promise<any> => {
    ...
    const signed: any = await port.sign(input);
  ...
  it("binds signatures to context and rejects unsupported purposes", async () : Promise<any> => {
    const { port } = await fixture();
    ...
    const first: any = await port.sign({ ...request, context: { moduleId: "sample-plugin", version: "one" } });
    const second: any = await port.sign({ ...request, context: { moduleId: "sample-plugin", version: "two" } });
  ...
  it("fails closed for scope mismatch and revoked custody", async () : Promise<any> => {
    const { dataDir, secretKeyProvider } = await fixture();
    const wrongServicePort: any = createArtifactSignerPort({ dataDir, secretKeyProvider, pluginId: "sample-plugin", serviceId: "different-service", allowedPurposes: ["plugin-artifact.sample-plugin.artifact"] });
    const request: Record<string, any> = {
      secretRef,
      purpose: "plugin-artifact.sample-plugin.artifact",
    ...
    await expect(createArtifactSignerPort({ dataDir, secretKeyProvider, pluginId: "sample-plugin", allowedPurposes: [request.purpose] }).sign(request))
      .rejects.toMatchObject({ code: "local_secret_revoked" });
  });

  it("does not mutate secret payload objects returned by an injected resolver", async () : Promise<any> => {
    const { privateKey } = generateKeyPairSync("ed25519");
    const sharedPayload: Record<string, any> = { privateKeyJwk: privateKey.export({ format: "jwk" }) };
    const before: any = structuredClone(sharedPayload);
    const port: any = createArtifactSignerPort({
      dataDir: "fixture-data",
      pluginId: "sample-plugin",
      allowedPurposes: ["plugin-artifact.sample-plugin.artifact"],
      resolveSecretPayload: async () : Promise<any> => ({ payload: sharedPayload, revision: 7 })
    });
    await port.sign({
      secretRef,
      purpose: "plugin-artifact.sample-plugin.artifact",
