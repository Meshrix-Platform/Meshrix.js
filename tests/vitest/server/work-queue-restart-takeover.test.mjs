...
import { ensureSqliteWorkQueueSchema } from "../../../packages/foundation/src/work-queue/sqlite-schema.mjs";

const DEFINITION_ID = "queue.jobs.import-parse";
const SCOPE = Object.freeze({ tenantId: "platform", workspaceId: "default" });
const execFileAsync = promisify(execFile);
const restartFixturePath = path.resolve("tools/server-scripts/lib/work-queue-process-restart-child.mjs");

async function runRestartFixture(args) {
  const result = await execFileAsync(process.execPath, [restartFixturePath, ...args], {
    timeout: 15_000,
    maxBuffer: 64 * 1024
  });
...
function resolved(definitionValue, dedupeKey) {
...
function createStore(userDataPath, timeSource) {
...
describe("work queue restart takeover", () => {
  it("reloads a stable definition in a fresh process and fences the stale lease", async () => {
    ...
    try {
      ...
      const recovered = await runRestartFixture([
        "recover",
        userDataPath,
        seeded.workItemId,
        seeded.leaseId
  ...
  it("enforces absolute work expiry before claim, renewal, and terminal completion", async () => {
    ...
    try {
      ...
      const beforeRejectedAdmissions = store.inspect({ queueDefinitionId: DEFINITION_ID }).items.length;
  ...
  it("keeps definition identity stable, reclaims an expired lease, and fences late completion", async () => {
    ...
    let firstStore = createStore(userDataPath, timeSource);
    try {
      ...
      const firstClaim = firstStore.claim({
        queueDefinitionId: DEFINITION_ID,
        scope: SCOPE,
      ...
      const staleLease = firstClaim.claimed[0];
      firstStore.close();

      const secondStore = createStore(userDataPath, timeSource);
      firstStore = null;
      try {
        secondStore.registerQueueDefinition(definition());
        const duplicate = secondStore.enqueue({
          ...admission,
          payloadRef: { kind: "import_parse_job", jobId: "job-1" },
          ownerRef: { capability: "platform.job-workflow", jobId: "job-1" }
        });
        ...
        const recovery = secondStore.claim({
          queueDefinitionId: DEFINITION_ID,
          scope: SCOPE,
        ...
        const takeover = secondStore.claim({
          queueDefinitionId: DEFINITION_ID,
          scope: SCOPE,
          workerId: "worker-after-restart",
          leaseTimeoutMs: 10
        ...
        const currentLease = takeover.claimed[0];
        ...
        const completed = secondStore.complete({
        ...
        const cancellationAdmission = resolved(queueDefinition, { jobId: "job-2", versionNumber: 1 });
        const cancellable = secondStore.enqueue({
          ...cancellationAdmission,
          payloadRef: { kind: "import_parse_job", jobId: "job-2" },
          ownerRef: { capability: "platform.job-workflow", jobId: "job-2" }
        });
        const cancellationClaim = secondStore.claim({
          queueDefinitionId: DEFINITION_ID,
          scope: SCOPE,
  ...
  it("uses typed canonical dedupe serialization without delimiter collisions", () => {
  ...
  it("upgrades a revision-one database with duplicate terminal dedupe rows before adding the global index", () => {
