...
describe("upstream audience projection", () => {
  it("keeps discovery and execution decisions identical across two subjects", () => {
    ...
    const service = candidate.serviceEntries[0][1];
    const capabilityId = operation._meta.dynamicCapability.capabilityId;
    ...
    const allowedParity = evaluateAudienceParity({
      grant: allowedGrant,
      operation,
      service
    });
    const deniedParity = evaluateAudienceParity({
      grant: deniedGrant,
      operation,
      service
    });
  ...
  it("applies deny-tag precedence and fails closed for missing metadata", () => {
    const candidate = snapshot(2, [serviceEntry({
      tagPolicy: {
    ...
    const denied = evaluateAudienceDecision({
      grant: baseGrant,
      operation,
      service,
      tagStore: denyStore,
  ...
  it("fails closed when a credential-bound capability has no granted binding", () => {
    const candidate = snapshot(2, [serviceEntry()]);
    const projection = compileUpstreamOperationProjection(candidate);
    const operation = projection.operations[0];
    const service = candidate.serviceEntries[0][1];
  ...
  it("uses the same audience authority for catalog discovery and execution", async () => {
    const runtimeExecute = vi.fn(async () => ({ ok: true, status: 200, payload: {} }));
    const tool = {
    ...
    expect(audience.mock.calls.map(([input]) => input.purpose)).toEqual(["discovery", "execution"]);
    expect(runtimeExecute).not.toHaveBeenCalled();
  });

  it("advances audience revision and diffs only affected opaque partitions", () => {
    const candidate = snapshot(3, [serviceEntry()]);
    const projection = compileUpstreamOperationProjection(candidate);
    const capabilityId = projection.operations[0]._meta.dynamicCapability.capabilityId;
    const bindingIds = projection.operations[0]._meta.dynamicCapability.credentialBindingIds;
    const first = compileAudienceProjection({
      sourceRevision: 3,
    ...
    const second = compileAudienceProjection({
      sourceRevision: 3,
      sourceDigest: candidate.setDigest,
      catalogFingerprint: "catalog-a",
      snapshot: candidate,
    ...
    const replay = compileAudienceProjection({
      sourceRevision: 3,
      sourceDigest: candidate.setDigest,
      catalogFingerprint: "catalog-a",
      snapshot: candidate,
    ...
    const event = createAudiencePublicationEvent(second);
  ...
  it("invalidates visible partitions when an operation descriptor changes without changing its id", () => {
    const firstSnapshot = snapshot(4, [serviceEntry()]);
    const firstOperations = compileUpstreamOperationProjection(firstSnapshot).operations;
    const capability = firstOperations[0]._meta.dynamicCapability;
    const audienceGrant = grant({
      id: "grant-descriptor",
    ...
    const secondSnapshot = snapshot(5, [serviceEntry({
      operations: [{
        operationKey: "read",
        method: "GET",
        path: "/read",
    ...
    const secondOperations = compileUpstreamOperationProjection(secondSnapshot).operations;
