...
describe("sensitive report URL scanning", () => {
  it("keeps ordinary public documentation URLs while ignoring URL path-shaped prose", () => {
    const value = ["https://docs.example.test", "/", ["home", "example", "guide"].join("/")].join("");
    expect(sensitiveReportFindings(value)).toEqual([]);
    expect(redactReportText(value)).toBe(value);
  });

  it("does not treat a SHA-256 digest beginning with an IPv6 prefix as an endpoint", () => {
  ...
  ])("detects %s before public URL normalization", (_label, value, finding) => {
