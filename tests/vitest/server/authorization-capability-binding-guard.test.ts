...
describe("authorization capability binding guard boundary behavior", () : any => {
  it("normalizes context aliases and hashes consistently across string and buffer lookup keys", () : any => {
    const normalized: any = normalizeCapabilityBindingContext({
    ...
    const lookupKey: any = Buffer.alloc(32, 23);
    ...
    expect(() : any => capabilityBindingKeyHash(Buffer.alloc(31), "cap-key"))
      .toThrow("Capability binding guard requires a 256-bit lookup key.");
    expect(() : any => capabilityBindingSubjectHash(Buffer.alloc(31), "namespace", "tenant-east"))
      .toThrow("Capability binding guard requires a 256-bit lookup key.");
  ...
  it("binds from aliased context fields, verifies wildcard-free matches, and leaves no-op invalidations untouched", async () : Promise<any> => {
    const dataDir: any = await tempDir("meshrix-binding-guard-extra-");
  ...
  it("supports memory guards with default namespace bindings and leaves alias sanitization intact", async () : Promise<any> => {
    const guard: any = createMemoryCapabilityBindingGuard({
