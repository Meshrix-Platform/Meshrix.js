...
describe("MCP release manifest", () : any => {
  it("projects canonical target details without a late runtime reference failure", () : any => {
    ...
    const manifest: any = releaseManifest(input);

    expect(manifest.portable.supportedTargetDetails.map(({ target }: Record<string, any>) : any => target))
      .toEqual(MCP_SUPPORTED_TARGETS);
    expect(manifest.install.supportedTargetDetails.map(({ target }: Record<string, any>) : any => target))
      .toEqual(MCP_SUPPORTED_TARGETS);
