...
} from "../../../packages/server-runtime/src/composition/console-domain/operation-executors/settings-agent-gateway-models.ts";
import { buildAgentSettingsConsoleProjection } from "../../../packages/server-runtime/src/composition/console-domain/state-projections.ts";
...
afterEach(async () : Promise<any> => {
  await Promise.all(
    temporaryRoots.splice(0).map((root?: any) : any => fs.rm(root, { recursive: true, force: true }))
  );
  vi.unstubAllEnvs();
});

function configuredOpenAiAgent(overrides: Record<string, any> = {}) : any {
  return {
...
function runConcurrentSettingsSave({ userDataPath, agentId }: Record<string, any>) : any {
  const source: any = `
  ...
  return new Promise((resolve?: any, reject?: any) : any => {
    const child: any = spawn(process.execPath, ["--input-type=module", "--eval", source], {
...
function runConcurrentRegistryReplace({ rootPath, agentId }: Record<string, any>) : any {
  const source: any = `
    import { AgentConfigRegistry } from "@meshrix/agents/agent-configs/config-registry";
    const agentId = process.env.MESHRIX_TEST_AGENT_ID;
...
describe("canonical model provider runtime", () : any => {
  it("probes the explicitly selected OpenAI agent through the real gateway transport", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-model-provider");
    const apiPatch: any = normalizeAgentModelPayload({
      tokenHeader: "Authorization",
      tokenPrefix: "Bearer "
    });
    expect(apiPatch.tokenPrefix).toBe("Bearer ");
    const entry: any = configuredOpenAiAgent(apiPatch);
    const fetchImpl: any = vi.fn(async (url?: any, init?: any) : Promise<any> => {
      ...
      const body: any = JSON.parse(init.body);
    ...
    const settings: Record<string, any> = { modelLibraryAgents: [entry] };
    expect(publicAgentGatewayRegistry(settings)).toMatchObject({
      defaultAlias: "",
      agents: [{ alias: entry.uid, provider: "openai", urlConfigured: true }]
    });
    const probe: any = await probeModelConnection({
      provider: "openai",
      modelAlias: entry.uid,
      settings,
      userDataPath,
      fetchImpl,
      egressLookup: async () : Promise<any> => [{ address: "192.0.2.1", family: 4 }]
    });
  ...
  it("rejects control characters in model credential prefixes", () : any => {
    expect(() : any => normalizeAgentModelPayload({ tokenPrefix: "Bearer\r\nInjected: yes" }))
      .toThrow(/CR, LF, or NUL/u);
    expect(() : any => normalizeAgentModelPayload({ tokenHeader: "Host" }))
      .toThrow(/reserved/u);
    expect(() : any => normalizeAgentModelPayload({ tokenHeader: "Authorization\nX-Test" }))
      .toThrow(/valid HTTP field name/u);
    expect(() : any => normalizeAgentModelPayload({
      baseUrl: ["https://user", "password@example.test/v1?credential=value"].join(":")
    })).toThrow(/userinfo, query, or fragment/u);
  });

  it("applies omitted, explicit-clear, replacement, and zero-timeout agent patches truthfully", () : any => {
    const current: any = configuredOpenAiAgent();
  ...
  it("fails closed when provider or model identity is not explicit", async () : Promise<any> => {
    const settings: Record<string, any> = { modelLibraryAgents: [configuredOpenAiAgent()] };
  ...
  it("persists agent credentials only in the canonical registry and redacts projections", async () : Promise<any> => {
    const rootPath: any = await temporaryRoot("meshrix-agent-config-registry");
    const registry: any = new AgentConfigRegistry({ rootPath });
    const entry: any = configuredOpenAiAgent();
    ...
    const modelFiles: any = (await fs.readdir(registry.modelListPath))
      .filter((file?: any) : any => file !== "manifest.json");
    expect(modelFiles).toHaveLength(1);
    const modelPath: any = path.join(registry.modelListPath, modelFiles[0]);
    const stat: any = await fs.stat(modelPath);
    expect(stat.mode & 0o777).toBe(0o600);
    const persistedModel: any = await fs.readFile(modelPath, "utf8");
    expect(persistedModel).not.toContain("test-only-key");
    expect(persistedModel).toContain("model-credential://");
    const credentialFiles: any = await fs.readdir(path.join(rootPath, "credentials"));
  ...
  it("keeps the previous generation intact when replacement staging fails", async () : Promise<any> => {
    const rootPath: any = await temporaryRoot("meshrix-agent-config-rollback");
    const registry: any = new AgentConfigRegistry({ rootPath });
    const entry: any = configuredOpenAiAgent();
    await registry.replaceFromModelLibraryAgents([entry]);
    const before: any = JSON.parse(await fs.readFile(
    ...
    const after: any = JSON.parse(await fs.readFile(
      path.join(rootPath, "current.json"),
      "utf8"
    ));
  ...
  it("rejects case-fold identity collisions without changing the active generation", async () : Promise<any> => {
    const rootPath: any = await temporaryRoot("meshrix-agent-config-collision");
    const registry: any = new AgentConfigRegistry({ rootPath });
    const entry: any = configuredOpenAiAgent({
      uid: "Agent_X",
      instanceId: "Agent_X",
    ...
    await registry.replaceFromModelLibraryAgents([entry]);
    const beforeGeneration: any = registry.generation;
    await expect(registry.replaceFromModelLibraryAgents([
      entry,
  ...
  it("commits one cross-process current-pointer replacement per expected revision", async () : Promise<any> => {
    const rootPath: any = await temporaryRoot("meshrix-agent-config-cross-process-cas");
    const registry: any = new AgentConfigRegistry({ rootPath });
    ...
    expect(attempts.every((attempt?: any) : any => attempt.stderr === "")).toBe(true);
    const results: any = attempts.map((attempt?: any) : any => JSON.parse(attempt.stdout));
    const success: any = results.find((result?: any) : any => result.ok);
    const conflict: any = results.find((result?: any) : any => !result.ok);
  ...
  it("never preserves or sends a redacted credential after its provider origin changes", async () : Promise<any> => {
    const rootPath: any = await temporaryRoot("meshrix-agent-credential-binding");
    const registry: any = new AgentConfigRegistry({ rootPath });
    const current: any = configuredOpenAiAgent({ baseUrl: "https://old.example.test/v1" });
    ...
    const merged: any = mergeSettingsForModelProbe(
  ...
  it("keeps registry-owned model records out of the main settings document", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-settings-ownership");
    const entry: any = configuredOpenAiAgent();
    ...
    const persisted: any = JSON.parse(
  ...
  it("rolls back registry and split settings when a multi-document save fails", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-settings-transaction-rollback");
    const original: any = configuredOpenAiAgent();
    await saveSettings(userDataPath, {
      modelLibraryRevision: 0,
      modelLibraryEntries: ["openai"],
    ...
    cyclic.self = cyclic;
    const replacement: any = configuredOpenAiAgent({
    ...
    const restored: any = await loadSettings(userDataPath);
  ...
  it("rejects stale model-library snapshots with revision compare-and-swap", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-model-library-cas");
    const original: any = configuredOpenAiAgent();
    const first: any = await saveSettings(userDataPath, {
      modelLibraryRevision: 0,
      modelLibraryEntries: ["openai"],
      modelLibraryAgents: [original]
    });
  ...
  it("serializes cross-process settings and model-pointer compare-and-swap", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-settings-cross-process-cas");
    await saveSettings(userDataPath, {
      modelLibraryRevision: 0,
      modelLibraryEntries: ["openai"],
      modelLibraryAgents: [configuredOpenAiAgent()]
    ...
    const loaded: any = await loadSettings(userDataPath);
    ...
    const pointer: any = JSON.parse(await fs.readFile(
  ...
  it("recovers a prepared cross-document transaction after process termination", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-settings-crash-recovery");
    const original: any = configuredOpenAiAgent();
    await saveSettings(userDataPath, {
      modelLibraryRevision: 0,
      modelLibraryEntries: ["openai"],
    ...
    const registryRoot: any = path.join(userDataPath, "agent-configs");
    const settingsPath: any = path.join(userDataPath, "settings.json");
    const agentToolPath: any = path.join(userDataPath, "operation-permission", "execution.json");
    const previousPointer: any = JSON.parse(await fs.readFile(path.join(registryRoot, "current.json"), "utf8"));
    const previousMain: any = JSON.parse(await fs.readFile(settingsPath, "utf8"));
    const previousAgentTool: any = JSON.parse(await fs.readFile(agentToolPath, "utf8"));

    const registry: any = new AgentConfigRegistry({ rootPath: registryRoot });
    const replacement: any = configuredOpenAiAgent({
    ...
    const interruptedGeneration: any = registry.generation;
    ...
    const transactionId: any = "a".repeat(32);
    ...
    const blockedRegistry: any = new AgentConfigRegistry({ rootPath: registryRoot });
    ...
    const recovered: any = await loadSettings(userDataPath);
  ...
  it("keeps module bindings and profiles attached to registry-owned agents across save and load", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-settings-agent-bindings");
    const entry: any = configuredOpenAiAgent({
  ...
  it("projects the same readiness rules for all five runtime providers", async () : Promise<any> => {
    const userDataPath: any = await temporaryRoot("meshrix-provider-readiness");
    const providers: any[] = ["openai", "deepseek", "openrouter", "copilot", "local-model"];
    const entries: any = providers.map((provider?: any) : any => configuredOpenAiAgent({
    ...
    const projection: any = await buildAgentSettingsConsoleProjection({
      userDataPath,
      settingsPort,
      getAgentConfigRegistry: () : any => new AgentConfigRegistry({
        rootPath: path.join(userDataPath, "agent-configs")
      })
    });
    expect(projection.agentSelector.options).toHaveLength(5);
    expect(projection.agentSelector.options.map((option?: any) : any => ({
      provider: option.provider,
      selectable: option.selectable,
      capabilities: option.capabilities
    }))).toEqual(providers.map((provider?: any) : any => ({
      provider,
      selectable: true,
      capabilities: ["agent.invoke", "gateway.forward"]
    })));

    const incomplete: any = publicAgentGatewayRegistry({
    ...
    const localWithoutTimeout: any = publicAgentGatewayRegistry({
