...
describe("External Gateway registry and normalization", () : any => {
  it("exposes the built-in adapters, accepts aliases, and rejects unknown adapters", () : any => {
    const adapters: any = listExternalGatewayAdapters();
    expect(adapters.map((adapter?: any) : any => adapter.adapterId)).toEqual(["caddy", "nginx"]);
    ...
    expect(() : any => getExternalGatewayAdapter("edge-router")).toThrow("Unsupported gateway adapter: edge-router");
    expect(() : any => registerExternalGatewayAdapter({ adapterId: "broken" })).toThrow(
      "gateway adapter broken must provide renderConfig(profile)"
    );
  });

  it("normalizes profile inputs, fills defaults, and derives route manifests", () : any => {
...
describe("traffic gateway rendering", () : any => {
  it("renders caddy and nginx configs from plain and pre-normalized inputs", () : any => {
    const caddy: any = renderCaddyConfig({
      directMode: {
        baseUrl: "https://direct.example.invalid:9443"
      },
      gatewayMode: {
  ...
  it("registers and renders a custom adapter", () : any => {
    const customAdapter: any = registerExternalGatewayAdapter({
      adapterId: "example-edge",
      label: "Example Edge",
      fileName: "example-edge.conf",
      mediaType: "text/example-edge",
      renderConfig(profile?: any) : any {
    ...
    expect(listExternalGatewayAdapters().map((adapter?: any) : any => adapter.adapterId)).toEqual(
      expect.arrayContaining(["caddy", "nginx", "example-edge"])
    );

    const rendered: any = renderExternalGatewayConfig({
      adapterId: "example-edge",
      directBaseUrl: "https://direct.example.invalid:9443",
      publicBaseUrl: "https://gateway.example.invalid:8443"
    });
...
describe("traffic gateway runtime planning and validation", () : any => {
  it("resolves runtime plans from env, explicit cache roots, and runtime URLs", () : any => {
    const homedirSpy: any = vi.spyOn(os, "homedir").mockReturnValue("/private/meshrix");
  ...
  it("validates the gateway contract and reports missing route and policy failures", () : any => {
    const report: any = validateExternalGatewayProfile({
      schemaVersion: "v0.0.1:schema:definition-1",
      protocol: EXTERNAL_GATEWAY_PROTOCOL_VERSION,
      adapterId: "caddy",
      directMode: {
