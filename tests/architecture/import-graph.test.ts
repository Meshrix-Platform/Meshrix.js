...
async function test() : Promise<any> {
  ...
  assert.equal(
    graph.summary.relativeEdgeCount +
      ...
      graph.edges.filter((edge?: any) : any => !["relative", "package-import", "workspace-package"].includes(edge.family)).length,
    graph.summary.totalEdges,
    "every resolved edge must belong to a counted specifier family"
  );
  assert.equal(
    violations.filter((violation?: any) : any => violation.rule === "unsupported-import-specifier-family").length,
    0,
