 ...
 *
 * This module derives its data from tools/registry/architecture-layout-manifest.ts.
 *
 ...
 *   node tools/server-scripts/verify-layout-manifest-consistency.ts
 */

import {
  PUBLIC_FACADES as _ARCH_PUBLIC_FACADES,
  PUBLIC_RESOURCES as _ARCH_PUBLIC_RESOURCES,
} from "./architecture-layout-manifest.ts";

export const SOURCE_LAYOUT_MANIFEST_VERSION: any = "source-layout-manifest";

// ── Derive publicModules from architecture manifest ────────────────────────────

function _buildPublicModules() : any {
...
function _buildPublicResources() : any {
  const resources: any = Object.create(null);
...
export const SOURCE_LAYOUT_MANIFEST: Readonly<Record<string, any>> = Object.freeze({
...
export function sourceLayoutModule(moduleId?: any) : any {
  const entry: any = SOURCE_LAYOUT_MANIFEST.publicModules[moduleId];
  if (!entry) {
...
export function sourceLayoutPath(moduleId?: any) : any {
  const moduleEntry: any = SOURCE_LAYOUT_MANIFEST.publicModules[moduleId];
  if (moduleEntry) {
    return moduleEntry.facadePath;
  }
  const resourceEntry: any = SOURCE_LAYOUT_MANIFEST.publicResources[moduleId];
  if (resourceEntry) {
...
export function sourceLayoutPackageImports() : any {
  return Object.freeze(
    Object.fromEntries(
      (Object.values(SOURCE_LAYOUT_MANIFEST.publicModules) as any[])
        .filter((entry?: any) : any => entry.importSpecifier && entry.facadePath)
        .map((entry?: any) : any => [entry.importSpecifier, `./${entry.facadePath}`])
    )
  );
}

export function validateSourceLayoutPackageImports(imports?: any, { label = "package.json#imports" }: Record<string, any> = {}) : any {
  if (!imports || typeof imports !== "object" || Array.isArray(imports)) {
    throw new Error(`${label} must define source-layout public module imports.`);
  }

  for (const [specifier, expectedTarget] of (Object.entries(sourceLayoutPackageImports()) as [string, any][])) {
