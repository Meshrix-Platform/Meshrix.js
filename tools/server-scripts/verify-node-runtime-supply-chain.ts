...
try {
  ...
  } = await import("./lib/mcp-release-portable.ts");
  ...
  for (const publicKeyUrl of [
    ...
    const invalidSignerLock: any = structuredClone(lock);
    invalidSignerLock.signer.publicKeyUrl = publicKeyUrl;
    assert.throws(
      () : any => verifyNodeRuntimeSignedChecksums({ lock: invalidSignerLock, checksumsText: "tampered\n" }),
      /node_runtime_lock_untrusted_signer_key/u
    );
  }
  const invalidMetadataSizeLock: any = structuredClone(lock);
  invalidMetadataSizeLock.signatureSizeBytes = 0;
  assert.throws(
    () : any => verifyNodeRuntimeSignedChecksums({ lock: invalidMetadataSizeLock, checksumsText: "tampered\n" }),
    /node_runtime_lock_invalid_metadata_size/u
  );
  const invalidTargetSizeLock: any = structuredClone(lock);
  invalidTargetSizeLock.targets[Object.keys(invalidTargetSizeLock.targets)[0]].sizeBytes = 0;
  assert.throws(
    () : any => verifyNodeRuntimeSignedChecksums({ lock: invalidTargetSizeLock, checksumsText: "tampered\n" }),
  ...
  const evidence: any = await verifyPinnedNodeRuntimeRelease({ outputDir: path.join(tempRoot, "verify") });
  ...
  const cacheDir: any = process.env.MESHRIX_MCP_NODE_RUNTIME_CACHE_DIR;
  const checksumsPath: any = path.join(cacheDir, `${lock.version}-${lock.checksumsFile}`);
  const signaturePath: any = path.join(cacheDir, `${lock.version}-${lock.signatureFile}`);
  const keyPath: any = path.join(cacheDir, `${lock.signer.fingerprint}.asc`);
  const wrongSignerLock: any = structuredClone(lock);
  wrongSignerLock.signer.fingerprint = lock.signer.fingerprint === "A".repeat(40)
    ? "B".repeat(40)
    : "A".repeat(40);
  await assert.rejects(
    () : any => verifyNodeReleaseSignature({
  ...
  const tamperedSignaturePath: any = path.join(tempRoot, "tampered-signature.sig");
  const tamperedSignature: any = await fs.readFile(signaturePath);
  ...
  await assert.rejects(
    () : any => downloadPinnedFile(
      "https://nodejs.org/dist/redirect-fixture",
      path.join(tempRoot, "redirect-fixture"),
      fixtureSha256,
      fixtureBytes.length,
      { fetchImpl: async () : Promise<any> => responseFor(fixtureBytes, { redirected: true }) }
    ),
  ...
  await assert.rejects(
    () : any => downloadPinnedFile(
      ...
      fixtureSha256,
      fixtureBytes.length,
      { fetchImpl: async () : Promise<any> => responseFor(Buffer.concat([fixtureBytes, Buffer.from("x")])) }
    ),
    /node_runtime_download_size_limit_exceeded/u
  );

  let transientRetryFetchCount: any = 0;
  const transientRetryDestination: any = path.join(tempRoot, "transient-retry-fixture");
  ...
  let concurrentFetchCount: any = 0;
  const concurrentDestination: any = path.join(tempRoot, "concurrent-fixture");
  const concurrentFetch: any = async () : Promise<any> => {
