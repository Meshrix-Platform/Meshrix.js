...
import {
  ...
  listHydrationFacts
} from "../../packages/foundation/src/composition-management/architecture/manifest.ts";
import { computeArchitectureFactsDigest } from "../generators/generate-architecture-diagram-digests.ts";

const __filename: any = fileURLToPath(import.meta.url);
const __dirname: any = path.dirname(__filename);
const repoRoot: any = path.resolve(__dirname, "../..");
...
async function assertPathExists(relativePath?: any) : Promise<any> {
  await fs.access(repoPath(relativePath));
}

function collectModuleIds(module?: any, ids: any = []) : any {
...
function extractHtmlCodeValues(html?: any) : any {
  return [...html.matchAll(/<code>(.*?)<\/code>/g)].map((match?: any) : any => match[1].trim()).filter(Boolean);
}

function assertHydrationShape(record?: any, context?: any) : any {
...
async function verifyDocumentationClassification() : Promise<any> {
  const promotional: any = listDocumentationAssetsByClassification("promotional");
  const developer: any = listDocumentationAssetsByClassification("developer-documentation");
...
async function verifyManifestShape() : Promise<any> {
  ...
  assert.equal(MESHRIX_ARCHITECTURE_FACTS.authority, "packages/contracts/src/modules/manifest.ts");
  ...
  const categoryIds: any = new Set<any>();
  ...
  for (const expectedCategory of (Object.values(ARCHITECTURE_MODULE_CATEGORY_BY_LAYER) as any[])) {
  ...
  const layerIds: any = new Set<any>(MESHRIX_ARCHITECTURE_FACTS.systemLayers.map((layer?: any) : any => layer.layerId));
  const moduleIds: any[] = [];
  ...
  assert.equal(new Set<any>(moduleIds).size, moduleIds.length, "architecture module ids must be unique");

  const nodeFacts: any = listArchitectureNodeFacts();
  const nodeFactIds: any = new Set<any>(nodeFacts.map((node?: any) : any => node.moduleId));
  ...
  const serviceLayerNumbers: any = MESHRIX_ARCHITECTURE_FACTS.serviceCapabilityLayers.map((layer?: any) : any => layer.layerNumber);
  ...
  const hydrationFacts: any = listHydrationFacts();
  assert.ok(hydrationFacts.some((item?: any) : any => item.hydratable === false), "manifest must include non-hydratable modules");
  assert.ok(hydrationFacts.some((item?: any) : any => item.hydratable === true), "manifest must include hydratable modules");
  const inventory: any = buildArchitectureComponentInventory();
  ...
  assert.ok(
    inventory.baseComponents.every((component?: any) : any => component.moduleCategory === "foundation"),
    "base components must be foundation components"
  );
  assert.ok(
    inventory.hydratableBaseComponents.every((component?: any) : any => component.moduleCategory === "foundation" && component.hydratable === true),
    "hydratable base components must be hydratable foundation components"
  );
  assert.ok(
    inventory.nonHydratableBaseComponents.every((component?: any) : any => component.moduleCategory === "foundation" && component.hydratable === false),
    "non-hydratable base components must be non-hydratable foundation components"
  );
  assert.ok(
    inventory.hydratableComponents.every((component?: any) : any => component.hydratable === true),
    "hydratable component list must only include hydratable components"
  );
}

async function verifyHtmlSourceCoverage() : Promise<any> {
  const expectedDigest: any = computeArchitectureFactsDigest();
  const systemHtml: any = await read("docs/architecture/MESHRIX-SYSTEM-ARCHITECTURE.html");
  const serviceHtml: any = await read("docs/architecture/MESHRIX-SERVICE-CAPABILITY-ARCHITECTURE.html");

  for (const [label, html] of [
    ["system", systemHtml],
    ["service", serviceHtml]
  ]) {
    const marker: any = html.match(DIGEST_MARKER_RE)?.[1];
    assert.equal(
      marker,
      expectedDigest,
  ...
  const serviceProtocolFieldIds: any = new Set<any>(MESHRIX_ARCHITECTURE_FACTS.serviceCapabilityProtocolFields.map((field?: any) : any => field.fieldId));
