...
const REPORT_PATH = "build/reports/core-platform-documentation-convergence.json";

const MCP_RELEASE_TARGET_LABELS = Object.freeze(
  MCP_SUPPORTED_TARGETS.map((target) => MCP_TARGET_LABELS[target])
);
...
const INTERNAL_DOCUMENTATION_REFERENCE_PATTERNS = Object.freeze([
...
const DOCUMENT_AUTHORING_MARKER_PATTERNS = Object.freeze([
  ["authoring_todo", /(?:^|\s)(?:TODO|FIXME)(?=\s|:|$)/imu]
]);

function documentAuthoringMarkerMatches(documentText) {
  return DOCUMENT_AUTHORING_MARKER_PATTERNS
    .filter(([, pattern]) => pattern.test(documentText))
    .map(([id]) => id);
}

function verifyDocumentationPolicyContract() {
  const truthfulLimitations = [
...
const PRE_RELEASE_READMES = Object.freeze([
  "README.md",
  "README.zh-CN.md"
]);

const INSTALLER_READMES = Object.freeze([
  "packages/protocols/mcp/adapter/native-installer/README.md"
]);
...
async function main() {
  ...
  const preReleasePolicyActive = /\bpre-release\b/iu.test(securityPolicy);

  const rows = Object.entries(docs).map(([relativePath, text]) => {
    const requiredTerms = REQUIRED_TERMS[relativePath] || [];
    const missingRequiredTerms = requiredTerms.filter((term) => !text.includes(term));
    const internalReferenceMatches = INTERNAL_DOCUMENTATION_REFERENCE_PATTERNS
      .filter(([, pattern]) => pattern.test(text))
      .map(([id]) => id);
    const staleMatches = documentAuthoringMarkerMatches(text);
    const policyMatches = [];
  ...
  const failingRows = rows.filter((row) => !row.releaseReady);
  const report = {
    schemaVersion: "v0.0.1:platform:documentation-convergence-report-1",
    generatedAt: new Date().toISOString(),
    verifier: "tools/server-scripts/verify-core-platform-documentation-convergence.mjs",
    algorithm: {
    ...
    summary: {
      ...
      policySelfTest: true,
      missingRequiredTermCount: rows.reduce((count, row) => count + row.missingRequiredTerms.length, 0),
      internalReferenceMatchCount: rows.reduce((count, row) => count + row.internalReferenceMatches.length, 0),
      staleMatchCount: rows.reduce((count, row) => count + row.staleMatches.length, 0),
      policyMatchCount: rows.reduce((count, row) => count + row.policyMatches.length, 0),
      failingDocuments: failingRows.map((row) => ({
        path: row.path,
