#!/usr/bin/env node
/**
 * verify-layout-audit.mjs — Comprehensive Repository Layout Audit
 *
 ...
 * - Script registry coverage (from tools/scripts/package-script-registry.mjs)
...
const strictMode = process.argv.includes("--strict") || !reportOnly; // default: strict unless report-only
const reportFormat = process.argv.includes("--markdown") ? "markdown" : "json";
const ROOT_AUDIT_IGNORED_ENTRIES = new Set([
  ".DS_Store",
  ".cache",
  ".git",
  "build",
...
async function packageExclusionTargetExists(pattern) {
  ...
  while (pending.length > 0) {
    ...
    for (const entry of await fs.readdir(absoluteDir, { withFileTypes: true })) {
      ...
      const relativeEntry = toPosix(path.join(relativeDir, entry.name));
...
/**
 * Scan a directory for .mjs/.js/.ts/.vue files and check for import patterns.
 */
async function* walkFiles(rootDir, extensions = [".mjs", ".js"]) {
...
async function main() {
  ...
  const archUrl = pathToFileURL(
    path.join(repoRoot, "tools/registry/architecture-layout-manifest.mjs")
  ).href;
  const arch = await import(archUrl);
  const packageJson = await readJson("package.json");
  const scripts = packageJson.scripts || {};

  const scriptReg = await import(pathToFileURL(path.join(repoRoot, "tools/scripts/package-script-registry.mjs")).href);
  const suiteReg = await readJson("tools/registry/tests.registry.json");

  // ── S1: Root directory classification ─────────────────────────────────────
  {
    const rootEntries = await fs.readdir(repoRoot, { withFileTypes: true });
    const actualNames = new Set(rootEntries
      .map((e) => e.name)
      .filter((name) => !ROOT_AUDIT_IGNORED_ENTRIES.has(name)));
    const allowedNames = new Set(arch.ROOT_ALLOWED_ENTRIES);

    const unclassified = [...actualNames].filter((n) => !allowedNames.has(n));
  ...
  {
    const canonicalRoots = arch.ROOT_ENTRIES
      .filter((entry) => entry.kind === "source-root" && entry.required === true)
  ...
  // ── S4: Public facade imports ────────────────────────────────────────────
  {
    ...
    for (const facade of arch.PUBLIC_FACADES) {
      const expected = `./${facade.facadePath}`;
  ...
  // ── S6: package.json#files vs runtime payload policy ─────────────────────
  {
    ...
    const exclusions = files.filter((f) => f.startsWith("!"));
    const manifestPatterns = new Set(arch.RUNTIME_PAYLOADS.map((rp) => rp.packageExclusion || rp.pattern).filter(Boolean));
    const staleExclusions = [];

    for (const rp of arch.RUNTIME_PAYLOADS) {
      const exclusion = rp.packageExclusion || rp.pattern;
    ...
    for (const exclusion of exclusions) {
      ...
      const cleanPath = exclusion.slice(1).replace(/\/\*\*$/, "");
      const exists = await packageExclusionTargetExists(cleanPath);
      const payload = arch.RUNTIME_PAYLOADS.find((rp) => rp.pattern === exclusion);
      if (!exists) {
  ...
  // ── S8: Test suite registry coverage ─────────────────────────────────────
  {
    ...
    const missing = [...suiteScriptRefs].filter((s) => !scripts[s]);
  ...
  {
    const architectureVerifier = await import(pathToFileURL(
      path.join(repoRoot, "tools/verifiers/architecture-graph.mjs")
    ).href);
    const architectureResult = await architectureVerifier.runArchitectureGraph({
      verbose: false,
      writeReport: false
    });
    const constraintViolations = architectureResult.violations;
    ...
    if (constraintViolations.length > 0) {
      const shown = constraintViolations.slice(0, 20);
  ...
  {
    try {
      const routesIndex = await fs.readFile(path.join(repoRoot, "apps/console/router/index.ts"), "utf8");
      const routesTs = await fs.readFile(path.join(repoRoot, "apps/console/router/routes.ts"), "utf8");
      // Strip single-line and block comments to avoid flagging documentation
      const stripComments = (s) => s.replace(/\/\/.*$/gm, "").replace(/\/\*[\s\S]*?\*\//g, "");
      const activeIndex = stripComments(routesIndex);
      const activeRoutes = stripComments(routesTs);
      const usesImGlob = activeIndex.includes("import.meta.glob") || activeIndex.includes("resolveAdminComponent") || activeRoutes.includes("import.meta.glob");
      const hasViteIgnore = activeIndex.includes("@vite-ignore") || activeRoutes.includes("@vite-ignore");
  ...
  const jsonPath = path.join(reportDir, "layout-audit.json");
  ...
  if (reportFormat === "markdown") {
    ...
    for (const severity of ["severe", "warning", "info"]) {
      const items = report.issues[severity];
      if (items.length > 0) {
    ...
    const mdPath = path.join(reportDir, "layout-audit.md");
  ...
  // ── Exit code ────────────────────────────────────────────────────────────
  if (!reportOnly) {
    const severeCount = report.issues.severe.length;
    const warningCount = report.issues.warning.length;
    if (severeCount > 0) {
