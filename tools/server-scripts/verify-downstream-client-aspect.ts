...
} from "../../packages/protocols/downstream-client-aspect/index.ts";

const FIXTURE_FRAMEWORK: Readonly<Record<string, any>> = Object.freeze({
...
const tempRoot: any = await fs.mkdtemp(path.join(os.tmpdir(), "meshrix-downstream-mcp-aspect-"));
try {
  const fixtureBin: any = path.join(tempRoot, "bin");
  await fs.mkdir(fixtureBin, { recursive: true });
  const executableName: any = (name?: any) : any => path.join(fixtureBin, process.platform === "win32" ? `${name}.cmd` : name);
  for (const name of ["fixture-client"]) {
    const filePath: any = executableName(name);
    await fs.writeFile(filePath, process.platform === "win32" ? "@echo off\r\nexit /b 0\r\n" : "#!/bin/sh\nexit 0\n", "utf8");
    if (process.platform !== "win32") await fs.chmod(filePath, 0o755);
  }
  const envWithAgentBins: Record<string, any> = { ...process.env, PATH: fixtureBin };
  ...
  assert.equal(capabilities.every((record?: any) : any => record.protocol === "mcp"), true);
  assert.equal(capabilities.every((record?: any) : any => record.capabilities.toolBoundary === "v0.0.1:operation-permission:projection-1"), true);

  const route: any = service.translateInboundRequest({
  ...
  const missingFixture: any = missingService.listCapabilities({ protocol: "mcp", frameworkId: "fixture-client" }).at(0);
  ...
  const layer: any = new McpAgentFrameworkAdapterLayer();
