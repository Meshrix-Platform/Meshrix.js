...
import { assertNoSensitiveReportLeak } from "./lib/sensitive-report-scan.mjs";
import { PRODUCTION_READINESS_GATES_REPORT_PATH } from "./lib/production-readiness-gates-evidence.mjs";
import {
...
const DEFAULT_PRODUCTION_READINESS_GATE_TIMEOUT_MS = 12 * 60 * 1000;
...
export const PRODUCTION_READINESS_GATES = Object.freeze([
  {
    ...
    verifier: "tools/server-scripts/verify-security-local-stdio-lockdown.mjs",
  ...
  },
  {
    ...
    owner: "security-boundary",
    verifier: "tools/server-scripts/verify-risk-control-model.mjs",
    command: process.execPath,
    args: ["tools/server-scripts/verify-risk-control-model.mjs"]
  }
...
function buildMissingEvidence(gateResults) {
  return gateResults
    .filter((result) => result.status === "blocked")
    .map((result) => `gate-blocked:${result.id}`);
}

function createProductionGateAggregateReadiness(gateResults, missingEvidence) {
  const failedGates = gateResults.filter((result) => result.status === "failed");
  return createAggregateReleaseEvidenceReadiness({
    ...
    failedCommands: failedGates.map((result) => result.id),
...
function buildProductionReadinessGateReport({
  ...
  const failedGates = gateResults.filter((result) => result.status === "failed");
  const blockedGates = gateResults.filter((result) => result.status === "blocked");
  const coreBlocked = aggregateReadiness.releaseReady !== true &&
  ...
  return {
    ...
    verifier: "tools/server-scripts/production-readiness-gate.mjs",
    algorithm: {
    ...
    gates: gateResults.map((result) => {
      const gate = PRODUCTION_READINESS_GATES.find((candidate) => candidate.id === result.id) || {};
      return {
        id: result.id,
        owner: gate.owner || "",
    ...
    summary: {
      ...
      failedGates: failedGates.map((result) => result.id),
      blockedGateCount: blockedGates.length,
      blockedGates: blockedGates.map((result) => result.id),
      missingEvidenceCount: missingEvidence.length,
      missingEvidence,
...
async function writeProductionReadinessGateReport(report) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const { results: rawGateResults, schedule: gateSchedule } = await runReleaseCommandDag({
    commands: PRODUCTION_READINESS_GATES.map((gate) => ({
      ...gate,
  ...
  const gateResults = rawGateResults.map((result) => ({
    id: result.id,
    status: result.status,
    exitCode: result.exitCode,
  ...
  const missingEvidence = buildMissingEvidence(gateResults);
  const aggregateReadiness = createProductionGateAggregateReadiness(gateResults, missingEvidence);
  const finishedAt = new Date();
  const report = buildProductionReadinessGateReport({
    startedAt,
    finishedAt,
    gateResults,
    missingEvidence,
  ...
  const reportReadiness = createReleaseEvidenceReadiness(
  ...
  const exitCode = liveReadinessExitCode(reportReadiness);
