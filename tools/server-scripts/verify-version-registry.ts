...
const registryRelativePath: any = "packages/foundation/src/version-control/version-registry.json";
const schemaRelativePath: any = "packages/foundation/src/version-control/version-registry.schema.json";
const artifactLifecycleRelativePath: any = "packages/foundation/src/workflow/state-machine/definitions/version.artifact.lifecycle.json";
const transitionLifecycleRelativePath: any = "packages/foundation/src/workflow/state-machine/definitions/version.transition.lifecycle.json";
const reportRelativePath: any = "build/reports/version-registry/latest.json";

const ARTIFACT_ID_PATTERN: any = /^meshrix(?:\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)+$/;
const VERSION_NAME_PATTERN: any = /^(?![a-z0-9-]*(?:legacy|compat|v[0-9]+))[a-z](?:[a-z0-9-]*[a-z0-9])?$/;
const VERSION_SUBSECTION_PATTERN: any = /^(?![a-z0-9-]*(?:legacy|compat|v[0-9]+))[a-z](?:[a-z0-9-]*[a-z0-9])?-[0-9]+(\.[0-9]+)*$/;
const ARTIFACT_REF_PATTERN: any = new RegExp(
  `^meshrix(?:\\.[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)+@${GOVERNED_VERSION_PATTERN.source.slice(1, -1)}$`
);
const ABSOLUTE_PATH_PATTERNS: any[] = [
  /\/Users\//i,
  /\/home\//i,
  /^[a-zA-Z]:\\/
];
...
function assertOnlyKeys(object?: any, allowedKeys?: any, label?: any) : any {
  const allowed: any = new Set<any>(allowedKeys);
  for (const key of Object.keys(object || {})) {
    assert.equal(allowed.has(key), true, `${label} contains unsupported key: ${key}`);
  }
}

function assertNoSensitiveStrings(value?: any, pathLabel: any = "registry") : any {
  ...
  if (Array.isArray(value)) {
    value.forEach((item?: any, index?: any) : any => assertNoSensitiveStrings(item, `${pathLabel}[${index}]`));
    return;
  }
...
function assertRefShape(ref?: any, label?: any, platformVersionBaseline?: any) : any {
...
function assertArtifactId(value?: any, label?: any) : any {
  assert.match(value, ARTIFACT_ID_PATTERN, `${label} must be a stable dotted meshrix.* identity`);
}

function assertGovernedVersion(value?: any, label?: any, platformVersionBaseline: any = "v0.0.1") : any {
  assert.match(
    value,
    GOVERNED_VERSION_PATTERN,
...
function expectedRef(artifactId?: any, version?: any) : any {
  return `${artifactId}@${version}`;
}

function isFixtureEvidenceUri(uri: any = "") : any {
  const normalized: any = normalizePath(uri);
  return normalized.startsWith("tests/") || /(?:^|[-/])fixture(?:[-./]|$)/u.test(normalized);
}

function isVerifierEntrypointUri(uri: any = "") : any {
  const normalized: any = normalizePath(uri);
  return /^tools\/(?:server-scripts\/verify-|verifiers\/)/u.test(normalized);
}

function verifierOwnedVersion(version: any = "") : any {
  const subsection: any = String(version || "").split(":").at(-1) || "";
  return /(?:report|audit|verification|readiness|conformance|evidence|proof|manifest|checkpoint|boundary|profile|candidate|runner|inventory)(?:-|$)/u
    .test(subsection);
}

function assertActiveVersionAuthority(artifact?: any, versionRecord?: any) : any {
  assert.equal(
  ...
  const evidenceUris: any = versionRecord.evidenceRefs.map((ref?: any) : any => normalizePath(ref.uri));
  const authorityUris: any = evidenceUris.filter((uri?: any) : any => !isFixtureEvidenceUri(uri));
  assert.ok(
    authorityUris.length > 0,
    `${artifact.artifactId} active version requires a non-test, non-fixture authority source`
  );
  assert.equal(
    verifierOwnedVersion(versionRecord.version) || authorityUris.some((uri?: any) : any => !isVerifierEntrypointUri(uri)),
    true,
    `${artifact.artifactId} non-report version requires runtime, schema, registry, or generator authority`
  );
}

async function assertReferenceExists(uri?: any, label?: any) : Promise<any> {
...
async function verifyEvidenceRefs(refs?: any, label?: any) : Promise<any> {
...
function verifyArtifactRefs(refs?: any, label?: any) : any {
...
async function verifyRegistry() : Promise<any> {
  ...
  assert.deepEqual(
    ...
    sorted(artifactLifecycleDef.states.map((state?: any) : any => state.id)),
  ...
  assert.deepEqual(
    ...
    sorted(transitionLifecycleDef.states.map((state?: any) : any => state.id)),
  ...
  const platformVersionBaseline: any = registry.authority.platformVersionBaseline;

  const artifactIds: any = new Set<any>();
  const artifactRefs: any = new Set<any>();
  const activeRefs: any = new Map<any, any>();
  const activeVersionEvidence: any = new Map<any, any>();
  ...
  for (const artifact of registry.artifacts) {
    ...
    const versions: any = new Set<any>();
    const activeVersions: any[] = [];
    for (const versionRecord of artifact.versions) {
      ...
      if (versionRecord.lifecycle === "active") {
        ...
        activeVersionEvidence.set(
          ...
          new Set<any>(versionRecord.evidenceRefs.map((evidenceRef?: any) : any => normalizePath(evidenceRef.uri)))
        );
  ...
  const compatibilityIds: any = new Set<any>();
  ...
  const transitionIds: any = new Set<any>();
  ...
  const assemblyIds: any = new Set<any>();
  ...
  const scannedOccurrences: any = collectGovernedVersionOccurrences({
    repoRoot,
    scanRoots: VERSION_SCAN_ROOTS,
    excludedRelativePaths: [registryRelativePath]
  });
  const unregisteredVersionFindings: any[] = [];
  const malformedVersionFindings: any[] = [];
  ...
  const unbackedActiveVersionFindings: any[] = [];
  for (const [version, evidenceUris] of activeVersionEvidence) {
    const occurrences: any = scannedOccurrences.get(version) || [];
    const sourcePaths: any = new Set<any>(occurrences.map((occurrence?: any) : any => occurrence.relativePath));
    if (sourcePaths.size === 0 || ![...evidenceUris].some((uri?: any) : any => sourcePaths.has(uri))) {
...
function artifactRefsHasVersion(artifactRefs?: any, version?: any) : any {
