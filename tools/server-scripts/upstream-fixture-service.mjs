...
function parseArgs(argv = []) {
  const options = { mode: "http", host: "127.0.0.1", port: 0, tokenEnv: UPSTREAM_FIXTURE_TOKEN_ENV };
...
const token = String(process.env[options.tokenEnv] || "").trim();

if (options.mode === "mcp-stdio") {
  runUpstreamFixtureMcpStdio({ token });
} else {
  const service = createUpstreamFixtureHttpService({ token });
  const started = await service.start({ host: options.host, port: options.port });
