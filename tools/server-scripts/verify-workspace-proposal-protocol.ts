...
async function rpc(server?: any, auth?: any, method?: any, params: Record<string, any> = {}, id: any = method) : Promise<any> {
  const payload: any = await fetchJson(`${server.url}/api/rpc`, {
    method: "POST",
    headers: {
...
for (const operationId of ["workspace.proposal.create", "workspace.proposal.apply"]) {
  assert.ok(catalog.tools.some((tool?: any) : any => tool.operationId === operationId), `${operationId} must be exposed as a tool`);
}

const userDataPath: any = await fs.mkdtemp(path.join(os.tmpdir(), "meshrix-workspace-proposal-protocol-"));
...
try {
  ...
  const createdProposal: any = await rpc(server, auth, "workspace.proposal.create", {
    workspaceId,
    runId: "verify-run",
  ...
  const appliedProposal: any = await rpc(server, auth, "workspace.proposal.apply", {
    workspaceId,
  ...
  const workspace: any = await rpc(server, auth, "agent_workspaces.get", { workspaceId });
  assert.ok(workspace.decisions.some((item?: any) : any => item.decisionId === appliedProposal.decision.decisionId));
