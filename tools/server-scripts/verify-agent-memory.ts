...
import {
  ...
  createAgentMemory
} from "../../packages/agents/src/agent-memory/index.ts";
import { createContextRuntime } from "../../packages/server-runtime/src/state/interface/index.ts";

const userDataPath: any = await fs.mkdtemp(path.join(os.tmpdir(), "meshrix-agent-memory-"));
const CONTEXT_PROFILE_ID: any = "memory-verifier-profile";
const CONTEXT_PROFILE: Readonly<Record<string, any>> = Object.freeze({
...
function message(index?: any) : any {
  return {
    id: `msg-${index}`,
...
const agentMemory: any = createAgentMemory({ userDataPath });
...
const appended: any = await agentMemory.appendSessionMemory({
...
const exact: any = await agentMemory.latestSessionMemory({
  sessionId: "direct-session",
  profileId: CONTEXT_PROFILE_ID,
  sourceHash: "hash-a"
});
...
const missingHash: any = await agentMemory.latestSessionMemory({
  sessionId: "direct-session",
  profileId: CONTEXT_PROFILE_ID,
  sourceHash: "hash-b"
});
...
const cleared: any = await agentMemory.clearSessionMemory({
  sessionId: "direct-session",
  profileId: CONTEXT_PROFILE_ID,
  reason: "verify"
});
assert.equal(cleared.ok, true);
const afterClear: any = await agentMemory.latestSessionMemory({
  sessionId: "direct-session",
  profileId: CONTEXT_PROFILE_ID
});
assert.equal(afterClear, null);

const contextRuntime: any = createContextRuntime({
  userDataPath,
  agentMemory
});
await contextRuntime.saveProfiles({ profiles: [CONTEXT_PROFILE] });
const messages: any = Array.from({ length: 24 }, (_?: any, index?: any) : any => message(index + 1));
const firstRun: any = await contextRuntime.runCompaction({
...
const runtimeRecords: any = await agentMemory.listSessionMemory({ sessionId: "runtime-session" });
...
const reusedRun: any = await contextRuntime.runCompaction({
  contextProfileId: CONTEXT_PROFILE_ID,
  sessionId: "runtime-session",
  messages,
  taskBrief: "Verify ContextRuntime persists through AgentMemory.",
