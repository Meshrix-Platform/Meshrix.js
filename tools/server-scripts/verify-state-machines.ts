...
const DEFAULT_DEFINITIONS_DIR: any = path.join(ROOT, "packages/foundation/src/workflow/state-machine/definitions");
const DEFAULT_INTEGRITY_REGISTRY_PATH: any = path.join(ROOT, "tools/registry/state-machines/state-machine-integrity.registry.json");
const REPORT_SCHEMA_VERSION: any = "v0.0.1:state-machine:verification-report-1";
const REPORT_VERIFIER: any = "tools/server-scripts/verify-state-machines.ts";
...
function loadIntegrityRegistry(registryPath?: any) : any {
  ...
  const machineIds: any = registry.machines.map((entry?: any) : any => String(entry?.machineId || "").trim());
  if (machineIds.some((machineId?: any) : any => !machineId) || new Set<any>(machineIds).size !== machineIds.length) {
    throw new Error("State machine integrity registry is invalid: machineId values must be non-empty and unique");
  }
  return {
    registry,
    index: new Map<any, any>(registry.machines.map((entry?: any) : any => [entry.machineId, entry]))
  };
}
...
function appendIntegrityChecks(result?: any, definition?: any, filePath?: any, integrityIndex?: any) : any {
  ...
  const relativePath: any = toPosixRelative(filePath);
  ...
  for (const [key, actual] of (Object.entries(counts) as [string, any][])) {
...
export function runVerifier(definitionsDir?: any, reportsDir?: any, {
  ...
  const files: any = listDefinitionFiles(definitionsDir);
  const integrity: any = loadIntegrityRegistry(integrityRegistryPath);
  const seenMachineIds: any = new Set<any>();

  for (const filePath of files) {
    const relativePath: any = toPosixRelative(filePath);
    try {
      const def: any = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
      const res: any = verifyMachineDefinition(def, { throwOnError: false, relativePath });
  ...
  const failedCount: any = report.machines.filter((machine?: any) : any => machine.ok !== true).length;
  ...
  let mdContent: any = `# State Machine Verification Report\n\nChecked At: ${report.checkedAt}\nStatus: ${report.ok ? '✅ PASS' : '❌ FAIL'}\n\n## Machines\n\n`;
  for (const machine of report.machines) {
    ...
    if (!machine.ok) {
      mdContent += `- **Errors**:\n`;
      if (machine.checks) {
        for (const check of machine.checks.filter((c?: any) : any => c.status !== 'passed')) {
...
const isMain: any = process.argv[1] === fileURLToPath(import.meta.url);
if (isMain) {
  const defs: any = DEFAULT_DEFINITIONS_DIR;
  const reps: any = path.join(process.cwd(), "build/reports/state-machines");
  const r: any = runVerifier(defs, reps);
  if (!r.ok) {
