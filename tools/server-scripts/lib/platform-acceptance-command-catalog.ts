...
const PLATFORM_ACCEPTANCE_EVIDENCE_COMMANDS: readonly any[] = Object.freeze([
  ...
  command("mcp-authorization-request-filters", "MCP authorization request filters", "downstream-gateway", nodeCommand(["tools/server-scripts/verify-mcp-authorization-request-filters.ts"]), "build/reports/mcp-authorization-request-filters.json", ["downstream-gateway", "authorization"], { timeoutMs: 2 * 60 * 1000 }),
  ...
  command("job-work-queue-capacity", "Job work queue capacity verifier", "platform-capability", nodeCommand(["tools/server-scripts/verify-job-work-queue-capacity.ts"]), "build/reports/job-work-queue-capacity.json", ["platform-capability", "jobs", "capacity"], { resourceLocks: ["work-queue"] }),
