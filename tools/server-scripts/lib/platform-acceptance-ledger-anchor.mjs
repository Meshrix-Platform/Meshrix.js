...
export async function anchorAcceptanceEvidence({
  ...
  const reportDigests = [];
  const logicalPaths = [...new Set(reportPaths.map((reportPath) =>
    String(reportPath || "").replace(/\\/gu, "/")
  ))].sort();
  if (logicalPaths.length !== reportPaths.length || logicalPaths.some((reportPath) => (
  ...
  for (const reportPath of logicalPaths) {
    try {
      const absolutePath = path.resolve(resolvedRoot, reportPath);
...
export async function verifyAcceptanceEvidenceAnchor({
  ...
  const eventId = String(ledgerEventId || "").trim();
  ...
  if (resolvedEnvelopeId && typeof proofSubstrate.exportProofBundle === "function") {
    try {
      ...
      const verification = typeof proofSubstrate.verifyReceipt === "function"
  ...
  const releaseId = String(workspaceId || "").replace(/^release:/u, "");
  const commitment = await proofSubstrate.verifyReceiptCommitment({
    ledgerEventId: eventId,
    commitment: {
      kind: "acceptance-evidence",
      releaseId,
      reportDigests: expectedReportDigests.map(({ path: reportPath, schemaVersion, contentHash }) => ({
