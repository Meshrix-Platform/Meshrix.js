...
export async function runNativeOrbBootstrapStage(context?: any) : Promise<any> {
  ...
  const ready: any = bootstrapOrbText(context.parsed.machine, ["sh", "-lc",
    ...
    "meshrix-dependencies-ready", marker, state, context.layout.currentDirectory], { allowFailure: true, timeout: 15_000 }) === "ready";
  if (!ready) {
    ...
    timeout: 1_200_000, code: "native_orb_bootstrap_dependencies_failed" });
    runOrb({ machine: context.parsed.machine, args: ["sh", "-lc", "umask 077; printf '%s\\n' \"$2\" > \"$1\"", "meshrix-dependency-marker", marker, state], timeout: 30_000 });
  }
