...
export async function runNativeOrbBootstrapStage(context?: any) : Promise<any> {
  ...
  const ready: any = bootstrapOrbText(context.parsed.machine, ["sh", "-lc",
    ...
    "meshrix-build-ready", marker, context.toolchainState, context.layout.currentDirectory], { allowFailure: true }) === "ready";
  if (!ready) {
    ...
    runOrb({ machine: context.parsed.machine, args: ["sh", "-lc", "cd \"$1\" && exec \"$2\" \"$3\" run build", "meshrix-bootstrap-build", context.layout.currentDirectory, context.nodeExecutable, context.npmCli], code: "native_orb_bootstrap_build_failed" });
    runOrb({ machine: context.parsed.machine, args: ["sh", "-lc", "umask 077; printf '%s\\n' \"$2\" > \"$1\"", "meshrix-build-marker", marker, context.toolchainState] });
  }
  const nativeReady: any = runOrb({ machine: context.parsed.machine, args: ["sh", "-lc", "cd \"$1\" && exec \"$2\" -e 'const D=require(\"better-sqlite3\");const db=new D(\":memory:\");db.close()'", "meshrix-native-proof", context.layout.currentDirectory, context.nodeExecutable], allowFailure: true }).status === 0;
