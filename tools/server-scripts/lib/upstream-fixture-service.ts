...
export const UPSTREAM_FIXTURE_SERVICE_NAME: any = "meshrix-upstream-fixture";
export const UPSTREAM_FIXTURE_SERVICE_VERSION: any = "0.0.1";
export const UPSTREAM_FIXTURE_TOKEN_ENV: any = "MESHRIX_UPSTREAM_FIXTURE_TOKEN";
export const UPSTREAM_FIXTURE_CLI_PATH: any = "tools/server-scripts/upstream-fixture-service.ts";
export const UPSTREAM_FIXTURE_MCP_PROTOCOL_VERSION: any = "2025-06-18";

export const UPSTREAM_FIXTURE_RECORDS: readonly any[] = Object.freeze([
...
export const UPSTREAM_FIXTURE_IDENTITY: Readonly<Record<string, any>> = Object.freeze({
  principal: "fixture-operator",
  accountId: "fixture-account-001"
});

export function fixtureTokenProof(token: any = "") : any {
  const raw: any = String(token || "").trim();
  return raw ? `sha256:${crypto.createHash("sha256").update(raw).digest("hex").slice(0, 16)}` : "";
}

function jsonSchema(properties: Record<string, any> = {}, required: any = []) : any {
  return {
    type: "object",
    properties,
...
export function upstreamFixtureMcpTools({ includeCancellationTools = false }: Record<string, any> = {}) : any {
  const tools: any[] = [
    {
      name: "records.search",
  ...
  return includeCancellationTools
    ? tools
    : tools.filter((tool?: any) : any => !["state.increment.delayed", "state.peer.wait"].includes(tool.name));
}

export function createUpstreamFixtureState({ token = "", supportsDelayedCancellation = false }: Record<string, any> = {}) : any {
  const state: Record<string, any> = {
    token: String(token || "").trim(),
    records: UPSTREAM_FIXTURE_RECORDS.map((record?: any) : any => ({ ...record })),
    notes: new Map<any, any>(),
    counter: 0,
    callCount: 0,
    purged: false,
  ...
  function requireRecord(recordId: any = "") : any {
    const record: any = state.records.find((item?: any) : any => item.recordId === String(recordId || ""));
    if (!record) {
      const error: Error & Record<string, any> = new Error(`Fixture record not found: ${String(recordId || "")}`);
  ...
  function authProof(presentedToken: any = "") : any {
    const presented: any = String(presentedToken || "").trim();
    return {
  ...
  return {
    ...
    authProof,
    reset() : any {
      state.records = UPSTREAM_FIXTURE_RECORDS.map((record?: any) : any => ({ ...record }));
      state.notes.clear();
      state.counter = 0;
      state.purged = false;
      return { ok: true, recordCount: state.records.length };
    },
    searchRecords({ query = "", perPage = 10 }: Record<string, any> = {}) : any {
      state.callCount += 1;
      const needle: any = String(query || "").toLowerCase();
      const limit: any = Math.max(1, Math.min(50, Number(perPage) || 10));
      const items: any = state.records
        .filter((record?: any) : any => !needle ||
    ...
    getRecord({ recordId = "" }: Record<string, any> = {}) : any {
      state.callCount += 1;
      const record: any = requireRecord(recordId);
      return { ok: true, record, note: state.notes.get(record.recordId) || "" };
    },
    sessionIdentity({ presentedToken = "" }: Record<string, any> = {}) : any {
      state.callCount += 1;
      return {
        ok: true,
    ...
    incrementCounter({ amount = 1 }: Record<string, any> = {}) : any {
      state.callCount += 1;
      const step: any = Number.isFinite(Number(amount)) ? Math.trunc(Number(amount)) : 1;
      state.counter += step === 0 ? 1 : step;
      return { ok: true, counter: state.counter };
    },
    beginDelayedIncrement({ amount = 1, delayMs = 500 }: Record<string, any> = {}) : any {
      state.callCount += 1;
      state.delayedOperations.incrementStarted += 1;
      state.delayedOperations.activeIncrement += 1;
      const step: any = Number.isFinite(Number(amount)) ? Math.trunc(Number(amount)) : 1;
      const boundedDelayMs: any = Math.max(25, Math.min(Number(delayMs) || 500, 10_000));
      let active: any = true;
      return {
    ...
    beginPeerWait({ delayMs = 1_000 }: Record<string, any> = {}) : any {
      state.callCount += 1;
      state.delayedOperations.peerStarted += 1;
      state.delayedOperations.activePeer += 1;
      const boundedDelayMs: any = Math.max(25, Math.min(Number(delayMs) || 1_000, 10_000));
      let active: any = true;
      return {
        delayMs: boundedDelayMs,
        complete() : any {
    ...
    purgeRecords() : any {
      state.callCount += 1;
      const purgedCount: any = state.records.length;
      state.records = [];
      state.notes.clear();
      state.purged = true;
...
export function callUpstreamFixtureTool(fixture?: any, name: any = "", args: Record<string, any> = {}) : any {
  const toolArguments: any = args && typeof args === "object" && !Array.isArray(args) ? args : {};
  switch (String(name || "")) {
    case "records.search":
    ...
    default: {
      const error: Error & Record<string, any> = new Error(`Fixture tool not found: ${String(name || "")}`);
...
function mcpToolCallResult(structured?: any) : any {
  return {
...
export function handleUpstreamFixtureMcpMessage(fixture?: any, message: Record<string, any> = {}) : any {
  if (!message || typeof message !== "object" || !message.method) return null;
  const { id, method, params = {} } = message;
  const isNotification: any = id === undefined || id === null;
  ...
  if (method === "tools/call") {
    try {
      const structured: any = callUpstreamFixtureTool(fixture, params.name, params.arguments || {});
...
function fixtureRequestKey(requestId?: any) : any {
  return `${typeof requestId}:${String(requestId)}`;
}

function scheduledFixtureOperation(fixture?: any, name: any = "", args: Record<string, any> = {}) : any {
...
function scheduleUpstreamFixtureMcpCall({ fixture, message, output, pending }: Record<string, any>) : any {
  if (message?.method !== "tools/call") return false;
  const operation: any = scheduledFixtureOperation(
    fixture,
    message.params?.name,
    message.params?.arguments || {}
  );
  if (!operation) return false;
  const key: any = fixtureRequestKey(message.id);
  const entry: Record<string, any> = { operation, timer: null };
  entry.timer = setTimeout(() : any => {
    if (pending.get(key) !== entry) return;
    pending.delete(key);
    const structured: any = operation.complete();
...
export function runUpstreamFixtureMcpStdio({
  ...
  const fixture: any = createUpstreamFixtureState({ token, supportsDelayedCancellation: true });
  const pending: any = new Map<any, any>();
  let buffer: any = "";
  input.setEncoding("utf8");
  input.on("data", (chunk?: any) : any => {
    buffer += chunk;
    ...
    for (const line of lines) {
      ...
      if (message?.method === "notifications/cancelled") {
        const key: any = fixtureRequestKey(message.params?.requestId);
        const entry: any = pending.get(key);
      ...
      const response: any = handleUpstreamFixtureMcpMessage(fixture, message);
      if (response) {
...
export function upstreamFixtureOpenApiDocument() : any {
  const record: any = jsonSchema({
    recordId: { type: "string" },
    name: { type: "string" },
...
function bearerToken(request?: any) : any {
  const header: any = String(request.headers.authorization || "");
  const match: any = header.match(/^Bearer\s+(\S+)$/i);
  return match ? match[1] : "";
}

export function createUpstreamFixtureHttpService({ token = "" }: Record<string, any> = {}) : any {
  const fixture: any = createUpstreamFixtureState({ token });
  const openPaths: any = new Set<any>(["/health", "/openapi.json"]);
  ...
  async function handleRequest(request?: any, response?: any) : Promise<any> {
    ...
    const presentedToken: any = bearerToken(request);
    ...
    const route: any = `${request.method} ${url.pathname}`;
    try {
      switch (route) {
        case "GET /health":
        ...
        case "POST /api/fault":
          ...
          return;
        case "POST /mcp": {
          const reply: any = handleUpstreamFixtureMcpMessage(fixture, body);
          if (!reply) {
      ...
      const status: any = error?.code === "fixture_record_not_found" ? 404 : 500;
  ...
  return {
    fixture,
    start({ host = "127.0.0.1", port = 0 }: Record<string, any> = {}) : any {
      return new Promise((resolve?: any, reject?: any) : any => {
        const server: any = http.createServer((request?: any, response?: any) : any => {
          handleRequest(request, response).catch(() : any => {
            try {
        ...
        server.listen(port, host, () : any => {
          ...
          resolve({
            ...
            close: () : any => new Promise((done?: any) : any => server.close(() : any => done()))
          });
        });
      });
    }
