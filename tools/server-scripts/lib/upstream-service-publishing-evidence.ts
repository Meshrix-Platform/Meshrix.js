import { assertNoSensitiveReportLeak, reportPayloadDigest } from "./sensitive-report-scan.ts";

export const UPSTREAM_SERVICE_PUBLISHING_REPORT_PATH: any = "build/reports/upstream-service-publishing.json";
export const UPSTREAM_SERVICE_PUBLISHING_REPORT_SCHEMA_VERSION: any =
  "v0.0.1:upstream-service-publishing:server-report-3";
export const UPSTREAM_SERVICE_PUBLISHING_OBSERVATION_SCHEMA_VERSION: any =
  "v0.0.1:upstream-service-publishing:observations-2";
export const UPSTREAM_SERVICE_PUBLISHING_VERIFIER: any =
  "tools/server-scripts/verify-upstream-service-publishing.ts";
export const UPSTREAM_SERVICE_PUBLISHING_COMMAND_ID: any = "verify:upstream-service-publishing";
export const UPSTREAM_SERVICE_PUBLISHING_REDUCER: any =
  "tools/server-scripts/lib/upstream-service-publishing-evidence.ts#createUpstreamServicePublishingReadiness";

export const UPSTREAM_SERVICE_PUBLISHING_REQUIREMENTS: any = Object.freeze(
  Array.from({ length: 13 }, (_?: any, index?: any) : any => `REQ-USP-${String(index + 1).padStart(3, "0")}`)
);

export const UPSTREAM_SERVICE_PUBLISHING_ASSERTIONS: any = Object.freeze(
...
export const UPSTREAM_SERVICE_PUBLISHING_BOUNDARIES: readonly any[] = Object.freeze([
...
export const UPSTREAM_SERVICE_PUBLISHING_SCENARIOS: readonly any[] = Object.freeze([
  "create", "replace", "disable", "remove", "republish", "identical-replay",
  "conflict", "stale", "invalid-command", "restart", "reconnect"
]);

export const UPSTREAM_SERVICE_PUBLISHING_COUNTERS: readonly any[] = Object.freeze([
...
const EDGE_OUTCOMES: Readonly<Record<string, any>> = Object.freeze({
...
const PROTOCOL_COHORTS: Readonly<Record<string, any>> = Object.freeze({
  "affected-ack": "acknowledged",
  "api-key-disconnect": "disconnected_fenced",
  "ack-timeout": "timed_out_fenced",
  "reconnect-fence": "disconnected_fenced"
});
const TOP_LEVEL_KEYS: readonly any[] = Object.freeze([
...
const OPTIONAL_TOP_LEVEL_KEYS: readonly any[] = Object.freeze(["releaseEvidenceProvenance"]);
const MAX_COUNTER: any = 1_000_000;
const MAX_REPORT_BYTES: any = 512 * 1024;
const MAX_DURATION_MS: any = 300_000;
...
function record(value?: any, code: any = "upstream_service_publishing_report_invalid") : any {
  if (!value || typeof value !== "object" || Array.isArray(value) ||
      ![Object.prototype, null].includes(Object.getPrototypeOf(value))) fail(code, "Expected an object.");
  return value;
}

function exactKeys(value?: any, keys?: any, field?: any) : any {
  record(value);
  const actual: any = Object.keys(value).sort();
  const expected: any = [...keys].sort();
  if (actual.length !== expected.length || actual.some((key?: any, index?: any) : any => key !== expected[index])) {
    fail("upstream_service_publishing_report_unknown_field", `${field} fields do not match the report contract.`);
  }
}

function exactReportKeys(report?: any) : any {
  record(report);
  const actual: any = Object.keys(report).sort();
  const allowed: any = new Set<any>([...TOP_LEVEL_KEYS, ...OPTIONAL_TOP_LEVEL_KEYS]);
  if (actual.some((key?: any) : any => !allowed.has(key)) ||
      TOP_LEVEL_KEYS.some((key?: any) : any => !Object.hasOwn(report, key))) {
    fail("upstream_service_publishing_report_unknown_field", "report fields do not match the report contract.");
  }
}

function exactStringSet(values?: any, expected?: any, field?: any) : any {
  if (!Array.isArray(values) || values.length !== expected.length ||
      values.some((value?: any, index?: any) : any => value !== expected[index])) {
    fail("upstream_service_publishing_report_set_mismatch", `${field} does not match the canonical set.`);
  }
}

function integer(value?: any, field?: any, maximum: any = MAX_COUNTER) : any {
...
function positiveInteger(value?: any, field?: any, maximum: any = MAX_COUNTER) : any {
  integer(value, field, maximum);
  if (value === 0) fail("upstream_service_publishing_observation_count_invalid", `${field} must be positive.`);
  return value;
}

function validateCounters(value?: any, field?: any) : any {
...
function sumScenarioCounters(scenarios?: any) : any {
  const total: any = Object.fromEntries(UPSTREAM_SERVICE_PUBLISHING_COUNTERS.map((key?: any) : any => [key, 0]));
...
const EVENT_KEYS: readonly any[] = Object.freeze(["id", "status", "scenario", "fromRevision", "toRevision", "count", "fact"]);
const EVENT_META_KEYS: readonly any[] = Object.freeze(["id", "status", "scenario", "fromRevision", "toRevision", "count"]);
const EVENT_CATALOG: any = Object.freeze([
  ...
  ["audience.scoped-api-key.projected", "projected", "replace", 2, 2],
  ["protocol.invalidation.received", "received", "replace", 1, 2],
  ["protocol.unaffected-stream.quiet", "preserved", "replace", 1, 2],
  ["catalog.refresh.pulled", "pulled", "replace", 1, 2],
  ["protocol.malformed-ack.rejected", "rejected", "replace", 1, 2],
  ["protocol.stale-ack.rejected", "rejected", "replace", 1, 2],
  ["protocol.exact-ack.accepted", "acknowledged", "replace", 1, 2],
  ["protocol.duplicate-ack.rejected", "rejected", "replace", 1, 2],
  ["forward.allowed.accepted", "accepted", "replace", 2, 2],
  ["forward.response-schema.validated", "validated", "replace", 2, 2],
  ["forward.response-projection-redaction.observed", "observed", "replace", 2, 2],
  ["forward.fixture-call.observed", "observed", "replace", 2, 2],
  ["sensitive-reference.materialized", "observed", "replace", 2, 2],
  ["forward.approval.pending", "pending", "replace", 2, 2],
  ["forward.approval.resolved", "approved", "replace", 2, 2],
  ["forward.approval.fixture-call.observed", "observed", "replace", 2, 2],
  ["forward.approval.replay.rejected", "rejected", "replace", 2, 2],
  ["forward.response-byte-bound.rejected", "rejected", "replace", 2, 2],
  ["forward.timeout.rejected", "rejected", "replace", 2, 2],
  ["forward.cancellation.observed", "cancelled", "replace", 2, 2],
  ["forward.traffic.admitted", "admitted", "replace", 2, 2],
  ["forward.traffic.rejected", "rejected", "replace", 2, 2],
  ["forward.denied.rejected", "denied", "replace", 2, 2],
  ...
  ["protocol.api-key.revoked", "revoked", "replace", 2, 2],
  ...
  ["protocol.timeout-audience.changed", "updated", "replace", 3, 3],
...
].map(([id, status, scenario, fromRevision, toRevision, count = 1]: any[]) : any =>
  Object.freeze({ id, status, scenario, fromRevision, toRevision, count })));

const EMPTY_COUNTER_DELTA: any = Object.freeze(Object.fromEntries(
  UPSTREAM_SERVICE_PUBLISHING_COUNTERS.map((counter?: any) : any => [counter, 0])
));

const FACT_POLICY: Readonly<Record<string, any>> = Object.freeze({
...
function counterDelta(overrides: Record<string, any> = {}) : any {
  return Object.freeze({ ...EMPTY_COUNTER_DELTA, ...overrides });
}

function syntheticFact(event?: any) : any {
  const [type = "assertion", deltas = {}] = FACT_POLICY[event.id] || [];
  const publication: any = ["manifest_commit", "runtime_snapshot", "runtime_restart", "catalog_audience_commit", "server_terminal"].includes(type);
  const terminal: any = ["runtime_restart", "server_terminal"].includes(type);
...
export const UPSTREAM_SERVICE_PUBLISHING_OBSERVATION_EVENTS: any = Object.freeze(
  EVENT_CATALOG.map((event?: any) : any => Object.freeze({ ...event, fact: syntheticFact(event) }))
);

const BOUNDARY_EVENT: Readonly<Record<string, any>> = Object.freeze({
  ...
  "operation-permission": "audience.scoped-api-key.projected",
...
const REVISION_EVENT: Readonly<Record<string, any>> = Object.freeze({
...
const PROTOCOL_EVENT: Readonly<Record<string, any>> = Object.freeze({
  "affected-ack": ["protocol.exact-ack.accepted", "acknowledged"],
  "api-key-disconnect": ["protocol.revoked-stream.closed", "disconnected_fenced"],
...
const REQUIREMENT_EVENTS: Readonly<Record<string, any>> = Object.freeze({
  ...
  "REQ-USP-006": ["catalog.initial.pulled", "audience.scoped-api-key.projected"],
  "REQ-USP-007": [
...
export function reduceUpstreamServicePublishingObservations(observations?: any) : any {
  ...
  const directFacts: any = new Set<any>();
  observations.forEach((observation?: any, index?: any) : any => {
    ...
    const expected: any = EVENT_CATALOG[index];
    ...
    const canonicalDelta: any = counterDelta(expectedDelta);
    ...
    const publicationFact: any = ["manifest_commit", "runtime_snapshot", "runtime_restart", "catalog_audience_commit", "server_terminal"].includes(expectedType);
    if (publicationFact) {
      ...
      const directKey: any = `${expectedType}:${observation.fact.sourceRevision}`;
    ...
    const terminalFact: any = ["runtime_restart", "server_terminal"].includes(expectedType);
    if (terminalFact) {
  ...
  const byId: any = new Map<any, any>(observations.map((observation?: any) : any => [observation.id, observation]));
  const publicationTriples: any[] = [
  ...
  for (const eventIds of publicationTriples) {
    const facts: any = eventIds.map((eventId?: any) : any => byId.get(eventId)?.fact);
    if (facts.some((fact?: any) : any => !fact) || facts.some((fact?: any) : any =>
  ...
  const restartFact: any = byId.get("restart.snapshot-restored")?.fact;
  const finalPublicationFact: any = byId.get("catalog.republish.admitted")?.fact;
  const serverTerminalFact: any = byId.get("publication.republish.server-published")?.fact;
  ...
  const assertions: any = UPSTREAM_SERVICE_PUBLISHING_ASSERTIONS.map((assertion?: any) : any => {
    const requiredEvents: any = REQUIREMENT_EVENTS[assertion.requirement];
    if (!Array.isArray(requiredEvents) || requiredEvents.some((eventId?: any) : any => !byId.has(eventId))) {
  ...
  const productionBoundaries: any = UPSTREAM_SERVICE_PUBLISHING_BOUNDARIES.map((boundary?: any) : any => {
  ...
  const revisionEdges: any = UPSTREAM_SERVICE_PUBLISHING_SCENARIOS.map((scenario?: any) : any => {
    const observation: any = byId.get(REVISION_EVENT[scenario]);
    return Object.freeze({
      scenario,
  ...
  const protocolCohorts: any = (Object.entries(PROTOCOL_EVENT) as [string, any][]).map(([id, [eventId, outcome]]: any[]) : any => Object.freeze({
    id,
    outcome,
    count: byId.get(eventId).count
  }));
  const scenarioCounters: any = Object.fromEntries(UPSTREAM_SERVICE_PUBLISHING_SCENARIOS.map((id?: any) : any => [id, Object.fromEntries(
    UPSTREAM_SERVICE_PUBLISHING_COUNTERS.map((counter?: any) : any => [counter, 0])
  )]));
  ...
  const scenarios: any = UPSTREAM_SERVICE_PUBLISHING_SCENARIOS.map((id?: any) : any => Object.freeze({
    id,
    counters: Object.freeze(scenarioCounters[id])
  }));
  const counters: any = Object.freeze(sumScenarioCounters(scenarios));
  return Object.freeze({
...
function sameJson(left?: any, right?: any) : any {
  return JSON.stringify(left) === JSON.stringify(right);
}

export function validateUpstreamServicePublishingReport(report?: any, {
  ...
  const derived: any = reduceUpstreamServicePublishingObservations(report.observations);
  for (const field of [
  ...
  const assertionIds: any = new Set<any>();
  report.assertions.forEach((assertion?: any, index?: any) : any => {
    exactKeys(assertion, ["id", "requirement", "phase", "expectedOutcome", "observedOutcome", "passed", "durationMs", "reasonCode"], `assertions[${index}]`);
    const expected: any = UPSTREAM_SERVICE_PUBLISHING_ASSERTIONS[index];
  ...
  exactStringSet(report.productionBoundaries.map((entry?: any) : any => entry.id), UPSTREAM_SERVICE_PUBLISHING_BOUNDARIES, "productionBoundaries");
  report.productionBoundaries.forEach((entry?: any, index?: any) : any => {
  ...
  exactStringSet(report.revisionEdges.map((edge?: any) : any => edge.scenario), UPSTREAM_SERVICE_PUBLISHING_SCENARIOS, "revisionEdges");
  report.revisionEdges.forEach((edge?: any, index?: any) : any => {
    ...
    const expectedOutcome: any = EDGE_OUTCOMES[edge.scenario];
  ...
  exactStringSet(report.protocolCohorts.map((cohort?: any) : any => cohort.id), Object.keys(PROTOCOL_COHORTS), "protocolCohorts");
  report.protocolCohorts.forEach((cohort?: any, index?: any) : any => {
  ...
  exactStringSet(report.scenarios.map((scenario?: any) : any => scenario.id), UPSTREAM_SERVICE_PUBLISHING_SCENARIOS, "scenarios");
  report.scenarios.forEach((scenario?: any, index?: any) : any => {
  ...
  const recomputedCounters: any = sumScenarioCounters(report.scenarios);
  if (UPSTREAM_SERVICE_PUBLISHING_COUNTERS.some((key?: any) : any => report.counters[key] !== recomputedCounters[key])) {
  ...
  const expectedSummary: Record<string, any> = {
    assertionCount: report.assertions.length,
    passedCount: report.assertions.filter((assertion?: any) : any => assertion.passed).length,
    failedCount: report.assertions.filter((assertion?: any) : any => !assertion.passed).length,
    boundaryCount: report.productionBoundaries.length,
    revisionEdgeCount: report.revisionEdges.length,
    verificationPassed: true,
    reportLeakScan: true
  };
  if (Object.keys(expectedSummary).some((key?: any) : any => report.summary[key] !== expectedSummary[key])) {
  ...
  const actualBytes: any = Buffer.byteLength(JSON.stringify(report), "utf8");
...
export function reduceUpstreamServicePublishingReport(report?: any, options: Record<string, any> = {}) : any {
  return validateUpstreamServicePublishingReport(report, options);
}

export function createUpstreamServicePublishingReadiness(report?: any, options: Record<string, any> = {}) : any {
  try {
...
export function finalizeUpstreamServicePublishingReport(report?: any) : any {
  const inputKeys: any[] = [
    "schemaVersion", "verifier", "generatedAt", "producer", "commandId", "sourceRevision",
  ...
  for (let attempt: any = 0; attempt < 4; attempt += 1) {
    const size: any = Buffer.byteLength(JSON.stringify(next), "utf8");
