const rel = (number) => `REQ-REL-${String(number).padStart(3, "0")}`;
const usp = (number) => `REQ-USP-${String(number).padStart(3, "0")}`;

export const PLATFORM_ACCEPTANCE_REQUIREMENTS = Object.freeze([
  ...Array.from({ length: 30 }, (_, index) => rel(index + 1)),
  ...Array.from({ length: 13 }, (_, index) => usp(index + 1))
]);
...
const mapping = {
  ...
  [rel(22)]: { commandIds: ["maintenance-agent", "work-queue-conformance"] },
  ...
  [rel(25)]: { commandIds: ["job-work-queue-capacity", "mcp-gateway-load", "gateway-platform-profile", "production-readiness-gates"] },
  ...
  [rel(28)]: { commandIds: ["agent-gateway", "model-routing", "surface-convergence"] },
...
export const PLATFORM_ACCEPTANCE_REQUIREMENT_EVIDENCE = Object.freeze(
  Object.fromEntries(PLATFORM_ACCEPTANCE_REQUIREMENTS.map((requirement) => [
    requirement,
    Object.freeze({
...
export function validatePlatformAcceptanceRequirementEvidence({ commands = [] } = {}) {
  const commandById = new Map(commands.map((command) => [String(command?.id || ""), command]));
  const reasons = [];
  const labels = Object.keys(PLATFORM_ACCEPTANCE_REQUIREMENT_EVIDENCE);
...
export function reducePlatformAcceptanceRequirementEvidence({
  ...
  const nodes = PLATFORM_ACCEPTANCE_REQUIREMENTS.map((requirement) => {
    const binding = PLATFORM_ACCEPTANCE_REQUIREMENT_EVIDENCE[requirement];
    const reasons = [];
    const reportPaths = [];
    for (const commandId of binding.commandIds) {
      const command = commandById.get(commandId);
      const status = resultById.get(commandId)?.status;
      if (status === "blocked") {
    ...
    return Object.freeze({
      ...
      reportPaths: Object.freeze([...new Set(reportPaths)].sort()),
  ...
  return Object.freeze({
    ...
    ready: nodes.every((node) => node.ready),
