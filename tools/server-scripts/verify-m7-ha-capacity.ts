...
const reportPath: any = M7_HA_DISCIPLINE.reports.capacity.path;
const childEntry: any = M7_HA_DISCIPLINE.reports.capacity.childEntry;
...
async function main() : Promise<any> {
  ...
  if (childResult.exitCode !== 0 || childResult.childReport?.accepted !== true) {
    const report: any = createBlockedReport({
      ...
      message: "HA capacity child verification did not pass in a fresh process.",
    });
  ...
  const report: any = createAcceptedReport({
    ...
    summary: {
      ...
      replayPassed: childResult.childReport.capacityReport?.verifierBudgetsPassed === true,
    },
  });
  await writeReportAtomically(repoRoot, reportPath, report);
  console.log("[m7-ha-capacity] accepted=true releaseReady=true");
}
...
if (invokedDirectly) {
  main().catch(async (error?: any) : Promise<any> => {
    ...
    console.error(`[m7-ha-capacity] failed reasonCode=${report.summary.reasonCode}`);
    process.exitCode = 1;
  });
}

export { main as verifyM7HaCapacity };
