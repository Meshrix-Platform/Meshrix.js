...
function isAllowedPublicProtocolMatch(check?: any, text?: any, match?: any) : any {
  if (check.id !== "non-governed-dotted-strategy-version") return false;
  const index: any = match.index || 0;
  if (text.slice(Math.max(0, index - 32), index).endsWith("meshrix.")) {
    return true;
  }
  const prefix: any = text.slice(Math.max(0, index - "application/".length), index);
  const suffix: any = text.slice(index + match[0].length, index + match[0].length + 16);
