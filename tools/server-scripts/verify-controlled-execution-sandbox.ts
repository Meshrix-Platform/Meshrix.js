...
if (invokedDirectly) {
  ...
  runControlledExecutionSandboxVerification({ verifyProductionBackend: true }).then((report?: any) : any => {
    ...
    if (report.productionBackendProbeFailures.length > 0) {
      console.log(`[controlled-execution-sandbox] productionBackendProbeFailures=${report.productionBackendProbeFailures.map((failure?: any) : any => `${failure.code}:${failure.failureStage}:${failure.failureReason}:${failure.exitCode}`).join(",")}`);
    }
