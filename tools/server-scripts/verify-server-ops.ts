...
export const SERVER_OPS_VERIFY_COMMANDS: readonly any[] = Object.freeze([
  nodeScript("backup-restore", "tools/server-scripts/verify-backup-restore.ts"),
  nodeScript("storage-production-restore-drill", "tools/server-scripts/verify-storage-production-restore-drill.ts"),
...
export async function main(argv: any = process.argv.slice(2)) : Promise<any> {
  return runRegressionCommandGroup({
    ...
    verifier: "tools/server-scripts/verify-server-ops.ts"
  });
}
