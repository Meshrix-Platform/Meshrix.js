...
const reportPath: any = M7_SCALE_DISCIPLINE.reports.fault.path;
const childEntry: any = M7_SCALE_DISCIPLINE.reports.fault.childEntry;
...
async function main() : Promise<any> {
  ...
  if (childResult.exitCode !== 0 || childResult.childReport?.accepted !== true) {
    const report: any = createBlockedReport({
      ...
      message: "Scale fault child verification did not pass in a fresh process.",
    });
  ...
  await writeReportAtomically(repoRoot, reportPath, report);
  console.log("[m7-scale-fault] accepted=true");
}
...
if (invokedDirectly) {
  main().catch(async (error?: any) : Promise<any> => {
    ...
    console.error(`[m7-scale-fault] failed reasonCode=${report.summary.reasonCode}`);
    process.exitCode = 1;
  });
}

export { main as verifyM7ScaleFault };
