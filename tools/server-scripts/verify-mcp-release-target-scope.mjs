...
const REPORT_PATH = "build/reports/mcp-release-target-scope.json";
const RELEASE_TARGETS = MCP_SUPPORTED_TARGETS;
...
const ADAPTER_BOUNDARY_DOCUMENT_FILES = Object.freeze([
  "CHANGELOG.md",
  ".github/RELEASE_TEMPLATE.md",
...
function assertReleaseTargets(value, label) {
  assert.deepEqual([...value].sort(), [...RELEASE_TARGETS].sort(), `${label} did not expose the release target set`);
}

function targetIds(items = []) {
  return items.map((item) => typeof item === "string" ? item : item?.target).filter(Boolean);
}
...
try {
  ...
  await test("public source, package metadata, and installer docs expose only the trusted catalog", async () => {
  ...
  await test("documentation declares the external client-adapter boundary", async () => {
  ...
  await test("real MCP discovery metadata exposes exactly the release targets", async () => {
    const discovery = await fetchJson("/api/mcp/discovery");
    assert.equal(discovery.status, 200);
    const connector = discovery.payload?.installer || {};
    ...
    assert.equal(RELEASE_LABELS.every((label) => JSON.stringify(connector).includes(label)), true);
    return {
  ...
  await test("real MCP initialize metadata exposes exactly the release targets", async () => {
    const initialize = await mcpInitialize();
    assert.equal(initialize.status, 200);
    const meta = initialize.payload?.result?._meta || {};
