...
import { runPluginVerifierWorkload } from "../../packages/foundation/src/module-system/plugin-verifier-runner.mjs";
import { resolveEnabledPluginSelection } from "./lib/runtime-plugin-selection.mjs";
import { collectPluginRuntimeOwnershipFailures } from "./lib/plugin-runtime-capability-bindings.mjs";
import { stagePluginArtifactVerificationFixture } from "./lib/plugin-artifact-verification-fixture.mjs";
import { applyFeatureSourcePlan, createPackagingPlan } from "./package-server-source.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, "../..");
const REPORT_PATH = path.join(ROOT, "build/reports/plugin-runtime.json");
const REPORT_SCHEMA_VERSION = "v0.0.1:plugin:runtime-verification-report-3";
const VERIFIER = "tools/server-scripts/verify-plugin-runtime.mjs";
...
function assertNoLeak(value, label = "plugin runtime report") {
...
async function writePlugin(repoRoot, record, source) {
  const directory = path.join(repoRoot, "plugins", record.id);
...
async function fixtureManager(repoRoot, userDataPath, enabledPlugins = [], extra = {}) {
...
async function runPackagedServer({
  ...
  const configPath = path.join(tempRoot, `${id}.runtime.json`);
  const dataPath = path.join(tempRoot, `${id}.data`);
  const readyPath = path.join(tempRoot, `${id}.ready.json`);
  ...
  const result = await new Promise((resolve) => {
    ...
    const pollReadyFile = async () => {
      if (settled) return;
      try {
        const payload = JSON.parse(await fs.readFile(readyPath, "utf8"));
...
async function copyPluginArtifact(sourceRoot, targetRoot, pluginId) {
  const source = path.join(sourceRoot, "plugins", pluginId);
  const target = path.join(targetRoot, "plugins", pluginId);
...
async function stageInstalledDependencyClosure(packageRoot) {
  ...
  for (const [packageName, relativeTarget] of Object.entries(workspacePackages)) {
    await fs.symlink(
...
async function verifyPhysicalRemovalMatrix({ tempRoot, plugins }) {
  if (plugins.length === 0) return [];
  const packageRoot = path.join(tempRoot, "removal-package");
  ...
  for (const removed of plugins) {
    const stagedPluginRoot = path.join(packageRoot, "plugins");
    await fs.rm(stagedPluginRoot, { recursive: true, force: true });
    const retained = plugins.filter((plugin) => plugin.id !== removed.id);
    const retainedFiles = {};
    ...
    const artifactFixture = await stagePluginArtifactVerificationFixture({ sourcePluginRoot: stagedPluginRoot });
    const stagedRegistry = await loadPluginRegistry({ artifactAuthority: artifactFixture.authority });
    const stagedIds = stagedRegistry.listPlugins().map((plugin) => plugin.id).sort();
    if (stagedIds.includes(removed.id) || stagedIds.join(",") !== retained.map((plugin) => plugin.id).sort().join(",")) {
      throw new Error(`Physical removal matrix did not stage the exact catalog for ${removed.id}.`);
    }
    const deployment = stagedRegistry.resolveDeployment({ enabledPluginIds: [] });
    if (
    ...
    let selectedRemovedRejected = false;
    try {
    ...
    await artifactFixture.close();
    const removedArtifactPresent = await fs.access(path.join(stagedPluginRoot, removed.id))
      .then(() => true, () => false);
    if (removedArtifactPresent) {
      throw new Error(`Physical removal matrix retained the artifact for ${removed.id}.`);
    }
    const boot = await runPackagedServer({
      tempRoot,
    ...
    const dataEntries = await fs.readdir(path.join(tempRoot, `removed-${removed.id}.data`), {
      recursive: true
    }).catch(() => []);
    if (dataEntries.some((entry) => String(entry).split(path.sep).includes(removed.id))) {
    ...
    results.push(Object.freeze({
      ...
      retainedFileCount: Object.values(retainedFiles).reduce((total, files) => total + files.length, 0)
    }));
  }
  return results;
}

function runtimeSource(id, mountName, mountId, kind, { postCommit = false, eventFile = "" } = {}) {
  return `
...
async function readFixtureEvents(filePath) {
  return fs.readFile(filePath, "utf8")
    .then((value) => value.split("\n").filter(Boolean), () => []);
}

function contributionRuntimeSource(id) {
  return `
...
async function collectExecutableSelectionFailures() {
  const startupPath = "tools/server-scripts/start-server.mjs";
  const source = await fs.readFile(path.join(ROOT, startupPath), "utf8");
...
export function finalizePluginRuntimeReport(report, scan = assertNoLeak) {
...
export function reducePluginRuntimeAcceptance({
  ...
  const everyCheckParticipates = Array.isArray(checks) &&
    checks.length > 0 &&
...
async function writePluginRuntimeReport(report) {
  const finalized = finalizePluginRuntimeReport(report);
...
async function main() {
  ...
  let pluginOwnershipFailures = [];
  let executableSelectionFailures = [];
  let physicalRemovalMatrix = [];
  let repoArtifactFixture = null;
  try {
    repoArtifactFixture = await stagePluginArtifactVerificationFixture({ sourcePluginRoot: path.join(ROOT, "plugins") });
    const repoRegistry = await loadPluginRegistry({ artifactAuthority: repoArtifactFixture.authority });

    await check("repo-plugin-catalog-is-explicit-and-default-off", async () => {
      const plugins = repoRegistry.listPlugins();
      const pluginRoot = path.join(ROOT, "plugins");
      const pluginDirectories = await fs.readdir(pluginRoot, { withFileTypes: true })
        .then((entries) => entries.filter((entry) => entry.isDirectory() && !entry.isSymbolicLink()), () => []);
      if (pluginDirectories.length > 0 && plugins.length === 0) {
        throw new Error("Repository plugin catalog is incomplete.");
      }
      if (plugins.some((plugin) => plugin.defaultEnabled !== false)) {
        throw new Error("Repository plugins must be explicit-selection only.");
      }
      const deployment = repoRegistry.resolveDeployment();
    ...
    await check("real-plugin-verifier-workloads-fail-closed-without-controlled-sandbox", async () => {
      for (const plugin of repoRegistry.listPlugins()) {
        const artifactSnapshot = await repoArtifactFixture.installed.get(plugin.id)?.port.loadSnapshot();
        ...
        for (const declaration of plugin.verifierHooks) {
          const result = await runPluginVerifierWorkload(declaration, {
            resolveSource: (source) => artifactSnapshot.resolveRuntimeModule(source),
            pluginId: plugin.id
          });
          if (
            result.ok !== false ||
    ...
    await check("every-real-optional-plugin-has-a-physical-removal-proof", async () => {
      ...
      if (
        ...
        physicalRemovalMatrix.some((entry) => entry.status !== "passed")
      ) {
        throw new Error("Physical removal coverage is incomplete.");
      }
    });

    await check("executable-selection-parser-preserves-explicit-empty-array", async () => {
      ...
      const selected = resolveEnabledPluginSelection({
    ...
    await check("executable-startup-preserves-empty-and-rejects-invalid-selection", async () => {
      const empty = await runPackagedServer({
        tempRoot,
        id: "packaged-empty",
        enabledPlugins: [],
        expectReady: true
      ...
      const unknown = await runPackagedServer({
        tempRoot,
        id: "packaged-unknown",
    ...
    await check("catalog-only-plugin-cannot-be-enabled", async () => {
      const catalogOnlyRoot = path.join(tempRoot, "catalog-only-repo");
    ...
    const fixtureEventPath = path.join(tempRoot, "fixture-events.jsonl");
    await writePlugin(
      fixtureRoot,
      manifest("dependency", {
    ...
    await check("disabled-plugin-is-never-imported", async () => {
      const manager = await fixtureManager(fixtureRoot, path.join(tempRoot, "disabled-state"));
    ...
    await check("selected-plugins-load-in-dependency-order-and-unload-in-reverse", async () => {
      const manager = await fixtureManager(
        fixtureRoot,
        path.join(tempRoot, "enabled-state"),
        ["demo", "dependency"]
      );
      const view = manager.createExecutionView();
      ...
      const activation = (await readFixtureEvents(fixtureEventPath)).filter((entry) => entry.endsWith(":activate"));
      ...
      await manager.close();
      const closing = (await readFixtureEvents(fixtureEventPath)).filter((entry) => entry.includes("close"));
    ...
    await check("plugin-contributions-are-executable-and-exactly-match-the-manifest", async () => {
      const manager = await fixtureManager(
        fixtureRoot,
      ...
      const operation = contributions.operations["contributor.run"];
      ...
      const result = await operation.implementation();
      if (result?.ok !== true) {
    ...
    await check("plugin-removal-is-safe-only-while-disabled", async () => {
      const removalRoot = path.join(tempRoot, "removal-repo");
      await writePlugin(
        removalRoot,
        manifest("removable", {
          runtime: { module: "./runtime.mjs" },
      ...
      const first = await fixtureManager(removalRoot, path.join(tempRoot, "removal-state-one"));
      await first.close();
      await fs.rm(path.join(removalRoot, "plugins", "removable"), { recursive: true, force: true });
      const second = await fixtureManager(removalRoot, path.join(tempRoot, "removal-state-two"));
    ...
    await check("missing-plugin-root-starts-only-with-empty-selection", async () => {
      const missingRoot = path.join(tempRoot, "missing-repo");
      const manager = await fixtureManager(missingRoot, path.join(tempRoot, "missing-state"));
    ...
    await check("dependency-omissions-and-unknown-selections-fail-closed", async () => {
      const artifactFixture = await stagePluginArtifactVerificationFixture({ sourcePluginRoot: path.join(fixtureRoot, "plugins") });
      try {
        const registry = await loadPluginRegistry({ artifactAuthority: artifactFixture.authority });
    ...
    await check("manifest-schema-path-and-claim-conflicts-fail-closed", async () => {
    ...
    await check("partial-activation-failure-unwinds-registered-resources", async () => {
      const failureRoot = path.join(tempRoot, "failure-repo");
      const failureEventPath = path.join(tempRoot, "failure-events.jsonl");
      await writePlugin(
        failureRoot,
...
export async function activatePlugin({ onClose }) {
  ...
  throw new Error("private fixture failure detail");
      ...
      const expected = ["alpha:activate", "beta:activate", "beta:registered-close", "alpha:close", "alpha:registered-close"];
    ...
    await check("configured-provider-and-plugin-mount-conflicts-fail-closed", async () => {
      const fixtureEventsBefore = await readFixtureEvents(fixtureEventPath);
      const closingBefore = fixtureEventsBefore.filter((entry) => entry.includes("close")).length;
      const activationBefore = fixtureEventsBefore.filter((entry) => entry.endsWith(":activate")).length;
      let rejected = false;
      try {
      ...
      const fixtureEventsAfter = await readFixtureEvents(fixtureEventPath);
      const closingAfter = fixtureEventsAfter.filter((entry) => entry.includes("close")).length;
      const activationAfter = fixtureEventsAfter.filter((entry) => entry.endsWith(":activate")).length;
      const acquired = activationAfter - activationBefore;
      const closed = closingAfter - closingBefore;
    ...
    await check("close-failure-is-surfaced-after-all-cleanups-run", async () => {
      const closeFailureRoot = path.join(tempRoot, "close-failure-repo");
      const closeFailureEventPath = path.join(tempRoot, "close-failure-events.jsonl");
      await writePlugin(
        closeFailureRoot,
...
export async function activatePlugin({ manifest, onClose }) {
  ...
  const executableChecks = checks.filter((item) => item.id.startsWith("executable-"));
  const runtimeChecks = checks.filter((item) => !item.id.startsWith("executable-"));
  const runtimeContractReady = runtimeChecks.every((item) => item.status === "passed");
  const pluginOwnershipReady = pluginOwnershipFailures.length === 0;
  const executableSelectionReady = executableSelectionFailures.length === 0 &&
    executableChecks.length === 2 &&
    executableChecks.every((item) => item.status === "passed");
  const failures = [...executableSelectionFailures, ...pluginOwnershipFailures];
  const blockers = failures.filter((item) => item.kind === "external-evidence");
  const localImplementationFailures = failures.filter((item) =>
