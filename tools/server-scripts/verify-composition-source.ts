...
const reportPath: any = path.join(sourceRoot, "build", "reports", "composition-source.json");
const manifestPath: any = path.join(sourceRoot, "meshrix-source-package-manifest.json");
const KEY_SYNTAX_FILES: readonly any[] = Object.freeze([
...
const PROHIBITED_PACKAGE_PATHS: readonly any[] = Object.freeze([
  ".git",
  "node_modules",
...
const REQUIRED_DOCKERIGNORE_ENTRIES: readonly any[] = Object.freeze([
  ".cache",
  "**/.cache",
  "docs/plans",
  "docs/reports",
...
const SENSITIVE_PATTERNS: readonly any[] = Object.freeze([
...
async function sha256File(relativePath?: any) : Promise<any> {
  const content: any = await fs.readFile(path.join(sourceRoot, relativePath));
  return createHash("sha256").update(content).digest("hex");
}

function assertNoLeak(report?: any) : any {
...
function syntaxCheck(relativePath?: any) : any {
  const result: any = spawnSync(process.execPath, ["--check", relativePath], {
    cwd: sourceRoot,
    encoding: "utf8",
...
const findings: any[] = [];
const manifest: any = await readJson(manifestPath).catch((error?: any) : any => {
  findings.push({
...
if (manifest) {
  ...
  const fileEntries: any = Array.isArray(manifest.files) ? manifest.files : [];
  ...
  for (const entry of fileEntries) {
    ...
    const actualHash: any = await sha256File(entry.path);
  ...
  for (const removedPath of manifest.featurePackagePlan?.removePaths || []) {
    ...
    if (fileEntries.some((entry?: any) : any => entry.path === removedPath || entry.path.startsWith(`${removedPath}/`))) {
      findings.push({ code: "feature_removed_manifest_entry_present", detail: removedPath });
    }
  }
}

const rootLicense: any = await readText("LICENSE").catch(() : any => "");
...
const dockerfile: any = await readText("Dockerfile").catch(() : any => "");
const dockerLicenseCopyCount: any = (dockerfile.match(/^COPY[^\n]*\bLICENSE\b[^\n]*$/gmu) || []).length;
if (dockerLicenseCopyCount < 2) {
  findings.push({ code: "docker_license_copy_missing", detail: "Dockerfile" });
}
const dockerignore: any = await readText(".dockerignore").catch(() : any => "");
const dockerignoreEntries: any = new Set<any>(
  dockerignore
    .split(/\r?\n/gu)
    .map((line?: any) : any => line.trim())
    .filter((line?: any) : any => line && !line.startsWith("#"))
);
...
const syntaxChecks: any = KEY_SYNTAX_FILES.map(syntaxCheck);
...
const report: Record<string, any> = {
  ...
  verifier: "tools/server-scripts/verify-composition-source.ts",
  ...
  summary: {
    ...
    internalDocumentationExcluded: INTERNAL_SOURCE_PACKAGE_EXCLUDED_PATHS.every((relativePath?: any) : any =>
      !findings.some((finding?: any) : any => {
        const detail: any = String(finding.detail || "");
        return detail === relativePath || detail.startsWith(relativePath + "/");
      })
    )
  },
  syntaxChecks: syntaxChecks.map((check?: any) : any => ({ file: check.file, ok: check.ok })),
