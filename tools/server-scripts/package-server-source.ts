...
function normalizeStringList(value: any = []) : any {
  const list: any = Array.isArray(value) ? value : String(value || "").split(",");
  return [...new Set<any>(list.map((item?: any) : any => String(item || "").trim()).filter(Boolean))];
}

function packageRemovePathMatches(relativePath: any = "", removePath: any = "") : any {
  const normalized: any = normalizeRelativePath(relativePath);
  const remove: any = normalizeRelativePath(removePath);
  return Boolean(remove && (normalized === remove || normalized.startsWith(`${remove}/`)));
}

function isRemovedByPackagePlan(relativePath: any = "", packageRemovePaths: any = []) : any {
  return packageRemovePaths.some((removePath?: any) : any => packageRemovePathMatches(relativePath, removePath));
}

function shouldSkipRelativePath(relativePath: any = "", packageRemovePaths: any = []) : any {
  const normalized: any = normalizeRelativePath(relativePath);
...
function assertSafeSourceRelativePath(relativePath: any = "") : any {
  const normalized: any = normalizeRelativePath(relativePath);
...
function displayPath(filePath: any = "", repoRoot: any = REPO_ROOT, externalLabel: any = "[external-output]") : any {
  const absolutePath: any = path.resolve(filePath);
  const absoluteRepoRoot: any = path.resolve(repoRoot);
...
async function collectSourceFiles(sourcePath?: any, relativePath?: any, packageRemovePaths: any = [], files: any = []) : Promise<any> {
  if (shouldSkipRelativePath(relativePath, packageRemovePaths)) {
    return files;
  }
  const normalizedRelativePath: any = assertSafeSourceRelativePath(relativePath);
  const stat: any = await fs.lstat(sourcePath);
  if (stat.isSymbolicLink()) {
    return files;
  }
  if (stat.isDirectory()) {
    const entries: any = await fs.readdir(sourcePath, { withFileTypes: true });
    for (const entry of entries.sort((left?: any, right?: any) : any => left.name.localeCompare(right.name))) {
      const childRelativePath: any = normalizeRelativePath(path.join(relativePath, entry.name));
...
async function ignoredSourcePaths(repoRoot?: any, relativePaths: any = []) : Promise<any> {
...
function resolveSourceRevision(repoRoot?: any) : any {
  const gitRepoRoot: any = resolveGitRepoRoot(repoRoot);
...
async function copySourceFile(sourceFile?: any, stagingPath?: any, manifestEntries?: any) : Promise<any> {
  const targetPath: any = path.join(stagingPath, sourceFile.relativePath);
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.copyFile(sourceFile.sourcePath, targetPath);
  const stat: any = await fs.stat(targetPath);
  const sha256: any = await sha256File(targetPath);
...
async function prepareStagingPath(stagingPath?: any) : Promise<any> {
  const absoluteStagingPath: any = path.resolve(stagingPath);
  if (await exists(absoluteStagingPath)) {
    const stat: any = await fs.lstat(absoluteStagingPath);
    if (stat.isSymbolicLink() || !stat.isDirectory()) {
      throw new Error("source_package_staging_path_invalid");
    }
    const entries: any = await fs.readdir(absoluteStagingPath);
    if (entries.length > 0) {
...
export function createSourceFeaturePlan(packagingPlan: any = createPackagingPlan()) : any {
  const featureRuntime: any = resolveFeatureRuntime({
  ...
  const packagePlan: any = collectPackagePlan(featureRuntime, {
    surface: packagingPlan.featureSurface || "all"
  });
  return { featureRuntime, packagePlan };
}

export async function applyFeatureSourcePlan(stagingPath?: any, packagingPlan: any = createPackagingPlan(), options: Record<string, any> = {}) : Promise<any> {
  const repoRoot: any = path.resolve(options.repoRoot || REPO_ROOT);
  const absoluteStagingPath: any = await prepareStagingPath(stagingPath);
  const { featureRuntime, packagePlan } = createSourceFeaturePlan(packagingPlan);
  const packageRemovePaths: any = packagePlan.removePaths || [];
  const manifestEntries: any[] = [];
  const copiedRoots: any[] = [];
  const sourceFiles: any[] = [];
  let copiedFileCount: any = 0;
  let totalBytes: any = 0;
  for (const relativePath of [...ROOT_SOURCE_FILES, ...SOURCE_PACKAGE_ROOTS]) {
    const sourcePath: any = path.join(repoRoot, relativePath);
    if (!await exists(sourcePath)) {
      continue;
    }
    const rootFiles: any = await collectSourceFiles(
      sourcePath,
      relativePath,
      packageRemovePaths
    );
  ...
  manifestEntries.sort((left?: any, right?: any) : any => left.path.localeCompare(right.path));
  const packageHash: any = createHash("sha256");
...
export async function resolveServerSourcePackageIdentity(repoRoot: any = REPO_ROOT) : Promise<any> {
  const packageJson: any = JSON.parse(await fs.readFile(path.join(repoRoot, "package.json"), "utf8"));
  const packageName: any = String(packageJson?.name || "").trim();
  const packageVersion: any = String(packageJson?.version || "").trim();
  ...
  const rootName: any = `${packageName}-server-source-${packageVersion}`;
  const archiveName: any = `${rootName}${SOURCE_PACKAGE_ARCHIVE_SUFFIX}`;
...
async function ensureOutputDirectory(outputDirectory?: any) : Promise<any> {
  await fs.mkdir(outputDirectory, { recursive: true });
  const stat: any = await fs.lstat(outputDirectory);
  if (stat.isSymbolicLink() || !stat.isDirectory()) {
    throw new Error("source_package_output_directory_invalid");
  }
}

async function validateArtifactTarget(targetPath: any, { force }: Record<string, any>) : Promise<any> {
  try {
    const stat: any = await fs.lstat(targetPath);
...
export async function createServerSourcePackage({
  ...
  const absoluteRepoRoot: any = path.resolve(repoRoot);
  const absoluteOutputDirectory: any = path.resolve(
    outputDirectory || path.join(absoluteRepoRoot, DEFAULT_SERVER_SOURCE_PACKAGE_OUTPUT_DIRECTORY)
  );
  const identity: any = await resolveServerSourcePackageIdentity(absoluteRepoRoot);
  const archivePath: any = path.join(absoluteOutputDirectory, identity.archiveName);
  const checksumPath: any = path.join(absoluteOutputDirectory, identity.checksumName);
  ...
  const workspacePrefix: any = path.join(absoluteOutputDirectory, ".meshrix-server-source-package-");
  const temporaryRoot: any = await fs.mkdtemp(workspacePrefix);
  const stagingRoot: any = path.join(temporaryRoot, identity.rootName);
  const generatedArchivePath: any = path.join(temporaryRoot, identity.archiveName);
  const generatedChecksumPath: any = path.join(temporaryRoot, identity.checksumName);
  try {
    const source: any = await applyFeatureSourcePlan(stagingRoot, packagingPlan, {
      repoRoot: absoluteRepoRoot
    });
    const artifactBoundaryScan: any = await scanPublicArtifact(stagingRoot, {
    ...
    const { createReproduciblePortableArchives } = await import(
      "./lib/mcp-release-reproducible-archives.ts"
    );
    ...
    const archiveSha256: any = await sha256File(generatedArchivePath);
    const archiveStat: any = await fs.stat(generatedArchivePath);
    await fs.writeFile(
      generatedChecksumPath,
...
function validateCliArgs(args?: any) : any {
  const booleanOptions: any = new Set<any>(["--dry-run", "--force"]);
...
async function main(args: any = process.argv.slice(2)) : Promise<any> {
  ...
  const identity: any = await resolveServerSourcePackageIdentity(REPO_ROOT);
