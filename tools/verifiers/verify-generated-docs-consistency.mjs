#!/usr/bin/env node
/**
 * verify-generated-docs-consistency.mjs — validates registry-governed docs.
 *
...
const docsRegistry = JSON.parse(
  await readFile(resolve(ROOT, 'tools/registry/docs.registry.json'), 'utf8')
);
const EXPECTED_DOCS = (docsRegistry.entries || []).map((entry) => ({
  id: entry.id,
  path: entry.path,
  source: entry.source,
}));
...
for (const doc of EXPECTED_DOCS) {
  const docPath = resolve(ROOT, doc.path);
  try {
    await access(docPath);
    const content = await readFile(docPath, 'utf8');
    const sourceAccepted = /^[a-z]+:\/\//i.test(doc.source) || await pathExists(doc.source);
    const verificationSectionAccepted = !doc.path.startsWith('docs/functionality/') || /^## Verification\b/mu.test(content);
    const executableVerificationAccepted = !doc.path.startsWith('docs/functionality/') ||
