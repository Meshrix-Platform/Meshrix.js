 ...
 *
 * Usage: node tools/generators/generate-operation-artifacts.ts [--check]
 *
 ...
 *   packages/contracts/src/generated/operations.generated.ts
...
const REGISTRY_PATH: any = resolve(ROOT, "tools/registry/operations/operations.registry.json");
const CAPABILITY_REGISTRY_PATH: any = resolve(ROOT, "tools/registry/capabilities/capabilities.registry.json");

const OUTPUTS: Record<string, any> = {
  registry: REGISTRY_PATH,
  capabilityRegistry: CAPABILITY_REGISTRY_PATH,
  operations: resolve(ROOT, "packages/contracts/src/generated/operations.generated.ts"),
  capabilities: resolve(ROOT, "packages/foundation/src/security/authorization/generated-capabilities.ts"),
...
const SOURCE_OPERATION_REGISTRY: any = "packages/contracts/src/operations/operation-registry.ts";

// --- Load registries ---

function loadRegistry(path?: any) : any {
  try {
    const raw: any = readFileSync(path, "utf8");
...
function otelOf(operation: Record<string, any> = {}) : any {
...
function operationProjection(operation: Record<string, any> = {}) : any {
  const projected: any = cloneJson(operation);
...
function createOperationRegistryProjection(previousRegistry: Record<string, any> = {}) : any {
  return {
...
function apiCapabilitiesForOperations(operations: any = [], nonOperationCapabilities: any = []) : any {
  return [
    ...operations.map((operation?: any) : any => ({
    id: `cap:api:${operation.id}`,
    operationId: operation.id,
    kind: "api",
    risk: riskOf(operation)
    })),
    ...nonOperationCapabilities.map((capability?: any) : any => ({
...
function toolCapabilitiesForOperations(operations: any = []) : any {
  const catalog: any = createToolCatalog({ operations });
  return catalog.tools.map((tool?: any) : any => ({
...
function createCapabilityRegistryProjection(registry: Record<string, any> = {}, previousCapabilityRegistry: Record<string, any> = {}) : any {
  return {
...
function generateOperationsModule(registry?: any) : any {
  const operations: any = registry.operations || [];
  const hash: any = _hash(JSON.stringify(registry, null, 2));
  const generatedAt: any = registry.generatedAt || "1970-01-01T00:00:00.000Z";

  const operationsJson: any = JSON.stringify(operations, null, 2);
...
export const SERVER_API_OPERATIONS: any = ${operationsJson};

export const OPERATION_IDS: any = Object.freeze(SERVER_API_OPERATIONS.map((op: any) => op.id));
export const OPERATIONS_BY_ID: any = Object.freeze(
  Object.fromEntries(SERVER_API_OPERATIONS.map((op: any) => [op.id, op]))
);
export const PUBLIC_OPERATION_IDS: any = Object.freeze(
  SERVER_API_OPERATIONS.filter((op: any) => op.public).map((op: any) => op.id)
);
export const DESTRUCTIVE_OPERATION_IDS: any = Object.freeze(
  SERVER_API_OPERATIONS.filter((op: any) => op.risk === 'destructive').map((op: any) => op.id)
);

export function getOperationById(id: any): any {
  return OPERATIONS_BY_ID[id] || null;
}

export function listOperationsByFeature(feature: any): any {
  return SERVER_API_OPERATIONS.filter((op: any) => op.feature === feature);
}

export function listOperationsByRisk(risk: any): any {
  return SERVER_API_OPERATIONS.filter((op: any) => op.risk === risk);
}
`;
}

// --- Generate generated-capabilities.ts ---

function generateCapabilitiesModule(registry?: any, capabilityRegistry?: any) : any {
  const capabilities: any = capabilityRegistry?.capabilities || [];
  const toolCapabilities: any = capabilityRegistry?.toolCapabilities || [];
  const wildcards: any = capabilityRegistry?.wildcards || [];

  const hash: any = _hash(JSON.stringify(capabilityRegistry, null, 2));
  const generatedAt: any = capabilityRegistry.generatedAt || registry.generatedAt || "1970-01-01T00:00:00.000Z";

  const apiCapIds: any = capabilities.map((c?: any) : any => `"${c.id}"`).join(",\n  ");
  const toolCapIds: any = toolCapabilities.map((c?: any) : any => `"${c.id}"`).join(",\n  ");
  const wildcardIds: any = wildcards.map((c?: any) : any => `"${c.id}"`).join(",\n  ");

  const allCaps: any[] = [...capabilities, ...toolCapabilities];
...
export const CAPABILITY_RISK_MAP = Object.freeze(${JSON.stringify(
    Object.fromEntries(allCaps.map((c?: any) : any => [c.id, c.risk || "read_only"])),
    null,
    2
  )});
`;
...
function openApiResponses(operation: Record<string, any> = {}) : any {
  const contract: any = operation.http?.responseContract;
  ...
  const byStatus: any = new Map<any, any>();
  for (const [code, errorContract] of (Object.entries(contract.errors || {}) as [string, any][])) {
    const status: any = String(errorContract.status || 500);
    const group: any = byStatus.get(status) || [];
    group.push({ code, contract: errorContract });
    byStatus.set(status, group);
  }
  for (const [status, errors] of byStatus) {
    responses[status] = {
      description: `Protocol errors: ${errors.map((entry?: any) : any => entry.code).join(", ")}`,
      content: {
        "application/json": { schema: errors[0].contract.responseSchema }
      },
      "x-meshrix-error-codes": Object.fromEntries(errors.map(({ code, contract: errorContract }: Record<string, any>) : any => [
        code,
...
function generateOpenApi(registry?: any) : any {
  const operations: any = registry.operations || [];

  const paths: Record<string, any> = {};
  for (const op of operations) {
    if (!op.http) continue;
    const path: any = op.http.path || `/${op.id.replace(/\./g, "/")}`;
    const method: any = String(op.http.method || "POST").toLowerCase();
...
function generateRoutesJson(registry?: any) : any {
  const operations: any = registry.operations || [];
  const routes: any[] = [];
...
function writeGenerated(path?: any, content?: any) : any {
  const dir: any = dirname(path);
  if (!existsSync(dir)) {
...
function checkGenerated(path?: any, content?: any) : any {
  if (!existsSync(path)) {
    console.error(`  MISSING: ${path}`);
    return false;
  }
  const existing: any = readFileSync(path, "utf8");
  if (existing !== content) {
...
function main() : any {
  ...
  const previousRegistry: any = loadRegistry(REGISTRY_PATH);
  const previousCapabilityRegistry: any = loadRegistry(CAPABILITY_REGISTRY_PATH);
  const registry: any = createOperationRegistryProjection(previousRegistry);
  const capabilityRegistry: any = createCapabilityRegistryProjection(registry, previousCapabilityRegistry);

  const errors: any = validateRegistry(registry);
  if (errors.length > 0) {
  ...
  // Generate outputs
  const opsContent: any = generateOperationsModule(registry);
  const capsContent: any = generateCapabilitiesModule(registry, capabilityRegistry);
  const openApiContent: any = JSON.stringify(generateOpenApi(registry), null, 2);
  const routesContent: any = JSON.stringify(generateRoutesJson(registry), null, 2);
  ...
  if (checkMode) {
    ...
    if (!allOk) {
      console.error("\nFAILED: Generated files are stale. Run: node tools/generators/generate-operation-artifacts.ts");
      process.exit(1);
    }
 ...
 */
function validateRegistry(registry?: any) : any {
  const errors: any[] = [];
  ...
  for (const op of registry.operations || []) {
    ...
    if (op.http) {
      ...
      const methods: any = Array.isArray(op.http.method)
        ? op.http.method
        : [op.http.method];
