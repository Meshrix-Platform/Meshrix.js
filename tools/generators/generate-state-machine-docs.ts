 ...
 *   node tools/generators/generate-state-machine-docs.ts --check
 */
...
const INTEGRITY_REGISTRY: any = path.resolve(
  ROOT,
  "tools/registry/state-machines/state-machine-integrity.registry.json"
);
const STATE_MACHINES_DOC: any = path.resolve(ROOT, "docs/architecture/STATE-MACHINES.md");
const DEFINITIONS_README: any = path.resolve(
  ROOT,
  "packages/foundation/src/workflow/state-machine/definitions/README.md"
);
const GENERATED_AT: any = "1970-01-01T00:00:00.000Z";
const DOC_SCHEMA: any = "v0.0.1:docs:state-machines-projection-1";

function toPosix(filePath?: any) : any {
  return path.relative(ROOT, filePath).split(path.sep).join("/");
}

function sha256Text(text?: any) : any {
  return `sha256:${createHash("sha256").update(String(text)).digest("hex")}`;
}

function loadIntegrityRegistry() : any {
  return JSON.parse(fs.readFileSync(INTEGRITY_REGISTRY, "utf8"));
}

function renderMachineTable(machines?: any, title?: any) : any {
  const rows: any = machines
    .map((machine?: any) : any => `| \`${machine.machineId}\` | \`${machine.canonicalSha256}\` | ${machine.stateCount}/${machine.eventCount}/${machine.matrixCellCount} | \`${machine.path}\` |`)
    .join("\n");
  return [
    `## ${title}`,
    "",
...
function renderDefinitionsReadme(registry?: any) : any {
  const lines: any = registry.machines.map((machine?: any) : any =>
...
function buildStateMachinesDoc(registry?: any) : any {
  const coreMachines: any = registry.machines.filter((machine?: any) : any => !machine.machineId.startsWith("acceptance."));
  const acceptanceMachines: any = registry.machines.filter((machine?: any) : any => machine.machineId.startsWith("acceptance."));
  const registryDigest: any = sha256Text(JSON.stringify(registry.machines.map((machine?: any) : any => ({
...
async function main() : Promise<any> {
  ...
  const registry: any = loadIntegrityRegistry();
  const stateMachinesDoc: any = buildStateMachinesDoc(registry);
  const definitionsReadme: any = renderDefinitionsReadme(registry);
  const outputs: any[] = [
  ...
  if (check && failed > 0) {
    console.error("Run: node tools/generators/generate-state-machine-docs.ts");
    process.exitCode = 1;
  }
}
