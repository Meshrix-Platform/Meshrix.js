...
export async function acceptM3EventDeliveryImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M3 node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M3 implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M3 focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M3 ${testPath}`);
  ...
  return {
    schema_version: "licomesh.m3-event-delivery-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM3EventDeliveryImplementation()
