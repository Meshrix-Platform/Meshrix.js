...
const EXPECTED_RECEIPT_DIGEST = "980c334ec973d3344786fed7a66c88ca7e7d8faf6bf20cb69919e95c7a130e31";

const modulePath = fileURLToPath(import.meta.url);
const defaultRepoRoot = path.resolve(path.dirname(modulePath), "../..");
const CHILD_PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m3-event-delivery";
const PARENT_PLAN_DIRECTORY = "end-to-end-release/high-concurrency";
const CHILD_FINAL_NODE_ID = "804cd00a-4bc3-404b-9b4a-7dc9347b4348";
const PARENT_INTEGRATION_NODE_ID = "3ff06d77-3295-4e34-a4a2-95b3c69530bb";
...
export async function acceptM3EventDeliveryParentIntegration({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(childMapPlan, "M3 DependencyMap entry is missing");

  const childFinalNode = childCheckpoints.find((node) => node.id === CHILD_FINAL_NODE_ID);
  requireCondition(childFinalNode, "M3 final validation node is missing");
  requireCondition(childFinalNode.status === "completed", "M3 final validation node is not completed");
  ...
  requireCondition(
    ...
    "M3 parent integration binding does not target the assigned node",
  );
  requireCondition(
    profilesEqual(integrationBinding.profiles, M3_PROFILES),
    "M3 parent integration profiles are not profile-scoped",
  );

  const receipt = acceptedFinalReceipt(childMapPlan, CHILD_FINAL_NODE_ID);
  requireCondition(receipt, "M3 accepted final receipt is missing");
  requireCondition(
    receipt.final_node_id === CHILD_FINAL_NODE_ID &&
      ...
      profilesEqual(receipt.profiles, M3_PROFILES) &&
      receipt.receipt_digest === EXPECTED_RECEIPT_DIGEST,
    "M3 accepted final receipt is not current or profile-scoped",
  );
  assertReceiptIntegrity(receipt);

  const parentIntegration = parentCheckpoints.find((node) => node.id === PARENT_INTEGRATION_NODE_ID);
  requireCondition(parentIntegration, "M3 parent integration node is missing");
  requireCondition(parentIntegration.role === "implementation", "M3 parent integration is not an implementation checkpoint");
  requireCondition(parentIntegration.status === "pending", "M3 parent integration node is not pending");
  requireCondition(
    parentIntegration.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M3 parent integration focused regression command is not canonical",
  );
  ...
  run(
    ...
    "M3 parent integration Better Plan verification",
  );
  ...
  requireCondition(
    ...
    "M3 DependencyMap receipt changed before parent checkpoint write",
  );
  ...
  requireCondition(canonical.accepted === true, "M3 parent integration canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m3-event-delivery-parent-integration-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM3EventDeliveryParentIntegration()
