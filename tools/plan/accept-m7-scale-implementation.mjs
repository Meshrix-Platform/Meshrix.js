...
const EVIDENCE_PATH = "build/reports/m7-scale/implementation-evidence.json";

const VERIFICATION_COMMANDS = Object.freeze([
  {
    kind: "capacity",
    script: M7_SCALE_DISCIPLINE.reports.capacity.verifier,
    reportPath: M7_SCALE_DISCIPLINE.reports.capacity.path,
  },
  {
    kind: "memory",
    script: M7_SCALE_DISCIPLINE.reports.memory.verifier,
    reportPath: M7_SCALE_DISCIPLINE.reports.memory.path,
  },
  {
    kind: "fault",
    script: M7_SCALE_DISCIPLINE.reports.fault.verifier,
    reportPath: M7_SCALE_DISCIPLINE.reports.fault.path,
  },
]);
...
async function loadVerificationReports(repoRoot) {
  ...
  for (const entry of VERIFICATION_COMMANDS) {
    ...
    assertM7ScaleReportShape(report, entry.kind);
...
export async function acceptM7ScaleImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M7 scale node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M7 scale implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M7 scale focused regression command is not canonical",
  );

  const verificationResults = [];
  for (const entry of VERIFICATION_COMMANDS) {
    runFreshProcess(repoRoot, entry.script, `M7 scale ${entry.kind}`);
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M7 scale ${testPath}`);
  ...
  const evidence = {
    schema_version: "licomesh.m7-scale-implementation-acceptance.v1",
    accepted: true,
