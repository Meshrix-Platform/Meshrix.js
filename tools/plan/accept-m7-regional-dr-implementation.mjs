...
import { readJson, resolveRepoRoot, writeReportAtomically } from "../server-scripts/lib/m7-regional-dr-report.mjs";

const modulePath = fileURLToPath(import.meta.url);
const defaultRepoRoot = resolveRepoRoot(path.resolve(path.dirname(modulePath), "../.."));
const PLAN_DIRECTORY = "end-to-end-release/m7-regional-dr";
const IMPLEMENTATION_NODE_ID = "0a141a84-d38b-4434-8052-5baf69b7d5d0";
const FOCUSED_COMMAND = "npx vitest run tests/vitest/server/plan-execution-eligibility.test.mjs";
const EVIDENCE_PATH = "build/reports/m7-regional-dr/implementation-evidence.json";

const VERIFICATION_COMMANDS = Object.freeze([
  {
    kind: "capacity",
    script: M7_REGIONAL_DR_DISCIPLINE.reports.capacity.verifier,
    reportPath: M7_REGIONAL_DR_DISCIPLINE.reports.capacity.path,
  },
  {
    kind: "memory",
    script: M7_REGIONAL_DR_DISCIPLINE.reports.memory.verifier,
    reportPath: M7_REGIONAL_DR_DISCIPLINE.reports.memory.path,
  },
  {
    kind: "fault",
    script: M7_REGIONAL_DR_DISCIPLINE.reports.fault.verifier,
    reportPath: M7_REGIONAL_DR_DISCIPLINE.reports.fault.path,
  },
]);
...
async function loadVerificationReports(repoRoot) {
  ...
  for (const entry of VERIFICATION_COMMANDS) {
    ...
    assertM7RegionalDrReportShape(report, entry.kind);
...
export async function acceptM7RegionalDrImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M7 regional-DR implementation node is missing");
  ...
  for (const entry of VERIFICATION_COMMANDS) {
    runFreshProcess(repoRoot, entry.script, `M7 regional-DR ${entry.kind}`);
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M7 regional-DR ${testPath}`);
  ...
  requireCondition(manifestPlan, "M7 regional-DR Manifest entry is missing");
  ...
  const evidence = {
    schema_version: "licomesh.m7-regional-dr-implementation-acceptance.v1",
    accepted: true,
    node_id: IMPLEMENTATION_NODE_ID,
    plan_directory: PLAN_DIRECTORY,
    profile: M7_REGIONAL_DR_DISCIPLINE.profile,
  ...
  requireCondition(canonical.accepted === true, "M7 regional-DR canonical Better Plan validation failed");
...
if (isDirectRun) {
  acceptM7RegionalDrImplementation()
