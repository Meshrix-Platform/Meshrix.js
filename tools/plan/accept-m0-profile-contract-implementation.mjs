...
export async function acceptM0ProfileContractImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M0 node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M0 implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M0 focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M0 ${testPath}`);
  ...
  return {
    schema_version: "licomesh.m0-profile-contract-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM0ProfileContractImplementation()
