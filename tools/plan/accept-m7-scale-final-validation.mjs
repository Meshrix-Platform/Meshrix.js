...
import { readJson, resolveRepoRoot, writeReportAtomically } from "../server-scripts/lib/m7-scale-report.mjs";
...
const PLAN_DIRECTORY = "end-to-end-release/m7-scale";
const IMPLEMENTATION_NODE_ID = "c5887ee1-d077-40f1-abf0-75dd884558ad";
const FINAL_NODE_ID = "a08d0585-1a0d-4f99-ae74-6c2da416c771";
const FINAL_COMMAND = "npm test";
const IMPLEMENTATION_EVIDENCE_PATH = "build/reports/m7-scale/implementation-evidence.json";
const FINAL_EVIDENCE_PATH = "build/reports/m7-scale/final-validation-evidence.json";
...
async function syncPlanManifest(repoRoot) {
  ...
  requireCondition(result.status === 0, "M7 scale Better Plan manifest sync failed");
}

async function assertScaleEvidenceCurrent(repoRoot) {
  const implementationEvidence = await readJson(path.join(repoRoot, IMPLEMENTATION_EVIDENCE_PATH));
  requireCondition(implementationEvidence.accepted === true, "M7 scale implementation evidence is not accepted");
  requireCondition(
    implementationEvidence.profile === M7_SCALE_DISCIPLINE.profile,
    "M7 scale implementation evidence profile is not scale",
  );
  requireCondition(
    implementationEvidence.node_id === IMPLEMENTATION_NODE_ID,
    "M7 scale implementation evidence node identity is mismatched",
  );
  assertM7ScaleReports(implementationEvidence.reports);
  return implementationEvidence;
}

export async function acceptM7ScaleFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M7 scale implementation node is missing");
  requireCondition(implementationNode.status === "completed", "M7 scale implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "M7 scale final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "M7 scale node is not a final validation checkpoint");
  requireCondition(
    finalNode.status === "pending" || finalNode.status === "completed",
    "M7 scale final validation node is not executable",
  );
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "M7 scale final regression command is not canonical",
  );
  requireCondition(finalNode.regression?.scope === "full", "M7 scale final regression scope is not full");
  ...
  requireCondition(mapPlan, "M7 scale DependencyMap entry is missing");
  const existingReceipt = acceptedFinalReceipt(mapPlan, FINAL_NODE_ID);
  requireCondition(!existingReceipt, "M7 scale final validation receipt is already accepted");

  const implementationEvidence = await assertScaleEvidenceCurrent(repoRoot);
  ...
  if (finalNode.status === "pending") {
    run(
      ...
      ["test"],
      "M7 scale final validation complete Core regression",
    );
    ...
    requireCondition(manifestPlan, "M7 scale Manifest entry is missing");
    ...
    requireCondition(finalNode.status === "completed", "M7 scale final validation node is incomplete");
    requireCondition(
      finalNode.acceptance_criteria?.every((criterion) => criterion.checked === true),
      "M7 scale final validation criteria are incomplete",
    );
    requireCondition(typeof recordedAt === "string" && recordedAt.length > 0, "M7 scale final validation evidence timestamp is missing");
  }

  await syncPlanManifest(repoRoot);

  const canonical = await validateCanonicalBetterPlanWorkspace({ repoRoot });
  requireCondition(canonical.accepted === true, "M7 scale canonical Better Plan validation failed");
  ...
  requireCondition(
    ...
    "M7 scale final receipt profiles are not scale-only",
  );
  requireCondition(
    receipt.prerequisite_receipts.every((entry) => profilesEqual(entry.profiles, M7_SCALE_PROFILES)),
    "M7 scale final receipt consumed a cross-profile prerequisite",
  );
  ...
  const evidence = {
    schema_version: "licomesh.m7-scale-final-validation-acceptance.v1",
    accepted: true,
    node_id: FINAL_NODE_ID,
    plan_directory: PLAN_DIRECTORY,
    profile: M7_SCALE_DISCIPLINE.profile,
