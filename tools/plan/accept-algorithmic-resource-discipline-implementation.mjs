...
export async function acceptAlgorithmicResourceDisciplineImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "Algorithmic Resource Discipline implementation node is missing");
  requireCondition(implementationNode.role === "implementation", "ARD node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "ARD implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "ARD focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `ARD ${testPath}`);
  ...
  return {
    schema_version: "licomesh.algorithmic-resource-discipline-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptAlgorithmicResourceDisciplineImplementation()
