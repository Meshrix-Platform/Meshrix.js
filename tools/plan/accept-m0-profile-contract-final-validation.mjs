...
const PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m0-profile-contract";
const IMPLEMENTATION_NODE_ID = "4ffcda15-107c-42c9-8d59-645cf532ae70";
const FINAL_NODE_ID = "e85d564e-4392-4ee8-9a09-9315339ad3b8";
...
export async function acceptM0ProfileContractFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M0 implementation node is missing");
  requireCondition(implementationNode.status === "completed", "M0 implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "M0 final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "M0 node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "M0 final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "M0 final regression command is not canonical",
  );
  ...
  run(
    ...
    "M0 Deployment Profile Contract Better Plan verification",
  );
  ...
  requireCondition(manifestPlan, "M0 Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "M0 canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m0-profile-contract-final-validation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM0ProfileContractFinalValidation()
