...
export async function acceptM2ObjectStorageImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M2 node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M2 implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M2 focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M2 ${testPath}`);
  ...
  return {
    schema_version: "licomesh.m2-object-storage-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM2ObjectStorageImplementation()
