...
import {
  ...
  type EligibleNodeProjection,
  type PlanExecutionEvaluation,
  type PlanExecutionInputs,
...
import { type CheckpointNode, type CheckpointsByDirectory, type JsonRecord, type PlanProfile, hasControlCharacter, isJsonRecord, isStringArray } from "./plan-types.ts";
...
function checkpointEntries(checkpoints: CheckpointsByDirectory): Array<[string, CheckpointNode[]]> {
  return checkpoints instanceof Map ? [...checkpoints.entries()] : Object.entries(checkpoints);
}

function selectedCheckpoint(selected: EligibleNodeProjection, checkpoints: CheckpointsByDirectory): CheckpointNode | undefined {
  const nodes = checkpointEntries(checkpoints).find(([directory]) => directory === selected.planDirectory)?.[1];
  return nodes?.find((node) => node.id === selected.nodeId);
}
...
function validRegressionPaths(value: unknown): value is string[] {
  ...
  if (normalized.some((entry, index) =>
    ...
    entry.startsWith("/") || entry.includes("\\") || hasControlCharacter(entry))) {
    return false;
  }
...
export function assertSelectionContract(selected: EligibleNodeProjection | undefined, checkpoints: CheckpointsByDirectory): void {
  if (!selected) {
...
function selectedNodeProjection(node: EligibleNodeProjection | undefined) {
  if (!node) {
    return null;
  }
  return {
...
export function boundedSelectionOutput(evaluation: PlanExecutionEvaluation, checkpoints: CheckpointsByDirectory) {
...
export async function selectNextPlanNode({
  ...
  verifyPlan?: (options: { repoRoot: string; writeReport: boolean; requireCompletedReceipts: boolean }) => Promise<unknown>;
  loadInputs?: (options: { repoRoot: string }) => Promise<PlanExecutionInputs>;
} = {}) {
  try {
    await verifyPlan({
...
export function boundedSelectionError(error: unknown) {
  return {
    schema_version: OUTPUT_SCHEMA,
    accepted: false,
...
async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  let selectedProfile: PlanProfile | undefined;
  for (let index  = 0; index < argv.length; index += 1) {
    ...
    if (profile !== "enterprise-single-node") throw new PlanExecutionPolicyError("unknown_profile", "unknown execution profile");
