...
const M6_PROFILES = Object.freeze(["ha", "regional-dr", "scale"]);
const EXPECTED_RECEIPT_DIGEST = "027c203015eb59177bc82dbcbc06920e11c824addfa606773a0f5b7fa41987db";

const modulePath = fileURLToPath(import.meta.url);
const defaultRepoRoot = path.resolve(path.dirname(modulePath), "../..");
const CHILD_PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m6-observability";
const PARENT_PLAN_DIRECTORY = "end-to-end-release/high-concurrency";
const CHILD_FINAL_NODE_ID = "c341e85b-1dfe-4515-8e45-9b0e9937605e";
const PARENT_INTEGRATION_NODE_ID = "48a06ac0-9dad-4917-a66f-258c1b71c4fd";
...
export async function acceptM6ObservabilityParentIntegration({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(childMapPlan, "M6 DependencyMap entry is missing");

  const childFinalNode = childCheckpoints.find((node) => node.id === CHILD_FINAL_NODE_ID);
  requireCondition(childFinalNode, "M6 final validation node is missing");
  requireCondition(childFinalNode.status === "completed", "M6 final validation node is not completed");
  ...
  requireCondition(
    ...
    "M6 parent integration binding does not target the assigned node",
  );
  requireCondition(
    profilesEqual(integrationBinding.profiles, M6_PROFILES),
    "M6 parent integration profiles are not profile-scoped",
  );

  const receipt = acceptedFinalReceipt(childMapPlan, CHILD_FINAL_NODE_ID);
  requireCondition(receipt, "M6 accepted final receipt is missing");
  requireCondition(
    receipt.final_node_id === CHILD_FINAL_NODE_ID &&
      ...
      profilesEqual(receipt.profiles, M6_PROFILES) &&
      receipt.receipt_digest === EXPECTED_RECEIPT_DIGEST,
    "M6 accepted final receipt is not current or profile-scoped",
  );
  assertReceiptIntegrity(receipt);

  const parentIntegration = parentCheckpoints.find((node) => node.id === PARENT_INTEGRATION_NODE_ID);
  requireCondition(parentIntegration, "M6 parent integration node is missing");
  requireCondition(parentIntegration.role === "implementation", "M6 parent integration is not an implementation checkpoint");
  requireCondition(parentIntegration.status === "pending", "M6 parent integration node is not pending");
  requireCondition(
    parentIntegration.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M6 parent integration focused regression command is not canonical",
  );
  ...
  run(
    ...
    "M6 parent integration Better Plan verification",
  );
  ...
  requireCondition(
    ...
    "M6 DependencyMap receipt changed before parent checkpoint write",
  );
  ...
  requireCondition(canonical.accepted === true, "M6 parent integration canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m6-observability-parent-integration-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM6ObservabilityParentIntegration()
