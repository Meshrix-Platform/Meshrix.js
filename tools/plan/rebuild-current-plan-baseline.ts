...
function oxlintOwnedBatch(ownedPaths?: any) : any {
  ...
  return `npx oxlint -c .oxlintrc.json ${lintPaths.join(" ")}`;
}
...
export function planNodes() : any[] {
  ...
  return [
    ...
    node({
      ...
      commands: ["npm run typecheck --workspace @meshrix/contracts", "npm test -- --suite gateway-split-contracts"],
      ...
      decisions: {
        composition: "the Service exposes one language-neutral HTTP and JSON contract; Meshrix consumes it through one default-disabled stateless adapter; every Agent MCP call traverses the selected downstream Gateway channel, an optional descriptor-selected Workspace application stage, and the selected upstream Gateway channel in that order; the local maintenance plugin calls the Service directly and Meshrix only as an ordinary external governed client",
