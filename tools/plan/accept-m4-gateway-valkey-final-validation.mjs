...
const PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m4-gateway-valkey";
const IMPLEMENTATION_NODE_ID = "aff5ac92-3df7-422a-a90d-6be781f4532f";
const FINAL_NODE_ID = "4e2ff224-2ef3-476a-b455-84c3513492e3";
...
export async function acceptM4GatewayValkeyFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M4 Gateway And Valkey implementation node is missing");
  requireCondition(implementationNode.status === "completed", "M4 Gateway And Valkey implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "M4 Gateway And Valkey final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "M4 node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "M4 Gateway And Valkey final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "M4 final regression command is not canonical",
  );
  ...
  run(
    ...
    "M4 Gateway And Valkey Better Plan verification",
  );
  ...
  requireCondition(manifestPlan, "M4 Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "M4 canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m4-gateway-valkey-final-validation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM4GatewayValkeyFinalValidation()
