...
const EXPECTED_RECEIPT_DIGEST = "18b0416630a315be66cb1b37cc1c2924b16fab9c61eebd2f506b84a3cd455677";

const modulePath = fileURLToPath(import.meta.url);
const defaultRepoRoot = path.resolve(path.dirname(modulePath), "../..");
const CHILD_PLAN_DIRECTORY = "end-to-end-release/m7-ha";
const PARENT_PLAN_DIRECTORY = "end-to-end-release";
const CHILD_FINAL_NODE_ID = "7895ddb1-a7c9-4abe-8550-6c9b86b2edf0";
const PARENT_INTEGRATION_NODE_ID = "aa7c73ae-4dd0-4773-b573-8ba954a27b4e";
...
export async function acceptM7HaParentIntegration({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(childMapPlan, "M7-ha DependencyMap entry is missing");

  const childFinalNode = childCheckpoints.find((node) => node.id === CHILD_FINAL_NODE_ID);
  requireCondition(childFinalNode, "M7-ha final validation node is missing");
  requireCondition(childFinalNode.status === "completed", "M7-ha final validation node is not completed");
  ...
  requireCondition(
    ...
    "M7-ha parent integration binding does not target the assigned node",
  );
  requireCondition(
    profilesEqual(integrationBinding.profiles, M7_HA_PROFILES),
    "M7-ha parent integration profiles are not profile-scoped",
  );

  const receipt = acceptedFinalReceipt(childMapPlan, CHILD_FINAL_NODE_ID);
  requireCondition(receipt, "M7-ha accepted final receipt is missing");
  requireCondition(
    ...
    "M7-ha accepted final receipt is not current or profile-scoped",
  );
  assertReceiptIntegrity(receipt);

  const parentIntegration = parentCheckpoints.find((node) => node.id === PARENT_INTEGRATION_NODE_ID);
  requireCondition(parentIntegration, "M7-ha parent integration node is missing");
  requireCondition(
    parentIntegration.role === "implementation",
    "M7-ha parent integration is not an implementation checkpoint",
  );
  requireCondition(parentIntegration.status === "pending", "M7-ha parent integration node is not pending");
  requireCondition(
    parentIntegration.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M7-ha parent integration focused regression command is not canonical",
  );
  ...
  run(
    ...
    "M7-ha parent integration Better Plan verification",
  );
  ...
  requireCondition(
    ...
    "M7-ha DependencyMap receipt changed before parent checkpoint write",
  );
  ...
  requireCondition(
    ...
    "M7-ha parent integration canonical Better Plan validation failed",
  );
  ...
  return {
    schema_version: "licomesh.m7-ha-parent-integration-acceptance.v1",
    accepted: true,
