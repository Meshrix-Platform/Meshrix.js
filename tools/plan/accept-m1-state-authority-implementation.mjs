...
export async function acceptM1StateAuthorityImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M1 node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M1 implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M1 focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M1 ${testPath}`);
  ...
  return {
    schema_version: "licomesh.m1-state-authority-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM1StateAuthorityImplementation()
