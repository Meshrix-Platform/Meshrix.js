...
const PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m6-observability";
const IMPLEMENTATION_NODE_ID = "3003e119-20d1-436e-b033-ca41ca8ac193";
const FINAL_NODE_ID = "c341e85b-1dfe-4515-8e45-9b0e9937605e";
...
export async function acceptM6ObservabilityFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M6 Observability Pipeline implementation node is missing");
  requireCondition(implementationNode.status === "completed", "M6 implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "M6 Observability Pipeline final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "M6 node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "M6 final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "M6 final regression command is not canonical",
  );
  ...
  run(
    ...
    "M6 Observability Pipeline Better Plan verification",
  );
  ...
  requireCondition(manifestPlan, "M6 Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "M6 canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m6-observability-final-validation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM6ObservabilityFinalValidation()
