...
export async function acceptM4GatewayValkeyImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M4 node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M4 implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M4 focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M4 ${testPath}`);
  ...
  return {
    schema_version: "licomesh.m4-gateway-valkey-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM4GatewayValkeyImplementation()
