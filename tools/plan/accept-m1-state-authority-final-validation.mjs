...
const PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m1-state-authority";
const IMPLEMENTATION_NODE_ID = "5a4e96be-cd0d-49d2-87e8-b5f96bf9592e";
const FINAL_NODE_ID = "054737d4-fddd-4d18-b083-cceaf72c5864";
...
export async function acceptM1StateAuthorityFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M1 implementation node is missing");
  requireCondition(implementationNode.status === "completed", "M1 implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "M1 final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "M1 node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "M1 final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "M1 final regression command is not canonical",
  );
  ...
  run(
    ...
    "M1 Shared State Authority Better Plan verification",
  );
  ...
  requireCondition(manifestPlan, "M1 Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "M1 canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m1-state-authority-final-validation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM1StateAuthorityFinalValidation()
