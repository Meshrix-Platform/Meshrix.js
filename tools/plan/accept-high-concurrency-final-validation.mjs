...
const PREREQUISITE_NODE_ID = "0e0a2b94-7d99-483d-8cac-a1f90042ed22";
const FINAL_NODE_ID = "767db312-c6ef-4b17-8a06-56e543a9dd62";
...
function canReuseCoreRegression(prerequisiteNode, revision) {
  return prerequisiteNode.status === "completed"
    && prerequisiteNode.commit?.delivered === revision
    && prerequisiteNode.regression?.commands?.[0] === FINAL_COMMAND
    && prerequisiteNode.regression?.last_pass?.content_fingerprint != null;
}

export async function acceptHighConcurrencyFinalValidation({
  ...
  requireCondition(prerequisiteNode, "Shared high-concurrency Core regression node is missing");
  requireCondition(prerequisiteNode.status === "completed", "Shared Core regression node is not completed");
  requireCondition(
    prerequisiteNode.regression?.commands?.[0] === FINAL_COMMAND,
  ...
  requireCondition(finalNode, "Shared high-concurrency final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "Node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "Shared high-concurrency final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "Shared high-concurrency final regression command is not canonical",
  );
  requireCondition(
    finalNode.regression?.scope === "full",
    "Shared high-concurrency final regression scope is not full",
  );

  const reuseCoreRegression = skipRegression ?? canReuseCoreRegression(prerequisiteNode, revision);
  if (!reuseCoreRegression) {
    run(
      repoRoot,
      process.platform === "win32" ? "npm.cmd" : "npm",
      ["test"],
      "Shared high-concurrency final validation complete Core regression",
    );
  }
  ...
  requireCondition(manifestPlan, "Shared high-concurrency Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "Shared high-concurrency canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.high-concurrency-final-validation-acceptance.v1",
    accepted: true,
    ...
    reused_core_regression: reuseCoreRegression,
...
if (isDirectRun) {
  acceptHighConcurrencyFinalValidation()
