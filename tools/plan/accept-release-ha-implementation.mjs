...
const IMPLEMENTATION_NODE_ID = "dc1d8b29-d976-48d0-b83c-3247bf6131fe";
const HA_FINAL_NODE_ID = "7a2201c9-6e61-481f-a9fe-83636af00292";
const FOCUSED_COMMAND = "npm run verify:acceptance";
const ACCEPTANCE_PROFILE = "ha";
const PLAN_PROFILE = "ha";
const ACCEPTANCE_VERIFIER = "tools/server-scripts/verify-platform-acceptance.mjs";
const EVIDENCE_PATH = "build/reports/release-ha-implementation-acceptance.json";
...
export async function acceptReleaseHaImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "HA release implementation node is missing");
  requireCondition(implementationNode.role === "implementation", "HA release node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "HA release implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "HA release focused regression command is not canonical",
  );

  const finalNode = checkpoints.find((node) => node.id === HA_FINAL_NODE_ID);
  requireCondition(finalNode?.status === "pending", "HA final validation must remain pending");
  ...
  requireCondition(planReceiptPreflight.planProfile === PLAN_PROFILE, "HA plan receipt profile mismatch");
  requireCondition(planReceiptPreflight.requiredReceiptCount === 3, "HA release must consume exactly three prerequisite receipts");
  requireCondition(
    ...
    "HA release must consume only the ha prerequisite receipts",
  );

  const frozenAcceptance = {
    schema_version: "licomesh.release-ha-frozen-acceptance.v1",
  ...
  run(
    ...
    "HA release canonical acceptance",
  );
  ...
  requireCondition(
    ...
    "HA acceptance must not mutate the frozen plan receipt set",
  );
  ...
  const evidence = {
    schema_version: "licomesh.release-ha-implementation-acceptance.v1",
    accepted: true,
    node_id: IMPLEMENTATION_NODE_ID,
    plan_directory: PLAN_DIRECTORY,
    plan_profile: PLAN_PROFILE,
    ...
    ha_final_node_status: finalNode.status,
  };
  await writePrivateFileAtomic(
