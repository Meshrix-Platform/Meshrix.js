...
const PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m2-object-storage";
const IMPLEMENTATION_NODE_ID = "10e31b24-9fab-4057-ab92-7406334949a0";
const FINAL_NODE_ID = "401b8364-dee3-4d61-9ff8-074b48541214";
...
export async function acceptM2ObjectStorageFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "M2 Object Storage implementation node is missing");
  requireCondition(implementationNode.status === "completed", "M2 Object Storage implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "M2 Object Storage final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "M2 node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "M2 Object Storage final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "M2 final regression command is not canonical",
  );
  ...
  run(
    ...
    "M2 Object Storage Better Plan verification",
  );
  ...
  requireCondition(manifestPlan, "M2 Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "M2 canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m2-object-storage-final-validation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM2ObjectStorageFinalValidation()
