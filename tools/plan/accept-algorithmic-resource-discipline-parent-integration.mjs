...
const ARD_PROFILES = Object.freeze(["ha", "regional-dr", "scale"]);

const modulePath = fileURLToPath(import.meta.url);
const defaultRepoRoot = path.resolve(path.dirname(modulePath), "../..");
const CHILD_PLAN_DIRECTORY = "end-to-end-release/high-concurrency/algorithmic-resource-discipline";
const PARENT_PLAN_DIRECTORY = "end-to-end-release/high-concurrency";
const CHILD_FINAL_NODE_ID = "4faef9cf-45d3-47e7-988c-9197df2096a7";
const PARENT_INTEGRATION_NODE_ID = "4b43ba1a-1b75-433a-b399-ca811ce017c1";
...
export async function acceptAlgorithmicResourceDisciplineParentIntegration({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(childMapPlan, "ARD DependencyMap entry is missing");

  const childFinalNode = childCheckpoints.find((node) => node.id === CHILD_FINAL_NODE_ID);
  requireCondition(childFinalNode, "ARD final validation node is missing");
  requireCondition(childFinalNode.status === "completed", "ARD final validation node is not completed");
  ...
  requireCondition(
    ...
    "ARD parent integration binding does not target the assigned node",
  );
  requireCondition(
    profilesEqual(integrationBinding.profiles, ARD_PROFILES),
    "ARD parent integration profiles are not profile-scoped",
  );

  const receipt = acceptedFinalReceipt(childMapPlan, CHILD_FINAL_NODE_ID);
  requireCondition(receipt, "ARD accepted final receipt is missing");
  requireCondition(
    receipt.final_node_id === CHILD_FINAL_NODE_ID &&
      ...
      profilesEqual(receipt.profiles, ARD_PROFILES),
    "ARD accepted final receipt is not current or profile-scoped",
  );
  assertReceiptIntegrity(receipt);

  const parentIntegration = parentCheckpoints.find((node) => node.id === PARENT_INTEGRATION_NODE_ID);
  requireCondition(parentIntegration, "ARD parent integration node is missing");
  requireCondition(parentIntegration.role === "implementation", "ARD parent integration is not an implementation checkpoint");
  requireCondition(parentIntegration.status === "pending", "ARD parent integration node is not pending");
  requireCondition(
    parentIntegration.regression?.commands?.[0] === FOCUSED_COMMAND,
    "ARD parent integration focused regression command is not canonical",
  );
  ...
  run(
    ...
    "ARD parent integration Better Plan verification",
  );
  ...
  requireCondition(canonical.accepted === true, "ARD parent integration canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.algorithmic-resource-discipline-parent-integration-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptAlgorithmicResourceDisciplineParentIntegration()
