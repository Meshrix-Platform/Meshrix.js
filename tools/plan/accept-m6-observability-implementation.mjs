...
export async function acceptM6ObservabilityImplementation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode.role === "implementation", "M6 node is not an implementation checkpoint");
  requireCondition(implementationNode.status === "pending", "M6 implementation node is not pending");
  requireCondition(
    implementationNode.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M6 focused regression command is not canonical",
  );
  ...
  run(repoRoot, process.execPath, [vitestEntry, "run", testPath], `M6 ${testPath}`);
  ...
  return {
    schema_version: "licomesh.m6-observability-implementation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM6ObservabilityImplementation()
