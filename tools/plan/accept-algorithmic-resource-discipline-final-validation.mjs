...
const PLAN_DIRECTORY = "end-to-end-release/high-concurrency/algorithmic-resource-discipline";
const IMPLEMENTATION_NODE_ID = "965474db-f28e-4bac-beef-c25dd33161e2";
const FINAL_NODE_ID = "4faef9cf-45d3-47e7-988c-9197df2096a7";
...
export async function acceptAlgorithmicResourceDisciplineFinalValidation({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(implementationNode, "ARD implementation node is missing");
  requireCondition(implementationNode.status === "completed", "ARD implementation node is not completed");

  const finalNode = checkpoints.find((node) => node.id === FINAL_NODE_ID);
  requireCondition(finalNode, "ARD final validation node is missing");
  requireCondition(finalNode.role === "final_validation", "ARD node is not a final validation checkpoint");
  requireCondition(finalNode.status === "pending", "ARD final validation node is not pending");
  requireCondition(
    finalNode.regression?.commands?.[0] === FINAL_COMMAND,
    "ARD final regression command is not canonical",
  );
  ...
  run(
    ...
    "Algorithmic Resource Discipline Better Plan verification",
  );
  ...
  requireCondition(manifestPlan, "ARD Manifest entry is missing");
  ...
  requireCondition(canonical.accepted === true, "ARD canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.algorithmic-resource-discipline-final-validation-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptAlgorithmicResourceDisciplineFinalValidation()
