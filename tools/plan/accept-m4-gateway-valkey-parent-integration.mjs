...
const M4_PROFILES = Object.freeze(["ha", "regional-dr", "scale"]);
const EXPECTED_RECEIPT_DIGEST = "6fc58d2528881b3a2229486712f8259477a655b0180efa91fd8402a55825dd6c";

const modulePath = fileURLToPath(import.meta.url);
const defaultRepoRoot = path.resolve(path.dirname(modulePath), "../..");
const CHILD_PLAN_DIRECTORY = "end-to-end-release/high-concurrency/m4-gateway-valkey";
const PARENT_PLAN_DIRECTORY = "end-to-end-release/high-concurrency";
const CHILD_FINAL_NODE_ID = "4e2ff224-2ef3-476a-b455-84c3513492e3";
const PARENT_INTEGRATION_NODE_ID = "40089b3d-9ab6-4e2d-8164-ef37ad5a0b19";
...
export async function acceptM4GatewayValkeyParentIntegration({ repoRoot = defaultRepoRoot } = {}) {
  ...
  requireCondition(childMapPlan, "M4 DependencyMap entry is missing");

  const childFinalNode = childCheckpoints.find((node) => node.id === CHILD_FINAL_NODE_ID);
  requireCondition(childFinalNode, "M4 final validation node is missing");
  requireCondition(childFinalNode.status === "completed", "M4 final validation node is not completed");
  ...
  requireCondition(
    ...
    "M4 parent integration binding does not target the assigned node",
  );
  requireCondition(
    profilesEqual(integrationBinding.profiles, M4_PROFILES),
    "M4 parent integration profiles are not profile-scoped",
  );

  const receipt = acceptedFinalReceipt(childMapPlan, CHILD_FINAL_NODE_ID);
  requireCondition(receipt, "M4 accepted final receipt is missing");
  requireCondition(
    receipt.final_node_id === CHILD_FINAL_NODE_ID &&
      ...
      profilesEqual(receipt.profiles, M4_PROFILES) &&
      receipt.receipt_digest === EXPECTED_RECEIPT_DIGEST,
    "M4 accepted final receipt is not current or profile-scoped",
  );
  assertReceiptIntegrity(receipt);

  const parentIntegration = parentCheckpoints.find((node) => node.id === PARENT_INTEGRATION_NODE_ID);
  requireCondition(parentIntegration, "M4 parent integration node is missing");
  requireCondition(parentIntegration.role === "implementation", "M4 parent integration is not an implementation checkpoint");
  requireCondition(parentIntegration.status === "pending", "M4 parent integration node is not pending");
  requireCondition(
    parentIntegration.regression?.commands?.[0] === FOCUSED_COMMAND,
    "M4 parent integration focused regression command is not canonical",
  );
  ...
  run(
    ...
    "M4 parent integration Better Plan verification",
  );
  ...
  requireCondition(
    ...
    "M4 DependencyMap receipt changed before parent checkpoint write",
  );
  ...
  requireCondition(canonical.accepted === true, "M4 parent integration canonical Better Plan validation failed");
  ...
  return {
    schema_version: "licomesh.m4-gateway-valkey-parent-integration-acceptance.v1",
    accepted: true,
...
if (isDirectRun) {
  acceptM4GatewayValkeyParentIntegration()
