...
- The bounded compiled-authorization cache binds each retained entry to the canonical structural content of the policy, profile, subject, actor, session user, and tool in addition to their identities and revisions. A capability, scope, source allowlist, tool policy, risk, or resource-boundary change is therefore visible to the next request even when a producer reuses a revision. Expiry, use count, and request source remain live per-request checks; facts that cannot be fingerprinted exactly bypass the cache.
...
- Job status, result, normalized-artifact, and raw-object ownership checks require an explicit authenticated subject at the controller boundary. A missing internal session denies access instead of inheriting authority from the dispatch path, process co-location, or a direct controller call; owner Console sessions and scoped API Key workload principals still use the same job and workspace bindings.
...
Runtime changes use the active generation as the comparison baseline. Ordinary configuration changes and authority reductions do not require human approval. Any effective authority expansion—including a new Host capability, artifact-signing purpose, interaction method, or wider Host-owned execution, egress, workspace, or resource policy—must remain pending until an administrator explicitly approves it in the Meshrix.js Console. Approval is bound to the candidate artifact generation, configuration revision, and exact authority delta; a stale or changed candidate is re-evaluated rather than inheriting the prior decision.

Approved activation still publishes authority through one reversible
contribution transaction. A missing transaction, partial publication, durable
generation failure, or cleanup uncertainty never creates an active plugin
receipt. The previous complete contribution generation is restored before an
ordinary activation failure is reported; unresolved rollback is exposed as
`PLUGIN_PACKAGE_ROLLBACK_FAILED` with the candidate remaining non-ready.
