...
| Downstream adapters | Pinned external LicoMesh-Plugins packages for local connector-managed clients; Core contains only the generic adapter protocol and security boundary. |
...
| LicoArc | External desktop/mobile client implementing published protocols | Optional and independently released; its repository, implementation, build, plans, tests, reports, and receipts are not required for core startup, server implementation, or server release readiness. |
...
in LicoMesh-Plugins. They do not block or promote a Core release. Core verifies
